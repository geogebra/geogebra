...
public class PredefMacros {
	...
	static {
		...
		NewEnvironmentMacro.addNewEnvironment("Vmatrix",
				"\\left\\|\\begin{matrix}", "\\end{matrix}\\right\\|", 0);
		NewEnvironmentMacro.addNewEnvironment("eqnarray", "\\begin{array}{rcl}",
				"\\end{array}", 0);
	...
	public static final Atom fcscore_macro(final String[] args)
	...
	public static final Atom spATbreve_macro() throws ParseException {
		VRowAtom vra = new VRowAtom(
				new TeXFormula("\\displaystyle\\!\\breve{}").root);
		vra.setRaise(TeXLength.Unit.EX, 0.6f);

		return new SmashedAtom(vra, null);
	}

	public static final Atom spAThat_macro() throws ParseException {
		VRowAtom vra = new VRowAtom(
	...
	public static final Atom hvspace_macro(final String[] args)
			throws ParseException {
		int i;
		for (i = 0; i < args[1].length()
				&& !java.lang.Character.isLetter(args[1].charAt(i)); i++) {
			;
		}
		double f = 0;
		try {
		...
		if (i != args[1].length()) {
			...
			unit = TeXLength.Unit.POINT;
		}

		if (unit == Unit.NONE) {
			throw new ParseException(
					"Unknown unit \"" + args[1].substring(i) + "\" !");
		}
	...
	public static final Atom rule_macro(final String[] args)
			throws ParseException {
		Object[] winfo = SpaceAtom.getLength(args[1]);
		if (winfo.length == 1) {
			throw new ParseException(
					"Error in getting width in \\rule command !");
		}
		Object[] hinfo = SpaceAtom.getLength(args[2]);
		if (hinfo.length == 1) {
			throw new ParseException(
					"Error in getting height in \\rule command !");
		}

		Object[] rinfo = SpaceAtom.getLength(args[3]);
		if (rinfo.length == 1) {
			throw new ParseException(
					"Error in getting raise in \\rule command !");
		}

		return new RuleAtom((Unit) winfo[0], (Double) winfo[1],
				(Double) hinfo[1], -((Double) rinfo[1]).doubleValue());
	}

	/* Thanks to Juan Enrique Escobar Robles for this macro */
	public static final Atom cfrac_macro(final TeXParser tp,
			final String[] args) throws ParseException {
		Align alig = TeXConstants.Align.CENTER;
		if ("r".equals(args[3])) {
			alig = TeXConstants.Align.RIGHT;
		} else if ("l".equals(args[3])) {
			alig = TeXConstants.Align.LEFT;
		}
	...
	public static final Atom sfrac_macro(final TeXParser tp,
			final String[] args) throws ParseException {
		TeXFormula num = new TeXFormula(tp, args[1], false);
		TeXFormula denom = new TeXFormula(tp, args[2], false);
		if (num.root == null || denom.root == null) {
		...
		if (!tp.isMathMode()) {
			...
			((VRowAtom) slash).setRaise(TeXLength.Unit.EX, 0.4f);
		}
		...
		at.add(new SpaceAtom(TeXLength.Unit.EM, shiftL, 0f, 0f));
		at.add(slash);
		at.add(new SpaceAtom(TeXLength.Unit.EM, shiftR, 0f, 0f));
	...
	public static final Atom genfrac_macro(final TeXParser tp,
		...
		Atom at = new FractionAtom(num.root, denom.root, rule, (Unit) ths[0],
				(Double) ths[1]);
		RowAtom rat = new RowAtom();
	...
	public static final Atom over_macro(final TeXParser tp)
			throws ParseException {
	...
	public static final Atom atop_macro(final TeXParser tp)
			throws ParseException {
		Atom num = tp.getFormulaAtom();
		Atom denom = new TeXFormula(tp, tp.getOverArgument(), false).root;
		if (num == null || denom == null) {
	...
	public static final Atom binom_macro(final TeXParser tp,
			final String[] args) throws ParseException {
		TeXFormula num = new TeXFormula(tp, args[1], false);
		TeXFormula denom = new TeXFormula(tp, args[2], false);
		if (num.root == null || denom.root == null) {
	...
	public static final Atom above_macro(final TeXParser tp)
			throws ParseException {
		Atom num = tp.getFormulaAtom();
		Object[] dim = tp.getLength();
		Atom denom = new TeXFormula(tp, tp.getOverArgument(), false).root;
	...
	public static final Atom mbox_macro(final TeXParser tp, final String[] args)
			throws ParseException {
		Atom group = new RomanAtom(
				new TeXFormula(tp, args[1], "mathnormal", false, false).root);
	...
	public static final Atom mhchem_ce_macro(final TeXParser tp,
			final String[] args) throws ParseException {
	...
	public static final Atom IJ_macro(final String[] args)
			throws ParseException {
		return new IJAtom(args[0].charAt(0) == 'I');
	}

	public static final Atom TStroke_macro(final String[] args)
			throws ParseException {
		return new TStrokeAtom(args[0].charAt(0) == 'T');
	}

	public static final Atom LCaron_macro(final String[] args)
			throws ParseException {
		return new LCaronAtom(args[0].charAt(0) == 'L');
	}

	public static final Atom tcaron_macro() throws ParseException {
	...
	public static final Atom nbsp_macro() throws ParseException {
	...
	public static final Atom sideset_macro(final TeXParser tp,
		...
		tf.add(new SpaceAtom(TeXLength.Unit.MU, -0.3f, 0f, 0f));
	...
	public static final Atom prescript_macro(final TeXParser tp,
		...
		tp.addAtom(new SpaceAtom(TeXLength.Unit.MU, -0.3f, 0f, 0f));
		return new TypedAtom(TeXConstants.TYPE_ORDINARY,
	...
	public static final Atom underbrace_macro(final TeXParser tp,
		...
		return new OverUnderDelimiter(new TeXFormula(tp, args[1], false).root,
				null, SymbolAtom.get("rbrace"), TeXLength.Unit.EX, 0, false);
	}

	public static final Atom overbrace_macro(final TeXParser tp,
			final String[] args) throws ParseException {
		return new OverUnderDelimiter(new TeXFormula(tp, args[1], false).root,
				null, SymbolAtom.get("lbrace"), TeXLength.Unit.EX, 0, true);
	}

	public static final Atom underbrack_macro(final TeXParser tp,
			final String[] args) throws ParseException {
		return new OverUnderDelimiter(new TeXFormula(tp, args[1], false).root,
				null, SymbolAtom.get("rsqbrack"), TeXLength.Unit.EX, 0, false);
	}

	public static final Atom overbrack_macro(final TeXParser tp,
			final String[] args) throws ParseException {
		return new OverUnderDelimiter(new TeXFormula(tp, args[1], false).root,
				null, SymbolAtom.get("lsqbrack"), TeXLength.Unit.EX, 0, true);
	}

	public static final Atom underparen_macro(final TeXParser tp,
			final String[] args) throws ParseException {
		return new OverUnderDelimiter(new TeXFormula(tp, args[1], false).root,
				null, SymbolAtom.get("rbrack"), TeXLength.Unit.EX, 0, false);
	}

	public static final Atom overparen_macro(final TeXParser tp,
			final String[] args) throws ParseException {
		return new OverUnderDelimiter(new TeXFormula(tp, args[1], false).root,
				null, SymbolAtom.get("lbrack"), TeXLength.Unit.EX, 0, true);
	}
	...
	public static final Atom joinrel_macro() throws ParseException {
		return new TypedAtom(TeXConstants.TYPE_RELATION,
				TeXConstants.TYPE_RELATION,
				new SpaceAtom(TeXLength.Unit.MU, -2.6f, 0, 0));
	}

	public static final Atom smash_macro(final TeXParser tp,
			final String[] args) throws ParseException {
		return new SmashedAtom(new TeXFormula(tp, args[1], false).root,
				args[2]);
	}

	public static final Atom vdots_macro() throws ParseException {
		return new VdotsAtom();
	}

	public static final Atom ddots_macro() throws ParseException {
		return new TypedAtom(TeXConstants.TYPE_INNER, TeXConstants.TYPE_INNER,
				new DdotsAtom());
	}

	public static final Atom iddots_macro() throws ParseException {
		return new TypedAtom(TeXConstants.TYPE_INNER, TeXConstants.TYPE_INNER,
				new IddotsAtom());
	}
	...
	public static final Atom cr_macro(final TeXParser tp)
			throws ParseException {
		if (tp.isArrayMode()) {
			tp.addRow();
		} else {
	...
	public static final Atom intertext_macro(final TeXParser tp,
		...
		Atom at = new RomanAtom(
				new TeXFormula(tp, str, "mathnormal", false, false).root);
	...
	public static final Atom multlineATATenv_macro(final TeXParser tp,
		...
		return new MultlineAtom(tp.getIsPartial(), array,
	...
	public static final Atom shoveright_macro(final TeXParser tp,
		...
		at.alignment = TeXConstants.Align.RIGHT;
		return at;
	}
	...
	public static final Atom shoveleft_macro(final TeXParser tp,
			final String[] args) throws ParseException {
		Atom at = new TeXFormula(tp, args[1]).root;
		at.alignment = TeXConstants.Align.LEFT;
		return at;
	}
	...
	public static final Atom newenvironment_macro(final String[] args)
			throws ParseException {
		int opt = args[4] == null ? 0 : Integer.parseInt(args[4]);
		// if (opt == null)
		// throw new ParseException("The optional argument should be an integer
		// !");
	...
	public static final Atom renewenvironment_macro(final String[] args)
			throws ParseException {
	...
	public static final Atom stackrel_macro(final TeXParser tp,
		...
		Atom at = new UnderOverAtom(new TeXFormula(tp, args[2], false).root,
				new TeXFormula(tp, args[3], false).root, TeXLength.Unit.MU,
				0.5f, true, new TeXFormula(tp, args[1], false).root,
				TeXLength.Unit.MU, 2.5f, true);
		return new TypedAtom(TeXConstants.TYPE_RELATION,
	...
	public static final Atom overset_macro(final TeXParser tp,
			final String[] args) throws ParseException {
		Atom at = new UnderOverAtom(new TeXFormula(tp, args[2], false).root,
				new TeXFormula(tp, args[1], false).root, TeXLength.Unit.MU,
				2.5f, true, true);
		return new TypedAtom(TeXConstants.TYPE_RELATION,
				TeXConstants.TYPE_RELATION, at);
	}
	...
	public static final Atom undertilde_macro(final TeXParser tp,
		...
		return new UnderOverAtom(at,
				new AccentedAtom(new PhantomAtom(at, true, false, false),
						"widetilde"),
				TeXLength.Unit.MU, 0.3f, true, false);
	}
	...
	public static final Atom rm_macro(final TeXParser tp)
			throws ParseException {
	...
	public static final Atom bf_macro(final TeXParser tp)
			throws ParseException {
		return new BoldAtom(
				new RomanAtom(new TeXFormula(tp, tp.getOverArgument(), null,
						false, tp.isIgnoreWhiteSpace()).root));
	}
	...
	public static final Atom tt_macro(final TeXParser tp)
			throws ParseException {
	...
	public static final Atom it_macro(final TeXParser tp)
			throws ParseException {
	...
	public static final Atom sf_macro(final TeXParser tp)
			throws ParseException {
		return new SsAtom(new TeXFormula(tp, tp.getOverArgument(), null, false,
				tp.isIgnoreWhiteSpace()).root);
	}

	public static final Atom LaTeX_macro() throws ParseException {
		return new LaTeXAtom();
	}

	// public static final Atom GeoGebra_macro(final TeXParser tp, final
	...
	public static final Atom biggl_macro(final TeXParser tp,
			final String[] args) throws ParseException {
		Atom at = new TeXFormula(tp, args[1], false).root;
		if (!(at instanceof SymbolAtom)) {
			return at;
	...
	public static final Atom biggr_macro(final TeXParser tp,
			final String[] args) throws ParseException {
		Atom at = new TeXFormula(tp, args[1], false).root;
		if (!(at instanceof SymbolAtom)) {
			return at;
	...
	public static final Atom scalebox_macro(final TeXParser tp,
		...
		return new ScaleAtom(new TeXFormula(tp, args[2]).root,
				...
				args[3] == null ? Double.parseDouble(args[1])
	...
	public static final Atom raisebox_macro(final TeXParser tp,
			final String[] args) throws ParseException {
		Object[] raise = SpaceAtom.getLength(args[1]);
		if (raise.length == 1) {
			throw new ParseException(
	...
	public static final Atom definecolor_macro(final String[] args)
			throws ParseException {
		Color color = null;
		if ("gray".equals(args[2])) {
			...
			double k = 1 - cmyk[3];
			color = graphics.createColor(k * (1 - cmyk[0]), k * (1 - cmyk[1]),
					k * (1 - cmyk[2]));
		} else {
	...
	public static final Atom selection_macro(final TeXParser tp,
			final String[] args) throws ParseException {
		try {
			return new SelectionAtom(new TeXFormula(tp, args[1]).root,
	...
	public static final Atom cursor_macro(final String[] args)
			throws ParseException {
		try {
			return new CursorAtom(
					FactoryProvider.getInstance().getGraphicsFactory()
							.createColor(GraphicsFactory.CURSOR_RED,
	...
	public static final Atom cong_macro() throws ParseException {
		VRowAtom vra = new VRowAtom(SymbolAtom.get("equals"));
		vra.add(new SpaceAtom(TeXLength.Unit.MU, 0f, 1.5f, 0f));
		vra.add(SymbolAtom.get("sim"));
		vra.setRaise(TeXLength.Unit.MU, -1f);
		return new TypedAtom(TeXConstants.TYPE_RELATION,
				TeXConstants.TYPE_RELATION, vra);
	}

	public static final Atom doteq_macro() throws ParseException {
		Atom at = new UnderOverAtom(SymbolAtom.get("equals"),
				SymbolAtom.get("ldotp"), TeXLength.Unit.MU, 3.7f, false, true);
		return new TypedAtom(TeXConstants.TYPE_RELATION,
				TeXConstants.TYPE_RELATION, at);
	}

	// public static final Atom jlmDynamic_macro(final TeXParser tp, final
	// String[] args) throws ParseException {
	...
	// public static final Atom DeclareMathSizes_macro(final TeXParser tp, final
	// String[] args)
	// throws ParseException {
	// TeXFont.setMathSizes(Double.parseDouble(args[1]),
	// Double.parseDouble(args[2]),
	// Double.parseDouble(args[3]), Double.parseDouble(args[4]));
	// return null;
	// }

	// public static final Atom magnification_macro(final TeXParser tp, final
	// String[] args)
	// throws ParseException {
	...
	public static final Atom hline_macro(final TeXParser tp)
			throws ParseException {
		if (!tp.isArrayMode()) {
			throw new ParseException(
	...
	public static final Atom size_macros(final TeXParser tp,
			final String[] args) throws ParseException {
		double f = 1f;
	...
	public static final Atom jlatexmathcumsub_macro(final TeXParser tp,
		...
		return new CumulativeScriptsAtom(tp.getLastAtom(),
				new TeXFormula(tp, args[1]).root, null);
	}

	public static final Atom dotminus_macro() throws ParseException {
		Atom at = new UnderOverAtom(SymbolAtom.get("minus"),
				SymbolAtom.get("normaldot"), TeXLength.Unit.MU, -3.3f, false,
				true);
		return new TypedAtom(TeXConstants.TYPE_BINARY_OPERATOR,
				TeXConstants.TYPE_BINARY_OPERATOR, at);
	}

	public static final Atom ratio_macro() throws ParseException {
		Atom at = new UnderOverAtom(SymbolAtom.get("normaldot"),
				SymbolAtom.get("normaldot"), TeXLength.Unit.MU, 5.2f, false,
				true);
		return new TypedAtom(TeXConstants.TYPE_RELATION,
				TeXConstants.TYPE_RELATION, at);
	}

	public static final Atom geoprop_macro() throws ParseException {
		RowAtom ddot = new RowAtom(SymbolAtom.get("normaldot"));
		ddot.add(new SpaceAtom(TeXLength.Unit.MU, 4f, 0f, 0f));
		ddot.add(SymbolAtom.get("normaldot"));
		Atom at = new UnderOverAtom(SymbolAtom.get("minus"), ddot,
				TeXLength.Unit.MU, -3.4f, false, ddot, TeXLength.Unit.MU, -3.4f,
				false);
		return new TypedAtom(TeXConstants.TYPE_RELATION,
				TeXConstants.TYPE_RELATION, at);
	}

	public static final Atom minuscolon_macro() throws ParseException {
		RowAtom at = new RowAtom(SymbolAtom.get("minus"));
		at.add(new SpaceAtom(TeXLength.Unit.EM, -0.095f, 0f, 0f));
	...
	public static final Atom minuscoloncolon_macro() throws ParseException {
		RowAtom at = new RowAtom(SymbolAtom.get("minus"));
		at.add(new SpaceAtom(TeXLength.Unit.EM, -0.095f, 0f, 0f));
	...
	public static final Atom simcolon_macro() throws ParseException {
	...
	public static final Atom simcoloncolon_macro() throws ParseException {
		RowAtom at = new RowAtom(SymbolAtom.get("sim"));
		at.add(new SpaceAtom(TeXLength.Unit.EM, -0.095f, 0f, 0f));
		Atom colon = new UnderOverAtom(SymbolAtom.get("normaldot"),
				SymbolAtom.get("normaldot"), TeXLength.Unit.MU, 5.2f, false,
	...
	public static final Atom approxcolon_macro() throws ParseException {
	...
	public static final Atom approxcoloncolon_macro() throws ParseException {
		RowAtom at = new RowAtom(SymbolAtom.get("approx"));
		at.add(new SpaceAtom(TeXLength.Unit.EM, -0.095f, 0f, 0f));
		Atom colon = new UnderOverAtom(SymbolAtom.get("normaldot"),
				SymbolAtom.get("normaldot"), TeXLength.Unit.MU, 5.2f, false,
	...
	public static final Atom equalscolon_macro() throws ParseException {
	...
	public static final Atom equalscoloncolon_macro() throws ParseException {
		RowAtom at = new RowAtom(SymbolAtom.get("equals"));
		at.add(new SpaceAtom(TeXLength.Unit.EM, -0.095f, 0f, 0f));
		Atom colon = new UnderOverAtom(SymbolAtom.get("normaldot"),
				SymbolAtom.get("normaldot"), TeXLength.Unit.MU, 5.2f, false,
	...
	public static final Atom colonminus_macro() throws ParseException {
		RowAtom at = new RowAtom(new UnderOverAtom(SymbolAtom.get("normaldot"),
				SymbolAtom.get("normaldot"), TeXLength.Unit.MU, 5.2f, false,
				true));
		at.add(new SpaceAtom(TeXLength.Unit.EM, -0.32f, 0f, 0f));
		at.add(SymbolAtom.get("minus"));
		return new TypedAtom(TeXConstants.TYPE_RELATION,
				TeXConstants.TYPE_RELATION, at);
	}

	public static final Atom coloncolonminus_macro() throws ParseException {
		Atom colon = new UnderOverAtom(SymbolAtom.get("normaldot"),
				SymbolAtom.get("normaldot"), TeXLength.Unit.MU, 5.2f, false,
				true);
	...
	public static final Atom colonequals_macro() throws ParseException {
		RowAtom at = new RowAtom(new UnderOverAtom(SymbolAtom.get("normaldot"),
				SymbolAtom.get("normaldot"), TeXLength.Unit.MU, 5.2f, false,
				true));
		at.add(new SpaceAtom(TeXLength.Unit.EM, -0.32f, 0f, 0f));
	...
	public static final Atom coloncolonequals_macro() throws ParseException {
		Atom colon = new UnderOverAtom(SymbolAtom.get("normaldot"),
				SymbolAtom.get("normaldot"), TeXLength.Unit.MU, 5.2f, false,
				true);
		RowAtom at = new RowAtom(colon);
	...
	public static final Atom coloncolon_macro() throws ParseException {
		Atom colon = new UnderOverAtom(SymbolAtom.get("normaldot"),
				SymbolAtom.get("normaldot"), TeXLength.Unit.MU, 5.2f, false,
				true);
		RowAtom at = new RowAtom(colon);
	...
	public static final Atom colonsim_macro() throws ParseException {
		RowAtom at = new RowAtom(new UnderOverAtom(SymbolAtom.get("normaldot"),
				SymbolAtom.get("normaldot"), TeXLength.Unit.MU, 5.2f, false,
				true));
		at.add(new SpaceAtom(TeXLength.Unit.EM, -0.32f, 0f, 0f));
	...
	public static final Atom coloncolonsim_macro() throws ParseException {
		Atom colon = new UnderOverAtom(SymbolAtom.get("normaldot"),
				SymbolAtom.get("normaldot"), TeXLength.Unit.MU, 5.2f, false,
				true);
		RowAtom at = new RowAtom(colon);
	...
	public static final Atom colonapprox_macro() throws ParseException {
		RowAtom at = new RowAtom(new UnderOverAtom(SymbolAtom.get("normaldot"),
				SymbolAtom.get("normaldot"), TeXLength.Unit.MU, 5.2f, false,
				true));
		at.add(new SpaceAtom(TeXLength.Unit.EM, -0.32f, 0f, 0f));
	...
	public static final Atom coloncolonapprox_macro() throws ParseException {
		Atom colon = new UnderOverAtom(SymbolAtom.get("normaldot"),
				SymbolAtom.get("normaldot"), TeXLength.Unit.MU, 5.2f, false,
				true);
		RowAtom at = new RowAtom(colon);
	...
	public static final Atom smallfrowneq_macro() throws ParseException {
		Atom at = new UnderOverAtom(SymbolAtom.get("equals"),
				SymbolAtom.get("smallfrown"), TeXLength.Unit.MU, -2f, true,
				true);
		return new TypedAtom(TeXConstants.TYPE_RELATION,
				TeXConstants.TYPE_RELATION, at);
	}
	...
	public static final Atom hstrok_macro(final TeXParser tp)
			throws ParseException {
		RowAtom ra = new RowAtom(
				new SpaceAtom(TeXLength.Unit.EX, -0.1f, 0f, 0f));
	...
	public static final Atom Hstrok_macro(final TeXParser tp)
			throws ParseException {
		RowAtom ra = new RowAtom(
				new SpaceAtom(TeXLength.Unit.EX, 0.28f, 0f, 0f));
	...
	public static final Atom dstrok_macro(final TeXParser tp)
			throws ParseException {
		RowAtom ra = new RowAtom(
				new SpaceAtom(TeXLength.Unit.EX, 0.25f, 0f, 0f));
		ra.add(SymbolAtom.get("bar"));
		VRowAtom vra = new VRowAtom(new LapedAtom(ra, 'r'));
		vra.setRaise(TeXLength.Unit.EX, -0.1f);
		RowAtom at = new RowAtom(vra);
	...
	public static final Atom kern_macro(final String[] args)
			throws ParseException {
		Object[] info = SpaceAtom.getLength(args[1]);
		if (info.length == 1) {
			throw new ParseException(
					"Error in getting kern in \\kern command !");
		}

		return new SpaceAtom((Unit) info[0], (Double) info[1], 0, 0);
	}
	...
	public static final Atom romannumeral_macro(final String[] args)
			throws ParseException {
		int[] numbers = { 1000, 900, 500, 400, 100, 90, 50, 40, 10, 9, 5, 4,
				1 };
	...
	public static final Atom textcircled_macro(final TeXParser tp,
			final String[] args) throws ParseException {
		return new TextCircledAtom(
				new RomanAtom(new TeXFormula(tp, args[1]).root));
	}
	...
	public static final Atom sc_macro(final TeXParser tp)
			throws ParseException {
		return new SmallCapAtom(new TeXFormula(tp, tp.getOverArgument(), null,
				false, tp.isIgnoreWhiteSpace()).root);
	}

	public static final Atom quad_macro() throws ParseException {
		return new SpaceAtom(TeXLength.Unit.EM, 1f, 0f, 0f);
	}

	public static final Atom qquad_macro() throws ParseException {
		return new SpaceAtom(TeXLength.Unit.EM, 2f, 0f, 0f);
	}
	...
	public static final Atom muskip_macros(final String[] args)
			throws ParseException {
		Muskip type = Muskip.THIN;
		if (args[0].equals(",")) {
			type = TeXConstants.Muskip.THIN;
		} else if (args[0].equals(":")) {
			type = TeXConstants.Muskip.MED;
		} else if (args[0].equals(";")) {
			type = TeXConstants.Muskip.THICK;
			...
			type = TeXConstants.Muskip.NEGTHIN;
		} else if (args[0].equals("negthinspace")) {
			type = TeXConstants.Muskip.NEGTHIN;
		} else if (args[0].equals("negmedspace")) {
			type = TeXConstants.Muskip.NEGMED;
		} else if (args[0].equals("negthickspace")) {
			type = TeXConstants.Muskip.NEGTHICK;
		}

		return new SpaceAtom(type);
	}

	public static final Atom surd_macro() throws ParseException {
	...
	public static final Atom iint_macro() throws ParseException {
		...
		ra.add(new SpaceAtom(TeXLength.Unit.MU, -6f, 0f, 0f));
		ra.add(integral);
	...
	public static final Atom idotsint_macro() throws ParseException {
		...
		integral.type_limits = TeXConstants.SCRIPT_NOLIMITS;
		RowAtom ra = new RowAtom(integral);
		ra.add(new SpaceAtom(TeXLength.Unit.MU, -1f, 0f, 0f));
	...
	public static final Atom insertBreakMark_macro() throws ParseException {
		...
		while ((pos = str.indexOf("$")) != -1) {
			if (pos < str.length() - 1) {
				...
				while (++start < str.length()
						&& java.lang.Character.isLetter(str.charAt(start))) {
					;
				}
