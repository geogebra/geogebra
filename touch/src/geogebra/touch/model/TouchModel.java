...
public class TouchModel {
  ...
  private void handleDraw(Point2D pointRW, boolean singlePointForIntersection) {
    ...
    case PerpendicularLine:
      ...
      break;
    case ParallelLine:
      newElements.add(this.kernel.getAlgoDispatcher().Line(null, (GeoPoint) this.getElement(Test.GEOPOINT), (GeoLine) this.getElement(Test.GEOLINE)));
      break;
    ...
    case ReflectObjectAboutLine:
      ...
      final GeoLine line = this.getNumberOf(Test.GEOLINE) > 1 ? (GeoLine) this.getElement(Test.GEOLINE, 1) : (GeoLine) this.getElement(Test.GEOLINE);
      this.deselect(line);
	...
	final AlgoJoinPointsSegment algoSegment = new AlgoJoinPointsSegment(this.kernel.getConstruction(), (GeoPoint) this.getElement(Test.GEOPOINT),
	...
	final AlgoCirclePointRadius algo = new AlgoCirclePointRadius(this.kernel.getConstruction(), null,
	    (GeoPoint) this.getElement(Test.GEOPOINT, 2), algoSegment.getSegment(), true);
	...
	final AlgoCirclePointRadius algo = new AlgoCirclePointRadius(this.kernel.getConstruction(), null, (GeoPoint) this.getElement(Test.GEOPOINT),
	    radius.getRadius());
  ...
  public boolean inputPanelClosed(String input) {
    ...
    if (this.command == ToolBarCommand.Slider) {
      ...
      final GeoElement slider = this.inputDialog.isNumber() ? new GeoNumeric(this.kernel.getConstruction()) : new GeoAngle(
	...
	break;
      ...
      case RotateObjectByAngle:
	final GeoPoint center = this.lastSelected() instanceof GeoPoint ? (GeoPoint) this.lastSelected() : (GeoPoint) this.getElement(Test.GEOPOINT);
  ...
  public boolean newInput(String input) {
	...
	if (!text.isTextCommand() && text.getStartPoint() == null) {
	  ...
	  final GeoPoint p = new GeoPoint(text.getConstruction(), null, (ev.getXmin() + ev.getXmax()) / 2, (ev.getYmin() + ev.getYmax()) / 2, 1.0);
