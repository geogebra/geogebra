...
public enum ToolBarCommand {
	...
	AttachDetachPoint(EuclidianConstants.MODE_ATTACH_DETACH, getLafIcons()
			.attach_detach_point(), StylingBarEntries.Point), ComplexNumbers(
			...
			StylingBarEntries.Point), PointOnObject(
	...
	LineThroughTwoPoints(EuclidianConstants.MODE_JOIN, getLafIcons()
			...
			getLafIcons().ray_through_two_points(), StylingBarEntries.Line), SegmentBetweenTwoPoints(
	...
	AngleBisector(EuclidianConstants.MODE_ANGULAR_BISECTOR, getLafIcons()
			.angle_bisector(), StylingBarEntries.Line), BestFitLine(
			EuclidianConstants.MODE_FITLINE, getLafIcons().best_fit_line(),
			StylingBarEntries.Line), Locus(EuclidianConstants.MODE_LOCUS,
			getLafIcons().locus(), StylingBarEntries.Line), ParallelLine(
			EuclidianConstants.MODE_PARALLEL, getLafIcons().parallel_line(),
			StylingBarEntries.Line), PerpendicularBisector(
	...
	Polygon(EuclidianConstants.MODE_POLYGON, getLafIcons().polygon(),
			StylingBarEntries.Polygon), RegularPolygon(
	...
	CircleThroughThreePoints(EuclidianConstants.MODE_CIRCLE_THREE_POINTS,
			getLafIcons().circle_through_three_points(), StylingBarEntries.Line), CircleWithCenterThroughPoint(
			...
			StylingBarEntries.Line), Semicircle(
	...
	FreehandShape(EuclidianConstants.MODE_FREEHAND_SHAPE, getLafIcons()
			.freehand_shape(), StylingBarEntries.Line), InsertImage(
			...
			StylingBarEntries.Line), RelationBetweenTwoObjects(
	...
	Move_Mobile(EuclidianConstants.MODE_MOVE, getLafIcons()
			.arrow_cursor_finger(), StylingBarEntries.Move), Select(-1,
			getLafIcons().arrow_cursor_grab(), StylingBarEntries.Move);
