...
public class TouchEventController implements TouchStartHandler, TouchMoveHandler, TouchEndHandler, MouseDownHandler, MouseUpHandler,
	...
	event.preventDefault();
	this.mc.onTouchEnd(getX(event.getChangedTouches().get(0)), this.getY(event.getChangedTouches().get(0)));
    }
	...
	if (event.getTouches().length() == 1) {
	    // proceed normally
	    this.mc.onTouchMove(getX(event.getTouches().get(0)), this.getY(event.getTouches().get(0)));
	...
	if (event.getTouches().length() == 1) {
	    ...
	    App.debug("Touch down" + getX(event.getTouches().get(0)) + "," + this.getY(event.getTouches().get(0)));
	    this.ignoreMouseEvents = true;
	    this.mc.onTouchStart(getX(event.getTouches().get(0)), this.getY(event.getTouches().get(0)));
	} else if (event.getTouches().length() == 2) {
