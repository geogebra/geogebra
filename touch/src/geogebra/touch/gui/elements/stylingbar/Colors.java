...
public class Colors extends FlowPanel {
	...
	private void addColorButton(final GColor color) {
		...
		button.getElement().setAttribute("style",
				"background: " + GColor.getColorString(color));
		...
		button.addDomHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				...
				if (Colors.this.touchModel.lastSelected() != null
						&& Colors.this.touchModel.isColorChangeAllowed()
						&& StyleBarStatic
								.applyColor(Colors.this.touchModel
										.getSelectedGeos(), color)) {
					Colors.this.touchModel.lastSelected().updateRepaint();
				}

				if (Colors.this.touchModel.getCommand().equals(
						ToolBarCommand.Pen)
						|| Colors.this.touchModel.getCommand().equals(
								ToolBarCommand.FreehandShape)) {
					Colors.this.stylingBar.euclidianView
