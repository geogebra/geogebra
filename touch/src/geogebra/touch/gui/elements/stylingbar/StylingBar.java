...
public class StylingBar extends DecoratorPanel {
	...
	public StylingBar(TouchModel touchModel, EuclidianViewM view,
		...
		this.showHide.addHandler(new MouseDownHandler() {
			@Override
			public void onMouseDown(MouseDownEvent event) {
				StylingBar.this.onTouchStartShowHide(event, euclidianViewPanel);
			}
		}, MouseDownEvent.getType());

		this.getElement().getStyle()
				.setBackgroundColor(GColor.WHITE.toString());

		EuclidianStyleBarStatic.lineStyleArray = EuclidianView.getLineTypes();
		if (this.guiModel.getCommand() != null) {
			this.rebuild(this.guiModel.getCommand().getStylingBarEntries());
		}
	...
	private StandardImageButton createStyleBarButton(final String process,
		...
		newButton.addHandler(TouchEntryPoint.getLookAndFeel()
				.getStyleBarButtonHandler(this, newButton, process),
				TouchEntryPoint.getLookAndFeel().getStylBarEventType());

		return newButton;
	}

	public void onTouchStartOptionalButton(
			DomEvent<? extends EventHandler> event,
			StandardImageButton eventSource, OptionType type) {
		...
		if (StylingBar.this.guiModel.getOptionTypeShown().equals(type)) {
			...
			final ColorBarBackground colorBar = new ColorBarBackground(
					StylingBar.this, StylingBar.this.touchModel);

			// includes closeOptions()
			StylingBar.this.guiModel.showOption(new OptionsBox(colorBar),
					OptionType.Color,
					StylingBar.this.button[StylingBar.this.colorButtonIndex]);
		} else if (type.equals(OptionType.LineStyle)) {
			StylingBar.this.guiModel.showOption(new OptionsBox(
					new LineStyleBar(StylingBar.this.touchModel,
							StylingBar.this)), OptionType.LineStyle,
					eventSource);
		} else if (type.equals(OptionType.CaptionStyle)) {
			StylingBar.this.guiModel.showOption(new OptionsBox(new CaptionBar(
					StylingBar.this.touchModel)), OptionType.CaptionStyle,
					eventSource);
		}
	}

	public void onTouchStartShowHide(DomEvent<? extends EventHandler> event,
		...
		if (StylingBar.this.visible) {
			...
			StylingBar.this.contentPanel.clear();
			for (final StandardImageButton b : StylingBar.this.button) {
	...
	public void onTouchStartStyleBarButton(
			DomEvent<? extends EventHandler> event,
	...
	public void rebuild() {
		if (this.lastCommand != null
				...
				&& this.lastCommand.equals(this.guiModel.getCommand())) {
			return;
		}
		...
		if (this.guiModel.getCommand() != null
				&& !this.rebuild(this.guiModel.getCommand()
						.getStylingBarEntries())) {
			this.clear();
	...
	private boolean rebuild(StylingBarEntries entry) {
		...
		if (entry == StylingBarEntries.Move
		...
		for (int i = 0; i < resource.length; i++) {
			if (resource[i].equals(LafIcons.label())) {
				...
				b[i].addHandler(
						TouchEntryPoint.getLookAndFeel()
								.getOptionalButtonHandler(this, b[i],
				...
				b[i].addHandler(
						TouchEntryPoint.getLookAndFeel()
								.getOptionalButtonHandler(this, b[i],
										OptionType.LineStyle), TouchEntryPoint
		...
		if (this.visible) {
			for (final StandardImageButton imageButton : this.button) {
	...
	public void updateColor(String color) {
		if (this.button[this.colorButtonIndex] != null) {
			this.button[this.colorButtonIndex].getElement().getStyle()
