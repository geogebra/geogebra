...
public class StylingBar extends PopupPanel {
	...
	public StylingBar(TouchModel touchModel, EuclidianViewM view, final EuclidianViewPanel euclidianViewPanel) {
		...
		this.showHide.addDomHandler(TouchEntryPoint.getLookAndFeel().getStyleBarHandlerShowHide(this, euclidianViewPanel), TouchEntryPoint
	...
	private boolean rebuild(StylingBarEntries entry) {
		...
		for (int i = 0; i < resource.length; i++) {
			if (resource[i].equals(LafIcons.label())) {
				...
				b[i].addDomHandler(TouchEntryPoint.getLookAndFeel().getOptionalButtonHandler(this, b[i], OptionType.CaptionStyle), TouchEntryPoint
						.getLookAndFeel().getStylBarEventType());
			} else if (resource[i].equals(LafIcons.properties_default())) {
				b[i] = new StandardImageButton(LafIcons.properties_default());

				b[i].addDomHandler(TouchEntryPoint.getLookAndFeel().getOptionalButtonHandler(this, b[i], OptionType.LineStyle), TouchEntryPoint
						.getLookAndFeel().getStylBarEventType());
