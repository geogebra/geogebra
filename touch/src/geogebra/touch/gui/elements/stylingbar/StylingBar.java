 ...
 * 
 */
public class StylingBar extends PopupPanel {
	...
	// euclidianViewPanel), TouchEntryPoint
	// .getLookAndFeel().getStylBarEventType());

	this.showHide.addDomHandler(new TouchStartHandler() {
	    @Override
	    public void onTouchStart(TouchStartEvent event) {
	...
	newButton.addDomHandler(TouchEntryPoint.getLookAndFeel().getStyleBarButtonHandler(this, newButton, process), TouchEntryPoint.getLookAndFeel()
		.getStylBarEventType());
	...
	if (StylingBar.this.guiModel.getOptionTypeShown().equals(type)) {
	    ...
	    final ColorBarBackground colorBar = new ColorBarBackground(StylingBar.this, StylingBar.this.touchModel);
    ...
    public void rebuild() {
	if (this.lastCommand != null && this.guiModel.getCommand() != ToolBarCommand.Move_Mobile
	...
	if (this.guiModel.getCommand() != null && !this.rebuild(this.guiModel.getCommand().getStylingBarEntries())) {
	    this.clear();
	...
	for (int i = 0; i < resource.length; i++) {
		...
		// b[i].addHandler(TouchEntryPoint.getLookAndFeel().getOptionalButtonHandler(this,
		// b[i], OptionType.CaptionStyle), TouchEntryPoint
		// .getLookAndFeel().getStylBarEventType());
		final int index = i;
		b[i].addDomHandler(new TouchStartHandler() {
		    @Override
		    public void onTouchStart(final TouchStartEvent event) {
			onTouchStartOptionalButton(event, b[index], OptionType.CaptionStyle);
		    }
		}, TouchStartEvent.getType());
	    } else if (resource[i].equals(LafIcons.properties_default())) {
		b[i] = new StandardImageButton(LafIcons.properties_default());

		// b[i].addDomHandler(TouchEntryPoint.getLookAndFeel().getOptionalButtonHandler(this,
		// b[i], OptionType.LineStyle), TouchEntryPoint
		// .getLookAndFeel().getStylBarEventType());
		final int index = i;
		b[i].addDomHandler(new TouchStartHandler() {
		    @Override
		    public void onTouchStart(final TouchStartEvent event) {
			onTouchStartOptionalButton(event, b[index], OptionType.LineStyle);
		    }
		}, TouchStartEvent.getType());
		...
		b[i].addDomHandler(TouchEntryPoint.getLookAndFeel().getOptionalButtonHandler(this, b[i], OptionType.Color), TouchEntryPoint
			.getLookAndFeel().getStylBarEventType());
