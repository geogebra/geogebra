...
public class AlgebraViewM extends AlgebraViewWeb {
  ...
  public void setUserObject(TreeItem ti, final Object ob) {
    ...
    if (ob instanceof GeoElement) {
      ti.setWidget(new RadioButtonTreeItemT((GeoElement) ob, getLafIcons().algebra_shown().getSafeUri(), DefaultResources.INSTANCE.algebra_hidden()
	  .getSafeUri(), null, this.controller));
  ...
  public void updateFonts() {
    ...
    this.getStyleElement().getStyle().setFontStyle(Style.FontStyle.valueOf(font.isItalic() ? "ITALIC" : "NORMAL"));
    this.getStyleElement().getStyle().setFontSize(font.getSize(), Style.Unit.PX);
    this.getStyleElement().getStyle().setFontWeight(Style.FontWeight.valueOf(font.isBold() ? "BOLD" : "NORMAL"));
  }
