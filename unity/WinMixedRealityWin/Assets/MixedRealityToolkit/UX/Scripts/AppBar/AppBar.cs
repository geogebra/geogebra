...
namespace MixedRealityToolkit.UX.AppBarControl
{
    public class AppBar : InteractionReceiver
    {
        ...
        /// </summary>
        [Tooltip("Default value was 0.25")]
        public float HoverOffsetYScale = 0.25f;

        /// <summary>
        /// Pushes the app bar away from the object
        /// </summary>
        [Tooltip("Pushes the app bar away from the object. Default value was 0.25")]
        ...
        private void FollowBoundingBox(bool smooth)
        {
            ...
            if (finalForward == boundingBox.transform.forward)
            {
                Debug.Log("boundingBox.transform.forward");
            }
            else if (finalForward == boundingBox.transform.right)
            {
                Debug.Log("boundingBox.transform.right");
            }
            else if (finalForward == -boundingBox.transform.forward)
            {
                Debug.Log("-boundingBox.transform.forward");
            }
            else if (finalForward == -boundingBox.transform.right)
