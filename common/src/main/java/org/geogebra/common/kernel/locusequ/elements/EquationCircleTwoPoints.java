...
/**
 * @author sergio EquationElement for {@link AlgoCircleTwoPoints}
 */
public class EquationCircleTwoPoints extends EquationGenericConic {
	 ...
	 *            {@link EquationScope}
	 */
	public EquationCircleTwoPoints(final GeoElement element,
	...
	protected void computeMatrix() {
		...
		AlgoCircleTwoPoints algo = (AlgoCircleTwoPoints) this.getResult()
				.getParentAlgorithm();

		EquationPoint ext = this.getScope()
				.getPoint((GeoPoint) algo.getExternalPoint());
		EquationPoint center = this.getScope()
				.getPoint((GeoPoint) algo.getCenter());

		matrix[0] = EquationNumericValue.from(1);
		matrix[1] = EquationNumericValue.from(1);
		matrix[2] = diff(sum(
				times(center.getXExpression(), center.getXExpression()),
