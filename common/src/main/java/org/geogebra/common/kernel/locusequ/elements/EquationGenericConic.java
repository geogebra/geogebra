...
import static org.geogebra.common.kernel.locusequ.arith.EquationArithHelper.dbl;
import static org.geogebra.common.kernel.locusequ.arith.EquationArithHelper.det3;
...
/**
 * @author sergio Common base class for EquationElements for conics. All
 *         constructor subclasses must call this.computeMatrix() at the end.
 */
public abstract class EquationGenericConic extends EquationElement {
	 ...
	 *            {@link EquationScope}
	 */
	public EquationGenericConic(final GeoElement conic,
	 ...
	 * @return matrix.
	 */
	public EquationExpression[] getMatrix() {
		return this.matrix;
	}
	...
	protected EquationList forPointImpl(EquationPoint p) {
		return equation(sum(
				times(this.getMatrix()[0],
						pow(p.getXExpression(), EquationNumericValue.from(2))),
				times(this.getMatrix()[1],
						pow(p.getYExpression(), EquationNumericValue.from(2))),
				this.getMatrix()[2],
				times(dbl(this.getMatrix()[3]), p.getXExpression(),
						p.getYExpression()),
	 ...
	 *            one vector (length 3)
	 * @param b
	 *            another vector (length 3)
	 * @return b * matrix * a
	 */
	protected EquationExpression applyVectorsToMatrix(EquationExpression[] a,
			EquationExpression[] b) {
		return sum(
				times(b[0],
						sum(times(a[0], this.getMatrix()[0]),
								times(a[1], this.getMatrix()[3]),
								times(a[2], this.getMatrix()[4]))),
				times(b[1],
						sum(times(a[0], this.getMatrix()[3]),
								times(a[1], this.getMatrix()[1]),
								times(a[2], this.getMatrix()[5]))),
				times(b[2],
						sum(times(a[0], this.getMatrix()[4]),
	 ...
	 * 
	 * @param p
	 *            an {@link EquationPoint}
	 * @return a 3-long vector containing expressions for the polar line.
	 */
	public EquationExpression[] getPolarLineCoefficientsFor(
			final EquationPoint p) {
		return new EquationExpression[] {
				sum(times(p.getXExpression(), getMatrix()[0]),
						times(p.getYExpression(), getMatrix()[3]),
						getMatrix()[4]),
				sum(times(p.getXExpression(), getMatrix()[3]),
						times(p.getYExpression(), getMatrix()[1]),
						getMatrix()[5]),
				sum(times(p.getXExpression(), getMatrix()[4]),
						times(p.getYExpression(), getMatrix()[5]),
						getMatrix()[2]) };
	}
	 ...
	 */
	protected EquationExpression[] matrixForCircle(final EquationPoint center,
			final EquationExpression radius2) {
		EquationExpression[] circleMatrix = new EquationExpression[6];

		circleMatrix[0] = circleMatrix[1] = EquationNumericValue.from(1);
		circleMatrix[2] = diff(
				sum(sqr(center.getXExpression()), sqr(center.getYExpression())),
				radius2);
	 ...
	 * @return a 6-long vector.
	 */
	protected EquationExpression[] matrixForCircle(final EquationPoint a,
			final EquationPoint b, final EquationPoint c) {
		EquationExpression[] circleMatrix = new EquationExpression[6];

		circleMatrix[0] = det3(a.getXExpression(), a.getYExpression(),
				EquationNumericValue.from(1), b.getXExpression(),
				b.getYExpression(), EquationNumericValue.from(1),
				c.getXExpression(), c.getYExpression(),
		...
		circleMatrix[3] = EquationNumericValue.from(0);
		circleMatrix[4] = div(
				det3(a.getYExpression(), ssqr(a), EquationNumericValue.from(1),
						b.getYExpression(), ssqr(b),
						EquationNumericValue.from(1), c.getYExpression(),
						ssqr(c), EquationNumericValue.from(1)),
				EquationNumericValue.from(2));
		circleMatrix[5] = div(
				det3(a.getXExpression(), ssqr(a), EquationNumericValue.from(1),
						b.getXExpression(), ssqr(b),
						EquationNumericValue.from(1), c.getXExpression(),
						ssqr(c), EquationNumericValue.from(1)),
