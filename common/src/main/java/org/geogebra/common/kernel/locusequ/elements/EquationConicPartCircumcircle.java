...
/**
 * @author sergio EquationElement for {@link AlgoConicPartCircumcircle}
 */
public class EquationConicPartCircumcircle extends EquationGenericConicPart {
	 ...
	 *            {@link EquationScope}
	 */
	public EquationConicPartCircumcircle(final GeoElement conic,
	...
	protected void computeMatrix() {
		...
		AlgoConicPartCircumcircle algo = (AlgoConicPartCircumcircle) this
	...
	protected EquationList forPointImpl(EquationPoint p) {
		...
		if (this.isSector()) {
			...
			EquationGenericLine l = new EquationLineBisector(A, B,
					this.getScope());
			EquationGenericLine m = new EquationLineBisector(A, C,
					this.getScope());

			EquationAuxiliarSymbolicPoint center = l.getNewIncidentPoint();
			center.addIncidence(m);

			el.addAll(new EquationJoinPoints(center, A, this.getScope())
					.forPoint(p));
			el.addAll(new EquationJoinPoints(center, C, this.getScope())
					.forPoint(p));
