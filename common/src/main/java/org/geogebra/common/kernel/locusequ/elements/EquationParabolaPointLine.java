...
/**
 * @author sergio {@link EquationElement} for {@link AlgoParabolaPointLine}
 */
public class EquationParabolaPointLine extends EquationGenericConic {
	 ...
	 *            {@link EquationScope}
	 */
	public EquationParabolaPointLine(final GeoElement conic,
	...
	protected void computeMatrix() {
		...
		AlgoParabolaPointLine algo = (AlgoParabolaPointLine) this.getResult()
				.getParentAlgorithm();

		EquationGenericLine line = (EquationGenericLine) this.getScope()
				.getElement((GeoElement) algo.getLine());
		...
		matrix[2] = diff(
				times(a2b2,
						sum(sqr(f.getXExpression()), sqr(f.getYExpression()))),
				sqr(line.getC()));
		matrix[3] = times(line.getA(), line.getB()).getOpposite();
		matrix[4] = sum(times(a2b2, f.getX()), times(line.getA(), line.getC()))
				.getOpposite();
		matrix[5] = sum(times(a2b2, f.getY()), times(line.getB(), line.getC()))
				.getOpposite();
