 ...
 *
 */
public class SymbolicVector extends EquationPoint
	...
	public boolean isSecondCoordinateZero() {
		return !this.getY().containsSymbolicValues()
				&& this.getY().computeValue() == 0.0;
	}
	...
	public boolean isThirdCoordinateZero() {
		return !this.getZ().containsSymbolicValues()
				&& this.getZ().computeValue() == 0.0;
	}
	...
	public EquationExpression getModule() {
		if (this.module == null) {
			this.module = sqrt(EquationArithHelper.sum(
					sqr(diff(this.a.getX(), this.b.getX())),
	...
	public SymbolicVector getUnitary() {
		if (this.unitary == null) {
			this.unitary = new SymbolicVector(
					new EquationUnitaryPoint(this.a, this.getModule()),
