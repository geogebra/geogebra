...
public class AlgoDependentGeoCopy extends AlgoElement implements DependentAlgo {
	...
	final public String toString(StringTemplate tpl) {
		...
		if (tpl.hasType(StringType.GEOGEBRA_XML) && !origGeo.isLabelSet()
				...
				&& ((GeoLine) origGeo).getEquationForm() == EquationLinear.Form.PARAMETRIC) {
			((GeoLine) origGeo).setEquationForm(EquationLinear.Form.EXPLICIT);
			String ret = origGeo.getLabel(tpl);
			((GeoLine) origGeo).setEquationForm(EquationLinear.Form.PARAMETRIC);
			return ret;
		}
