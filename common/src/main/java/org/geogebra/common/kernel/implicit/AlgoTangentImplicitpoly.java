...
public class AlgoTangentImplicitpoly extends AlgoElement
	...
	public AlgoTangentImplicitpoly(Construction c, String[] labels,
		...
		if (!pointOnPath) {
			AlgoImplicitPolyTangentCurve algoTangentPoly = new AlgoImplicitPolyTangentCurve(
					c, p, R);
	...
	public void compute() {
		...
		for (int i = 0; i < ip.length; i++) {
			...
			tangents.adjustOutputSize(n + 1);
			tangents.getElement(n).setCoords(ip[i].getY() - this.R.getInhomY(),
					this.R.getInhomX() - ip[i].getX(),
					ip[i].getX() * this.R.getInhomY()
							- this.R.getInhomX() * ip[i].getY());
