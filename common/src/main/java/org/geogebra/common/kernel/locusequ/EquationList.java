...
/**
 * Represents a list of equations. There are some algos represented by more than
 * one equation (intersection between two objects, for example). This class
 * should be used in general instead of {@link Equation} when dealing with
...
public class EquationList extends ArrayList<Equation> {
	...
	/*
	 * A subclass that guarantees that objects cannot be added nor removed (and
	 * so it will be always empty).
	 */
	private static class EmptyEquationList extends EquationList {
		private static final long serialVersionUID = 1L;

		public EmptyEquationList() {
			super(0);
		}
	...
	/**
	 * Creates an empty {@link EquationList} with space for <code>i</code>
	 * items.
	 * 
	 * @param i
	 *            size of initial array.
	 */
	public EquationList(int i) {
		super(i);
	}
	 ...
	 * 
	 * @param c
	 *            Collection whose items will be imported.
	 */
	...
	/**
	 * Returns <code>true</code> if all elements in this collection are
	 * algebraic. <code>false</code> otherwise.
	 * 
	...
	/**
	 * Returns an empty {@link EquationList}. This list is a special one.
	 * Elements cannot be added nor removed from it, so it is always empty.
	 * 
