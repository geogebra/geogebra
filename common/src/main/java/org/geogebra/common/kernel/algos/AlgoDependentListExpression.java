...
public class AlgoDependentListExpression extends AlgoElement implements
	 ...
	 */
	public AlgoDependentListExpression(Construction cons, String label,
			ExpressionNode root) {
		this(cons, root);
		list.setLabel(label);
	}
	...
	public final void compute() {
		...
		for (int i = 0; i < evalListSize; i++) {
			...
			// eg {{1,3,5},{2,4,6}}*{{11,14},{12,15},{13,a}}
				...
				/*
				 * removed Michael Borcherds 20080602 bug: 9PointCubic.ggb
				 * (matrix multiplication) // try to use cached element of type
				 * GeoList GeoList list2 = null; if (i < cachedListSize) {
				 * GeoElement cachedGeo = list.getCached(i);
				 * 
				...
				if (i < cachedListSize) {
					...
					if (cachedGeo.getGeoClassType().equals(
							geo0.getGeoClassType())) {
						cachedGeo.set(geo0);
						geo = cachedGeo;
					}
				}
