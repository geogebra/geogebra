...
/**
 * @author sergio Represents the square root of a given expression.
 */
public class EquationSqrtOperator extends EquationUnaryOperator {
	 ...
	 * 
	 * @see
	 * geogebra.common.kernel.locusequ.arith.EquationUnaryOperator#operation(
	 * double)
	 */
	@Override
	...
	protected <T> T translateImpl(EquationTranslator<T> translator) {
		if (this.getOriginalExpression().containsSymbolicValuesImpl()) {
			return translator
					.sqrt(this.getOriginalExpression().translate(translator));
		}
		return translator
				.number(Math.sqrt(this.getOriginalExpression().computeValue()));
	}
}
