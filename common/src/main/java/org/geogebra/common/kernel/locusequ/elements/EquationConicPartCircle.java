...
public class EquationConicPartCircle extends EquationGenericConicPart {
	 ...
	 *            {@link EquationScope}
	 */
	public EquationConicPartCircle(final GeoElement conic,
	...
	protected void computeMatrix() {
		...
		AlgoConicPartCircle algo = (AlgoConicPartCircle) this.getResult()
				.getParentAlgorithm();

		EquationPoint center = this.getScope().getPoint(algo.getCenter());
		EquationPoint startPoint = this.getScope()
				.getPoint(algo.getStartPoint());
	...
	protected EquationList forPointImpl(EquationPoint p) {
		...
		if (this.isSector()) {
			...
			EquationPoint startPoint = this.getScope()
					.getPoint(algo.getStartPoint());
			EquationPoint endPoint = this.getScope()
					.getPoint(algo.getEndPoint());

			el.addAll(
					new EquationJoinPoints(center, startPoint, this.getScope())
							.forPoint(p));
			el.addAll(new EquationJoinPoints(center, endPoint, this.getScope())
					.forPoint(p));
