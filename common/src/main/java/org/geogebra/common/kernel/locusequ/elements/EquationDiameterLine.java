...
/**
 * @author sergio EquationElement for {@link AlgoDiameterLine}
 */
public class EquationDiameterLine extends EquationGenericLine {
	 ...
	 *            {@link EquationScope}
	 */
	public EquationDiameterLine(final GeoElement line,
			final EquationScope scope) {
		super(line, scope);

		AlgoDiameterLine algo = (AlgoDiameterLine) this.getResult()
				.getParentAlgorithm();

		EquationGenericLine l = (EquationGenericLine) this.getScope()
				.getElement((GeoElement) algo.getLine());
		EquationGenericConic c = (EquationGenericConic) this.getScope()
				.getElement(algo.getConic());
