...
/**
 * @author sergio EquationElement for {@link AlgoJoinPointsRay}
 */
public class EquationJoinPointsRay extends EquationGenericRay {
	 ...
	 *            {@link EquationScope}
	 */
	public EquationJoinPointsRay(final GeoElement element,
			final EquationScope scope) {
		super(element, scope);

		AlgoJoinPointsRay algo = (AlgoJoinPointsRay) element
				.getParentAlgorithm();

		this.setPoint(algo.getP());
		this.setVector(new SymbolicVector(this.getEquationPoint(),
				this.getScope().getPoint(algo.getQ())));
	}
}
