...
/**
 * @author sergio EquationElement for {@link AlgoTangentPoint}
 */
public class EquationTangentPoint extends EquationGenericLine {
	 ...
	 *            {@link EquationScope}
	 */
	public EquationTangentPoint(final GeoElement line,
			final EquationScope scope) {
		super(line, scope);

		AlgoTangentPoint algo = (AlgoTangentPoint) this.getResult()
				.getParentAlgorithm();

		GeoPoint p = algo.getPoint();

		this.setPoint(p);
		EquationGenericConic conic = (EquationGenericConic) this.getScope()
