...
/**
 * @author sergio A restriction is a special kind of EquationElement. In
 *         general, an EquationElement retrieves a list of equations given a
 *         point. A EquationRestriction has a GeoElement that is itself a point
 *         so it can generate a list of equations without any other point.
 * 
 *         The resst of EquationElements are there just for help.
 * 
 *         Example:
 * 
 *         When building a circle, the important part is the conic and it does
 *         not generate any kind of restriction in the locus. A point in that
 *         conic is generated by AlgoIntersectConic, and the important part of
 *         that construction is the algo, not the geo (always a point).
 */
public abstract class EquationRestriction extends EquationElement {
	 ...
	 *            {@link EquationScope}
	 */
	public EquationRestriction(final GeoElement element, final AlgoElement algo,
	...
	/**
	 * Computes and sets the equation list for this restriction. Though, it is
	 * encourage just to override forPointImpl.
	 */
	protected void computeEquationList() {
		...
		for (GeoElement out : output) {
			if (out.isGeoPoint()) {
				list.addAll(this
						.forPoint(this.getScope().getPoint((GeoPoint) out)));
			}
		}
