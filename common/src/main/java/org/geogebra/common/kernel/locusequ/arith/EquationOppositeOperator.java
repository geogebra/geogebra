...
/**
 * @author sergio Represents the opposite of a given expression.
 */
public class EquationOppositeOperator extends EquationUnaryOperator {
	 ...
	 * 
	 * @Override protected <T> T translateImpl(EquationTranslator<T> translator)
	 * { T orig = this.getOriginalExpression().translate(translator); return
	 * translator.opposite(orig); }
	 */
	...
	protected <T> T translateImpl(EquationTranslator<T> translator) {
		return translator
				.opposite(this.getOriginalExpression().translate(translator));
	}
}
