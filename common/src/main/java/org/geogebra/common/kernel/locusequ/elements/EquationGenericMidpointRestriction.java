...
/**
 * @author sergio Base class for all midpoint restrictions. Subclasses need to
 *         set ends and midpoint in initialization.
 */
public abstract class EquationGenericMidpointRestriction
		extends EquationRestriction {
	 ...
	 *            {@link EquationScope}
	 */
	public EquationGenericMidpointRestriction(final GeoElement geo,
	...
	protected EquationList forPointImpl(EquationPoint p) {
		...
		EquationPoint mEqu = this.getScope().getPoint(getMidpoint()); // middle
																		// point
		EquationList res = new EquationList(2);

		res.add(equation(middle(pEqu.getXExpression(), qEqu.getXExpression(),
				mEqu.getXExpression())));
		res.add(equation(middle(pEqu.getYExpression(), qEqu.getYExpression(),
				mEqu.getYExpression())));
