...
/**
 * @author sergio Scope for points. Maps points to symbolic representation.
 */
public class EquationPointMap {

	/*
	 * Because EquationPoint holds three coordinates but for this project only
	 * two are needed, GEOGEBRA_DIMENSION is defined.
	 */
	private static final int GEOGEBRA_DIMENSION = 2;
	private GeoPoint locusPoint, movingPoint; // TODO: Look into using
												// GeoPointND
	private int curInd;
	...
	/**
	 * @param locusPoint
	 *            locusPoint from locus.
	 * @param movingPoint
	 *            movingPoint from locus.
	 * @param scope
	 *            scope.
	 */
	public EquationPointMap(GeoPoint locusPoint, GeoPoint movingPoint,
			EquationScope scope) {
		this(scope);
	 ...
	 *            scope.
	 */
	public EquationPointMap(GeoElement path, GeoPoint movingPoint,
			EquationScope scope) {
		this(scope);
	...
	/**
	 * @param p
	 *            The point whose {@link EquationPoint} you are looking for.
	 * @return The {@link EquationPoint} for p. If p is not in the map, returns
	 *         null.
	 */
	...
	private EquationPoint constructEquationPoint(GeoPoint geoPoint) {
		...
		if (mustTakeNumericCoordinates(geoPoint)) {
			...
			Log.info("[LocusEquation] Point " + geoPoint.getLabelSimple()
					+ " index " + this.curInd);
	...
	/**
	 * Restricts two different {@link GeoPoint} to use the same variables.
	 * 
	 * @param orig
	 *            Original point.
	 * @param target
	 *            Target point. Its coordinates will be preserved.
	 */
	...
	protected boolean mustTakeNumericCoordinates(final GeoPoint p) {
		return p.isIndependent()
				|| (isPointOnPath(p) && p != this.movingPoint
						&& !hasMovingPointAsPredecessor(p))
				|| isIntersectionOfAxis(p) || isAuxiliarPointOnAPolygon(p)
	...
	protected boolean isIntersectionOfAxis(GeoPoint p) {
		...
		// AlgoIntersectLines algo = (AlgoIntersectLines)
		// p.getParentAlgorithm();

		// FIXME: wait for an email answer.
		return false; /*
						 * (algo.getg() instanceof GeoAxis && algo.geth()
						 * instanceof GeoAxis);
						 */
	}
	...
	/**
	 * Check if p is the exact same point object as the moving point. Equality
	 * is checked with ==.
	 * 
	 * @param p
	 *            point to check.
