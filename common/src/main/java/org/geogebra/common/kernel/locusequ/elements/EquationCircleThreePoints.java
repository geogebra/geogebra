...
/**
 * @author sergio EquationElement for AlgoCircleThreePoints.
 */
public class EquationCircleThreePoints extends EquationGenericConic {
	 ...
	 *            {@link EquationScope}
	 */
	public EquationCircleThreePoints(final GeoElement circle,
			final EquationScope scope) {
		super(circle, scope);
		this.setScope(scope);
		this.setResult(circle);
		AlgoCircleThreePoints algo = (AlgoCircleThreePoints) circle
				.getParentAlgorithm();
		this.p = algo.getA();
		this.q = algo.getB();
		this.r = algo.getC();
