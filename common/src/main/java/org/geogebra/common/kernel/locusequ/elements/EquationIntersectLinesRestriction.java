...
/**
 * @author sergio Restriction for {@link AlgoIntersectLines
 */
public class EquationIntersectLinesRestriction extends EquationRestriction {
	 ...
	 *            element.
	 * @param algo
	 *            {@link AlgoIntersectLines}
	 * @param scope
	 *            {@link EquationScope}
	 */
	public EquationIntersectLinesRestriction(final GeoElement geo,
			final AlgoIntersectLines algo, final EquationScope scope) {
		super(geo, algo, scope);
	}

	@Override
	public AlgoIntersectLines getAlgo() {
		return (AlgoIntersectLines) super.getAlgo();
	}
	...
	protected EquationList forPointImpl(EquationPoint p) {
		...
		list.addAll(
				this.getScope().getElement(this.getAlgo().getg()).forPoint(p));
		list.addAll(
				this.getScope().getElement(this.getAlgo().geth()).forPoint(p));
