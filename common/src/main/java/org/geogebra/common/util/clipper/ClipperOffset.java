...
//    private LongPoint lowest;
    ...
    public void addPath( Path path, JoinType joinType, EndType endType ) {
        ...
        if (lowest.getX() < 0) {
            ...
            final DoublePoint ip = polyNodes.getChilds().get( (int) lowest.getX() ).getPolygon().get( (int) lowest.getY() );
    ...
    private void doMiter( int j, int k, double r ) {
        ...
        destPoly.add( new DoublePoint(  ( srcPoly.get( j ).getX() + (normals.get( k ).getX() + normals.get( j ).getX()) * q ), ( srcPoly.get( j ).getY() + (normals.get( k ).getY() + normals.get( j ).getY()) * q ) ) );
    }
        ...
        for (int i = 0; i < polyNodes.getChildCount(); i++) {
            ...
            if (len == 1) {
                if (node.getJoinType() == JoinType.ROUND) {
                    ...
                    for (int j = 1; j <= steps; j++) {
                        destPoly.add( new DoublePoint( ( srcPoly.get( 0 ).getX() + X * delta ), ( srcPoly.get( 0 ).getY() + Y
                                        * delta ) ) );
                        final double X2 = X;
                        X = X * cos - sin * Y;
                        Y = X2 * sin + Y * cos;
            ...
            if (node.getEndType() == EndType.CLOSED_POLYGON) {
                ...
                if (node.getEndType() == EndType.OPEN_BUTT) {
                    final int j = len - 1;
                    pt1 = new DoublePoint(( srcPoly.get( j ).getX() + normals.get( j ).getX() * delta ), ( srcPoly.get( j )
                ...
                if (node.getEndType() == EndType.OPEN_BUTT) {
                    pt1 = new DoublePoint(( srcPoly.get( 0 ).getX() - normals.get( 0 ).getX() * delta ), ( srcPoly.get( 0 )
    ...
    private void doRound( int j, int k ) {
        ...
        for (int i = 0; i < steps; ++i) {
            destPoly.add( new DoublePoint( ( srcPoly.get( j ).getX() + X * delta ), ( srcPoly.get( j ).getY() + Y * delta ) ) );
            X2 = X;
            X = X * cos - sin * Y;
            Y = X2 * sin + Y * cos;
        }
        destPoly.add( new DoublePoint( ( srcPoly.get( j ).getX() + normals.get( j ).getX() * delta ), ( srcPoly.get( j ).getY()
    ...
    private void doSquare( int j, int k ) {
        ...
        destPoly.add( new DoublePoint( ( sjx + delta * (nkx - nky * dx) ), ( sjy + delta * (nky + nkx * dx) ), 0 ) );
        destPoly.add( new DoublePoint( ( sjx + delta * (njx + njy * dx) ), ( sjy + delta * (njy - njx * dx) ), 0 ) );
    }
