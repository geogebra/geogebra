        ...
        if (IsSum) {
            for (int i = 0; i < pathCnt; i++) {
                ...
                for (final DoublePoint ip : pattern) {
                    p.add( new DoublePoint( path.get( i ).getX() + ip.getX(), path.get( i ).getY() + ip.getY(), 0 ) );
                }
                result.add( p );
            }
        }
            ...
            while (e != null) {
                ...
                if (dir[0] == Direction.LEFT_TO_RIGHT && e.getCurrent().getX() <= horzRight[0] || dir[0] == Direction.RIGHT_TO_LEFT
                    ...
                    if (e == eMaxPair && IsLastHorz) {
                        ...
                        final DoublePoint Pt = new DoublePoint( e.getCurrent().getX(), horzEdge.getCurrent().getY() );
