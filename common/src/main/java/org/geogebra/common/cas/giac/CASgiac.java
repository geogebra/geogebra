...
public abstract class CASgiac implements CASGenericInterface {
	...
	public static enum CustomFunctions {
		 ...
		 * Used internally.
		 */
		JACOBI_PREPARE("jacobiPrepare", "jacobiPrepare(polys,excludevars):=begin local ii, degrees, pos, vars, linvar, c, substval, keep; vars:=lvar(polys); print(\"input: \"+size(polys)+\" eqs in \"+size(vars)+\" vars\"); c:=1; keep:=[]; while (c<size(lvar(polys))) do ii:=0; while (ii<size(polys)-1) do degrees:=degree(polys[ii],vars); if ((sum(degrees)=c) and (isLinear(polys[ii]))) begin pos:=find(1,degrees); if (size(pos)=c) begin linvar:=vars[pos[0]]; substval:=op(solve(polys[ii]=0,linvar)[0])[1]; if (is_element(linvar,excludevars)) begin keep:=append(keep,polys[ii]); end; polys:=remove(0,expand(subs(polys,[linvar],[substval]))); vars:=lvar(polys); ii:=-1; end; end; ii:=ii+1; od; c:=c+1; od; polys:=flatten(append(polys,keep)); vars:=lvar(polys); print(\"output: \"+size(polys)+\" eqs in \"+size(vars)+\" vars\");  return polys; end"),
		/**
