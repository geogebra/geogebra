...
public class Ggb2giac {
	...
	public static Map<String, String> getMap(App app) {
		...
		p("Expand.1",
				"[[ggbexpandarg:=%0],when(contains(ggbexpandarg,i),normal(real(ggbexpandarg))+normal(i*im(ggbexpandarg)),when(inString(string(ggbexpandarg),\"sqrt\"),expand(ggbfactor(ggbexpandarg,x,0,1)),normal(lnexpand(ggbexpandarg))))][1]");
		...
		p("Factor.1",
				"[[ggbfacans:=%0],[if type(ggbfacans)==DOM_INT then ggbfacans:=unquote(ifactor(ggbfacans)); elif inString(string(ggbfacans),\"sqrt\") then ggbfacans:=regroup(ggbfactor(ggbfacans,x,0,1)); else ggbfacans:=ggbfactor(lncollect(ggbfacans),x, 0, 1); fi],ggbfacans][2]");
		p("Factor.2",
