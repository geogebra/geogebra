 ...
 * averaged). The ranking algorithm can be set using a constructor argument.
 * </p>
 *
 * @since 2.0
...
public class SpearmansCorrelation {
	...
	/**
	 * Create a SpearmansCorrelation with the given input data matrix and
	 * ranking algorithm.
	 *
	 * @param dataMatrix
	 *            matrix of data with columns representing variables to
	 *            correlate
	 ...
	 * <code>new SpearmansCorrelation(matrix).getRankCorrelation()</code> is
	 * equivalent to <code>new PearsonsCorrelation(rankTransform(matrix))</code>
	 * where <code>rankTransform(matrix)</code> is the result of applying the
	 ...
	 * or their common length is less than 2
	 * </p>
	 *
	 * @param xArray
	 *            first data array
	 ...
	 *             if the arrays lengths do not match or there is insufficient
	 *             data
	 */
