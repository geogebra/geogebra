...
public abstract class AbstractLeastSquaresOptimizer
	 ...
	 * derived class (the {@link LevenbergMarquardtOptimizer Levenberg-Marquardt
	 * optimizer} does this).
	 * </p>
	 */
	...
	protected void incrementIterationsCounter() throws OptimizationException {
		if (++iterations > maxIterations) {
			throw new OptimizationException(
					new MaxIterationsExceededException(maxIterations));
		}
	}
	 ...
	 *                if the function jacobian cannot be evaluated or its
	 *                dimension doesn't match problem dimension
	 */
	 ...
	 * 
	 * @exception FunctionEvaluationException
	 *                if the function cannot be evaluated or its dimension
	 *                doesn't match problem dimension or maximal number of of
	 ...
	 * is related to the criterion that is minimized by the optimizer as
	...
	/**
	 * Get a Chi-Square-like value assuming the N residuals follow N distinct
	 * normal distributions centered on 0 and whose variances are the reciprocal
	 * of the weights.
	 * 
	 ...
	 * @exception OptimizationException
	 *                if the covariance matrix cannot be computed (singular
	 *                problem)
	 */
	public double[][] getCovariances()
		...
		try {
			...
			RealMatrix inverse = new LUDecompositionImpl(
					MatrixUtils.createRealMatrix(jTj)).getSolver().getInverse();
	 ...
	 * @exception OptimizationException
	 *                if the covariances matrix cannot be computed or the number
	 *                of degrees of freedom is not positive (number of
	 *                measurements lesser or equal to number of parameters)
	 */
	public double[] guessParametersErrors()
			throws FunctionEvaluationException, OptimizationException {
		if (rows <= cols) {
			throw new OptimizationException(
					LocalizedFormats.NO_DEGREES_OF_FREEDOM, rows, cols);
		}
	...
	public VectorialPointValuePair optimize(
			...
			final double[] startPoint) throws FunctionEvaluationException,
			OptimizationException, IllegalArgumentException {

		if (target.length != weights.length) {
			throw new OptimizationException(
					LocalizedFormats.DIMENSIONS_MISMATCH_SIMPLE, target.length,
					weights.length);
		}
