 ...
 * so it remains very close to a set of observed points (x<sub>0</sub>, y
 * <sub>0</sub>), (x<sub>1</sub>, y<sub>1</sub>) ... (x<sub>k-1</sub>, y
 * <sub>k-1</sub>). This fitting is done by finding the parameters values that
 * minimizes the objective function &sum;(y<sub>i</sub>-f(x<sub>i</sub>))
 * <sup>2</sup>. This is really a least squares problem.
 * </p>
 * 
...
public class CurveFitter {
	...
	public WeightedObservedPoint[] getObservations() {
		return observations
				.toArray(new WeightedObservedPoint[observations.size()]);
	}
	...
	public double[] fit(final ParametricRealFunction f,
			final double[] initialGuess) throws FunctionEvaluationException,
		...
		VectorialPointValuePair optimum = optimizer.optimize(
				new TheoreticalValuesFunction(f), target, weights,
				initialGuess);
