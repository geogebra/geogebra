...
/**
 * This interface represents a first order integrator for differential
 * equations.
 * 
 * <p>
 * The classes which are devoted to solve first order differential equations
 * should implement this interface. The problems which can be handled should
 * implement the {@link FirstOrderDifferentialEquations} interface.
 * </p>
 *
...
public interface FirstOrderIntegrator extends ODEIntegrator {
	 ...
	 * </p>
	 * <p>
	 * Since this method stores some internal state variables made available in
	 * its public interface during integration (
	 ...
	 *            than <code>t0</code> for backward integration)
	 * @param y
	 *            placeholder where to put the state vector at each successful
	 *            step (and hence at the end of integration), can be the same
	 *            object as y0
	 * @return stop time, will be the same as target time if integration reached
	 *         its target, but may be different if some
	 *         {@link org.apache.commons.math.ode.events.EventHandler} stops it
	 *         at some point.
	 * @throws DerivativeException
	 *             this exception is propagated to the caller if the underlying
	 *             user function triggers one
	 * @throws IntegratorException
	 *             if the integrator cannot perform integration
	 */
	double integrate(FirstOrderDifferentialEquations equations, double t0,
