 ...
 * {@link NaNStrategy#MAXIMAL} and {@link TiesStrategy#AVERAGE}, respectively.
...
public class NaturalRanking implements RankingAlgorithm {
	...
	/**
	 * Rank <code>data</code> using the natural ordering on Doubles, with NaN
	 * values handled according to <code>nanStrategy</code> and ties resolved
	 * using <code>tiesStrategy.</code>
	 *
	 * @param data
	 *            array to be ranked
	...
	public double[] rank(double[] data) {
		...
		for (int i = 1; i < ranks.length; i++) {
			if (Double.compare(ranks[i].getValue(),
					ranks[i - 1].getValue()) > 0) {
	...
	private IntDoublePair[] removeNaNs(IntDoublePair[] ranks) {
		...
		for (int i = 0; i < ranks.length; i++) {
			if (Double.isNaN(ranks[i].getValue())) {
				...
				outRanks[j] = new IntDoublePair(ranks[i].getValue(),
						ranks[i].getPosition());
				j++;
			}
		}
	 ...
	 *            array of ranks
	 * @param tiesTrace
	 *            list of indices where <code>ranks</code> is constant -- that
	 *            is, for any i and j in TiesTrace, <code> ranks[i] == ranks[j]
	 * </code>
	 */
	private void resolveTie(double[] ranks, List<Integer> tiesTrace) {
		...
		case MINIMUM: // Replace ties with minimum
			...
			break;
		/*
		 * case RANDOM: // Fill with random integral values in [c, c + length -
		 * 1] Iterator<Integer> iterator = tiesTrace.iterator(); long f =
		 * FastMath.round(c); while (iterator.hasNext()) {
	 ...
	 * IntDoublePairs by value. Note that the implicitly defined natural
