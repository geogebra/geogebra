...
public class SingularValueDecompositionImpl
	...
	public SingularValueDecompositionImpl(final RealMatrix matrix)
		...
		if (m >= n) {
			...
			cachedU = eigenDecomposition.getV().getSubMatrix(0, m - 1, 0,
					p - 1);
		} else {
			p = m;
			// compute eigen decomposition of A*A^T
			...
			eigenDecomposition = new EigenDecompositionImpl(
					new Array2DRowRealMatrix(matATA), 1.0);
			cachedV = eigenDecomposition.getV().getSubMatrix(0, n - 1, 0,
					p - 1);
		}
		for (int i = 0; i < p; i++) {
		...
		for (int i = 0; i < p; i++) {
			...
			double product = matrix.operate(cachedV.getColumnVector(i))
					.dotProduct(tmp);
			if (product < 0) {
