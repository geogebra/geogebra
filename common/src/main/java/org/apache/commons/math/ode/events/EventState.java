...
public class EventState {
	...
	public void reinitializeBegin(final StepInterpolator interpolator)
		...
		try {
			...
			// occurring
			// less than epsilon after the solver start in the first step should
			// be ignored,
			// where epsilon is the convergence threshold of the event. The sign
			// of the g
			// function should be evaluated after this initial ignore zone, not
			// exactly at
			// beginning (if there are no event at the very beginning g(t0) and
			// g(t0+epsilon)
			// have the same sign, so this does not hurt ; if there is an event
			// at the very
			// beginning, g(t0) and g(t0+epsilon) have opposite signs and we
			// want to start
			// with the second one. Of course, the sign of epsilon depend on the
			// integration
			...
			if (g0 == 0) {
				// extremely rare case: there is a zero EXACTLY at end of ignore
				// zone
	...
	public boolean evaluateStep(final StepInterpolator interpolator)
		...
		try {
			...
			final double start = forward ? (t0 + convergence)
					: t0 - convergence;
			final double dt = t1 - start;
			final int n = Math.max(1,
					(int) Math.ceil(Math.abs(dt) / maxCheckInterval));
			final double h = dt / n;
			...
			for (int i = 0; i < n; ++i) {
				...
				if (g0Positive ^ (gb >= 0)) {
					...
					if ((!Double.isNaN(previousEventTime))
							...
							&& (Math.abs(
									root - previousEventTime) <= convergence)) {
						// we have either found nothing or found (again ?) a
						// past event, we simply ignore it
						ta = tb;
						ga = gb;
					} else if (Double.isNaN(previousEventTime) || (Math
							.abs(previousEventTime - root) > convergence)) {
	 ...
	 *         positive infinity if no events are triggered
	 */
	...
	public boolean reset(final double t, final double[] y)
		...
		if (!(pendingEvent
				&& (Math.abs(pendingEventTime - t) <= convergence))) {
			return false;
		}
