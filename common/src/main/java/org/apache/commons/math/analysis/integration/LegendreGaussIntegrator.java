 ...
 * <p>
 * Legendre-Gauss integrators are efficient integrators that can accurately
 * integrate functions with few functions evaluations. A Legendre-Gauss
 * integrator using an n-points quadrature formula can integrate exactly 2n-1
 * degree polynomials.
 * </p>
 * <p>
 * These integrators evaluate the function on n carefully chosen abscissas in
 * each step interval (mapped to the canonical [-1 1] interval). The evaluation
 * abscissas are not evenly spaced and none of them are at the interval
 * endpoints. This implies the function integrated can be undefined at
 * integration interval endpoints.
 * </p>
 * <p>
 * The evaluation abscissas x<sub>i</sub> are the roots of the degree n Legendre
 * polynomial. The weights a<sub>i</sub> of the quadrature formula integrals
 * from -1 to +1 &int; Li<sup>2</sup> where Li (x) = &prod; (x-x<sub>k</sub>)/(x
 * <sub>i</sub>-x<sub>k</sub>) for k != i.
 * </p>
 * <p>
 * 
...
public class LegendreGaussIntegrator extends UnivariateRealIntegratorImpl {
	...
	private static final double[] ABSCISSAS_5 = {
			...
			-FastMath.sqrt((35.0 - 2.0 * FastMath.sqrt(70.0)) / 63.0), 0.0,
			FastMath.sqrt((35.0 - 2.0 * FastMath.sqrt(70.0)) / 63.0),
			FastMath.sqrt((35.0 + 2.0 * FastMath.sqrt(70.0)) / 63.0) };
	...
	public double integrate(final UnivariateRealFunction f, final double min,
		...
		for (int i = 0; i < maximalIterationCount; ++i) {
			...
			final double limit = FastMath.max(absoluteAccuracy, relativeAccuracy
	 ...
	 */
	private double stage(final UnivariateRealFunction f, final double min,
			final double max, final int n) throws FunctionEvaluationException {
