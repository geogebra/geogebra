 ...
 *
 * @version $Revision: 949535 $ $Date: 2010-05-30 19:00:15 +0200 (dim. 30 mai
 *          2010) $
 */
public interface IntegerDistribution extends DiscreteDistribution {
	 ...
	 * represents the probability mass function for the distribution.
	 *
	 * @param x
	 ...
	 * method represents the probability distribution function, or PDF for the
	 * distribution.
	 *
	 * @param x
	 ...
	 * probability under (the density of) X, then <code>m - 1</code> is returned
	 * by <code>inverseCumulativeProbability(0).</code> If there is no such
	 * value <code>m,  Integer.MIN_VALUE</code> is returned.</li>
	 * <li>If there is a maximum value, <code>M</code>, such that P(X &le; M)
	 * =1, then <code>M</code> is returned by
	 * <code>inverseCumulativeProbability(1).</code> If there is no such value,
	 * <code>M, Integer.MAX_VALUE</code> is returned.</li>
	 * </ul>
	 * </p>
	 *
	 * @param p
