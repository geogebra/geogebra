...
public abstract class AbstractRealVector implements RealVector {
	...
	public double getLInfDistance(RealVector v)
		...
		while (it.hasNext() && (e = it.next()) != null) {
			d = FastMath.max(
					FastMath.abs(e.getValue() - v.getEntry(e.getIndex())), d);
		}
		return d;
	}
	 ...
	 * 
	 * @return index of the minimum entry or -1 if vector length is 0 or all
	 *         entries are NaN
	 */
	 ...
	 * 
	 * @return index of the maximum entry or -1 if vector length is 0 or all
	 *         entries are NaN
	 */
	...
	public RealMatrix outerProduct(RealVector v)
			throws IllegalArgumentException {
		RealMatrix product;
		// if (v instanceof SparseRealVector || this instanceof
		// SparseRealVector) {
	...
	/**
	 * This class should rare be used, but is here to provide a default
	 * implementation of sparseIterator(), which is implemented by walking over
	 * the entries, skipping those whose values are the default one.
	 *
	 * Concrete subclasses which are SparseVector implementations should make
	 * their own sparse iterator, not use this one.
	 *
	 * This implementation might be useful for ArrayRealVector, when expensive
	 * operations which preserve the default value are to be done on the
	 * entries, and the fraction of non-default values is small (i.e. someone
	 * took a SparseVector, and passed it into the copy-constructor of
	 * ArrayRealVector)
	 */
