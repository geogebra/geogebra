...
public class MathException extends Exception {
	 ...
	 * @since 1.2
	 */
	public MathException(Throwable rootCause, String pattern,
			Object... arguments) {
		super(rootCause);
		this.pattern = pattern;
		this.arguments = (arguments == null) ? new Object[0] : arguments;
	 ...
	 */
	/*
	 * AGprivate static String translate(String s, Locale locale) { try {
	 * ResourceBundle bundle =
	 * ResourceBundle.getBundle("org.apache.commons.math.MessagesResources",
	 * locale); if
	 * (bundle.getLocale().getLanguage().equals(locale.getLanguage())) { // the
	 * value of the resource is the translated string return
	 * bundle.getString(s); }
	 * 
	 * } catch (MissingResourceException mre) { // do nothing here }
	 * 
	 * // the locale is not supported or the resource is unknown // don't
	 ...
	 */
	/*
	 * AGpublic String getMessage(final Locale locale) { return (pattern ==
	 * null) ? "" : new MessageFormat(translate(pattern, locale),
	 * locale).format(arguments); }
	 * 
	 * /** {@inheritDoc}
	 */
	/*
	 * AG@Override public String getMessage() { return getMessage(Locale.US); }
	 * 
	 * /** {@inheritDoc}
	 */
	/*
	 * AG @Override public String getLocalizedMessage() { return
	 * getMessage(Locale.getDefault()); }
	 * 
	 * /** Prints the stack trace of this exception to the standard error
	 * stream.
	 */
	@Override
	...
	public void printStackTrace(/* AGPrintStream out */String out) {
		/*
		 * AG synchronized (out) { PrintWriter pw = new PrintWriter(out, false);
		 * printStackTrace(pw); // Flush the PrintWriter before it's GC'ed.
		 * pw.flush(); }
		 */
		Log.debug(out);
	}
