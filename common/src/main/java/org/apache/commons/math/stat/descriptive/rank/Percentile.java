 ...
 * <li>Let <code>lower</code> be the element in position <code>floor(pos)</code>
 * in the array and let <code>upper</code> be the next element in the array.
 ...
 * (50th percentile) of <code>{0, 1, 2, 3, 4, Double.NaN}</code> evaluates to
 * <code>2.5.</code>
 * </p>
 * <p>
 ...
 * </p>
 *
 * @version $Revision: 1006299 $ $Date: 2010-10-10 16:47:17 +0200 (dim. 10 oct.
 *          2010) $
 */
public class Percentile extends AbstractUnivariateStatistic
	...
	/**
	 * Minimum size under which we use a simple insertion sort rather than
	 * Hoare's select.
	 */
	private static final int MIN_SELECT_SIZE = 15;

	/**
	 * Maximum number of partitioning pivots cached (each level double the
	 * number of pivots).
	 */
	...
	/**
	 * Copy constructor, creates a new {@code Percentile} identical to the
	 * {@code original}
	 *
	 * @param original
	...
	/** {@inheritDoc} */
	@Override
	public void setData(final double[] values, final int begin,
			final int length) {
		if (values == null) {
			cachedPivots = null;
		} else {
	 ...
	 * <code>0</code></li>
	 * <li>Returns (for any value of <code>p</code>) <code>values[0]</code> if
	 * <code>values</code> has length <code>1</code></li>
	 * <li>Throws <code>IllegalArgumentException</code> if <code>values</code>
	 * is null or p is not a valid quantile value (p must be greater than 0 and
	...
	/**
	 * Returns an estimate of the <code>p</code>th percentile of the values in
	 * the <code>values</code> array, starting with the element in (0-based)
	 * position <code>begin</code> in the array and including
	 ...
	 *            at their sorted location, stored as an implicit heap (i.e. a
	 *            sorted binary tree stored in a flat array, where the children
	 *            of a node at index n are at indices 2n+1 for the left child
	 *            and 2n+2 for the right child, with 0-based indices)
	 * @param k
	 *            index of the desired element
	 * @return k<sup>th</sup> smallest element
	 */
	private double select(final double[] work, final int[] pivotsHeap,
			final int k) {
		...
		while (end - begin > MIN_SELECT_SIZE) {
			...
			if (k == pivot) {
				...
				node = Math.min(2 * node + 1, pivotsHeap.length); // the min is
																	// here to
																	// avoid
																	// integer
																	// overflow
				...
				begin = pivot + 1;
				node = Math.min(2 * node + 2, pivotsHeap.length); // the min is
																	// here to
																	// avoid
																	// integer
																	// overflow
	 ...
	 * @return the index of the median element chosen between the first, the
	 *         middle and the last element of the array slice
	 */
	 ...
	 * <p>
	 * Partitioning exchanges array elements such that all elements smaller than
	 * pivot are before it and all elements larger than pivot are after it
	 * </p>
	 * 
	 * @param work
	 *            data array
	...
	public static void copy(Percentile source, Percentile dest) {
		...
		if (source.cachedPivots != null) {
			System.arraycopy(source.cachedPivots, 0, dest.cachedPivots, 0,
