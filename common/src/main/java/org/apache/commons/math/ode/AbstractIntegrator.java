 ...
 * 
 * @version $Revision: 1073267 $ $Date: 2011-02-22 10:06:20 +0100 (mar. 22 fevr.
 *          2011) $
 * @since 2.0
 */
public abstract class AbstractIntegrator implements FirstOrderIntegrator {
	 ...
	 * 
	 * @return true if there is at least one event handler or if one of the step
	 *         handlers requires dense output
	 */
	...
	public void computeDerivatives(final double t, final double[] y,
			final double[] yDot) throws DerivativeException {
	 ...
	 *                if the value of one event state cannot be evaluated
	 * @since 2.2
	 */
	protected double acceptStep(final AbstractStepInterpolator interpolator,
		...
		try {
			...
			while (!occuringEvents.isEmpty()) {
				...
				// check if the same event occurs again in the remaining part of
				// the step
	...
	protected void sanityChecks(final FirstOrderDifferentialEquations ode,
		...
		if (FastMath.abs(t - t0) <= 1.0e-12
				* FastMath.max(FastMath.abs(t0), FastMath.abs(t))) {
			throw new IntegratorException(
	...
	protected CombinedEventsManager addEndTimeChecker(final double startTime,
		...
		for (final EventState state : manager.getEventsStates()) {
			newManager.addEventHandler(state.getEventHandler(),
					state.getMaxCheckInterval(), state.getConvergence(),
					state.getMaxIterationCount());
		}
		newManager.addEventHandler(new EndTimeChecker(endTime),
				Double.POSITIVE_INFINITY, FastMath.ulp(FastMath
						.max(FastMath.abs(startTime), FastMath.abs(endTime))),
				100);
		return newManager;
	}
