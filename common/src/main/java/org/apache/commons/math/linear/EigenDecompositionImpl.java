 ...
 * </p>
 * 
 * @version $Revision: 1002040 $ $Date: 2010-09-28 09:18:31 +0200 (mar. 28 sept.
 *          2010) $
 * @since 2.0
 */
public class EigenDecompositionImpl implements EigenDecomposition {
	...
	private void findEigenVectors(double[][] householderMatrix) {
		...
		if (maxAbsoluteValue != 0.0) {
			for (int i = 0; i < n; i++) {
				if (FastMath.abs(realEigenvalues[i]) <= MathUtils.EPSILON
						* maxAbsoluteValue) {
					realEigenvalues[i] = 0.0;
				}
				if (FastMath.abs(e[i]) <= MathUtils.EPSILON
						* maxAbsoluteValue) {
					e[i] = 0.0;
				}
			}
