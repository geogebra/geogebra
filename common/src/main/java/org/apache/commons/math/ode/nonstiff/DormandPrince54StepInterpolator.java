...
class DormandPrince54StepInterpolator extends RungeKuttaStepInterpolator {
	 ...
	 * cases. The {@link EmbeddedRungeKuttaIntegrator} uses the prototyping
	 ...
	 */
	public DormandPrince54StepInterpolator(
			final DormandPrince54StepInterpolator interpolator) {
	...
	protected void computeInterpolatedStateAndDerivatives(final double theta,
			final double oneMinusThetaH) throws DerivativeException {
		...
		if (!vectorsInitialized) {
			...
			for (int i = 0; i < interpolatedState.length; ++i) {
				...
				v1[i] = A70 * yDot0 + A72 * yDot2 + A73 * yDot3 + A74 * yDot4
						+ A75 * yDot5;
				v2[i] = yDot0 - v1[i];
				v3[i] = v1[i] - v2[i] - yDot6;
				v4[i] = D0 * yDot0 + D2 * yDot2 + D3 * yDot3 + D4 * yDot4
						+ D5 * yDot5 + D6 * yDot6;
			}
		...
		for (int i = 0; i < interpolatedState.length; ++i) {
			interpolatedState[i] = currentState[i] - oneMinusThetaH
					* (v1[i] - theta * (v2[i] + theta * (v3[i] + eta * v4[i])));
