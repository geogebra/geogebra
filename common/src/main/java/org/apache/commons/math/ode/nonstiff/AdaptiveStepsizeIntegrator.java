 ...
 *
 * <p>
 * If the Ordinary Differential Equations is an
 * {@link ExtendedFirstOrderDifferentialEquations extended ODE} rather than a
 * {@link FirstOrderDifferentialEquations basic ODE}, then <em>only</em> the
 * {@link ExtendedFirstOrderDifferentialEquations#getMainSetDimension() main
 * set} part of the state vector is used for stepsize control, not the complete
 * state vector.
 * </p>
 *
 * <p>
...
public abstract class AdaptiveStepsizeIntegrator extends AbstractIntegrator {
	 ...
	 */
	public AdaptiveStepsizeIntegrator(final String name, final double minStep,
	...
	protected void sanityChecks(final FirstOrderDifferentialEquations equations,
			...
			final double[] y) throws IntegratorException {

		super.sanityChecks(equations, t0, y0, t, y);

		if (equations instanceof ExtendedFirstOrderDifferentialEquations) {
			mainSetDimension = ((ExtendedFirstOrderDifferentialEquations) equations)
					.getMainSetDimension();
		} else {
		...
		if ((vecAbsoluteTolerance != null)
				&& (vecAbsoluteTolerance.length != mainSetDimension)) {
		...
		if ((vecRelativeTolerance != null)
				&& (vecRelativeTolerance.length != mainSetDimension)) {
			throw new IntegratorException(
					LocalizedFormats.DIMENSIONS_MISMATCH_SIMPLE,
	...
	public double initializeStep(
		...
		final double maxInv2 = FastMath.max(FastMath.sqrt(yDotOnScale2),
				yDDotOnScale);
	 ...
	 */
	protected double filterStep(final double h, final boolean forward,
			final boolean acceptSmall) throws IntegratorException {
