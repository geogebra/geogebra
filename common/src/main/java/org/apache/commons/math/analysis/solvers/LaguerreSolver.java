...
/**
 * Implements the
 * <a href="http://mathworld.wolfram.com/LaguerresMethod.html"> Laguerre's
 * Method</a> for root finding of real coefficient polynomials. For reference,
 * see <b>A First Course in Numerical Analysis</b>, ISBN 048641454X, chapter 8.
 * <p>
...
public class LaguerreSolver extends UnivariateRealSolverImpl {
	...
	public LaguerreSolver(UnivariateRealFunction f)
		...
		if (f instanceof PolynomialFunction) {
			...
			throw MathRuntimeException.createIllegalArgumentException(
					LocalizedFormats.FUNCTION_NOT_POLYNOMIAL);
		}
	}
	 ...
	 * Complex) may not be a real zero inside [min, max]. For example, p(x) =
	 * x^3 + 1, min = -2, max = 2, initial = 0. We can either try another
	 * initial value, or, as we did here, call solveAll() to obtain all roots
	 * and pick up the one that we're looking for.
	 * </p>
	 *
	 * @param f
	...
	/**
	 * Returns true iff the given complex root is actually a real zero in the
	 * given interval, within the solver tolerance level.
	 *
	 * @param min
	 *            the lower bound for the interval
	 * @param max
