...
/**
 * Base class for {@link UnivariateRealFunction} that can be composed with other
 * functions.
 *
 * @since 2.1
 * @version $Revision: 924453 $ $Date: 2010-03-17 16:05:20 -0400 (Wed, 17 Mar
 *          2010) $
 */
public abstract class ComposableFunction implements UnivariateRealFunction {
	 ...
	 * <p>
	 * The composed function h created by {@code h = g.of(f)} is such that
	 * {@code h.value(x) == g.value(f.value(x))} for all x.
	 * </p>
	 * 
	 * @param f
	 ...
	 * @return a new function which computes
	 *         {@code combine.value(this.value(x), f.value(x))}
	 */
	 ...
	 * {@link #asCollector(BivariateRealFunction, double)
