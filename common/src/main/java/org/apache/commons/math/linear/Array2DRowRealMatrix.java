 ...
 * LU decomposition</a> to support linear system solution and inverse.
 * <p>
 ...
 * <ul>
 * <li>The LU decomposition is cached and reused on subsequent calls. If data
 * are modified via references to the underlying array obtained using
 * <code>getDataRef()</code>, then the stored LU decomposition will not be
 * discarded. In this case, you need to explicitly invoke
 * <code>LUDecompose()</code> to recompute the decomposition before using any of
 * the methods above.</li>
 * <li>As specified in the {@link RealMatrix} interface, matrix element indexing
 * is 0-based -- e.g., <code>getEntry(0, 0)</code> returns the element in the
...
public class Array2DRowRealMatrix extends AbstractRealMatrix
	...
	/**
	 * Create a new RealMatrix using the input array as the underlying data
	 * array.
	 * <p>
	 * The input array is copied, not referenced. This constructor has the same
	 * effect as calling {@link #Array2DRowRealMatrix(double[][], boolean)} with
	 * the second argument set to <code>true</code>.
	 * </p>
	 *
	 * @param d
	 *            data for new matrix
	 * @throws IllegalArgumentException
	 *             if <code>d</code> is not rectangular (not all rows have the
	 ...
	 * array.
	 * <p>
	 * If an array is built specially in order to be embedded in a RealMatrix
	 * and not used directly, the <code>copyArray</code> may be set to
	 * <code>false</code. This will prevent the copying and improve
	...
	public Array2DRowRealMatrix(final double[][] d, final boolean copyArray)
		...
		if (copyArray) {
			...
			for (int r = 1; r < nRows; r++) {
				if (d[r].length != nCols) {
					throw MathRuntimeException.createIllegalArgumentException(
							LocalizedFormats.DIFFERENT_ROWS_LENGTHS, nCols,
							d[r].length);
				}
			}
			data = d;
	...
	/**
	 * Create a new (column) RealMatrix using <code>v</code> as the data for the
	 * unique column of the <code>v.length x 1</code> matrix created.
	 * <p>
	 ...
	 * <p>
	 * Does <strong>not</strong> make a fresh copy of the underlying data.
	 * </p>
	 *
	...
	public double getEntry(final int row, final int column)
		...
		try {
			...
			throw new MatrixIndexException(
					LocalizedFormats.NO_SUCH_MATRIX_ENTRY, row, column,
	...
	public void addToEntry(final int row, final int column,
			final double increment) throws MatrixIndexException {
		try {
	...
	public void multiplyEntry(final int row, final int column,
			final double factor) throws MatrixIndexException {
		try {
	...
	public double walkInRowOrder(final RealMatrixChangingVisitor visitor,
		...
		MatrixUtils.checkSubMatrixIndex(this, startRow, endRow, startColumn,
				endColumn);
		visitor.start(getRowDimension(), getColumnDimension(), startRow, endRow,
	 ...
	 *            data to copy in
	 * @throws IllegalArgumentException
	 *             if input array is empty or not rectangular
	 * @throws NullPointerException
