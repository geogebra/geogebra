 ...
 * <p>
 * This implementation is specially designed to be cache-friendly. Square blocks
 * are stored as small arrays and allow efficient traversal of data both in row
 * major direction and columns major direction, one block at a time. This
 * greatly increases performances for algorithms that use crossed directions
 * loops like multiplication or transposition.
 * </p>
 * <p>
 * The size of square blocks is a static parameter. It may be tuned according to
 * the cache size of the target computer processor. As a rule of thumbs, it
 * should be the largest value that allows three blocks to be simultaneously
 * cached (this is necessary for example for matrix multiplication). The default
 * value is to use 52x52 blocks which is well suited for processors with 64k L1
 * cache (one block holds 2704 values or 21632 bytes). This value could be
 * lowered to 36x36 for processors with 32k L1 cache.
 * </p>
 * <p>
 * The regular blocks represent {@link #BLOCK_SIZE} x {@link #BLOCK_SIZE}
 * squares. Blocks at right hand side and bottom side which may be smaller to
 * fit matrix dimensions. The square blocks are flattened in row major order in
 * single dimension arrays which are therefore {@link #BLOCK_SIZE}<sup>2</sup>
 * elements long for regular blocks. The blocks are themselves organized in row
 * major order.
 * </p>
 * <p>
 * As an example, for a block size of 52x52, a 100x60 matrix would be stored in
 * 4 blocks. Block 0 would be a double[2704] array holding the upper left 52x52
 * square, block 1 would be a double[416] array holding the upper right 52x8
 * rectangle, block 2 would be a double[2496] array holding the lower left 48x52
 * rectangle and block 3 would be a double[384] array holding the lower right
 * 48x8 rectangle.
 * </p>
 * <p>
 * The layout complexity overhead versus simple mapping of matrices to java
 * arrays is negligible for small matrices (about 1%). The gain from cache
 * efficiency leads to up to 3-fold improvements for matrices of moderate to
 * large size.
 * </p>
 * 
...
public class BlockRealMatrix extends AbstractRealMatrix
	 ...
	 *                if <code>blockData</code> shape is inconsistent with block
	 *                layout
	 ...
	 * column j is in array element <code>rawData[i][j]</code>. Blocks layout is
	 * the layout used in {@link BlockRealMatrix} instances, where the matrix is
	 * split in square blocks (except at right and bottom side where blocks may
	 * be rectangular to fit matrix size) and each block is stored in a
	 * flattened one-dimensional array.
	 * </p>
	 * <p>
	 * This method creates an array in blocks layout from an input array in raw
	 * layout. It can be used to provide the array argument of the
	 ...
	 * @exception IllegalArgumentException
	 *                if <code>rawData</code> is not rectangular (not all rows
	...
	public static double[][] toBlocksLayout(final double[][] rawData)
		...
		for (int i = 0; i < rawData.length; ++i) {
			...
			if (length != columns) {
				throw MathRuntimeException.createIllegalArgumentException(
						LocalizedFormats.DIFFERENT_ROWS_LENGTHS, columns,
						length);
			}
		}
	 ...
	 * @see #BlockRealMatrix(int, int, double[][], boolean)
	 */
	public static double[][] createBlocksLayout(final int rows,
			final int columns) {
		...
		for (int iBlock = 0; iBlock < out.blockRows; ++iBlock) {
			...
			for (int jBlock = 0; jBlock < out.blockColumns; ++jBlock) {
				...
				for (int kBlock = 0; kBlock < blockColumns; ++kBlock) {
					...
					final double[] tBlock = blocks[iBlock * blockColumns
							+ kBlock];
					final double[] mBlock = m.blocks[kBlock * m.blockColumns
							+ jBlock];
					int k = 0;
					for (int p = pStart; p < pEnd; ++p) {
						final int lStart = (p - pStart) * kWidth;
		...
		for (int iBlock = 0; iBlock < blockRows; ++iBlock) {
			...
			for (int p = pStart; p < pEnd; ++p) {
				...
				for (int jBlock = 0; jBlock < blockColumns - 1; ++jBlock) {
					System.arraycopy(blocks[blockIndex++], regularPos, dataP,
							dataPos, BLOCK_SIZE);
					dataPos += BLOCK_SIZE;
				}
				System.arraycopy(blocks[blockIndex], lastPos, dataP, dataPos,
						lastColumns);
	...
	public BlockRealMatrix getSubMatrix(final int startRow, final int endRow,
		...
		final BlockRealMatrix out = new BlockRealMatrix(endRow - startRow + 1,
		...
		for (int iBlock = 0; iBlock < out.blockRows; ++iBlock) {
			...
			for (int jBlock = 0; jBlock < out.blockColumns; ++jBlock) {
				...
				if (heightExcess > 0) {
					...
					// original matrix
					if (widthExcess > 0) {
						// the submatrix block spans on two blocks columns from
						// the original matrix
						final int width2 = blockWidth(qBlock + 1);
						copyBlockPart(blocks[index], width, rowsShift,
								BLOCK_SIZE, columnsShift, BLOCK_SIZE, outBlock,
								jWidth, 0, 0);
						copyBlockPart(blocks[index + 1], width2, rowsShift,
								BLOCK_SIZE, 0, widthExcess, outBlock, jWidth, 0,
								jWidth - widthExcess);
						copyBlockPart(blocks[index + blockColumns], width, 0,
								heightExcess, columnsShift, BLOCK_SIZE,
								outBlock, jWidth, iHeight - heightExcess, 0);
						copyBlockPart(blocks[index + blockColumns + 1], width2,
								0, heightExcess, 0, widthExcess, outBlock,
								jWidth, iHeight - heightExcess,
								jWidth - widthExcess);
					} else {
						// the submatrix block spans on one block column from
						// the original matrix
						copyBlockPart(blocks[index], width, rowsShift,
								BLOCK_SIZE, columnsShift, jWidth + columnsShift,
								outBlock, jWidth, 0, 0);
						copyBlockPart(blocks[index + blockColumns], width, 0,
								heightExcess, columnsShift,
								jWidth + columnsShift, outBlock, jWidth,
								iHeight - heightExcess, 0);
					}
				} else {
					...
					if (widthExcess > 0) {
						...
						final int width2 = blockWidth(qBlock + 1);
						copyBlockPart(blocks[index], width, rowsShift,
								iHeight + rowsShift, columnsShift, BLOCK_SIZE,
								outBlock, jWidth, 0, 0);
						copyBlockPart(blocks[index + 1], width2, rowsShift,
								iHeight + rowsShift, 0, widthExcess, outBlock,
								jWidth, 0, jWidth - widthExcess);
					} else {
						// the submatrix block spans on one block column from
						// the original matrix
						copyBlockPart(blocks[index], width, rowsShift,
								iHeight + rowsShift, columnsShift,
								jWidth + columnsShift, outBlock, jWidth, 0, 0);
					}
				}
	...
	public void setSubMatrix(final double[][] subMatrix, final int row,
		...
		for (final double[] subRow : subMatrix) {
			if (subRow.length != refLength) {
				throw MathRuntimeException.createIllegalArgumentException(
						LocalizedFormats.DIFFERENT_ROWS_LENGTHS, refLength,
						subRow.length);
			}
		}
		...
		for (int iBlock = blockStartRow; iBlock < blockEndRow; ++iBlock) {
			...
			for (int jBlock = blockStartColumn; jBlock < blockEndColumn; ++jBlock) {
				...
				final int jEnd = FastMath.min(endColumn + 1,
						firstColumn + jWidth);
	...
	public BlockRealMatrix getRowMatrix(final int row)
		...
		for (int jBlock = 0; jBlock < blockColumns; ++jBlock) {
			...
			if (jWidth > available) {
				...
				System.arraycopy(block, iRow * jWidth, outBlock, 0,
						jWidth - available);
	...
	/**
	 * Sets the entries in row number <code>row</code> as a row matrix. Row
	 * indices start at 0.
	 *
	 * @param row
	 *            the row to be set
	...
	public void setRowMatrix(final int row, final BlockRealMatrix matrix)
		...
		if ((matrix.getRowDimension() != 1)
			...
			throw new InvalidMatrixException(
					...
					matrix.getRowDimension(), matrix.getColumnDimension(), 1,
					nCols);
		}
		...
		for (int jBlock = 0; jBlock < blockColumns; ++jBlock) {
			...
			if (jWidth > available) {
				System.arraycopy(mBlock, mIndex, block, iRow * jWidth,
						available);
				mBlock = matrix.blocks[++mBlockIndex];
				System.arraycopy(mBlock, 0, block, iRow * jWidth,
						jWidth - available);
	...
	/**
	 * Sets the entries in column number <code>column</code> as a column matrix.
	 * Column indices start at 0.
	 *
	 * @param column
	 *            the column to be set
	 * @param matrix
	 *            column matrix (must have one column and the same number of
	...
	public void setRow(final int row, final double[] array)
		...
		if (array.length != nCols) {
			throw new InvalidMatrixException(
					LocalizedFormats.DIMENSIONS_MISMATCH_2x2, 1, array.length,
					1, nCols);
		}
	...
	public void setColumn(final int column, final double[] array)
		...
		if (array.length != nRows) {
			throw new InvalidMatrixException(
					LocalizedFormats.DIMENSIONS_MISMATCH_2x2, array.length, 1,
					nRows, 1);
		}
	...
	public double[] operate(final double[] v) throws IllegalArgumentException {
		...
		if (v.length != columns) {
			throw MathRuntimeException.createIllegalArgumentException(
					LocalizedFormats.VECTOR_LENGTH_MISMATCH, v.length, columns);
		}
		final double[] out = new double[rows];
		...
		for (int iBlock = 0; iBlock < blockRows; ++iBlock) {
			...
			for (int jBlock = 0; jBlock < blockColumns; ++jBlock) {
				...
				for (int p = pStart; p < pEnd; ++p) {
					...
					int q = qStart;
					while (q < qEnd - 3) {
						sum += block[k] * v[q] + block[k + 1] * v[q + 1]
								+ block[k + 2] * v[q + 2]
								+ block[k + 3] * v[q + 3];
						k += 4;
						q += 4;
	...
	public double[] preMultiply(final double[] v)
		...
		if (v.length != rows) {
			throw MathRuntimeException.createIllegalArgumentException(
					LocalizedFormats.VECTOR_LENGTH_MISMATCH, v.length, rows);
		}
		final double[] out = new double[columns];
		...
		for (int jBlock = 0; jBlock < blockColumns; ++jBlock) {
			...
			for (int iBlock = 0; iBlock < blockRows; ++iBlock) {
				...
				for (int q = qStart; q < qEnd; ++q) {
					...
					int p = pStart;
					while (p < pEnd - 3) {
						sum += block[k] * v[p] + block[k + jWidth] * v[p + 1]
		...
		for (int iBlock = 0; iBlock < blockRows; ++iBlock) {
			...
			for (int p = pStart; p < pEnd; ++p) {
				for (int jBlock = 0; jBlock < blockColumns; ++jBlock) {
					...
					final int qStart = jBlock * BLOCK_SIZE;
					final int qEnd = FastMath.min(qStart + BLOCK_SIZE, columns);
					final double[] block = blocks[iBlock * blockColumns
							+ jBlock];
	...
	public double walkInRowOrder(final RealMatrixChangingVisitor visitor,
		...
		for (int iBlock = startRow / BLOCK_SIZE; iBlock < 1
				+ endRow / BLOCK_SIZE; ++iBlock) {
			final int p0 = iBlock * BLOCK_SIZE;
			final int pStart = FastMath.max(startRow, p0);
			final int pEnd = FastMath.min((iBlock + 1) * BLOCK_SIZE,
					1 + endRow);
			for (int p = pStart; p < pEnd; ++p) {
				for (int jBlock = startColumn / BLOCK_SIZE; jBlock < 1
						+ endColumn / BLOCK_SIZE; ++jBlock) {
					final int jWidth = blockWidth(jBlock);
					final int q0 = jBlock * BLOCK_SIZE;
					final int qStart = FastMath.max(startColumn, q0);
					final int qEnd = FastMath.min((jBlock + 1) * BLOCK_SIZE,
							1 + endColumn);
					final double[] block = blocks[iBlock * blockColumns
							+ jBlock];
	...
	private int blockHeight(final int blockRow) {
		return (blockRow == blockRows - 1) ? rows - blockRow * BLOCK_SIZE
				: BLOCK_SIZE;
	}
	...
	private int blockWidth(final int blockColumn) {
		return (blockColumn == blockColumns - 1)
