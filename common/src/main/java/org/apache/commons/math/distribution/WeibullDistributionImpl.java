...
public class WeibullDistributionImpl extends AbstractContinuousDistribution
	...
	private void setShapeInternal(double alpha) {
		if (alpha <= 0.0) {
			throw MathRuntimeException.createIllegalArgumentException(
					LocalizedFormats.NOT_POSITIVE_SHAPE, alpha);
		}
		this.shape = alpha;
	}
	...
	private void setScaleInternal(double beta) {
		if (beta <= 0.0) {
			throw MathRuntimeException.createIllegalArgumentException(
					LocalizedFormats.NOT_POSITIVE_SCALE, beta);
		}
		this.scale = beta;
	}
	 ...
	 * Calculates the mean.
	 *
	 * The mean is <code>scale * Gamma(1 + (1 / shape))</code> where
	 * <code>Gamma(...)</code> is the Gamma-function
	 *
	 * @return the mean
	 * @since 2.2
	 */
	 ...
	 *
	 * @return the variance (possibly Double.POSITIVE_INFINITY as for certain
	 *         cases in {@link TDistributionImpl}) or Double.NaN if it's not
	 *         defined
	 * @since 2.2
	 */
