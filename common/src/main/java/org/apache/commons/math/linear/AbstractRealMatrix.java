...
public abstract class AbstractRealMatrix implements RealMatrix {
	...
	protected AbstractRealMatrix(final int rowDimension,
		...
		if (columnDimension <= 0) {
			throw MathRuntimeException.createIllegalArgumentException(
					LocalizedFormats.INSUFFICIENT_DIMENSION, columnDimension,
					1);
		}
		lu = null;
	}
	...
	public RealMatrix getSubMatrix(final int startRow, final int endRow,
		...
		for (int i = startRow; i <= endRow; ++i) {
			for (int j = startColumn; j <= endColumn; ++j) {
				subMatrix.setEntry(i - startRow, j - startColumn,
						getEntry(i, j));
			}
		}
	...
	public RealMatrix getSubMatrix(final int[] selectedRows,
		...
		// copy entries
		final RealMatrix subMatrix = createMatrix(selectedRows.length,
				selectedColumns.length);
		subMatrix.walkInOptimizedOrder(new DefaultRealMatrixChangingVisitor() {

			/** {@inheritDoc} */
			@Override
			public double visit(final int row, final int column,
					final double value) {
	...
	public void copySubMatrix(final int startRow, final int endRow,
		...
		if ((destination.length < rowsCount)
				|| (destination[0].length < columnsCount)) {
			throw MathRuntimeException.createIllegalArgumentException(
					LocalizedFormats.DIMENSIONS_MISMATCH_2x2,
					destination.length, destination[0].length, rowsCount,
					columnsCount);
		}
	...
	public void setRowVector(final int row, final RealVector vector)
		...
		if (vector.getDimension() != nCols) {
			throw new InvalidMatrixException(
					LocalizedFormats.DIMENSIONS_MISMATCH_2x2, 1,
