...
public class PolynomialFunction
	...
	public PolynomialFunction add(final PolynomialFunction p) {
		...
		final int lowLength = FastMath.min(coefficients.length,
				p.coefficients.length);
		final int highLength = FastMath.max(coefficients.length,
				p.coefficients.length);
	...
	public PolynomialFunction subtract(final PolynomialFunction p) {
		...
		// identify the lowest degree polynomial
		int lowLength = FastMath.min(coefficients.length,
				p.coefficients.length);
		int highLength = FastMath.max(coefficients.length,
				p.coefficients.length);
	...
	public PolynomialFunction multiply(final PolynomialFunction p) {
		...
		for (int i = 0; i < newCoefficients.length; ++i) {
			...
			for (int j = FastMath.max(0,
					i + 1 - p.coefficients.length); j < FastMath
							.min(coefficients.length, i + 1); ++j) {
