 ...
 * for finding zeros of real univariate functions.
 * <p>
...
public class BrentSolver extends UnivariateRealSolverImpl {
	 ...
	 *            lower bound for absolute accuracy of solutions returned by the
	 *            solver
	 * @since 2.1
	 */
	...
	/**
	 * Contstruct a solver with the given maximum iterations and absolute
	 * accuracy.
	 *
	...
	public double solve(final UnivariateRealFunction f, final double min,
		...
		if (sign > 0) {
			...
			if (FastMath.abs(yMin) <= functionValueAccuracy) {
				...
				// root.
				throw MathRuntimeException.createIllegalArgumentException(
						LocalizedFormats.SAME_SIGN_AT_ENDPOINTS, min, max, yMin,
						yMax);
			}
		} else if (sign < 0) {
	...
	private double solve(final UnivariateRealFunction f, double x0, double y0,
		...
		while (i < maximalIterationCount) {
			...
			double dx = x2 - x1;
			double tolerance = FastMath.max(relativeAccuracy * FastMath.abs(x1),
					absoluteAccuracy);
