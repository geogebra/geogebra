...
public class Ggb2MPReduce {
	...
	public Map<String,String> getMap() {
		...
		p("Sum.4",
				"<<begin scalar sb, sm!!; on combinelogs,combineexpt;sb:=sum(%0,%1,%2,%3)$ sm!!:=if freeof(sb,\\'sum) and freeof(sb,\\'infinity) then sb else if %2=-infinity and %3=infinity then limit(limit(sum(%0,%1,k!!,m!!),k!!,-infinity),m!!,infinity) else if %2=-infinity then limit(sum(%0,%1,k!!,%3),k!!,-infinity) else if %3=infinity then limit(sum(%0,%1,%2,k!!),k!!,infinity) else sb; return sm!! end>>");
		p("Tangent.2",
