...
public abstract class CASgiac implements CASGenericInterface {
	...
	protected final static String specialFunctions =
					...
					"xcoord(x):=when(type(x)==DOM_IDENT,xcoordsymb(x),when(x[0]=='pnt',real(x[1]),x[0]));"+ 
					"ycoord(x):=when(type(x)==DOM_IDENT,ycoordsymb(x),when(x[0]=='pnt',im(x[1]),x[1]));"+ 
					// make sure z((1,2)) = 0
					"zcoord(x):=when(type(x)==DOM_IDENT,zcoordsymb(x),when(x[0]=='pnt',0,when(length(x)<3,0,x[2])));"+ 
	...
	final public String evaluateRaw(final String input) throws Throwable {
		...
		if (result.trim().startsWith("\"")) {
			...
			else { // this is a special string output (only for the prover at the moment)
				result = result.substring(1, result.length() - 1); // removing \" from left and right
			}
		}
