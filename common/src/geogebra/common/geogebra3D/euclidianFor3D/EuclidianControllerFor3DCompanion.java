...
public class EuclidianControllerFor3DCompanion extends EuclidianControllerCompanion {
	...
	protected GeoAngle createAngle(GeoPointND A, GeoPointND B, GeoPointND C){
		...
		if (((GeoElement) A).isGeoElement3D() || ((GeoElement) B).isGeoElement3D() || ((GeoElement) C).isGeoElement3D()) { // at least one 3D geo		
			...
			return ec.kernel.getManager3D().Angle3D(null, A, B, C, orientation); // use view orientation
	...
	protected GeoElement[] createAngles(GeoPolygon p){
		...
		if (p.isGeoElement3D()){ // 3D polygon
			...
			return ec.kernel.getManager3D().Angles3D(null, p, orientation); // use view orientation
		}
	...
	protected GeoAngle createAngle(GeoVectorND v1, GeoVectorND v2){
		...
		if (v1.isGeoElement3D() || v2.isGeoElement3D()){ // at least one 3D geo	
			...
			return ec.kernel.getManager3D().Angle3D(null, v1, v2, orientation); // use view orientation
		}
	...
	public GeoAngle createAngle(GeoPointND p1, GeoPointND p2, GeoNumberValue a, boolean clockWise) {
		...
		if (direction == ec.kernel.getXOYPlane() || direction == ec.kernel.getSpace()){ // use xOy plane
			if (p1.isGeoElement3D() || p2.isGeoElement3D()){
				return (GeoAngle) ec.kernel.getManager3D().Angle(null, p1, p2, a, ec.kernel.getXOYPlane(), !clockWise)[0];
			}

			return super.createAngle(p1, p2, a, clockWise);
		}
		
		return (GeoAngle) ec.kernel.getManager3D().Angle(null, p1, p2, a, direction, !clockWise)[0];
	}
	...
	protected GeoAngle createLineAngle(GeoLineND g, GeoLineND h){
		...
		if (g.isGeoElement3D() || h.isGeoElement3D()){ // at least one 3D geo
			...
			return ec.kernel.getManager3D().createLineAngle(g, h, orientation); // use view orientation
		}
	...
	public GeoPointND getSingleIntersectionPoint(GeoElement a, GeoElement b, boolean coords2D) {
		...
		// first hit is a conic
			...
			if (b.isGeoLine()) {
				point = ec.kernel.getManager3D().IntersectLineConicSingle(null, 
						(GeoLineND)b, (GeoConicND)a, ec.xRW, ec.yRW, ec.view.getInverseMatrix());
			} else if (b.isGeoConic() && !a.isEqual(b)) {
				point = ec.kernel.getManager3D().IntersectConicsSingle(null, 
						(GeoConicND)a, (GeoConicND)b, ec.xRW , ec.yRW, ec.view.getInverseMatrix());
			} else {
				return null;
			}
		}
	...
	protected GeoElement[] orthogonal(GeoPointND point, GeoLineND line){
		return new GeoElement[] {(GeoElement) ec.kernel.getManager3D().OrthogonalLine3D(null, point, line, ec.view.getDirection())};		
	...
	public GeoPointND createNewPoint(String label, boolean forPreviewable, Path path, double x,
		...
		if (geo.isGeoElement3D() || (geo.isGeoList() && ((GeoList) geo).containsGeoElement3D())) {
			...
			GeoPointND point = ec.kernel.getManager3D().Point3D(label, path, x, y, z,
	...
	protected GeoElement midpoint(GeoSegmentND segment){
		...
		if (((GeoElement) segment).isGeoElement3D()) {
			return (GeoElement) ec.kernel.getManager3D().Midpoint(null, segment);
		} 
	...
	protected GeoElement midpoint(GeoPointND p1, GeoPointND p2){
		...
		if (((GeoElement) p1).isGeoElement3D()
			...
			AlgoMidpoint3D algo = new AlgoMidpoint3D(ec.kernel.getConstruction(), p1, p2);
	...
	public GeoElement[] regularPolygon(GeoPointND geoPoint1, GeoPointND geoPoint2, GeoNumberValue value){
		...
		if (geoPoint1.isGeoElement3D() || geoPoint2.isGeoElement3D()){
			return  ec.kernel.getManager3D().RegularPolygon(null, geoPoint1, geoPoint2, value, ec.view.getDirection());
		}
		
		return ec.kernel.getAlgoDispatcher().RegularPolygon(null, geoPoint1, geoPoint2, value);
	}
	...
	protected GeoElement[] createCircle2For3D(GeoPointND p0, GeoPointND p1) {
		return new GeoElement[] { ec.kernel.getManager3D().Circle3D(null, p0, p1,
	...
	protected GeoConicND circleFor3D(Construction cons, GeoPointND center, NumberValue radius){
		return ec.kernel.getManager3D().Circle3D(null, center, radius, ec.view.getDirection());
	}
	...
	@Override
	protected GeoElement circleArcSector(GeoPointND A, GeoPointND B, GeoPointND C, int type){
		
		GeoDirectionND orientation = ec.view.getDirection();
		
		if (((GeoElement) A).isGeoElement3D() || ((GeoElement) B).isGeoElement3D() || ((GeoElement) C).isGeoElement3D()) { // at least one 3D geo		
			if (orientation == ec.kernel.getSpace()){ // space is default orientation for 3D objects
				return (GeoElement) ec.kernel.getManager3D().CircleArcSector3D(null, A, B, C, type);
			}
			return (GeoElement) ec.kernel.getManager3D().CircleArcSector3D(null, A, B, C, orientation, type); // use view orientation
			
		}
			
		// 2D geos
		if (orientation == ec.kernel.getXOYPlane()){ // xOy plane is default orientation for 2D objects
			return super.circleArcSector(A, B, C, type);
		}
		return (GeoElement) ec.kernel.getManager3D().CircleArcSector3D(null, A, B, C, orientation, type); // use view orientation
	...
	protected GeoElement circumcircleArc(GeoPointND p1, GeoPointND p2, GeoPointND p3){
		if (p1.isGeoElement3D() || p2.isGeoElement3D() || p3.isGeoElement3D()){
			return (GeoElement) ec.kernel.getManager3D().CircumcircleArc3D(null, p1, p2, p3);
		}
	...
	protected GeoElement circumcircleSector(GeoPointND p1, GeoPointND p2, GeoPointND p3){
		if (p1.isGeoElement3D() || p2.isGeoElement3D() || p3.isGeoElement3D()){
			return (GeoElement) ec.kernel.getManager3D().CircumcircleSector3D(null, p1, p2, p3);
		}
