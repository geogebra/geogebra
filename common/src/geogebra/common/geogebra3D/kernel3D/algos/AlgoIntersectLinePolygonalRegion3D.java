...
public class AlgoIntersectLinePolygonalRegion3D extends
		AlgoIntersectLinePolygonalRegion {
	...
	/**
	 * This assumes that the line is in the plane of polygon and the polygon
	 * acts as a region
	 * 
	 * @param c
	 * @param labels
	...
	protected void intersectionsCoords(GeoLineND g, GeoPolygon p,
		...
		if (!lineInPlaneOfPolygon) {
			...
			// AlgoIntersectCS1D2D algo = new AlgoIntersectCS1D2D(cons, null,
			// (GeoElement) g, p);
			// GeoPoint3D point = (GeoPoint3D) algo.getIntersection();
			Coords globalCoords = new Coords(4);
			Coords inPlaneCoords = new Coords(4);
			Coords singlePoint = AlgoIntersectCS1D2D.getIntersectLinePlane(g,
					p, globalCoords, inPlaneCoords);
	...
	protected void intersectionsSegments(GeoLineND g, GeoPolygon p,
		...
		if (isEnteringRegion && !Kernel.isEqual(tOld, maxKey))
