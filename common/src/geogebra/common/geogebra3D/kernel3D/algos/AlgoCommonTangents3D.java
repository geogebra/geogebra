...
import geogebra.common.kernel.algos.AlgoCommonTangentsND;
...
public class AlgoCommonTangents3D extends AlgoCommonTangentsND {
    ...
    protected void setCoordsAsPoint(int index, double x, double y){
    	...
    	((GeoPoint3D) P[index]).setCoords(c[index].getCoordSys().getPoint(x,y));
    }
	...
	protected void setCoordsAsVector(int index, double x, double y){
    	...
    	((GeoPoint3D) P[index]).setCoords(c[index].getCoordSys().getVector(x,y));
	...
	public void initForNearToRelationship() {       
        AlgoTangentPoint3D.initForNearToRelationship(tangentPoints, tangents[0], algoIntersect);
        AlgoTangentPoint3D.initForNearToRelationship(tangentPoints2, tangents[2], algoIntersect2);
    }
	...
	protected boolean isIntersectionPointIncident(int index, GeoConicND conic){
    	return conic.isIntersectionPointIncident(coords2D[index], Kernel.MIN_PRECISION);
    }
...
//    			((GeoLine3D) tangents[0+2*index]).setCoord(tangentPoints[0].getCoords(), P[index].getCoords());
//    		}else{
    			((GeoLine3D) tangents[0+2*index]).setCoord(P[index], tangentPoints[0]);
//    		}
    	}
...
//    			((GeoLine3D) tangents[1+2*index]).setCoord(tangentPoints[1].getCoords(), P[index].getCoords());
//    		}else{
    			((GeoLine3D) tangents[1+2*index]).setCoord(P[index], tangentPoints[1]);
//    		}
    	}
     }
	...
	protected void setTangentFromPolar(int i, GeoLine line){
    	...
    	if (i == 0 || i == 2){ // for second tangent, calculations are already done
	...
	protected boolean checkUndefined(){
    	...
    	for (int csIndex = 0 ; csIndex < 2 ; csIndex++){
    		...
    		midpointProjected[csIndex][mpIndex] = c[csIndex].getCoordSys().getNormalProjection(c[mpIndex].getMidpoint3D())[1];
    	...
    	for (int csIndex = 0 ; csIndex < 2 ; csIndex++){
    		midpointProjected[csIndex][csIndex] = c[csIndex].getCoordSys().getNormalProjection(c[csIndex].getMidpoint3D())[1];   		
    	}
