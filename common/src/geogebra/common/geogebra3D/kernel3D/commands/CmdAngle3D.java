...
public class CmdAngle3D extends CmdAngle {
	...
	protected GeoElement[] process2(Command c, GeoElement[] arg, boolean[] ok){
		...
		if ((ok[0] = (arg[0].isGeoLine()))
			...
			GeoElement[] ret = { kernelA.getManager3D().Angle3D(c.getLabel(), (GeoLineND) arg[0], (GeoPlane3D) arg[1]) };
			return ret;
		}
		if ((ok[1] = (arg[1].isGeoLine()))
				&& (ok[0] = (arg[0].isGeoPlane()))) {
			GeoElement[] ret = { kernelA.getManager3D().Angle3D(c.getLabel(), (GeoLineND) arg[1], (GeoPlane3D) arg[0]) };
			return ret;
		}
		...
		if ((ok[0] = (arg[0].isGeoPlane()))
				&& (ok[1] = (arg[1].isGeoPlane()))) {
			GeoElement[] ret = { kernelA.getManager3D().Angle3D(c.getLabel(), (GeoPlane3D) arg[0], (GeoPlane3D) arg[1]) };
			return ret;
		}
