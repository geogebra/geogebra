...
public class CmdMirror3D extends CmdMirror {
	...
	protected GeoElement[] process2(String label, GeoElement[] arg, boolean[] ok){
		...
		if(arg[1] instanceof GeoCoordSys2D && !arg[1].isGeoConic()) { // no override for mirror at circle				
			ret = kernelA.getManager3D().Mirror3D(label, arg[0], (GeoCoordSys2D) arg[1]);
			return ret;
			
		}else if (arg[0].isGeoElement3D() || arg[1].isGeoElement3D()){ // check if there is a 3D geo
