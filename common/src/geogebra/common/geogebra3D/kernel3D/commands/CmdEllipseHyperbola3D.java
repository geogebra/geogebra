...
public class CmdEllipseHyperbola3D extends CmdEllipseHyperbola {
	...
	private final GeoElement ellipseHyperbola(String label, GeoPointND a, GeoPointND b, GeoNumberValue v, GeoDirectionND orientation){
		
		// check if all 2D
		if (!a.isGeoElement3D() && !b.isGeoElement3D() && orientation == kernelA.getXOYPlane()){
		...
		if (type == GeoConicNDConstants.CONIC_HYPERBOLA){
			algo = new AlgoHyperbolaFociLength3D(kernelA.getConstruction(), label, 
					a,b,v,orientation);
		}else{ // ellipse
