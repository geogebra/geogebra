...
import geogebra.common.kernel.commands.CmdSemicircle;
	...
	protected GeoElement semicircle(String label, GeoPointND A, GeoPointND B){	
		...
		if (orientation != null){
			return (GeoElement) kernelA.getManager3D().
					Semicircle3D(label, A, B, orientation) ;
			}
		
		if (A.isGeoElement3D() || B.isGeoElement3D()){
			return (GeoElement) kernelA.getManager3D().Semicircle3D(label, A, B, kernelA.getXOYPlane());
		}
