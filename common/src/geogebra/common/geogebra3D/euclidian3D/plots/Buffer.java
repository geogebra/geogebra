 ...
 * write elements from index zero to <code>limit - 1</code>. Accessing elements
 * out of the scope will cause an exception. Limit may not be negative and not
 * greater than capacity.</li>
 ...
 * <li>A buffer can be read-only or read-write. Trying to modify the elements of
 * a read-only buffer will cause a <code>ReadOnlyBufferException</code>, while
 * changing the position, limit and mark of a read-only buffer is OK.</li>
