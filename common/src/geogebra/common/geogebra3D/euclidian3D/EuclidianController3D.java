...
import geogebra.common.geogebra3D.euclidian3D.draw.DrawConic3D;
import geogebra.common.geogebra3D.euclidian3D.draw.DrawConicSection3D;
import geogebra.common.geogebra3D.euclidian3D.draw.DrawExtrusionOrConify3D;
import geogebra.common.geogebra3D.euclidian3D.draw.DrawIntersectionCurve3D;
...
public abstract class EuclidianController3D extends EuclidianController {
	...
	protected GeoPointND getNewPoint(Hits hits, boolean onPathPossible,
		...
		default:
			pointCreated = super.getNewPoint(hits, onPathPossible, inRegionPossible,
					intersectPossible, false);
			return pointCreated;
	...
	protected GeoPointND getSingleIntersectionPoint(Hits hits) {
		...
		// line/line or line/plane (only one intersection point)
			...
			point = (GeoPoint3D) getKernel().getManager3D().Intersect(null, a,
					b);
		}
		...
		if (point.isDefined()) {
			if (((GeoElement) point).isGeoElement3D()) {
				...
				Coords toScreenCoords = view3D
						.projectOnScreen(((GeoPoint3D) point).getCoords()
								.getCoordsLast1());
	...
	final protected GeoElement[] pyramidOrPrism(Hits hits) {
		...
		if (draggingOccuredBeforeRelease && (pyramidBasis == null) && selPoints() == 0 && selPolygons() == 0){
		...
		if (pyramidBasis == null) { // try to find/create a polygon
			...
			if (selPolygons() == 0) { // try to create a polygon
				...
				if (addSelectedPoint(hits, GeoPolygon.POLYGON_MAX_POINTS, false) != 0 
						|| 
						(!selectionPreview && !selectedPoints.isEmpty() && hits.contains(selectedPoints.get(0)))
						){
	...
	@Override
	public void showDrawingPadPopup(geogebra.common.awt.GPoint mouseLoc1) {
