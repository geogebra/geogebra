...
import geogebra.common.geogebra3D.euclidian3D.draw.DrawAngle3D;
import geogebra.common.geogebra3D.euclidian3D.draw.DrawAxis3D;
import geogebra.common.geogebra3D.euclidian3D.draw.DrawClippingCube3D;
import geogebra.common.geogebra3D.euclidian3D.draw.DrawConic3D;
import geogebra.common.geogebra3D.euclidian3D.draw.DrawConicPart3D;
import geogebra.common.geogebra3D.euclidian3D.draw.DrawConicSection3D;
import geogebra.common.geogebra3D.euclidian3D.draw.DrawConify3D;
import geogebra.common.geogebra3D.euclidian3D.draw.DrawCurve3D;
...
import geogebra.common.geogebra3D.euclidian3D.draw.DrawPointDecorations;
import geogebra.common.geogebra3D.euclidian3D.draw.DrawPolyLine3D;
...
import geogebra.common.geogebra3D.euclidian3D.draw.DrawQuadric3D;
import geogebra.common.geogebra3D.euclidian3D.draw.DrawQuadric3DLimited;
	...
	public void updateMatrixForCursor3D() {
		...
		if (getEuclidianController().getMode() == EuclidianConstants.MODE_VIEW_IN_FRONT_OF) {
			...
			case PREVIEW_POINT_REGION:
				...
				CoordMatrix4x4.createOrthoToDirection(getCursor3D().getDrawingMatrix()
						.getOrigin(), v, CoordMatrix4x4.VZ, tmpCoords1, tmpCoords2, tmpMatrix4x4);
			...
			case PREVIEW_POINT_PATH:
				...
				v = ((GeoElement) getCursor3D().getPath()).getMainDirection();
				CoordMatrix4x4.completeOrtho(v, tmpCoords1, tmpCoords2, tmpMatrix4x4_2);

				getCursor3D().getDrawingMatrix().setVx(
						tmpMatrix4x4_2.getVx().normalized().mul(t));
				t *= (10 + ((GeoElement) getCursor3D().getPath())
						.getLineThickness());
				getCursor3D().getDrawingMatrix().setVy(tmpMatrix4x4_2.getVy().mul(t));
				getCursor3D().getDrawingMatrix().setVz(tmpMatrix4x4_2.getVz().mul(t));
			...
			case PREVIEW_POINT_ALREADY:
				...
				if (getCursor3D().hasPath()) {
					...
					CoordMatrix4x4.completeOrtho(v, tmpCoords1, tmpCoords2, tmpMatrix4x4);
					...
					CoordMatrix4x4.createOrthoToDirection(getCursor3D().getCoordsInD3(),
							v, CoordMatrix4x4.VZ, tmpCoords1, tmpCoords2, tmpMatrix4x4_2);
				} else{
