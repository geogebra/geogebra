...
public class PolyGamma {
	private static final double klog10Of2 = log10(2), kDefaultWDTol = max(
			pow(2, -53), 0.5e-18);
	private static final int kMaxValue = 100, DBL_MANT_DIG = 53,
			DBL_MIN_EXP = -1021;
	private static final String sErrorDomain = "Math Error: DOMAIN"; //$NON-NLS-1$

	// Bernoulli Numbers
	static private double bvalues[] = { 1.00000000000000000e+00,
	...
	public static final double[] dpsifn(double x, int n, int kode, int m) {
		...
		if (x <= 0.) {
			/*
			 * use Abramowitz & Stegun 6.4.7 "Reflection Formula" psi(k, x) =
			 * (-1)^k psi(k, 1-x) - pi^{n+1} (d/dx)^n cot(x)
			 */
			if (x == (long) x) {
				/* non-positive integer : +Inf or NaN depends on n */
				for (j = 0; j < m; j++)
					/* k = j + n : */
					ans[j] = ((j + n) % 2 == 1) ? Double.POSITIVE_INFINITY
							: Double.NaN;
				return ans;
			}
			dpsifn(1. - x, n, 1, m);
			/*
			 * ans[j] == (-1)^(k+1) / gamma(k+1) * psi(k, 1 - x) for j = 0:(m-1)
			 * , k = n + j
			 */
			...
			s = (n % 2 == 1) ? -1. : 1.;/* s = (-1)^n */
			/*
			 * t := pi^(n+1) * d_n(x) / gamma(n+1) , where d_n(x) := (d/dx)^n
			 * cot(x)
			 */
			t1 = t2 = s = 1.;
		...
		// wdtol = kDefaultWDTol; //max(pow(FLT_RADIX, 1-DBL_MANT_DIG) * 0.5,
		// 0.5e-18);
		...
		if (abs(s) >= tst) {
			tk = 2.0;
			for (k = 4; k <= 22; k++) {
				t = t * ((tk + fn + 1) / (tk + 1.0)) * ((tk + fn) / (tk + 2.0))
						* rxsq;
				trm[k] = t * bvalues[k - 1];
				/* !* if (fabs(trm[k]) < tst) *! */
				if (abs(trm[k]) < tst)
