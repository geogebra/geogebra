...
public class MyMathExact {
	...
	public static class MyDecimal extends BigDecimal {
		...
		public MyDecimal divide(MyDecimal md) {
			return new MyDecimal(this.getScale(),super.divide(md, this.getScale(), BigDecimal.ROUND_HALF_EVEN));
		}
		public MyDecimal sqrt() {
			...
			int iterCount = 0;
			while (ret.multiply(ret).subtract(radicand).divide(radicand).divide(new MyDecimal(thisScalePlusOne,lower_bound*2)).abs().doubleValue()
					> Math.pow(10, -thisScale) && iterCount < 5) {
	...
	public static class MyDecimalMatrix implements AnyMatrix {
		...
		public MyDecimalMatrix copy() {
			...
			MyDecimalMatrix mdm = new MyDecimalMatrix(
					fixedScale, rowD, colD);
		...
		public MyDecimalMatrix multiply(MyDecimalMatrix m) {
			...
			if (this.getColumnDimension()!=m.getRowDimension()) {
				throw MathRuntimeException.createIllegalArgumentException(
						"Cannot multiply " + getRowDimension() + " x " + getColumnDimension() + 
						" and " + m.getRowDimension() + " x " + m.getColumnDimension() + 
						"matrices!");
	                   
			}
			
			MyDecimalMatrix ret = new MyDecimalMatrix(this.getScale(), this.getRowDimension(), m.getColumnDimension());
			for (int i=0; i<this.getRowDimension(); i++) {
