...
public class ClipLine {
	 ...
	 */
	public static geogebra.common.awt.GPoint2D[] getClipped(double x1,
	 ...
	 * @return <code>null</code> (does not clip) or array of two points
	 */
	protected static geogebra.common.awt.GPoint2D[] getClipped(double x1,
		...
		if ((mask & LEFT) != 0) {
			...
			geogebra.common.awt.GPoint2D p = intersect(x1, y1, x2, y2, xmin,
					ymin, xmin, ymax);
			if (p != null) {
				if (p1 == null) {
					p1 = p;
		...
		if ((mask & RIGHT) != 0) {
			...
			geogebra.common.awt.GPoint2D p = intersect(x1, y1, x2, y2, xmax,
					ymin, xmax, ymax);
			if (p != null) {
				if (p1 == null) {
					p1 = p;
		...
		if (p1 != null && p1.getY() == (ymin + 0.5)) {
			...
			if ((mask & ABOVE) != 0) {
				...
				geogebra.common.awt.GPoint2D p = intersect(x1, y1, x2, y2,
						xmin, ymax, xmax, ymax);
				if (p != null) {
					geogebra.common.awt.GPoint2D[] ret = new geogebra.common.awt.GPoint2D[2];
					ret[0] = p1;
	 ...
	 */
	private static geogebra.common.awt.GPoint2D intersect(double x11,
		...
		if (det != 0.0) {
			...
			if (mu >= 0.0 && mu <= 1.0) {
				geogebra.common.awt.GPoint2D p = AwtFactory.prototype
