...
public abstract class EuclidianView implements EuclidianViewInterfaceCommon {
	...
	// or use volatile image
		...
		private static final geogebra.common.awt.GColor colZoomRectangle = 
				geogebra.common.factories.AwtFactory.prototype.newColor(200, 200, 230);
		private static final geogebra.common.awt.GColor colZoomRectangleFill = 
		...
		private static geogebra.common.awt.GBasicStroke defAxesStroke = geogebra.common.factories.AwtFactory.prototype.newBasicStroke(1.0f,
				geogebra.common.awt.GBasicStroke.CAP_BUTT, geogebra.common.awt.GBasicStroke.JOIN_MITER);
		private static geogebra.common.awt.GBasicStroke boldAxesStroke = 
	...
	private geogebra.common.awt.GBasicStroke axesStroke, tickStroke, gridStroke;
	/** kernel */
	...
	private geogebra.common.awt.GAffineTransform coordTransform = 
	 ...
	 */
	public EuclidianView(EuclidianController ec,
	 ...
	 */
	protected void setCoordTransform(geogebra.common.awt.GAffineTransform coordTransform) {
	 ...
	 */
	public abstract  GGraphics2D getTempGraphics2D(geogebra.common.awt.GFont fontForGraphics);
	/**
	 * @return font
	 */
		...
		case NUMERIC:
			...
			if (d != null) {
				if (!geo.isColorSet()) {
					...
					if (geo.isIndependent()) {
						geogebra.common.awt.GColor col =consDef
										.getDefaultGeo(
	...
	public void updateFonts() {
		...
		setFontCoords(getApplication().getPlainFontCommon().deriveFont(GFont.PLAIN, getFontSize() - 2));
	...
	// Michael Borcherds 2008-03-01
		 ...
		 */
		protected void drawBackgroundWithImages(geogebra.common.awt.GGraphics2D g, boolean transparency) {
			if (!transparency) {
				clearBackground(g);
			}
		 ...
		 */
		final protected void drawBackground(geogebra.common.awt.GGraphics2D g, boolean clear) {
			if (clear) {
				clearBackground(g);
			}
		...
		private GEllipse2DDouble circle = geogebra.common.factories.AwtFactory.prototype.newEllipse2DDouble(); //polar grid circles
		private GLine2D tempLine = geogebra.common.factories.AwtFactory.prototype.newLine2D();
		/**
		 * Get styleBar
		 */
		...
		private double getLabelLength(double rw, GFontRenderContext frc) {
			GTextLayout layout = geogebra.common.factories.AwtFactory.prototype.newTextLayout(
		...
		protected void drawAxes(geogebra.common.awt.GGraphics2D g2) {
			...
			GColor bgCol = showGrid && !moveAxesLabels ? getBackgroundCommon() : null;
			...
			if (xAxisOnscreen()) {
				...
				if (axesLabels[0] != null) {
					GTextLayout layout = geogebra.common.factories.AwtFactory.prototype.newTextLayout(axesLabels[0], getFontLine().deriveFont(axesLabelsStyle[0]), frc);
					layout.draw(g2, 
					...
					if (pix <= maxX) {
						if (showAxesNumbers[0]) {
							...
							if ((labelno % unitsPerLabelX) == 0) {
								...
								GTextLayout layout = geogebra.common.factories.AwtFactory.prototype.newTextLayout(sb.toString(),
										getFontAxes(), frc);
			...
			if (yAxisOnscreen()) {
				...
				if (axesLabels[1] != null) {
					GTextLayout layout = geogebra.common.factories.AwtFactory.prototype.newTextLayout(axesLabels[1], getFontLine().deriveFont(axesLabelsStyle[1]), frc);
			...
			if (showAxesCornerCoords) {
				if ((getXmin() > 0) || (getXmax() < 0) || (getYmin() > 0) || (getYmax() < 0)) {
					...
					GTextLayout layout = geogebra.common.factories.AwtFactory.prototype.newTextLayout(sb.toString(), getFontAxes(), frc);
		 ...
		 */
		private void drawStringWithBackground(GGraphics2D g2, String text, double x, double y, GColor bgCol, GTextLayout layout, int spaceToLeft, int spaceBelow) {
			...
			for (; pix <= yAxisHeight; rw -= axesNumberingDistances[1], pix += axesStep) {
				if (pix <= maxY) {
					if (showAxesNumbers[1]) {
						...
						geogebra.common.awt.font.GTextLayout layout = geogebra.common.factories.AwtFactory.prototype.newTextLayout(sb.toString(), 
								getFontAxes(),frc);
		...
		public void drawPoints(GeoImage ge, double[] x, double[] y) {
			...
			doDrawPoints(ge, ptList, GColor.black, EuclidianStyleConstants.LINE_TYPE_FULL, 1);

		}

		@SuppressWarnings("javadoc")
		protected abstract void doDrawPoints(GeoImage gi, List<geogebra.common.awt.GPoint> penPoints2, GColor penColor, int penLineStyle, int penSize);
	...
	final public void mouseMovedOverList(ArrayList<GeoElement> geoList) {
		...
		protected void initView(boolean repaint) {
			...
			setAxesColor(geogebra.common.awt.GColor.black); // Michael Borcherds 2008-01-26 was darkgray
