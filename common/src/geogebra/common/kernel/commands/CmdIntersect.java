	...
	public  GeoElement[] process(Command c) throws MyError {
		...
		case 2 :
			...
			// Line - PolyLine
				...
				GeoElement[] ret =
						getAlgoDispatcher().IntersectLinePolyLine(
								c.getLabels(),
								(GeoLine) arg[0],
								(GeoPolyLine) arg[1]);
				return ret;
			...
			// Line - Polygon(as boudary)
				...
				GeoElement[] ret =
						getAlgoDispatcher().IntersectLinePolygon(
								c.getLabels(),
								(GeoLine) arg[0],
								(GeoPolygon) arg[1]);
				return ret;
			...
			// intersection of two lists
				...
				GeoElement[] ret = { 
						getAlgoDispatcher().Intersection(c.getLabel(),
		...
		case 3 : // only one of the intersection points: the third argument
			...
			// check before GeoFunctionable as GeoLine is now GeoFunctionable
				...
				GeoElement[] ret =
					{
						getAlgoDispatcher().IntersectFunctionLine(
								c.getLabel(),
								((GeoFunctionable) arg[0]).getGeoFunction(),
								(GeoLine) arg[1],
								(GeoPoint) arg[2])};
			...
			// Function - Function with startPoint
				...
				GeoElement[] ret =
					{
						getAlgoDispatcher().IntersectFunctions(
								c.getLabel(),
								((GeoFunctionable) arg[0]).getGeoFunction(),
								((GeoFunctionable) arg[1]).getGeoFunction(),
								(GeoPoint) arg[2])};
