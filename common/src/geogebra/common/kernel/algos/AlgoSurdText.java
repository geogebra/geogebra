...
import org.apache.commons.math.dfp.DfpDec;
...
public class AlgoSurdText extends AlgoElement {
    ...
    int[][] mPSLQ(int n, double[] x, double AccuracyFactor, int bound) {
    	...
    	Array2DRowFieldMatrix<Dfp> B_comp;
    	Array2DRowFieldMatrix<Dfp> result2;
    	...
    	*/
    	...
    	IntRelationFinder irf = new IntRelationFinder(oldp, x, decFull, decLess, AccuracyFactor, bound);
    	...
    	r2 = new Array2DRowFieldMatrix<Dfp> (m.getBMatrix().getField(),n,n);
    	Dfp ZERO = m.getBMatrix().getField().getZero();
    	...
    	while(oldp>=2 && p<oldp && rCols<n-1) {
    		...
    		oldp=p; p=0;
    		irf = new IntRelationFinder(oldp, x, new DfpField(64), new DfpField(16), AccuracyFactor, bound);
        	if( irf.result.size() == 0 ) {
        		break;
        	}
        	m = irf.result.get(0);
    		...
    		for (int j = 0; j<q; j++) {
    			...
    			for (int i=0; i<n; i++)
    				allZero = allZero && result2.getEntry(i, j).rint().equals(ZERO);
    			if (allZero)
    				break;
    			...
    			for (int i=0; i<n; i++)
    				tooLargeEntry = tooLargeEntry || Math.abs(new Double(result2.getEntry(i, j).rint().toString())) > bound;
    			if (tooLargeEntry)
    				break;
    	...
    	for (int i=0; i<n; i++){
    		for (int j=0; j<rCols; j++) {
    			result[i][j]=(new Double(r2.getEntry(i,j).rint().toString())).intValue();
    		}
    	}
    	return result;
    }
	...
	* from Ferguson and Bailey (1992)
	...
	public FullDec frobNormSq(Array2DRowFieldMatrix<Dfp> matrix, int m, int n) {
		//m is number of rows; n is number of columns
		FullDec ret = new FullDec(0);
		...
		for (int i=0; i<m; i++)
			for (int j=0; j<n; j++)
				ret = ret.newInstance(ret.add(matrix.getEntry(i, j).multiply(matrix.getEntry(i, j))));
	...
	public class FullDec extends DfpDec{
		...
		public Dfp newInstance(BigReal multiply) {
		...
		protected FullDec(Dfp x) {
			super(x);
		}
		
		
		public FullDec newInstance(Dfp x) {
			return new FullDec(x);
		}
		
		
		//round to the nearest 10Es
		public FullDec roundTo(int s) {
			
			FullDec r = new FullDec(this);
			if (s!=0) {
				r = r.newInstance(r.multiply(r.power10(-s)));
			}
			r = r.newInstance(r.rint());
			if (s!=0) {
				r = r.newInstance(r.multiply(r.power10(s)));
			}
	...
	private class DoubleDec extends DfpDec{
		...
		protected DoubleDec(Dfp x) {
			super(decLess, x.toString());
		}
		
		public DoubleDec newInstance(Dfp x) {
			return new DoubleDec(x);
		}
	...
	private class IntRelationFinder {
		...
		//constants
		private FullDec ZERO = new FullDec(0);
		private FullDec ONE = new FullDec(1); 
		private DoubleDec ZERO_LESS = new DoubleDec(0);
		private DoubleDec ONE_LESS = new DoubleDec(1);
		...
		private int n;
		private double[] x;
		private FullDec[] x_full;
		private DoubleDec[] x_double;
		
		
		//working variables
		private DfpField decFull;
		private DfpField decLess;
		
		private FullDec xNorm;
		private Array2DRowFieldMatrix<Dfp> H_full;
		private Array2DRowFieldMatrix<Dfp> H;
		Array2DRowFieldMatrix<Dfp> I;
		private Array2DRowFieldMatrix<Dfp> A;
		private Array2DRowFieldMatrix<Dfp> B;
		private Array2DRowFieldMatrix<Dfp> D;
		//private Array2DRowFieldMatrix<Dfp> B_comp; //B_comp: *= new B_rest (B_rest is in IntRelation class)
		private Array2DRowFieldMatrix<Dfp> xB;
		
		private DoubleDec a,b,l,d;
		private int r;
		...
		IntRelationFinder(int n, double[]x, DfpField decFull, DfpField decLess, double err, double bound) { 

			
			this.decFull = decFull;
			this.decLess = decLess;
			this.n=n;
			this.err = err;
			this.M = bound;
			...
			int digitsAllocated = decLess.getRadixDigits() *4;
			...
			if (needSizeChange)
				this.decLess = new DfpField(digitsAllocated);
			
			needSizeChange = false;
			digitsNeeded = n*digitsNeeded;
			digitsAllocated = decFull.getRadixDigits() *4;
			while (digitsAllocated <= digitsNeeded) {
				needSizeChange = true;
				digitsAllocated *=2;
			}
			if (needSizeChange)
				this.decFull = new DfpField(digitsAllocated);
	    	...
	    	if (n==1) {
	    		...
	    		B = new Array2DRowFieldMatrix<Dfp>(this.decLess,1,1);
	    		B.setEntry(0, 0, ONE_LESS);
	    		xB = new Array2DRowFieldMatrix<Dfp>(this.decLess,1,1);
			...
			this.x = new double[n];
			this.x_full = new FullDec[n]; //normalized, full precision
			this.x_double = new DoubleDec[n]; //normalized, double precision
			
			//int[] orthoIndices = new int[n];
			
			for (int i=0; i<n; i++) {
				this.x[i] = x[i];
				this.x_full[i] = new FullDec(x[i]);
			}
			...
			while (iterCount < iterBound) {
				...
				for (int index=0; index<n-1; index++) {
					gammaPow *= gamma;
					temp = gammaPow * Math.abs(H.getEntry(index, index).toDouble());
					if (max<temp) {
						max = temp;
						r = index;
					}
				...
				if (r<n-2) { //for r=n-2 we don't need to define these. Also l will be undefined
					a = a.newInstance(H.getEntry(r, r));
					b = b.newInstance(H.getEntry(r+1, r));
					l = l.newInstance(H.getEntry(r+1, r+1));
					d = d.newInstance(b.multiply(b).add(l.multiply(l)).sqrt());
				}
				
				DoubleDec temp2 = new DoubleDec(xB.getEntry(0, r));
				xB.setEntry(0, r,xB.getEntry(0, r+1));
				xB.setEntry(0, r+1, temp2);
				
				Dfp[] temp3 = H.getRow(r);
				...
				for (int j=0; j<n-1; j++) {
					if (Math.abs(new Double(H.getEntry(j, j).toString()))<Math.pow(10,-H.getEntry(j, j).getRadixDigits())) {
						relationExhausted = true;
						//System.out.println("relation Exhausted at iteration " + iterCount + "with j = " + j);
					}
				}
				for (int j=0; j<n; j++) { // we look at the j-th column of B
					...
					double maxxB = 0;
					for (int i=0; i<n; i++) {
						sumBCol += Math.abs(new Double(B.getEntry(i, j).toString()));
						maxxB = Math.max(maxxB, Math.abs(new Double(xB.getEntry(0, i).toString())));
					}
										
					if (xB.getEntry(0, j).equals(ZERO_LESS) || Math.abs(new Double(xB.getEntry(0, j).toString())) < sumBCol * err) {
		...
		void initialize_full() {
			...
			//normalize x
			for (int i=0; i<n; i++) {
				xNorm = xNorm.newInstance(xNorm.add(x_full[i].multiply(x_full[i])));
			}
			
			xNorm = xNorm.newInstance(xNorm.sqrt());
			
			//System.out.println(xNorm.toFullString());
			
			for (int i=0; i<n; i++) {
				x_full[i] = x_full[i].newInstance(x_full[i].divide(xNorm));
				x_double[i] = new DoubleDec(x_full[i]);
			}
			 ...
			 */
			FullDec[] ss = new FullDec[n];
			for (int i=0; i<n; i++) {
				ss[i] = new FullDec();
			}
			
			ss[n-1]= ss[n-1].newInstance(x_full[n-1].multiply(x_full[n-1]));
			
			for (int i = n-2; i>=0; i--) {
				ss[i]= ss[i].newInstance(ss[i+1].add(x_full[i].multiply(x_full[i])));
			}
			
			for (int i=0; i<n; i++) {
				ss[i] = ss[i].newInstance(ss[i].sqrt());
			}
			...
			//pre-calculate ss[j]*ss[j+1]
			FullDec[] Pss = new FullDec[n-1];
			for (int i=0; i<n-1; i++) {
				Pss[i] = new FullDec(ss[i].multiply(ss[i+1]));
			}
			
			//initialize Matrix H (lower trapezoidal
			H_full = new Array2DRowFieldMatrix<Dfp>(decFull, n,n-1);
			...
			*/
			...
			H = new Array2DRowFieldMatrix<Dfp>(decLess, n,n-1);
			for (int i=0; i<n; i++) {
				for (int j=0; j<n-1; j++) {
					H.setEntry(i, j, new DoubleDec(H_full.getEntry(i, j)));
				}
			}
			...
			*/
			...
			I = new Array2DRowFieldMatrix<Dfp>(decLess, n,n);
			for (int i=0; i<n; i++) {
				for (int j=0; j<n; j++) {
					if(i==j)
						I.setEntry(i, i, new DoubleDec(ONE)); //need a better method to treat this more directly
					else
						I.setEntry(i, j, new DoubleDec(ZERO));
				}
				
			}
			
			A = (Array2DRowFieldMatrix<Dfp>) I.copy();
			B = (Array2DRowFieldMatrix<Dfp>) I.copy();
			xB = new Array2DRowFieldMatrix<Dfp> (decLess, 1,n);
			for (int i=0; i<n; i++) {
				xB.setEntry(0, i, new DoubleDec(x_double[i]));
			}
		...
		private void hermiteReduction() {
			D = (Array2DRowFieldMatrix<Dfp>) I.copy();
			
			for (int i=1; i<n; i++) {
				for (int j=i-1; j>=0; j--) {
					DoubleDec q = new DoubleDec(H.getEntry(i, j).divide(H.getEntry(j, j)).rint());
					for (int k=0; k<=j; k++) {
		...
		private class IntRelation implements Comparable<IntRelation> {
			...
			private Array2DRowFieldMatrix<Dfp> B, B_sol, B_rest;
			Array2DRowFieldMatrix<Dfp> xB;
			int[] orthoIndices;

			
			public IntRelation(int n, Array2DRowFieldMatrix<Dfp> B, Array2DRowFieldMatrix<Dfp> xB, double sig) {
				
				if (n==0)
					return;
				this.size = n;
				this.B = (Array2DRowFieldMatrix<Dfp>) B.copy();
				this.xB = (Array2DRowFieldMatrix<Dfp>) xB.copy();
				this.sig = sig;
				...
				for (int j=0; j<n; j++) { // we look at the j-th column of B
					...
					if (xB.getEntry(0, j).equals(ZERO_LESS) || Math.abs(xB.getEntry(0, j).toDouble()) < sumBCol * err) {
				...
				if(nilDim>0) {
					B_sol = new Array2DRowFieldMatrix<Dfp>(B.getField(), size,nilDim);
				} else {
					B_sol = null;
				}
				if(nilDim<size) {
					B_rest = new Array2DRowFieldMatrix<Dfp>(B.getField(), size,size-nilDim);
				} else {
					B_rest = null;
				}
			...
			public Array2DRowFieldMatrix<Dfp> getBMatrix() {
				return (Array2DRowFieldMatrix<Dfp>) B.copy();
			}

			public Array2DRowFieldMatrix<Dfp> getBSolMatrix() {
				if (B_sol!=null)
					return (Array2DRowFieldMatrix<Dfp>) B_sol.copy();
				else
					return null;
			}
			
			public Array2DRowFieldMatrix<Dfp> getBRestMatrix() {
				if (B_rest!=null)
					return (Array2DRowFieldMatrix<Dfp>) B_rest.copy();
				else
					return null;
			}
	...
	private class AlgebraicFit  {
		...
		public AlgebraicFit(int numOfConstants, int numOfRadicals, String[] ConstStrings, double[] ConstValues,
			...
			if (ConstValues!=null)
				this.constValues = ConstValues.clone();
			if (ConstStrings!=null)
				this.constStrings = ConstStrings.clone();
			this.aft = aft;
			this.tpl = tpl;
