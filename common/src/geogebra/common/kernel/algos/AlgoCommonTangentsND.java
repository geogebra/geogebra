...
public abstract class AlgoCommonTangentsND extends AlgoElement implements TangentAlgo {
     ...
     * @param cons construction
     * @param p point
     * @param line polar line
     * @param conic conic
     * @return intersect algo
     */
    abstract protected AlgoIntersectND createAlgo(Construction cons, GeoPointND p, GeoLine line, GeoConicND conic);
    ...
    /**
     * @param csIndex coord sys index
     * @param mpIndex midpoint index
	...
	public final void compute() {
        ...
        // outer
        if( Math.abs(r2-r) > Kernel.MIN_PRECISION) {
            setCoordsAsPoint(0, (getMidpointX(0, 0)*r2-getMidpointX(0, 1)*r)/(r2-r),
                        (getMidpointY(0, 0)*r2-getMidpointY(0, 1)*r)/(r2-r));
        } else {
            setCoordsAsVector(0, (getMidpointX(0, 0)*r2-getMidpointX(0, 1)*r),
                        (getMidpointY(0, 0)*r2-getMidpointY(0, 1)*r));
        }
        // inner
        setCoordsAsPoint(1, (getMidpointX(1, 0)*r2+getMidpointX(1, 1)*r)/(r2+r),
     ...
     * @param index first/second tangents
     */
     ...
     */
    abstract protected boolean isIntersectionPointIncident(int index, GeoConicND conic);
    
    public GeoPointND getTangentPoint(GeoElement conic, GeoLine line) {
        if (conic != c[0] && conic != c[1]) return null;
        if (conic == c[0]) {
        	if (line == tangents[0]) {
