...
public class AlgoCauchyDF extends AlgoElement {
	...
	public void compute() {
		...
		ExpressionValue pi = newDoub(Math.PI);

		if (cumulative != null && cumulative.getBoolean()) {

			
			en = en.subtract(x0).divide(g.abs()).atan().divide(pi).plus(newDoub(0.5));
			...
			en = g.abs().divide(g.square().plus(en.subtract(x0).square()).multiply(pi));
