...
public class BigPolynomial implements Cloneable {
	...
	/**
	 * 
	 * @param p BigPolynomial to copy precision etc. from
	 * @param coeff used as the coefficients of this polynomial (directly, no clone)
	 */
	...
	public BigPolynomial add(BigPolynomial p){
		BigDecimal[] coeff=new BigDecimal[]{BigDecimal.ZERO};//=new BigDecimal[Math.max(coefficients.length, p.coefficients.length)];
...
//		a.coefficients=coeff;
	...
	public BigPolynomial multiply(BigPolynomial p){
		BigDecimal[] coeff=new BigDecimal[]{BigDecimal.ZERO};//new BigDecimal[(coefficients.length+p.coefficients.length)-1];
		for (int i=(coefficients.length+p.coefficients.length)-2;i>=0;i--){
			BigDecimal sum=BigDecimal.ZERO;
			for (int j=0;j<=i;j++){
				if (j<coefficients.length&&(i-j)<p.coefficients.length){
					sum=sum.add(coefficients[j].multiply(p.coefficients[i-j],mc),mc);
				}
...
//		}
		...
		quotient=new BigDecimal[]{BigDecimal.ZERO};//new BigDecimal[k+1-degD];
		...
		for (int i=k-degD;i>=0;i--){
			...
			if (!smallerEps(q)){
				...
				for (int j=0;j<=degD-1;j++){
					remainder[j+i]=remainder[j+i].subtract(quotient[i].multiply(divisor.coefficients[j],mc),mc);
				}
			}else{
				if (quotient.length>1){
...
//			}
	...
	public BigComplex getRoot(BigComplex[] coeff,BigComplex start,MathContext mc){
		BigComplex x=start;
		for (int i=0;i<MAX_ITERATIONS;i++){
			...
			BigComplex sq=h.multiply(BigDecimal.valueOf(n),mc).subtract(g2, mc).multiply(BigDecimal.valueOf(n-1), mc).sqrt(mc);
...
//			return BigComplex.ZERO;
//		}
//		BigComplex sum=poly[poly.length-1].multiply(new BigDecimal(poly.length-1));
...
//			return BigComplex.ZERO;
//		}
//		BigComplex sum=poly[poly.length-1].multiply(new BigDecimal((poly.length-1)*(poly.length-2)));
