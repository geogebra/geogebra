 ...
 * @author Michael
 * 
 * http://en.wikipedia.org/w/index.php?title=Curvature&section=8#Local_expressions_2
...
public class AlgoCurvatureVectorCurve3D extends AlgoElement {
	...
	private GeoCurveCartesian3D f, f1, f2; // f = f(x), f1 is f'(x), f2 is f''(x)
...
/*
	public AlgoCurvatureVectorCurve3D(Construction cons, GeoPoint A,
			GeoConic geoConic) {
		super(cons);
		this.A = A;
		this.gc = geoConic;
	...
	public final void compute() {
		try {
			...
			/*
			if (gc!=null){
				f=new GeoCurveCartesian2D(cons);
				gc.toGeoCurveCartesian(f);
				cas();
			}*/
