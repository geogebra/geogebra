...
public class RealSenseInfo {
	...
	public static native void isSupported(AsyncCallback clb) /*-{
		...
		if (typeof $wnd.RealSenseInfo !== 'function') {
			...
			clb.@geogebra.html5.util.AsyncCallback::onSuccess(Lcom/google/gwt/core/client/JavaScriptObject;)(result);

			return false;
		}
		
		$wnd.RealSenseInfo(['hand'], function (info) {
		if (info.IsReady == true) {
			...
			result.statusText = 'Platform not supported: ' + info.responseText
			...
			if (info.IsPlatformSupported != true) {
				...
				for (i = 0; i < info.Updates.length; i++) {
					resut.mgs = result.msg + '<a href="' + info.Updates[i].url + '">' + info.Updates[i].href + '</a><br>';
				}
			}
			result.isReady = false;
			clb.@geogebra.html5.util.AsyncCallback::onSuccess(Lcom/google/gwt/core/client/JavaScriptObject;)(result);
