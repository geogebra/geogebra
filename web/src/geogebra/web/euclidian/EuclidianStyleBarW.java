...
public class EuclidianStyleBarW extends StyleBarW implements
	...
	private MyToggleButton2 btnBold, btnFixPosition;
	...
	private MyToggleButton2[] newToggleBtnList() {
		return new MyToggleButton2[] { getAxesOrGridToggleButton(), btnBold,
		        btnItalic, btnFixPosition, btnDeleteSizes[0],
		        btnDeleteSizes[1],
		        btnDeleteSizes[2] };
	}
		...
		btnLabelStyle = new PopupMenuButton(app, captionArray, -1, 1,
			...
			public void update(Object[] geos) {
				...
				if (mode == EuclidianConstants.MODE_MOVE) {
					...
					for (int i = 0; i < geos.length; i++) {
						if (((GeoElement) geos[i]).isGeoAngle()) { // angle is
																   // always
																   // specific
																   // case
							geo = (GeoElement) geos[i];
					...
					for (int i = 0; i < geos.length; i++) {
						if ((((GeoElement) geos[i]).isLabelShowable() && ((GeoElement) geos[i])
						        .isGeoPoint())
						        || ((GeoElement) geos[i]).isGeoAngle()) { // angle
																		  // is
																		  // always
																		  // specific
																		  // case)
	...
	private void createAngleIntervalBtn() {
		...
		String[] angleIntervalString = new String[GeoAngle.INTERVAL_MIN.length];
		for (int i = 0; i < GeoAngle.INTERVAL_MIN.length; i++) {
			angleIntervalString[i] = app.getLocalization().getPlain(
			        "anglebetweenAandB", GeoAngle.INTERVAL_MIN[i],
	...
	public void setLabels() {
		...
		String[] angleIntervalArray = new String[GeoAngle.INTERVAL_MIN.length];
		for (int i = 0; i < GeoAngle.INTERVAL_MIN.length; i++) {
