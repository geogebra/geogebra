...
public class GeoGebraKeys implements KeyDownHandler, KeyUpHandler {
	...
	public void onKeyPress(KeyPressEvent e) {  
		...
		// ctrl pressed on Mac
		// or alt on Windows
		boolean modifierKeyPressed = e.isAltKeyDown();
		...
		if (modifierKeyPressed) {
			...
			String keyString = /*KeyEvent.getKeyText(*/e.getCharCode()+""/*)*/.toLowerCase();
			...
			if (e.isAltKeyDown() && (e.getUnicodeCharCode() >= MyKeyCodes.KEY_NUMPAD0 || e.getUnicodeCharCode() <= MyKeyCodes.KEY_NUMPAD9)) {
				char c = e.getCharCode();
				
				// make sure it's not eg alt-*
				if (c >= '0' && c <= '9')
					altCodes.append(e.getCharCode());
				//Application.debug("alt:"+altCodes);
			}
			
			boolean numpad = (e.getUnicodeCharCode() >= MyKeyCodes.KEY_NUMPAD0 || e.getUnicodeCharCode() <= MyKeyCodes.KEY_NUMPAD9);
			
			// Numeric keypad numbers eg NumPad-8, NumPad *
			if (!e.isAltKeyDown() && numpad)
				keyString = e.getCharCode() + "";
			
			AbstractApplication.debug("Key pressed "+StringUtil.toHexString(e.getCharCode())+" "+keyString);
			...
			boolean italian = app.getLocaleStr().toString().startsWith("it");
			...
			if (keyString.length() == 1)
				...
				case '*' :
					if (italian && !numpad) insertStr = e.isShiftKeyDown() ? "}" : "]"; // Italian keyboard			
					else insertStr = ExpressionNodeConstants.strVECTORPRODUCT; // alt-* -> vector product
					break;
				case '+' :
					if (italian) insertStr = e.isShiftKeyDown() ? "}" : "]"; // Italian keyboard			
					else insertStr = "\u00b1"; // alt-+ -> plusOrMinus
					break;
				case Unicode.eGrave :
					AbstractApplication.debug("matched eGrave "+e.isShiftKeyDown());
					insertStr = e.isShiftKeyDown() ? "{" : "["; // for Italian keyboard
					break;					
				case Unicode.eAcute : // Shift+eGrave on Italian keyboard
					AbstractApplication.debug("matched eAcute "+e.isShiftKeyDown());
					insertStr = e.isShiftKeyDown() ? "{" : "["; // for Italian keyboard
					break;					
				case '=' :
 ...
 */
			...
			if (!insertStr.equals("")) {
				...
				/*AGif (comp instanceof DynamicTextInputPane) {
				...
				// all other cases use setText
				{
					...
					sb.append(oldText.substring(pos));
					comp.setText(sb.toString());
					comp.setCursorPos(pos + insertStr.length());
				}
