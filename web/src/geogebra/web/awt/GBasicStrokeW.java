...
public class GBasicStrokeW implements geogebra.common.awt.GBasicStroke {
    ...
    public GBasicStrokeW(float width, int cap, int join, float miterLimit, float[] dash, float dashPhase) {
        if (width < 0.0f) {
        ...
        if (join == JOIN_MITER && miterLimit < 1.0f) {
            ...
            throw new IllegalArgumentException(("miterLimit less than 1.0f awt.136")); //$NON-NLS-1$
        }
        if (dash != null) {
            if (dashPhase < 0.0f) {
                // awt.137=Negative dashPhase
                throw new IllegalArgumentException(("Negative dashPhase awt.137")); //$NON-NLS-1$
            }
            if (dash.length == 0) {
    ...
    public geogebra.common.awt.GShape createStrokedShape(geogebra.common.awt.GShape s) {
        w2 = width / 2.0;
     ...
     * @param pathIterator - the PathIterator of source shape
     */
    void createSolidShape(geogebra.common.awt.GPathIterator pathIterator) {
     ...
     * @param pathIterator - the PathIterator of source shape
     */
    void createDashedShape(geogebra.common.awt.GPathIterator pathIterator) {
        double coords[] = new double[6];
        mx = my = cx = cy = 0.0;
        smx = smy = scx = scy = 0.0;
        isMove = false;
    ...
    static class BufferedPath {
        ...
        geogebra.common.awt.GGeneralPath createGeneralPath() {
            geogebra.web.awt.GeneralPath p = new geogebra.web.awt.GeneralPath();
            int j = 0;
	...
	public GBasicStrokeW(geogebra.common.awt.GBasicStroke objStroke) {
