 ...
 * the License.
 */
package org.geogebra.web.web.gui.app.docklayout;

import org.geogebra.web.web.gui.app.docklayout.MyLayout.Layer;
 ...
 * <p>
 * DockLayoutPanel elements in {@link com.google.gwt.uibinder.client.UiBinder
 * UiBinder} templates lay out their children in elements tagged with the
 ...
 * <p>
 * (Note that the tags of the child elements are not capitalized. This is meant
 * to signal that they are not runtime objects, and so cannot have a
 ...
 * </pre>
 */
public class MyDockLayoutPanel extends ComplexPanel implements
        MyAnimatedLayout, RequiresResize, ProvidesResize {
	...
	private class DockAnimateCommand extends MyLayoutCommand {
		public DockAnimateCommand(MyLayout layout) {
			super(layout);
		}
	...
	private final MyLayoutCommand layoutCmd;
	...
	public MyDockLayoutPanel(Unit unit) {
		...
		layout = new MyLayout(getElement());
		layoutCmd = new MyLayoutCommand(layout);
	}
	...
	/**
	 * Adds a widget to the start of the line. In LTR mode, the widget is added
	 * to the west. In RTL mode, the widget is added to the east.
	 * 
	 * @param widget
	 *            the widget to be added
	 * @param size
	...
	public void animate(int duration, final MyLayout.AnimationCallback callback) {
	 ...
	 * @param child
	 *            the widget to be queried
	 * @return the widget's layout size, or <code>null</code> if it is not a
	 *         child of this panel
	 * @throws AssertionError
	 *             if the widget is not a child and assertions are enabled
	 */
	 ...
	 * 
	 * @param widget
	 *            the widget that needs to update its size
	 * @param size
	...
	protected Direction getResolvedDirection(Direction direction) {
		if (direction == Direction.LINE_START) {
			return LocaleInfo.getCurrentLocale().isRTL() ? Direction.EAST
			        : Direction.WEST;
		} else if (direction == Direction.LINE_END) {
			return LocaleInfo.getCurrentLocale().isRTL() ? Direction.WEST
			        : Direction.EAST;
		}
		return direction;
	}
