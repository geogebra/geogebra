...
public class GBasicStrokeW implements org.geogebra.common.awt.GBasicStroke {
	...
	/*
	 * @Override public int hashCode() { HashCode hash = new HashCode();
	 * hash.append(width); hash.append(cap); hash.append(join);
	 ...
	 * join && bs.miterLimit == miterLimit && bs.dashPhase == dashPhase &&
	 * java.util.Arrays.equals(bs.dash, dash); } return false; }
	 */
	...
	void createSolidShape(org.geogebra.common.awt.GPathIterator pathIterator) {
		...
		while (!pathIterator.isDone()) {
			...
			case GPathIteratorW.SEG_MOVETO:
				if (!isClosed) {
					closeSolidShape();
				}
				rp.clean();
	...
	static class BufferedPath {
		...
		void moveTo(double x, double y) {
			checkBuf(1, 2);
			types[typeSize++] = GPathIteratorW.SEG_MOVETO;
		...
		void lineTo(double x, double y) {
			checkBuf(1, 2);
			types[typeSize++] = GPathIteratorW.SEG_LINETO;
		...
		void quadTo(double x1, double y1, double x2, double y2) {
			checkBuf(1, 4);
			types[typeSize++] = GPathIteratorW.SEG_QUADTO;
		...
		void cubicTo(double x1, double y1, double x2, double y2, double x3,
		        double y3) {
			checkBuf(1, 6);
			types[typeSize++] = GPathIteratorW.SEG_CUBICTO;
			points[pointSize++] = (float) x1;
			points[pointSize++] = (float) y1;
		...
		void closePath() {
			checkBuf(1, 0);
			types[typeSize++] = GPathIteratorW.SEG_CLOSE;
		}
		...
		void appendReverse(BufferedPath p) {
			...
			for (int i = p.typeSize - 1; i >= 0; i--) {
				byte type = p.types[i];
				if (type == GPathIteratorW.SEG_MOVETO) {
					types[closeIndex] = GPathIteratorW.SEG_MOVETO;
					types[typeSize++] = GPathIteratorW.SEG_CLOSE;
				} else {
					if (type == GPathIteratorW.SEG_CLOSE) {
						closeIndex = typeSize;
					}
					types[typeSize++] = type;
				}
	...
	public GBasicStrokeW(org.geogebra.common.awt.GBasicStroke objStroke) {
		this.lineWidth = ((GBasicStrokeW) objStroke).getLineWidth();
		this.lineCap = ((GBasicStrokeW) objStroke).getLineCap();
		this.lineJoin = ((GBasicStrokeW) objStroke).getLineJoin();
		this.dasharray = ((GBasicStrokeW) objStroke).getDashArray();
	}
