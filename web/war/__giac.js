...
194: { cmd:"SolveODE[y'=y / x]", result:"y = c_0 x", notes:"check in ExpressionNode for missing space, why c_0?" },
...
211: { cmd:"ToExponential[1+2ί]", result:"sqrt(5) ℯ^(atan(2) ί)|OR|sqrt(5)ℯ^(ί atan(2))", notes:"check missing space in ExpressionNode" },
...
255: { cmd:"Derivative[asinh(x)]", result:"1/(x+sqrt(x^2+1))*(1+1/2*2*x*(sqrt(x^2+1))^-1)" },
256: { cmd:"Derivative[acosh(x)]", result:"1/(x+sqrt(x^2-1))*(1+1/2*2*x*(sqrt(x^2-1))^-1)" },
...
273: { cmd:"Numeric[fractionalPart(-2.3)]", result:"-0.3", notes:"fractionalPart(x):=sgn(x)*(abs(x)-floor(abs(x)));" },
