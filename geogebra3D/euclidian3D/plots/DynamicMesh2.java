...
abstract class DynamicMeshElement2 {
	...
	protected final FastBucketPQ splitQueue;
	protected final FastBucketPQ mergeQueue;
	
	/////bucket stuff
	DynamicMeshElement2 prev;
	DynamicMeshElement2 next;
	int bucketIndex;
	Object data;
	FastBucketPQ owner;
	 ...
	 */
	public DynamicMeshElement2(int nChildren, int nParents, int level,
			boolean ignoreFlag, int version, FastBucketPQ splitQueue,
		...
		children = new DynamicMeshElement2[nChildren];
		parents = new DynamicMeshElement2[nParents];
	}
	 ...
	 * @param mergeQueue
	 */
	public void updateCullInfo(double radSq, DynamicMeshTriList2 drawList,
			FastBucketPQ splitQueue, FastBucketPQ mergeQueue) {

		if (ignoreCull())
			return;

		CullInfo2 parentCull = getParentCull();
		CullInfo2 prev = cullInfo;

		// update cull flag
		// if (parentCull == CullInfo2.SOMEIN || parentCull == null) {
		if (maxRadSq < radSq)
			cullInfo = CullInfo2.ALLIN;
		else if (minRadSq < radSq)
			cullInfo = CullInfo2.SOMEIN;
		else
			cullInfo = CullInfo2.OUT;
		// } else
		// cullInfo = parentCull;
		...
		if (prev != cullInfo || cullInfo == CullInfo2.SOMEIN) {
			...
			setHidden(drawList, cullInfo == CullInfo2.OUT);
	 ...
	 */
	abstract protected void setHidden(DynamicMeshTriList2 drawList, boolean val);
