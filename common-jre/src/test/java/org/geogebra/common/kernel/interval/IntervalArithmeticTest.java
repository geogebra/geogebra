...
public class IntervalArithmeticTest {
	...
	public void testMultiplicationEmpty() {
		assertTrue(new Interval().getEvaluate().multiply(interval(1, 1)).isEmpty());
	}

	@Test
	public void testMultiplicationPositiveWithPositive() {
		assertEquals(interval(2, 6),
				interval(1, 2).getEvaluate().multiply(interval(2, 3)));

		assertEquals(interval(4, POSITIVE_INFINITY),
				interval(1, POSITIVE_INFINITY).getEvaluate().multiply(interval(4, 6)));

		assertEquals(interval(POSITIVE_INFINITY, POSITIVE_INFINITY),
				interval(1, POSITIVE_INFINITY)
						.getEvaluate().multiply(interval(POSITIVE_INFINITY, POSITIVE_INFINITY)));
	}

	@Test
	public void testMultiplicationPositiveWithNegative() {
		assertEquals(interval(-6, -2),
				interval(1, 2).getEvaluate().multiply(interval(-3, -2)));

		assertEquals(interval(NEGATIVE_INFINITY, -2),
				interval(1, POSITIVE_INFINITY).getEvaluate().multiply(interval(-3, -2)));
	}
	...
	public void testMultiplicationPositiveWithMixed() {
		...
		assertEquals(interval(NEGATIVE_INFINITY, POSITIVE_INFINITY),
				interval(1, POSITIVE_INFINITY).getEvaluate().multiply(interval(-2, 3)));
	}

	@Test
	public void testMultiplicationPositiveWithZero() {
		assertEquals(interval(0, 0),
				interval(1, 2).getEvaluate().multiply(interval(0, 0)));

		assertEquals(interval(0, 0),
				interval(1, POSITIVE_INFINITY).getEvaluate().multiply(interval(0, 0)));
	}

	@Test
	public void testMultiplicationNegativeWithPositive() {
		assertEquals(interval(-6, -2),
				interval(-3, -2).getEvaluate().multiply(interval(1, 2)));

		assertEquals(interval(NEGATIVE_INFINITY, -2),
				interval(-3, -2).getEvaluate().multiply(interval(1, POSITIVE_INFINITY)));
	}

	@Test
	public void testMultiplicationNegativeWithNegative() {
		assertEquals(interval(2, 6),
				interval(-2, -1).getEvaluate().multiply(interval(-3, -2)));

		assertEquals(interval(4, POSITIVE_INFINITY),
				interval(NEGATIVE_INFINITY, -1).getEvaluate().multiply(interval(-6, -4)));

		assertEquals(interval(POSITIVE_INFINITY, POSITIVE_INFINITY),
				interval(NEGATIVE_INFINITY, -1)
						.getEvaluate().multiply(interval(NEGATIVE_INFINITY, NEGATIVE_INFINITY)));
	}

	@Test
	public void testMultiplicationNegativeWithMixed() {
		assertEquals(interval(-6, 4),
				interval(-2, -1).getEvaluate().multiply(interval(-2, 3)));

		assertEquals(IntervalConstants.whole(),
				interval(NEGATIVE_INFINITY, -1).getEvaluate().multiply(interval(-2, 3)));
	}

	@Test
	public void testMultiplicationNegativeWithZero() {
		assertEquals(zero(), interval(-2, -1).getEvaluate().multiply(zero()));
		assertEquals(zero(), interval(NEGATIVE_INFINITY, -1).getEvaluate().multiply(zero()));
	}
	...
	public void testMultiplicationMixedWithNegative() {
		assertEquals(interval(-6, 4),
				interval(-2, 3).getEvaluate().multiply(interval(-2, -1)));

		assertEquals(whole(),
				interval(-2, 3).getEvaluate().multiply(interval(NEGATIVE_INFINITY, -1)));
	}
	...
	public void testMultiplicationZeroWithAny() {
		assertEquals(zero(), zero().getEvaluate().multiply(interval(-2, 1)));
		assertEquals(zero(), zero().getEvaluate().multiply(interval(-2, -1)));
		assertEquals(zero(), zero().getEvaluate().multiply(interval(1, 2)));
		assertEquals(zero(), zero().getEvaluate().multiply(interval(0, 0)));
	}
	}