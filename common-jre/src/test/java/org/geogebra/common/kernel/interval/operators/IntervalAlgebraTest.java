...
public class IntervalAlgebraTest {
	...
	public void testNegativePowerOfEven() {
		assertEquals(
				interval(4), pow(interval(-2), 2));
		assertEquals(interval(4),
				pow(interval(-2), 1 + 1. / 3 + 1. / 3 + 1. / 3));
	}
	...
	public void testPowerOnPositiveFraction() {
		assertEquals(sqrt(interval(1, 2)), pow(
				interval(1, 2), 0.5));
	}
	...
	public void testBaseLessThanOne() {
		assertEquals(
				interval(0.5), pow(interval(0.5), one()));
		assertEquals(
				interval(0.25, 0.5),
				pow(interval(0.5), interval(1, 2)));
	}
