...
public class CellDragPasteHandlerTest extends BaseUnitTest {
	...
	private final TestSpreadsheetCellEditor editor = new TestSpreadsheetCellEditor();
	...
	private void setRangeToCopy(int fromRow, int toRow, int fromColumn, int toColumn) {
		cellDragPasteHandler = new CellDragPasteHandler(
				TabularRange.range(fromRow, toRow, fromColumn, toColumn), tabularData, editor);
	}
	...
	private void assertCellContentIsEqual(int originRow, int originColumn,
		...
		try {
			GeoElement copied = (GeoElement) tabularData.contentAt(originRow, originColumn);
			GeoElement pasted = (GeoElement) editor.getContentAt(destinationRow, destinationColumn);
			assertTrue(copied.isEqual(pasted));
		} catch (AssertionError | ClassCastException | NullPointerException e) {
			fail(String.format("The content of cell (%d, %d) should be equal to the content"
