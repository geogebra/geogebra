...
//		inputPosition2DOldX = Float.NaN;
	...
	public void updateInput3D() {
		if (input3D.update()) {
			...
			inputPosition2DX = input3D.getMouse2DX();
			inputPosition2DY = input3D.getMouse2DY();
			inputPotition2DFactor = input3D.getMouse2DFactor();
			

			// 2D cursor pos
			if (robot != null) {
				if (Float.isNaN(inputPosition2DOldX)){
					Point p = MouseInfo.getPointerInfo().getLocation();
					robotX = p.x;
					robotY = p.y;
					inputPosition2DOldX = inputPosition2DX;
					inputPosition2DOldY = inputPosition2DY;
				}else{
					int x, y;
					if (inputPotition2DFactor > 1f){
						App.debug(((int) inputPosition2DX)+","+((int) inputPosition2DY));
						x = (int) (inputPosition2DX * screenHalfWidth / 320);
						y = (int) (inputPosition2DY * screenHalfHeight / 240);
						if (x > screenHalfWidth * 2){
							x = (int) (screenHalfWidth * 2);
						}
						if (y > screenHalfHeight * 2){
							y = (int) (screenHalfHeight * 2);
						}
						App.error("ici");
					}else{
						int dx = (int) ((inputPosition2DX - inputPosition2DOldX) * inputPotition2DFactor);
						int dy = (int) ((inputPosition2DY - inputPosition2DOldY) * inputPotition2DFactor);
						x = robotX + dx;
						y = robotY + dy;
					}
