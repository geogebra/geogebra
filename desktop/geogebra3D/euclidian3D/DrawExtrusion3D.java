 ...
 *
 */
public class DrawExtrusion3D extends Drawable3DSurfaces implements Previewable{
		...
		if (extrusionComputer == null){
			if (selectedPolygons.size()==1){
				...
				extrusionComputer = new ExtrusionComputer(
						new AlgoPolyhedronPointsPrismForExtrusion(
				...
				//create the algo
				extrusionComputer = new ExtrusionComputer(
						new AlgoQuadricLimitedConicHeightCylinderForExtrusion(
								getView3D().getKernel().getConstruction(),
								null, 
								(GeoConicND) basis, 
								height)
			...
			if (extrusionComputer!=null){	
				...
				getView3D().getEuclidianController().setHandledGeo(extrusionComputer.getAlgo().getTopFace());
