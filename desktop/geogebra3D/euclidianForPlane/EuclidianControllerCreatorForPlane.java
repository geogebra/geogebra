...
public class EuclidianControllerCreatorForPlane extends EuclidianControllerCreatorFor3D{
	...
	protected GeoPointND createNewPoint(boolean forPreviewable, boolean complex){
		...
		GeoPointND ret = ec.kernel.getManager3D().Point3DIn(null, ec.view.getPlaneContaining(), coords, !forPreviewable, false);
		return ret;
	}
	...
	protected GeoPointND createNewPoint(boolean forPreviewable, Region region, boolean complex){
		Coords coords = getCoordsFromView(ec.xRW,ec.yRW);
		return ec.createNewPoint(null, forPreviewable, region, coords.getX(), coords.getY(), coords.getZ(), complex, false);
	}
	...
	public GeoElement[] rotateByAngle(GeoElement geoRot, GeoNumberValue phi, GeoPointND Q) {
		...
		return ec.kernel.getManager3D().Rotate3D(null, geoRot, phi, Q, ec.view.getDirection());
