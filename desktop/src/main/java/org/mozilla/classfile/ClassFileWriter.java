...
public class ClassFileWriter {
	...
	public void addExceptionHandler(int startLabel, int endLabel,
		 ...
		 */
		short catch_type_index = (catchClassName == null) ? 0
	...
	final class StackMapTable {
		...
		private void killSuperBlock(SuperBlock sb) {
			...
			int[] stack = new int[] {
					TypeInfo.OBJECT("java/lang/Throwable", itsConstantPool) };
		...
		private void executeBlock(SuperBlock work) {
			...
			for (int bci = work.getStart(); bci < work.getEnd(); bci += next) {
				...
				if (isBranch(bc)) {
					...
					if (DEBUGSTACKMAP) {
						System.out.println(
								"type state of " + targetSB + " after merge:");
					...
					SuperBlock targetSB = getSuperBlockFromOffset(
							bci + defaultOffset);
					if (DEBUGSTACK) {
		...
		private int execute(int bci) {
			...
			case ByteCode.DLOAD_3:
				...
				break;
			case ByteCode.ISTORE:
				executeStore(getOperand(bci + 1, wide ? 2 : 1),
						TypeInfo.INTEGER);
				break;
		...
		private void computeRawStackMap() {
			...
			for (int i = 1; i < superBlocks.length; i++) {
				...
				if (currentStack.length == 0) {
					...
					int delta = Math
							.abs(prevLocals.length - currentLocals.length);
					int j;
		...
		private void writeSameLocalsOneStackItemFrame(int[] locals, int[] stack,
				int offsetDelta) {
			if (offsetDelta <= 63) {
	...
	private int[] createInitialLocals() {
		...
		while (start < rParenIndex) {
			...
			String internalType = descriptorToInternalName(
					paramType.toString());
	...
	static {
		...
		try {
			is = ClassFileWriter.class
					.getResourceAsStream("ClassFileWriter.class");
			if (is == null) {
