...
final class NativeString extends IdScriptableObject {
	...
	protected void fillConstructorProperties(IdFunctionObject ctor) {
		...
		addIdFunctionProperty(ctor, STRING_TAG, ConstructorId_lastIndexOf,
				"lastIndexOf", 2);
		addIdFunctionProperty(ctor, STRING_TAG, ConstructorId_split, "split",
				3);
		...
		addIdFunctionProperty(ctor, STRING_TAG, ConstructorId_concat, "concat",
				2);
		addIdFunctionProperty(ctor, STRING_TAG, ConstructorId_slice, "slice",
				3);
		addIdFunctionProperty(ctor, STRING_TAG, ConstructorId_equalsIgnoreCase,
				"equalsIgnoreCase", 2);
		addIdFunctionProperty(ctor, STRING_TAG, ConstructorId_match, "match",
				2);
		...
		addIdFunctionProperty(ctor, STRING_TAG, ConstructorId_localeCompare,
				"localeCompare", 2);
		addIdFunctionProperty(ctor, STRING_TAG, ConstructorId_toLocaleLowerCase,
				"toLocaleLowerCase", 1);
	...
	public Object execIdCall(IdFunctionObject f, Context cx, Scriptable scope,
		...
		again: for (;;) {
			...
			case Id_indexOf:
				return ScriptRuntime.wrapInt(js_indexOf(Id_indexOf,
						ScriptRuntime.toString(thisObj), args));

			case Id_includes:
			case Id_startsWith:
			case Id_endsWith:
				String s = ScriptRuntime
						.toString(requireObjectCoercible(thisObj, f));

				if (args.length > 0 && args[0] instanceof NativeRegExp) {
					throw ScriptRuntime.typeError2("msg.first.arg.not.regexp",
							String.class.getSimpleName(), f.getFunctionName());
				}
			...
			case Id_substring:
				return js_substring(cx, ScriptRuntime.toCharSequence(thisObj),
						args);
			...
			case Id_equalsIgnoreCase: {
				...
				return ScriptRuntime.wrapBoolean((id == Id_equals)
			...
			case Id_localeCompare: {
				...
				return ScriptRuntime.wrapNumber(
						collator.compare(ScriptRuntime.toString(thisObj),
			...
			case Id_trim: {
				...
				int start = 0;
				while (start < chars.length && ScriptRuntime
						.isJSWhitespaceOrLineTerminator(chars[start])) {
					start++;
				}
				int end = chars.length;
				while (end > start && ScriptRuntime
			...
			case Id_codePointAt: {
				...
				return (cnt < 0 || cnt >= str.length()) ? Undefined.instance
	...
	private static int js_indexOf(int methodId, String target, Object[] args) {
		...
		if (position > target.length() && methodId != Id_startsWith
	...
	private static final int ConstructorId_fromCharCode = -1,
			...
			Id_constructor = 1, Id_toString = 2, Id_toSource = 3,
			Id_valueOf = 4, Id_charAt = 5, Id_charCodeAt = 6, Id_indexOf = 7,
			...
			Id_toLocaleUpperCase = 36, Id_trim = 37, Id_trimLeft = 38,
			Id_trimRight = 39, Id_includes = 40, Id_startsWith = 41,
			Id_endsWith = 42, Id_normalize = 43, Id_repeat = 44,
			Id_codePointAt = 45, MAX_PROTOTYPE_ID = Id_codePointAt;
