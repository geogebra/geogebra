...
public class NativeArray extends IdScriptableObject implements List {
	 ...
	 * turns out that js calls the equivalent of "new Array" in the current
	 * scope, except that it always gets at least an object back, even when
	 * Array == null.
	 */
	...
	protected void fillConstructorProperties(IdFunctionObject ctor) {
		...
		addIdFunctionProperty(ctor, ARRAY_TAG, ConstructorId_reverse, "reverse",
				0);
		...
		addIdFunctionProperty(ctor, ARRAY_TAG, ConstructorId_unshift, "unshift",
				1);
		addIdFunctionProperty(ctor, ARRAY_TAG, ConstructorId_splice, "splice",
				2);
		addIdFunctionProperty(ctor, ARRAY_TAG, ConstructorId_concat, "concat",
				1);
		addIdFunctionProperty(ctor, ARRAY_TAG, ConstructorId_slice, "slice", 2);
		addIdFunctionProperty(ctor, ARRAY_TAG, ConstructorId_indexOf, "indexOf",
				1);
		...
		addIdFunctionProperty(ctor, ARRAY_TAG, ConstructorId_filter, "filter",
				1);
		addIdFunctionProperty(ctor, ARRAY_TAG, ConstructorId_forEach, "forEach",
				1);
		...
		addIdFunctionProperty(ctor, ARRAY_TAG, ConstructorId_findIndex,
				"findIndex", 1);
		addIdFunctionProperty(ctor, ARRAY_TAG, ConstructorId_reduce, "reduce",
				1);
		addIdFunctionProperty(ctor, ARRAY_TAG, ConstructorId_reduceRight,
				"reduceRight", 1);
		addIdFunctionProperty(ctor, ARRAY_TAG, ConstructorId_isArray, "isArray",
				1);
	...
	public Integer[] getIndexIds() {
		...
		for (Object id : ids) {
			...
			if (int32Id >= 0 && ScriptRuntime.toString(int32Id)
					.equals(ScriptRuntime.toString(id))) {
				indices.add(int32Id);
			}
		}
	...
	protected void defineOwnProperty(Context cx, Object id,
			ScriptableObject desc, boolean checkValid) {
		if (dense != null) {
			Object[] values = dense;
			dense = null;
			denseOnly = false;
			...
			if (args.length > 1 || !(arg0 instanceof Number)) {
				...
				if (len != ((Number) arg0).doubleValue()) {
					String msg = ScriptRuntime
							.getMessage0("msg.arraylength.bad");
	...
	/**
	 * Change the value of the internal flag that determines whether all storage
	 * is handed by a dense backing array rather than an associative store.
	 * 
	 * @param denseOnly
	 *            new value for denseOnly flag
	 * @throws IllegalArgumentException
	 *             if an attempt is made to enable denseOnly after it was
	 *             disabled; NativeArray code is not written to handle switching
	...
	private static String toStringHelper(Context cx, Scriptable scope,
		...
		try {
			if (!iterating) {
				...
				// make toSource print null and undefined values in recent
				// versions
				...
				for (i = 0; i < length; i++) {
					...
					if (toSource) {
						...
						if (toLocale) {
							Callable fun;
							Scriptable funThis;
							fun = ScriptRuntime.getPropFunctionAndThis(elem,
									"toLocaleString", cx, scope);
	...
	private static Object js_splice(Context cx, Scriptable scope,
		...
		if (count != 0) {
			...
			if (cx.getLanguageVersion() == Context.VERSION_1_2) {
				/*
				 * Emulate C JS1.2; if no elements are removed, return
				 * undefined.
				 */
				result = Undefined.instance;
			} else {
		...
		if (thisObj instanceof NativeArray && result instanceof NativeArray) {
			...
			if (denseThis.denseOnly && denseResult.denseOnly) {
				...
				if (canUseDense && denseResult.ensureCapacity(length)) {
					System.arraycopy(denseThis.dense, 0, denseResult.dense, 0,
							(int) denseThis.length);
					int cursor = (int) denseThis.length;
					for (int i = 0; i < args.length && canUseDense; i++) {
						if (args[i] instanceof NativeArray) {
							NativeArray arg = (NativeArray) args[i];
							System.arraycopy(arg.dense, 0, denseResult.dense,
	...
	private static Object js_indexOf(Context cx, Scriptable thisObj,
		 ...
		 * Objects:Array:indexOf The index at which to begin the search.
		 * Defaults to 0, i.e. the whole array will be searched. If the index is
		 * greater than or equal to the length of the array, -1 is returned,
		 * i.e. the array will not be searched. If negative, it is taken as the
		 ...
		 * calculated index is less than 0, the whole array will be searched.
		 */
		long start;
		if (args.length < 2) {
			// default
	...
	private static Object js_lastIndexOf(Context cx, Scriptable thisObj,
		 ...
		 * From
		 * http://developer.mozilla.org/en/docs/Core_JavaScript_1.5_Reference:
		 * Objects:Array:lastIndexOf The index at which to start searching
		 * backwards. Defaults to the array's length, i.e. the whole array will
		 * be searched. If the index is greater than or equal to the length of
		 * the array, the whole array will be searched. If negative, it is taken
		 * as the offset from the end of the array. Note that even when the
		 * index is negative, the array is still searched from back to front. If
		 * the calculated index is less than 0, -1 is returned, i.e. the array
	...
	private static Object iterativeMethod(Context cx, int id, Scriptable scope,
		...
		if (callbackArg == null || !(callbackArg instanceof Function)) {
			throw ScriptRuntime.notFunctionError(callbackArg);
				...
				&& !(callbackArg instanceof NativeFunction)) {
			throw ScriptRuntime.notFunctionError(callbackArg);
		}
	...
	private static Object reduceMethod(Context cx, int id, Scriptable scope,
		...
		for (long i = 0; i < length; i++) {
			...
			if (value == Scriptable.NOT_FOUND) {
				// no initial value passed, use first element found as inital
				// value
				value = elem;
			} else {
	...
	private static final int Id_constructor = 1, Id_toString = 2,
			Id_toLocaleString = 3, Id_toSource = 4, Id_join = 5, Id_reverse = 6,
			Id_sort = 7, Id_push = 8, Id_pop = 9, Id_shift = 10,
			Id_unshift = 11, Id_splice = 12, Id_concat = 13, Id_slice = 14,
			Id_indexOf = 15, Id_lastIndexOf = 16, Id_every = 17, Id_filter = 18,
			Id_forEach = 19, Id_map = 20, Id_some = 21, Id_find = 22,
			Id_findIndex = 23, Id_reduce = 24, Id_reduceRight = 25,
	...
	private static final int ConstructorId_join = -Id_join,
			ConstructorId_reverse = -Id_reverse, ConstructorId_sort = -Id_sort,
			ConstructorId_push = -Id_push, ConstructorId_pop = -Id_pop,
			ConstructorId_shift = -Id_shift,
			ConstructorId_unshift = -Id_unshift,
			ConstructorId_splice = -Id_splice,
			ConstructorId_concat = -Id_concat, ConstructorId_slice = -Id_slice,
			ConstructorId_indexOf = -Id_indexOf,
			ConstructorId_lastIndexOf = -Id_lastIndexOf,
			ConstructorId_every = -Id_every, ConstructorId_filter = -Id_filter,
			ConstructorId_forEach = -Id_forEach, ConstructorId_map = -Id_map,
			ConstructorId_some = -Id_some, ConstructorId_find = -Id_find,
	...
	private static final int MAX_PRE_GROW_SIZE = (int) (Integer.MAX_VALUE
			/ GROW_FACTOR);
}
