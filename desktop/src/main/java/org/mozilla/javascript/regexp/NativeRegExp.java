...
public class NativeRegExp extends IdScriptableObject implements Function {
	...
	Scriptable compile(Context cx, Scriptable scope, Object[] args) {
		...
		String s = args.length == 0 || args[0] instanceof Undefined ? ""
				: escapeRegExp(args[0]);
	...
	private Object execSub(Context cx, Scriptable scopeObj, Object[] args,
		...
		if (d < 0 || str.length() < d) {
			...
			if ((re.flags & JSREG_GLOB) != 0) {
				lastIndex = (rval == null || rval == Undefined.instance) ? 0d
						: (double) indexp[0];
			}
		}
		return rval;
	...
	static RECompiled compileRE(Context cx, String str, String global,
		...
		if (debug) {
			if (regexp.anchorCh >= 0) {
				System.out.println(
						"Anchor ch = '" + (char) regexp.anchorCh + "'");
			}
		}
		return regexp;
	}
	...
	private static int emitREBytecode(CompilerState state, RECompiled re,
		...
		while (t != null) {
			...
			case REOP_ALTPREREQ2:
				...
				pc += INDEX_LEN;
				addIndex(program, pc,
						ignoreCase ? upcase((char) t.index) : t.index);
				pc += INDEX_LEN;
	...
	private static void pushProgState(REGlobalData gData, int min, int max,
			...
			int continuationPc) {
		gData.stateStackTop = new REProgState(gData.stateStackTop, min, max, cp,
				backTrackLastToSave, continuationOp, continuationPc);
	}
	...
	private static boolean classMatcher(REGlobalData gData, RECharSet charSet,
		...
		int byteIndex = ch >> 3;
		return (charSet.length == 0 || ch >= charSet.length
			...
			if (reopIsSimple(op)) {
				...
				case REOP_ALTPREREQ2: {
					...
					if (op == REOP_ALTPREREQ2) {
						if (c != matchCh1 && !classMatcher(gData,
				...
				case REOP_ASSERT: {
					...
					op = program[pc++];
					if (reopIsSimple(op) && simpleMatch(gData, input, op,
							program, pc, end, false) < 0) {
						result = false;
						break;
					}
				...
				case REOP_MINIMALREPEAT: {
					...
					if (!result) {
						...
						if (state.max == -1 || state.max > 0) {
							pushProgState(gData, state.min, state.max, gData.cp,
									null, state.continuationOp,
	...
	private static void reportWarning(Context cx, String messageId,
			String arg) {
