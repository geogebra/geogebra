...
public class RegExpImpl implements RegExpProxy {
	...
	public Object action(Context cx, Scriptable scope, Scriptable thisObj,
		...
		case RA_MATCH: {
			...
			Object rval = matchOrReplace(cx, scope, thisObj, args, this, data,
					re);
		...
		case RA_REPLACE: {
			...
			if (useRE) {
				...
				if (index >= 0) {
					...
					this.rightContext = new SubString(str, index + slen,
							str.length() - index - slen);
					val = Boolean.TRUE;
				} else {
					val = Boolean.FALSE;
				}
	...
	private static NativeRegExp createRegExp(Context cx, Scriptable scope,
			Object[] args, int optarg, boolean forceFlat) {
		NativeRegExp re;
	...
	private static Object matchOrReplace(Context cx, Scriptable scope,
			Scriptable thisObj, Object[] args, RegExpImpl reImpl, GlobData data,
			NativeRegExp re) {
		String str = data.str;
	 ...
	 */
	private static void match_glob(GlobData mdata, Context cx, Scriptable scope,
			int count, RegExpImpl reImpl) {
