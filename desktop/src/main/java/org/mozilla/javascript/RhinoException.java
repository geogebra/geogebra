...
public abstract class RhinoException extends RuntimeException {
	private static final Pattern JAVA_STACK_PATTERN = Pattern
			.compile("_c_(.*)_\\d+");
		...
		for (StackTraceElement e : stack) {
			...
			if (e.getMethodName().startsWith("_c_") && e.getLineNumber() > -1
				...
				if (!printStarted && hideFunction.equals(methodName)) {
					...
					list.add(new ScriptStackElement(fileName, methodName,
							e.getLineNumber()));
					count++;
				}

			} else if ("org.mozilla.javascript.Interpreter"
				...
				for (ScriptStackElement elem : interpreterStack[interpreterStackIndex++]) {
					if (!printStarted
							&& hideFunction.equals(elem.functionName)) {
						printStarted = true;
