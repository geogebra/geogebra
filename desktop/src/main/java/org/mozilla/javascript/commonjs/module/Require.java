 ...
 * <h1>Thread safety</h1> You will ordinarily create one instance of require()
 * for every top-level scope. This ordinarily means one instance per program
 ...
 * <h1>Creation</h1> If you need to create many otherwise identical require()
 * functions for different scopes, you might want to use {@link RequireBuilder}
 * for convenience.
 * <h1>Making it available</h1> In order to make the require() function
 * available to your JavaScript program, you need to invoke either
 * {@link #install(Scriptable)} or {@link #requireMain(Context, String)}.
 * 
...
public class Require extends BaseFunction {
	...
	public Scriptable requireMain(Context cx, String mainModuleId) {
		if (this.mainModuleId != null) {
			if (!this.mainModuleId.equals(mainModuleId)) {
				throw new IllegalStateException(
						"Main module already set to " + this.mainModuleId);
			}
			return mainExports;
		}
		...
		try {
			...
			moduleScript = moduleScriptProvider.getModuleScript(cx,
					mainModuleId, null, null, paths);
		...
		if (moduleScript != null) {
			mainExports = getExportedModuleInterface(cx, mainModuleId, null,
					null, true);
		} else if (!sandboxed) {
	...
	private Scriptable getExportedModuleInterface(Context cx, String id,
		...
		Map<String, Scriptable> threadLoadingModules = loadingModuleInterfaces
				.get();
	...
	private Scriptable executeModuleScript(Context cx, String id,
		...
		final ScriptableObject moduleObject = (ScriptableObject) cx
				.newObject(nativeScope);
		...
		final Scriptable executionScope = new ModuleScope(nativeScope, uri,
				base);
	...
	private ModuleScript getModule(Context cx, String id, URI uri, URI base) {
		try {
			final ModuleScript moduleScript = moduleScriptProvider
					.getModuleScript(cx, id, uri, base, paths);
			if (moduleScript == null) {
				throw ScriptRuntime.throwError(cx, nativeScope,
						"Module \"" + id + "\" not found.");
			}
