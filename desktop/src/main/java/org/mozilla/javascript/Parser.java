...
public class Parser {
	...
	private void addStrictWarning(String messageId, String messageArg,
			int position, int length, int line, String lineSource,
			int lineOffset) {
		if (compilerEnv.isStrictMode()) {
			addWarning(messageId, messageArg, position, length, line,
					lineSource, lineOffset);
		}
	}

	private void addWarning(String messageId, String messageArg, int position,
			int length, int line, String lineSource, int lineOffset) {
		String message = lookupMessage(messageId, messageArg);
		if (compilerEnv.reportWarningAsError()) {
			addError(messageId, messageArg, position, length, line, lineSource,
					lineOffset);
		} else if (errorCollector != null) {
			errorCollector.warning(message, sourceURI, position, length);
		} else {
			errorReporter.warning(message, sourceURI, line, lineSource,
					lineOffset);
		}
	}

	private void addError(String messageId, String messageArg, int position,
			int length, int line, String lineSource, int lineOffset) {
		++syntaxErrorCount;
		String message = lookupMessage(messageId, messageArg);
		if (errorCollector != null) {
			...
			errorReporter.error(message, sourceURI, line, lineSource,
					lineOffset);
		}
	}
		...
		try {
			...
			if (compilerEnv.isStrictMode()
				...
				String msg = (name != null && name.length() > 0)
						? "msg.no.return.value" : "msg.anon.no.return.value";
			...
			if (pn != null) {
				if (compilerEnv.isStrictMode() && !pn.hasSideEffects()) {
					...
					addStrictWarning(pn instanceof EmptyStatement
							? "msg.extra.trailing.semi" : "msg.no.side.effects",
							"", beg, nodeEnd(pn) - beg);
				}
				return pn;
			}
		...
		try {
			...
			if (isForIn) {
				...
				if (init instanceof VariableDeclaration) {
					...
					if (((VariableDeclaration) init).getVariables()
							.size() > 1) {
	...
	private AstNode returnOrYield(int tt, boolean exprContext)
		...
		if (!insideFunction()) {
			reportError(
					tt == Token.RETURN ? "msg.bad.return" : "msg.bad.yield");
		}
		consumeToken();
		int lineno = ts.lineno, pos = ts.tokenBeg, end = ts.tokenEnd;
	...
	private VariableDeclaration variables(int declType, int pos,
			boolean isStatement) throws IOException {
		int end;
	...
	void defineSymbol(int declType, String name, boolean ignoreNotInBlock) {
		...
		if (symbol != null && (symDeclType == Token.CONST
				|| declType == Token.CONST || (definingScope == currentScope
		...
		switch (declType) {
		case Token.LET:
			if (!ignoreNotInBlock && ((currentScope.getType() == Token.IF)
	...
	private AstNode condExpr() throws IOException {
		...
		if (matchToken(Token.HOOK)) {
			 ...
			 * ternary, where it's unambiguous, even if we might be parsing the
			 * init of a for statement.
			 */
		...
		for (;; tt = ts.getNextXMLToken()) {
			...
			case Token.XML:
				...
				AstNode expr = (peekToken() == Token.RC)
						? new EmptyExpression(beg, ts.tokenEnd - beg) : expr();
	...
	private AstNode generatorExpression(AstNode result, int pos,
			boolean inFunctionParams) throws IOException {
		...
		commaLoop: for (;;) {
			...
			if (tt == Token.RC) {
				...
				if (pname == null) {
					...
					if (peeked != Token.COMMA && peeked != Token.COLON
							&& peeked != Token.RC) {
						if (peeked == Token.LP) {
						...
						if (pname == null) {
							...
							propertyName = ts.getString();
							ObjectProperty objectProp = methodDefinition(ppos,
									pname, entryKind);
	...
	private AstNode objliteralProperty() throws IOException {
		...
		case Token.NUMBER:
			pname = new NumberLiteral(ts.tokenBeg, ts.getString(),
					ts.getNumber());
			break;
	...
	private ObjectProperty methodDefinition(int pos, AstNode propName,
			int entryKind) throws IOException {
	...
	protected void checkActivationName(String name, int token) {
		...
		if ("arguments".equals(name)
			...
			if (token == Token.GETPROP && compilerEnv
	...
	private void warnMissingSemi(int pos, int end) {
		...
		if (compilerEnv.isStrictMode()) {
			...
			// case ide-mode here
			int beg = compilerEnv.isIdeMode() ? Math.max(pos, end - linep[1])
					: pos;
			if (line != null) {
	...
	Node destructuringAssignmentHelper(int variableType, Node left, Node right,
		...
		result.addChildToFront(
				new Node(Token.LET, createName(Token.NAME, tempName, right)));
		try {
			pushScope(result);
		...
		for (AstNode n : array.getElements()) {
			...
			if (n.getType() == Token.NAME) {
				...
				parent.addChildToBack(new Node(setOp,
						createName(Token.BINDNAME, name, null), rightElem));
				if (variableType != -1) {
