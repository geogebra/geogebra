...
final class NativeDate extends IdScriptableObject {
	...
	private static final double SecondsPerDay = (MinutesPerDay
			* SecondsPerMinute);
	private static final double SecondsPerHour = (MinutesPerHour
			* SecondsPerMinute);
	...
	private static double parseISOString(String s) {
		...
		syntax: {
			...
			int hour = values[HOUR], min = values[MIN], sec = values[SEC],
					msec = values[MSEC];
			int tzhour = values[TZHOUR], tzmin = values[TZMIN];
			if (year > 275943 // ceil(1e8/365) + 1970 = 275943
					|| (month < 1 || month > 12)
					|| (day < 1 || day > DaysInMonth(year, month)) || hour > 24
	...
	private static String date_format(double t, int methodId) {
		...
		if (methodId != Id_toDateString) {
			...
			int minutes = (int) Math
					.floor((LocalTZA + DaylightSavingTA(t)) / msPerMinute);
	...
	private static String toLocale_helper(double t, int methodId) {
		...
		case Id_toLocaleString:
			if (localeDateTimeFormatter == null) {
				localeDateTimeFormatter = DateFormat
