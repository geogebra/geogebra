...
public abstract class ScriptableObject
	...
	static {
		try {
			GET_ARRAY_LENGTH = ScriptableObject.class
					.getMethod("getExternalArrayLength");
			...
			if (setter == null) {
				if (getter != null) {
					if (Context.getContext()
							.hasFeature(Context.FEATURE_STRICT_MODE)) {
	...
	public boolean isConst(String name) {
		...
		return (slot.getAttributes() & (PERMANENT | READONLY)) == (PERMANENT
				| READONLY);
	...
	private void setGetterOrSetter(String name, int index,
		...
		if (isExtensible()) {
			gslot = (GetterSlot) getSlot(name, index,
	...
	public void setExternalArrayData(ExternalArrayData array) {
		...
		if (array == null) {
			...
			defineProperty("length", null, GET_ARRAY_LENGTH, null,
					READONLY | DONTENUM);
		}
	}
	...
	static <T extends Scriptable> BaseFunction buildClassCtor(Scriptable scope,
		...
		if (existing instanceof BaseFunction) {
			Object existingProto = ((BaseFunction) existing)
		...
		if (mapInheritance) {
			...
			if (ScriptRuntime.ScriptableClass.isAssignableFrom(superClass)
				...
				Class<? extends Scriptable> superScriptable = extendsScriptable(
						superClass);
				String name = ScriptableObject.defineClass(scope,
						superScriptable, sealed, mapInheritance);
				if (name != null) {
					superProto = ScriptableObject.getClassPrototype(scope,
							name);
				}
			}
		}
	...
	private static <T extends Scriptable> Class<T> extendsScriptable(
			Class<?> c) {
	...
	public static void defineConstProperty(Scriptable destination,
		...
		if (destination instanceof ConstProperties) {
			...
			defineProperty(destination, propertyName, Undefined.instance,
					CONST);
	}
	 ...
	 */
	public void defineOwnProperty(Context cx, Object id,
	...
	protected void defineOwnProperty(Context cx, Object id,
		...
		if (checkValid) {
			ScriptableObject current = slot == null ? null
					: slot.getPropertyDescriptor(cx, this);
	...
	protected boolean sameValue(Object newValue, Object currentValue) {
		...
		if (currentValue instanceof Number && newValue instanceof Number) {
			...
			if (d1 == 0.0 && Double.doubleToLongBits(d1) != Double
					.doubleToLongBits(d2)) {
				return false;
			}
		}
	 ...
	 * @since 1.7R3
	 */
	public static <T> T getTypedProperty(Scriptable s, int index,
			Class<T> type) {
	...
	private static Slot unwrapSlot(Slot slot) {
		return (slot instanceof RelinkedSlot) ? ((RelinkedSlot) slot).slot
				: slot;
	}
	...
	private Slot getSlot(String name, int index, int accessType) {
		...
		if (slotsLocalRef != null) {
			...
			for (slot = slotsLocalRef[slotIndex]; slot != null; slot = slot.next) {
				...
				if (indexOrHash == slot.indexOrHash && (sname == name
	...
	private synchronized Slot createSlot(String name, int indexOrHash,
		...
		if (count == 0) {
			...
			Slot slot = prev;
			while (slot != null) {
				if (slot.indexOrHash == indexOrHash && (slot.name == name
			...
			if (slot != null) {
				...
				if (accessType == SLOT_MODIFY_GETTER_SETTER
					...
					newSlot = new GetterSlot(name, indexOrHash,
						...
						&& (inner instanceof GetterSlot)) {
					newSlot = new Slot(name, indexOrHash,
							inner.getAttributes());
	...
	private static void copyTable(Slot[] oldSlots, Slot[] newSlots, int count) {
		...
		for (;;) {
			...
			while (slot != null) {
				...
				Slot insSlot = slot.next == null ? slot
						: new RelinkedSlot(slot);
	...
	Object[] getIds(boolean getAll) {
		Slot[] s = slots;
		Object[] a;
		int externalLen = (externalData == null ? 0
				: externalData.getArrayLength());
