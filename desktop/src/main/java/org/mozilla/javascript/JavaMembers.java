		...
		for (Field field : fields) {
			...
			try {
				...
				if (member == null) {
					...
					Map<String, FieldAndMethods> fmht = isStatic
							? staticFieldAndMethods : fieldAndMethods;
					if (fmht == null) {
				...
				// skip this field
				Context.reportWarning(
						"Could not access field " + name + " of class "
								+ cl.getName() + " due to lack of privileges.");
			}
		}
				...
				if (memberIsGetMethod || memberIsIsMethod
					...
					if (v != null) {
						...
						if (!includePrivate || !(v instanceof Member)
								|| !Modifier
										.isPrivate(((Member) v).getModifiers()))
