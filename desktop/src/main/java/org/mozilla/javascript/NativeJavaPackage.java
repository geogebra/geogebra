 ...
 * reflect classes and subpackages, and use a caching/sharing system to ensure
 * that members reflected into one JavaPackage appear in all other references to
 * the same package (as with Packages.java.lang and java.lang).
 *
 * @author Mike Shaver
 * @see NativeJavaArray
...
public class NativeJavaPackage extends ScriptableObject {
	...
	NativeJavaPackage forcePackage(String name, Scriptable scope) {
		...
		if (cached != null && cached instanceof NativeJavaPackage) {
			...
			String newPackage = packageName.length() == 0 ? name
					: packageName + "." + name;
	...
	synchronized Object getPkgProperty(String name, Scriptable start,
		...
		if (shutter == null || shutter.visibleToScripts(className)) {
			...
			if (cl != null) {
				...
				newValue = wrapFactory.wrapJavaClass(cx, getTopLevelScope(this),
						cl);
	...
	private void readObject(ObjectInputStream in)
		...
		this.classLoader = Context.getCurrentContext()
