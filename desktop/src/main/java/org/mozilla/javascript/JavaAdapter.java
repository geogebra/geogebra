...
public final class JavaAdapter implements IdFunctionCall {
	...
	public static Object convertResult(Object result, Class<?> c) {
		if (result == Undefined.instance && (c != ScriptRuntime.ObjectClass
	...
	static Object js_createAdapter(Context cx, Scriptable scope,
			Object[] args) {
		int N = args.length;
		if (N == 0) {
		...
		// and/or interfaces to implement, followed by one NativeObject
		// providing
			...
			if (argsCount > 0) {
				...
				if (index < 0) {
					...
					throw Context.reportRuntimeError2("msg.no.java.ctor",
							adapterClass.getName(), sig);
				}

				// Found the constructor, so try invoking it.
				adapter = NativeJavaClass.constructInternal(ctorArgs,
						ctors.methods[index]);
			} else {
				Class<?>[] ctorParms = { ScriptRuntime.ScriptableClass,
						ScriptRuntime.ContextFactoryClass };
				Object[] ctorArgs = { obj, cx.getFactory() };
				adapter = adapterClass.getConstructor(ctorParms)
	...
	private static ObjToIntMap getObjectFunctionNames(Scriptable obj) {
		...
		for (int i = 0; i != ids.length; ++i) {
			...
			if (value instanceof Function) {
				...
				int length = ScriptRuntime
						.toInt32(ScriptableObject.getProperty(f, "length"));
				if (length < 0) {
					length = 0;
				}
				map.put(id, length);
	...
	private static void generateCtor(ClassFileWriter cfw, String adapterName,
		...
		if (parameters.length == 0) {
			...
			int marker = sig.length(); // lets us reuse buffer for super
										// signature
			for (Class<?> c : parameters) {
				appendTypeString(sig, c);
			}
			sig.append(")V");
			cfw.startMethod("<init>", sig.toString(),
					ClassFileWriter.ACC_PUBLIC);
	...
	private static int generateWrapArg(ClassFileWriter cfw, int paramOffset,
		...
		if (!argType.isPrimitive()) {
			...
			cfw.addInvoke(ByteCode.INVOKESPECIAL, "java/lang/Boolean", "<init>",
					"(Z)V");
		...
		if (retType == Void.TYPE) {
			...
			default:
				throw new RuntimeException(
						"Unexpected return type " + retType.toString());
			}
	...
	private static StringBuilder appendTypeString(StringBuilder sb,
			Class<?> type) {
		while (type.isArray()) {
			sb.append('[');
