...
public class ClassCache implements Serializable {
	...
	public static ClassCache get(Scriptable scope) {
		ClassCache cache = (ClassCache) ScriptableObject.getTopScopeValue(scope,
				AKEY);
		if (cache == null) {
	 ...
	 * <tt>Class.getMethods()</tt> and similar calls. This can speed execution
	 * dramatically, but increases the memory footprint. Also, with caching
	 * enabled, references may be held to objects past the lifetime of any real
	 * usage.
	 * <p>
	...
	Map<Class<?>, JavaMembers> getClassCacheMap() {
		if (classTable == null) {
			// Use 1 as concurrency level here and for other concurrent hash
			// maps
			// as we don't expect high levels of sustained concurrent writes.
			classTable = new ConcurrentHashMap<Class<?>, JavaMembers>(16, 0.75f,
					1);
		}
		return classTable;
	}
	...
	Object getInterfaceAdapter(Class<?> cl) {
		return interfaceAdapterCache == null ? null
