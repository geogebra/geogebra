 ...
 * {@link ForInLoop}, except that it has no body statement. Node type is
 * {@link Token#FOR}.
 * <p>
 */
public class ArrayComprehensionLoop extends ForInLoop {
	...
	public String toSource(int depth) {
		return makeIndent(depth) + " for " + (isForEach() ? "each " : "") + "("
				+ iterator.toSource(0) + " in " + iteratedObject.toSource(0)
				+ ")";
	}

	/**
	 * Visits the iterator expression and the iterated object expression. There
	 * is no body-expression for this loop type.
	 */
	@Override
