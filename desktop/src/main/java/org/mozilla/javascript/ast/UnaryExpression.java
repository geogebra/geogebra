...
/**
 * AST node representing unary operators such as {@code ++}, {@code ~},
 * {@code typeof} and {@code delete}. The type field is set to the appropriate
 * Token type for the operator. The node length spans from the operator to the
 * end of the operand (for prefix operators) or from the start of the operand to
 * the operator (for postfix).
 * <p>
 *
 * The {@code default xml namespace = &lt;expr&gt;} statement in E4X (JavaScript
 * 1.6) is represented as a {@code UnaryExpression} of node type
 * {@link Token#DEFAULTNAMESPACE}, wrapped with an {@link ExpressionStatement}.
 */
public class UnaryExpression extends AstNode {
	 ...
	 * It sets the parent of the operand, and sets its own bounds to encompass
	 ...
	 * @param postFix
	 *            true if the operator follows the operand. Int
	 * @throws IllegalArgumentException}
	 *             if {@code operand} is {@code null}
	 */
	public UnaryExpression(int operator, int operatorPosition, AstNode operand,
		...
		int end = postFix ? operatorPosition + 2
	...
	/**
	 * Sets operator &ndash; same as {@link #setType}, but throws an exception
