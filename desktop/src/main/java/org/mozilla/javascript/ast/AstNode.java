 ...
 * physical source code, to make it useful for code-processing tools such as
 * IDEs or pretty-printers. The parser must not rewrite the parse tree when
 * producing this representation.
 * <p>
 *
 * The {@code AstNode} hierarchy sits atop the older {@link Node} class, which
 * was designed for code generation. The {@code Node} class is a flexible,
 * weakly-typed class suitable for creating and rewriting code trees, but using
 * it requires you to remember the exact ordering of the child nodes, which are
 * kept in a linked list. The {@code AstNode} hierarchy is a strongly-typed
 * facade with named accessors for children and common properties, but under the
 * hood it's still using a linked list of child nodes. It isn't a very good idea
 * to use the child list directly unless you know exactly what you're doing.
 * </p>
 *
 * Note that {@code AstNode} records additional information, including the
 * node's position, length, and parent node. Also, some {@code AstNode}
 * subclasses record some of their child nodes in instance members, since they
 * are not needed for code generation. In a nutshell, only the code generator
 * should be mixing and matching {@code AstNode} and {@code Node} objects.
 * <p>
 *
 * All offset fields in all subclasses of AstNode are relative to their parent.
 * For things like paren, bracket and keyword positions, the position is
 * relative to the current node. The node start position is relative to the
 * parent node.
 * <p>
 *
 ...
 * annotations to the tree. The Rhino code generator acts as a client and uses
 * node properties extensively. You are welcome to use the property-list API for
 * anything your client needs.
 * <p>
 *
 * This hierarchy does not have separate branches for expressions and
 * statements, as the distinction in JavaScript is not as clear-cut as in Java
 * or C++.
 * <p>
 */
public abstract class AstNode extends Node implements Comparable<AstNode> {
	...
	/**
	 * Returns the absolute document position of the node. Computes it by adding
	 * the node's relative position to the relative positions of all its
	 * parents.
	 */
	...
	/**
	 * Sets the node start and end positions. Computes the length as (
	...
	/**
	 * Make this node's position relative to a parent. Typically only used by
	 * the parser when constructing the node.
	 * 
	 * @param parentPosition
	 *            the absolute parent position; the current node position is
	 *            assumed to be absolute and is decremented by parentPosition.
	 */
	...
	/**
	 * Sets the node parent. This method automatically adjusts the current
	 * node's start position to be relative to the new parent.
	 * 
	 * @param parent
	 *            the new parent. Can be {@code null}.
	 */
	...
	/**
	 * Adds a child or function to the end of the block. Sets the parent of the
	 * child to this node, and fixes up the start position of the child to be
	 * relative to this node. Sets the length of this node to include the new
	 * child.
	 * 
	 * @param kid
	 *            the child
	 ...
	 * 
	 * @return the {@link AstRoot} at the root of this node's parent chain, or
	 *         {@code null} if the topmost parent is not an {@code AstRoot}.
	 */
	 ...
	 * {@code null} children that are expected to be non-{@code null} when no
	 * errors are present. In this situation, the behavior of the
	 * {@code toSource} method is undefined: {@code toSource} implementations
	 * may assume that the AST node is error-free, since it is intended to be
	 * invoked only at runtime after a successful parse.
	 * <p>
	 *
	 * @param depth
	 *            the current recursion depth, typically beginning at 0 when
	 ...
	 * 
	 * @param op
	 *            the token type, e.g. {@link Token#ADD} or {@link Token#TYPEOF}
	 ...
	 * <p>
	 *
	 * It's up to each node subclass to decide the order for processing its
	 * children. The subclass also decides (and should document) which child
	 * nodes are not passed to the {@code NodeVisitor}. For instance, nodes
	 * representing keywords like {@code each} or {@code in} may not be passed
	 * to the visitor object. The visitor can simply query the current node for
	 * these children if desired.
	 * <p>
	 *
	 * Generally speaking, the order will be deterministic; the order is
	 * whatever order is decided by each child node. Normally child nodes will
	 * try to visit their children in lexical order, but there may be exceptions
	 * to this rule.
	 * <p>
	 *
	 * @param visitor
	 ...
	 *            a list to print
	 * @param sb
	 *            a {@link StringBuilder} into which to print
	 */
	...
	/**
	 * Returns the innermost enclosing {@link Scope} node, or {@code null} if
	 * we're not nested in a scope. Begins the search with this node's parent.
	...
	/**
	 * Permits AST nodes to be sorted based on start position and length. This
	 * makes it easy to sort Comment and Error nodes into a set of other AST
	 * nodes: just put them all into a {@link java.util.SortedSet}, for
	 * instance.
	 * 
	 * @param other
	 *            another node
	 ...
	 *         {@code other}'s length. If the lengths are equal, sorts
	...
	/**
	 * Returns the depth of this node. The root is depth 0, its children are
	 * depth 1, and so on.
	 * 
	...
	/**
	 * Return the line number recorded for this node. If no line number was
	 * recorded, searches the parent chain.
	 * 
	...
	/**
	 * Returns a debugging representation of the parse tree starting at this
	 * node.
	 * 
	 * @return a very verbose indented printout of the tree. The format of each
	 *         line is: abs-pos name position length [identifier]
	 */
