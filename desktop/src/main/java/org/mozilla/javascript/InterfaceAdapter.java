...
public class InterfaceAdapter {
	...
	/**
	 * Make glue object implementing interface cl that will call the supplied JS
	 * function when called. Only interfaces were all methods have the same
	 * signature is supported.
	 *
	 * @return The glue object or null if <tt>cl</tt> is not interface or has
	...
	static Object create(Context cx, Class<?> cl, ScriptableObject object) {
		...
		if (adapter == null) {
			...
			if (object instanceof Callable) {
				...
				// We allow this if the interface has only one method or
				// multiple
	...
	Object invokeImpl(Context cx, Object target, Scriptable topScope,
			Object thisObject, Method method, Object[] args) {
		Callable function;
		if (target instanceof Callable) {
			...
			if (!(value instanceof Callable)) {
				throw Context.reportRuntimeError1("msg.not.function.interface",
						methodName);
			}
		...
		Scriptable thisObj = wf.wrapAsJavaObject(cx, topScope, thisObject,
				null);
