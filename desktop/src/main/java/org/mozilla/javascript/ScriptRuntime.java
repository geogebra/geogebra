...
public class ScriptRuntime {
	...
	public static ScriptableObject initSafeStandardObjects(Context cx,
		...
		if (((cx.getLanguageVersion() >= Context.VERSION_1_8)
			...
			new LazilyLoadedCtor(scope, "DataView",
					"org.mozilla.javascript.typedarrays.NativeDataView", sealed,
					true);
		}
	 ...
	 * See ECMA 9.9.
	 */
	public static Scriptable toObject(Context cx, Scriptable scope,
			Object val) {
		if (val instanceof Scriptable) {
			return (Scriptable) val;
		}
	...
	public static long indexFromString(String str) {
		...
		if (len > 0) {
			...
			c -= '0';
			if (0 <= c && c <= 9 && len <= (negate ? MAX_VALUE_LENGTH + 1
					: MAX_VALUE_LENGTH)) {
				...
				if (index != 0) {
					...
					while (i != len && 0 <= (c = str.charAt(i) - '0')
							&& c <= 9) {
						oldIndex = index;
						index = 10 * index - c;
						i++;
	...
	public static Object getObjectProp(Scriptable obj, String property,
		...
		if (result == Scriptable.NOT_FOUND) {
			if (cx.hasFeature(Context.FEATURE_STRICT_MODE)) {
				Context.reportWarning(ScriptRuntime
	...
	public static Scriptable toIterator(Context cx, Scriptable scope,
		...
		if (ScriptableObject.hasProperty(obj,
			...
			Object[] args = new Object[] {
					keyOnly ? Boolean.TRUE : Boolean.FALSE };
	 ...
	 */
	@Deprecated
	public static Object enumInit(Object value, Context cx,
			boolean enumValues) {
		return enumInit(value, cx,
				enumValues ? ENUMERATE_VALUES : ENUMERATE_KEYS);
	}
	...
	public static Object callSpecial(Context cx, Callable fun,
			...
			Scriptable callerThis, int callType, String filename,
			int lineNumber) {
	...
	private static boolean eqString(CharSequence x, Object y) {
		for (;;) {
			if (y == null || y == Undefined.instance) {
				...
				return toNumber(x.toString()) == (((Boolean) y).booleanValue()
						? 1.0 : 0.0);
			} else if (y instanceof Scriptable) {
				if (y instanceof ScriptableObject) {
					Object test = ((ScriptableObject) y)
							.equivalentValues(x.toString());
					if (test != Scriptable.NOT_FOUND) {
						return ((Boolean) test).booleanValue();
					}
				}
		...
		getObj: if (t instanceof JavaScriptException) {
			...
			if (javaException != null && isVisible(cx, javaException)) {
				Object wrap = cx.getWrapFactory().wrap(cx, scope, javaException,
						null);
				ScriptableObject
						.defineProperty(errorObject, "javaException", wrap,
			...
			if (isVisible(cx, re)) {
				...
				ScriptableObject
						.defineProperty(errorObject, "rhinoException", wrap,
								ScriptableObject.PERMANENT
										| ScriptableObject.READONLY
										| ScriptableObject.DONTENUM);
			}
	...
	public static String getMessage2(String messageId, Object arg1,
			Object arg2) {
	...
	public static RuntimeException undefReadError(Object object, Object id) {
		return typeError2("msg.undef.prop.read", toString(object),
				toString(id));
	}
	...
	public static RuntimeException notFunctionError(Object value,
		...
		String msg = (messageHelper == null) ? "null"
				: messageHelper.toString();
		if (value == Scriptable.NOT_FOUND) {
	...
	public static JavaScriptException throwCustomError(Context cx,
			Scriptable scope, String constructorName, String message) {
		int[] linep = { 0 };
		String filename = Context.getSourcePositionFromStack(linep);
	...
	public static Scriptable requireObjectCoercible(Scriptable val,
			IdFunctionObject idFuncObj) {
		Scriptable val1 = val.getParentScope() != null ? val : null;
		if (val1 == null || val1 == Undefined.instance) {
			throw ScriptRuntime.typeError2("msg.called.null.or.undefined",
					idFuncObj.getTag(), idFuncObj.getFunctionName());
		}
