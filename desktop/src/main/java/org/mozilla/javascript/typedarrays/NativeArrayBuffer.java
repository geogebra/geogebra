...
/**
 * A NativeArrayBuffer is the backing buffer for a typed array. Used inside
 * JavaScript code, it implements the ArrayBuffer interface. Used directly from
 * Java, it simply holds a byte array.
 */
...
public class NativeArrayBuffer extends IdScriptableObject {
	...
	/**
	 * Return the actual bytes that back the buffer. This is a reference to the
	 * real buffer, so changes to bytes here will be reflected in the actual
	 * object and all its views.
	 */
	...
	/**
	 * Return a new buffer that represents a slice of this buffer's content,
	 * starting at position "start" and ending at position "end". Both values
	 * will be "clamped" as per the JavaScript spec so that invalid values may
	 * be passed and will be adjusted up or down accordingly. This method will
	 * return a new buffer that contains a copy of the original buffer. Changes
	 * there will not affect the content of the buffer.
	 *
	 * @param s
	 *            the position where the new buffer will start
	 * @param e
	...
	public Object execIdCall(IdFunctionObject f, Context cx, Scriptable scope,
		...
		case ConstructorId_isView:
			return (isArg(args, 0)
					&& (args[0] instanceof NativeArrayBufferView));
		...
		case Id_slice:
			...
			int end = isArg(args, 1) ? ScriptRuntime.toInt32(args[1])
					: self.buffer.length;
	...
	private static final int Id_constructor = 1, Id_slice = 2, Id_isView = 3,
	...
	protected void fillConstructorProperties(IdFunctionObject ctor) {
		addIdFunctionProperty(ctor, CLASS_NAME, ConstructorId_isView, "isView",
				1);
	}
