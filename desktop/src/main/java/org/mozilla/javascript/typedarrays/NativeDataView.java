...
/**
 * This class represents the JavaScript "DataView" interface, which allows
 * direct manipulations of the bytes in a NativeArrayBuffer. Java programmers
 * would be best off getting the underling "byte[]" array from the
 * NativeArrayBuffer and manipulating it directly, perhaps using the "ByteIo"
 * class as a helper.
 */
...
public class NativeDataView extends NativeArrayBufferView {
	...
	private NativeDataView js_constructor(NativeArrayBuffer ab, int offset,
			int length) {
		if (length < 0) {
			throw ScriptRuntime.constructError("RangeError",
					"length out of range");
	...
	private Object js_getInt(int bytes, boolean signed, Object[] args) {
		...
		boolean littleEndian = (isArg(args, 1) && (bytes > 1)
		...
		case 2:
			return (signed
					...
					: ByteIo.readUint16(arrayBuffer.buffer, offset,
							littleEndian));
		case 4:
			return (signed
					? ByteIo.readInt32(arrayBuffer.buffer, offset, littleEndian)
					: ByteIo.readUint32(arrayBuffer.buffer, offset,
							littleEndian));
		default:
	...
	private void js_setInt(int bytes, boolean signed, Object[] args) {
		...
		boolean littleEndian = (isArg(args, 2) && (bytes > 1)
				&& ScriptRuntime.toBoolean(args[2]));
	...
	public Object execIdCall(IdFunctionObject f, Context cx, Scriptable scope,
		...
		case Id_constructor:
			if (isArg(args, 0) && (args[0] instanceof NativeArrayBuffer)) {
				...
				int len = isArg(args, 2) ? ScriptRuntime.toInt32(args[2])
						: ab.getLength() - off;
	...
	private static final int Id_constructor = 1, Id_getInt8 = 2,
			Id_getUint8 = 3, Id_getInt16 = 4, Id_getUint16 = 5, Id_getInt32 = 6,
			Id_getUint32 = 7, Id_getFloat32 = 8, Id_getFloat64 = 9,
			Id_setInt8 = 10, Id_setUint8 = 11, Id_setInt16 = 12,
			Id_setUint16 = 13, Id_setInt32 = 14, Id_setUint32 = 15,
			Id_setFloat32 = 16, Id_setFloat64 = 17,
