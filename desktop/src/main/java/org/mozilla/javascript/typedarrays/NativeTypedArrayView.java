...
public abstract class NativeTypedArrayView<T> extends NativeArrayBufferView
	...
	protected NativeTypedArrayView(NativeArrayBuffer ab, int off, int len,
			int byteLen) {
	...
	protected abstract NativeTypedArrayView construct(NativeArrayBuffer ab,
			int off, int len);
	...
	protected abstract NativeTypedArrayView realThis(Scriptable thisObj,
			IdFunctionObject f);

	private NativeArrayBuffer makeArrayBuffer(Context cx, Scriptable scope,
			int length) {
		return (NativeArrayBuffer) cx.newObject(scope,
				NativeArrayBuffer.CLASS_NAME, new Object[] { length });
	}

	private NativeTypedArrayView js_constructor(Context cx, Scriptable scope,
			Object[] args) {
		if (!isArg(args, 0)) {
			...
			NativeArrayBuffer buffer = makeArrayBuffer(cx, scope,
			...
			NativeArrayBuffer na = makeArrayBuffer(cx, scope,
					src.length * getBytesPerElement());
			...
			if ((byteOff % getBytesPerElement()) != 0) {
				throw ScriptRuntime.constructError("RangeError",
						"offset must be a multiple of the byte size");
			}
	...
	private Object js_subarray(Context cx, Scriptable scope, int s, int e) {
		...
		int byteOff = Math.min(start * getBytesPerElement(),
				arrayBuffer.getLength());

		return cx.newObject(scope, getClassName(),
		...
		case Id_get:
			if (args.length > 0) {
				return realThis(thisObj, f)
						.js_get(ScriptRuntime.toInt32(args[0]));
			} else {
		...
		case Id_set:
			if (args.length > 0) {
				...
				if (args[0] instanceof NativeTypedArrayView) {
					int offset = isArg(args, 1) ? ScriptRuntime.toInt32(args[1])
							: 0;
	...
	private static final int Id_constructor = 1, Id_get = 2, Id_set = 3,
			Id_subarray = 4;

	protected static final int MAX_PROTOTYPE_ID = Id_subarray;
	 ...
	 */
	private static final int Id_length = 10, Id_BYTES_PER_ELEMENT = 11,
	...
	public <U> U[] toArray(U[] ts) {
		...
		if (ts.length >= length) {
			...
			a = (U[]) Array.newInstance(ts.getClass().getComponentType(),
					length);
		}
