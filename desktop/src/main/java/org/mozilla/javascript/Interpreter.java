...
public final class Interpreter extends Icode implements Evaluator {
	...
	public Object compile(CompilerEnvirons compilerEnv, ScriptNode tree,
		...
		itsData = cgen.compile(compilerEnv, tree, encodedSource,
				returnFunction);
		return itsData;
	}
	...
	public void captureStackInfo(RhinoException ex) {
		...
		if (cx.previousInterpreterInvocations == null
			...
			if (cx.previousInterpreterInvocations
					.peek() == cx.lastInterpreterFrame) {
	...
	public ScriptStackElement[][] getScriptStackElements(RhinoException ex) {
		...
		while (arrayIndex != 0) {
			...
			while (frame != null) {
				...
				frame = frame.parentFrame;
				group.add(new ScriptStackElement(fileName, functionName,
						lineNumber));
			}
		...
		StateLoop: for (;;) {
			withoutExceptions: try {
				...
				Loop: for (;;) {
					...
					jumplessRun: {
						...
						case Icode_GENERATOR: {
							if (!frame.frozen) {
								// First time encountering this opcode: create
								// new generator
								// object and return
								frame.pc--; // we want to come back here when we
											// resume
								CallFrame generatorFrame = captureFrameForGenerator(
										frame);
								generatorFrame.frozen = true;
								NativeGenerator generator = new NativeGenerator(
										frame.scope, generatorFrame.fnOrScript,
										generatorFrame);
								frame.result = generator;
								break Loop;
							} else {
								// We are now resuming execution. Fall through
								// to YIELD case.
							}
						}
						// fall through...
						case Token.YIELD: {
							if (!frame.frozen) {
								return freezeGenerator(cx, frame, stackTop,
										generatorState);
							} else {
								Object obj = thawGenerator(frame, stackTop,
										generatorState, op);
						...
						case Token.GT:
						case Token.LT: {
							stackTop = doCompare(frame, op, stack, sDbl,
									stackTop);
							continue Loop;
						}
						case Token.IN:
						case Token.INSTANCEOF: {
							stackTop = doInOrInstanceof(cx, op, stack, sDbl,
									stackTop);
							continue Loop;
						}
						case Token.EQ:
						...
						case Token.SHNE: {
							--stackTop;
							boolean valBln = doShallowEquals(stack, sDbl,
									stackTop);
						...
						case Token.LSH:
						case Token.RSH: {
							stackTop = doBitOp(frame, op, stack, sDbl,
									stackTop);
							continue Loop;
						}
						case Token.URSH: {
							...
							stack[--stackTop] = DBL_MRK;
							sDbl[stackTop] = ScriptRuntime
									.toUint32(lDbl) >>> rIntValue;
							continue Loop;
						}
						case Token.NEG:
						case Token.POS: {
						...
						case Token.DIV:
						case Token.MOD: {
							stackTop = doArithmetic(frame, op, stack, sDbl,
									stackTop);
							continue Loop;
						}
						case Token.NOT:
						...
						case Token.DELPROP:
						case Icode_DELNAME: {
							stackTop = doDelName(cx, frame, op, stack, sDbl,
									stackTop);
							continue Loop;
						}
						...
						case Token.GETELEM: {
							stackTop = doGetElem(cx, frame, stack, sDbl,
									stackTop);
							continue Loop;
						}
						case Token.SETELEM: {
							stackTop = doSetElem(cx, frame, stack, sDbl,
									stackTop);
							continue Loop;
						}
						case Icode_ELEM_INC_DEC: {
							stackTop = doElemIncDec(cx, frame, iCode, stack,
									sDbl, stackTop);
							continue Loop;
						}
						case Token.GET_REF: {
						...
						case Icode_CALLSPECIAL: {
							...
							stackTop = doCallSpecial(cx, frame, stack, sDbl,
									stackTop, iCode, indexReg);
							continue Loop;
						}
						case Token.CALL:
						case Icode_TAIL_CALL:
						case Token.REF_CALL: {
							...
							if (fun instanceof InterpretedFunction) {
								...
								if (frame.fnOrScript.securityDomain == ifun.securityDomain) {
									...
									initFrame(cx, calleeScope, funThisObj,
											...
											ifun, callParentFrame, calleeFrame);
									if (op != Icode_TAIL_CALL) {
							...
							if (fun instanceof IdFunctionObject) {
								...
								// Bug 405654 -- make best effort to keep
								// Function.apply and
								// Function.call within this interpreter loop
								// invocation
								if (BaseFunction.isApplyOrCall(ifun)) {
									...
									if (applyCallable instanceof InterpretedFunction) {
										...
										if (frame.fnOrScript.securityDomain == iApplyCallable.securityDomain) {
											frame = initFrameForApplyOrCall(cx,
													frame, indexReg, stack,
													sDbl, stackTop, op,
													calleeScope, ifun,
													iApplyCallable);
											continue StateLoop;
							...
							if (fun instanceof NoSuchMethodShim) {
								...
								// if the method is in fact an
								// InterpretedFunction
								if (noSuchMethodMethod instanceof InterpretedFunction) {
									InterpretedFunction ifun = (InterpretedFunction) noSuchMethodMethod;
									if (frame.fnOrScript.securityDomain == ifun.securityDomain) {
										frame = initFrameForNoSuchMethod(cx,
												frame, indexReg, stack, sDbl,
												stackTop, op, funThisObj,
												calleeScope, noSuchMethodShim,
												ifun);
										continue StateLoop;
									}
								}
							...
							stack[stackTop] = fun.call(cx, calleeScope,
									funThisObj, getArgsArray(stack, sDbl,
											stackTop + 2, indexReg));
						...
						case Icode_TYPEOFNAME:
							stack[++stackTop] = ScriptRuntime
									.typeofName(frame.scope, stringReg);
							continue Loop;
						case Token.STRING:
						...
						case Token.SETCONSTVAR:
							stackTop = doSetConstVar(frame, stack, sDbl,
									stackTop, vars, varDbls, varAttributes,
									indexReg);
							continue Loop;
						case Icode_SETVAR1:
							indexReg = iCode[frame.pc++];
							// fallthrough
						case Token.SETVAR:
							stackTop = doSetVar(frame, stack, sDbl, stackTop,
						...
						case Icode_GETVAR1:
							...
							// fallthrough
						case Token.GETVAR:
							stackTop = doGetVar(frame, stack, sDbl, stackTop,
									vars, varDbls, indexReg);
							continue Loop;
						case Icode_VAR_INC_DEC: {
							stackTop = doVarIncDec(cx, frame, stack, sDbl,
									stackTop, vars, varDbls, varAttributes,
									indexReg);
							continue Loop;
						}
						...
						case Token.CATCH_SCOPE: {
							...
							Throwable caughtException = (Throwable) stack[stackTop
									+ 1];
						...
						case Token.REF_MEMBER: {
							// indexReg: flags
							stackTop = doRefMember(cx, stack, sDbl, stackTop,
									indexReg);
							continue Loop;
						}
						case Token.REF_NS_MEMBER: {
							// indexReg: flags
							stackTop = doRefNsMember(cx, stack, sDbl, stackTop,
									indexReg);
							continue Loop;
						}
						case Token.REF_NAME: {
						...
						case Token.REF_NS_NAME: {
							// indexReg: flags
							stackTop = doRefNsName(cx, frame, stack, sDbl,
									stackTop, indexReg);
							continue Loop;
						}
						...
						case Token.REGEXP:
							...
							stack[++stackTop] = ScriptRuntime.wrapRegExp(cx,
									frame.scope, re);
							continue Loop;
			...
			if (generatorState != null
				...
				exState = cx.hasFeature(Context.FEATURE_ENHANCED_JAVA_ACCESS)
						? EX_CATCH_STATE : EX_FINALLY_STATE;
			} else if (throwable instanceof Error) {
				exState = cx.hasFeature(Context.FEATURE_ENHANCED_JAVA_ACCESS)
						? EX_CATCH_STATE : EX_NO_JS_STATE;
			} else if (throwable instanceof ContinuationJump) {
	...
	private static int doCallSpecial(Context cx, CallFrame frame,
			Object[] stack, double[] sDbl, int stackTop, byte[] iCode,
			int indexReg) {
		...
		if (isNew) {
			...
			Object[] outArgs = getArgsArray(stack, sDbl, stackTop + 1,
					indexReg);
			stack[stackTop] = ScriptRuntime.newSpecial(cx, function, outArgs,
					frame.scope, callType);
		} else {
			...
			stack[stackTop] = ScriptRuntime.callSpecial(cx, function,
					functionThis, outArgs, frame.scope, frame.thisObj, callType,
					frame.idata.itsSourceFile, sourceLine);
		}
		frame.pc += 4;
		return stackTop;
	...
	private static int doSetConstVar(CallFrame frame, Object[] stack,
			double[] sDbl, int stackTop, Object[] vars, double[] varDbls,
		...
		if (!frame.useActivation) {
			...
			if ((varAttributes[indexReg]
					& ScriptableObject.UNINITIALIZED_CONST) != 0) {
	...
	private static int doSetVar(CallFrame frame, Object[] stack, double[] sDbl,
			int stackTop, Object[] vars, double[] varDbls, int[] varAttributes,
			int indexReg) {
		if (!frame.useActivation) {
			if ((varAttributes[indexReg] & ScriptableObject.READONLY) == 0) {
				vars[indexReg] = stack[stackTop];
	...
	private static int doGetVar(CallFrame frame, Object[] stack, double[] sDbl,
			int stackTop, Object[] vars, double[] varDbls, int indexReg) {
		++stackTop;
		if (!frame.useActivation) {
	...
	private static int doVarIncDec(Context cx, CallFrame frame, Object[] stack,
			double[] sDbl, int stackTop, Object[] vars, double[] varDbls,
			int[] varAttributes, int indexReg) {
		// indexReg : varindex
		++stackTop;
	...
	private static int doRefNsName(Context cx, CallFrame frame, Object[] stack,
			double[] sDbl, int stackTop, int flags) {
		Object name = stack[stackTop];
		if (name == DOUBLE_MARK) {
			name = ScriptRuntime.wrapNumber(sDbl[stackTop]);
		}
	...
	private static Object freezeGenerator(Context cx, CallFrame frame,
		...
		return (frame.result != DOUBLE_MARK) ? frame.result
	...
	private static CallFrame initFrameForApplyOrCall(Context cx,
			CallFrame frame, int indexReg, Object[] stack, double[] sDbl,
			int stackTop, int op, Scriptable calleeScope, IdFunctionObject ifun,
	...
	public static NativeContinuation captureContinuation(Context cx) {
		...
		return captureContinuation(cx, (CallFrame) cx.lastInterpreterFrame,
				true);
	}
	...
	private static void doAdd(Object[] stack, double[] sDbl, int stackTop,
		...
		if (lhs instanceof Scriptable) {
			...
			double lDbl = (lhs instanceof Number) ? ((Number) lhs).doubleValue()
					: ScriptRuntime.toNumber(lhs);
			stack[stackTop] = DOUBLE_MARK;
