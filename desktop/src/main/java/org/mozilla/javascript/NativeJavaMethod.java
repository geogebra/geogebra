...
public class NativeJavaMethod extends BaseFunction {
	...
	public Object call(Context cx, Scriptable scope, Scriptable thisObj,
		...
		Object wrapped = cx.getWrapFactory().wrap(cx, scope, retval,
				staticType);
		if (debug) {
			Class<?> actualType = (wrapped == null) ? null : wrapped.getClass();
			System.err.println(
					" ----- Wrapped as " + wrapped + " class = " + actualType);
		}
			...
			if (firstBestFit < 0) {
				...
				for (int j = -1; j != extraBestFitsCount; ++j) {
					...
					if (cx.hasFeature(Context.FEATURE_ENHANCED_JAVA_ACCESS)
							&& (bestFit.member().getModifiers()
									& Modifier.PUBLIC) != (member.member()
											.getModifiers()
											& Modifier.PUBLIC)) {
						...
						if (preference == PREFERENCE_AMBIGUOUS) {
							...
							if (bestFit.isStatic() && bestFit
									.getDeclaringClass().isAssignableFrom(
