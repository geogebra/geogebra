...
public class Token {
	...
	ERROR = -1, // well-known as the only code < EOF
			...
			ENTERWITH = 2, LEAVEWITH = 3, RETURN = 4, GOTO = 5, IFEQ = 6,
			IFNE = 7, SETNAME = 8, BITOR = 9, BITXOR = 10, BITAND = 11, EQ = 12,
			NE = 13, LT = 14, LE = 15, GT = 16, GE = 17, LSH = 18, RSH = 19,
			URSH = 20, ADD = 21, SUB = 22, MUL = 23, DIV = 24, MOD = 25,
			NOT = 26, BITNOT = 27, POS = 28, NEG = 29, NEW = 30, DELPROP = 31,
			TYPEOF = 32, GETPROP = 33, GETPROPNOWARN = 34, SETPROP = 35,
			GETELEM = 36, SETELEM = 37, CALL = 38, NAME = 39, NUMBER = 40,
			STRING = 41, NULL = 42, THIS = 43, FALSE = 44, TRUE = 45, SHEQ = 46, // shallow
																					// equality
																					// (===)
			SHNE = 47, // shallow inequality (!==)
			REGEXP = 48, BINDNAME = 49, THROW = 50, RETHROW = 51, // rethrow
																	// caught
																	// exception:
																	// catch (e
																	// if ) use
																	// it
			IN = 52, INSTANCEOF = 53, LOCAL_LOAD = 54, GETVAR = 55, SETVAR = 56,
			CATCH_SCOPE = 57, ENUM_INIT_KEYS = 58, ENUM_INIT_VALUES = 59,
			ENUM_INIT_ARRAY = 60, ENUM_NEXT = 61, ENUM_ID = 62, THISFN = 63,
			...
			ESCXMLATTR = 75, ESCXMLTEXT = 76, REF_MEMBER = 77, // Reference for
																// x.@y, x..y
																// etc.
	...
	public final static int LAST_BYTECODE_TOKEN = REF_NS_NAME,
			...
			TRY = 81, SEMI = 82, // semicolon
			LB = 83, // left and right brackets
			RB = 84, LC = 85, // left and right curlies (braces)
			RC = 86, LP = 87, // left and right parentheses
			RP = 88, COMMA = 89, // comma operator
	...
	public final static int FIRST_ASSIGN = ASSIGN, LAST_ASSIGN = ASSIGN_MOD,
			...
			COLON = 103, OR = 104, // logical or (||)
			AND = 105, // logical and (&&)
			INC = 106, // increment/decrement (++ --)
			DEC = 107, DOT = 108, // member operator (.)
			...
			/*
			 * types used for the parse tree - these never get returned by the
			 * scanner.
			 */

			BLOCK = 129, // statement block
			LABEL = 130, // label
			TARGET = 131, LOOP = 132, EXPR_VOID = 133, // expression statement
														// in functions
			EXPR_RESULT = 134, // expression statement in scripts
			JSR = 135, SCRIPT = 136, // top-level node for entire script
			TYPEOFNAME = 137, // for typeof(simple-name)
			USE_STACK = 138, SETPROP_OP = 139, // x.y op= something
			SETELEM_OP = 140, // x[y] op= something
			LOCAL_BLOCK = 141, SET_REF_OP = 142, // *reference op= something
			...
			TO_OBJECT = 149, TO_DOUBLE = 150,

			GET = 151, // JS 1.5 get pseudo keyword
			SET = 152, // JS 1.5 set pseudo keyword
			LET = 153, // JS 1.7 let pseudo keyword
			CONST = 154, SETCONST = 155, SETCONSTVAR = 156, ARRAYCOMP = 157, // array
																				// comprehension
			LETEXPR = 158, WITHEXPR = 159, DEBUGGER = 160, COMMENT = 161,
			GENEXPR = 162, METHOD = 163, // ES6 MethodDefinition
			LAST_TOKEN = 164;

	/**
	 * Returns a name for the token. If Rhino is compiled with certain hardcoded
	 * debugging flags in this file, it calls {@code #typeToName}; otherwise it
	 * returns a string whose value is the token number.
	 */
