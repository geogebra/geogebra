...
class DToA {
	...
	static final int DTOSTR_STANDARD = 0, /*
											 * Either fixed or exponential format;
											 * round-trip
											 */
	...
	private static final double tens[] = { 1e0, 1e1, 1e2, 1e3, 1e4, 1e5, 1e6,
			1e7, 1e8, 1e9, 1e10, 1e11, 1e12, 1e13, 1e14, 1e15, 1e16, 1e17, 1e18,
			1e19, 1e20, 1e21, 1e22 };
	...
	/*
	 * Convert d into the form b*2^e, where b is an odd integer. b is the
	 * returned Bigint and e is the returned binary exponent. Return the number
	...
	static String JS_dtobasestr(int base, double d) {
		...
		if (d == dfloor) {
			...
			if ((word1 == 0) && ((word0 & Bndry_mask) == 0)
				 ...
				 * last input significant digit instead of one half of it when
				 * the output string's value is less than d.
				 */
				s2 += Log2P;
			...
			s = s.shiftLeft(s2);
			/*
			 * At this point we have the following: s = 2^s2; 1 > df = b/2^s2 >
			 * 0; (d - prevDouble(d))/2 = mlo/2^s2; (nextDouble(d) - d)/2 =
			 * mhi/2^s2.
			 */
			...
			do {
				...
				if (j1 == 0 && ((word1 & 1) == 0)) {
					...
					if (j1 > 0) {
						 ...
						 * significant digit. Use whichever would produce an
						 * output value closer to d.
						 */
						b = b.shiftLeft(1);
						j1 = b.compareTo(s);
						if (j1 > 0) /*
									 * The even test (|| (j1 == 0 && (digit &
									 * 1))) is not here because it messes up odd
									 * base output such as 3.5 in base 3.
									 */
							digit++;
					}
					done = true;
	...
	/*
	 * If biasUp is set, then rounding in modes 2 and 3 will round away from
	 * zero when the number is exactly halfway between two representable values.
	 * For example, rounding 2.5 to zero digits after the decimal point will
	 * return 3 and not 2. 2.49 will still round to 2, and 2.51 will still round
	 * to 3.
	 */
	/*
	 * bufsize should be at least 20 for modes 0 and 1. For the other modes,
	 * bufsize should be two greater than the maximum number of output
	 * characters expected.
	 */
	static int JS_dtoa(double d, int mode, boolean biasUp, int ndigits,
		 ...
		 * This gives a return value similar to that from fcvt, except that
		 * trailing zeros are suppressed, and ndigits can be negative. 4-9
		 * should give the same return values as 2-3, i.e., 4 <= mode <= 9 ==>
		...
		if ((word0(d) & Exp_mask) == Exp_mask) {
			/* Infinity or NaN */
			buf.append(((word1(d) == 0) && ((word0(d) & Frac_mask) == 0))
					? "Infinity" : "NaN");
			return 9999;
		}
		if (d == 0) {
		...
		if ((i = (word0(d) >>> Exp_shift1 & (Exp_mask >> Exp_shift1))) != 0) {
			 ...
			 * accurate k by invoking log10, but this is probably not
			 * worthwhile.)
			 */
			i -= Bias;
			denorm = false;
			...
			x = (i > 32) ? ((long) word0(d)) << (64 - i) | word1(d) >>> (i - 32)
					: ((long) word1(d)) << (32 - i);
			// d2 = x;
		...
		/*
		 * At this point d = f*2^i, where 1 <= f < 2. d2 is an approximation of
		 * f.
		 */
		...
		/*
		 * At this point floor(log10(d)) <= k <= floor(log10(d))+1. If k_check
		 * is zero, we're guaranteed that k = floor(log10(d)).
		 */
		j = bbits[0] - i - 1;
		...
		/*
		 * At this point d/10^k = (b * 2^b2 * 5^b5) / (2^s2 * 5^s5), where b is
		 * an odd integer, b2 >= 0, b5 >= 0, s2 >= 0, and s5 >= 0.
		 */
		if (mode < 0 || mode > 9)
			mode = 0;
		try_quick = true;
		...
		/*
		 * ilim is the maximum number of significant digits we want, based on k
		 * and ndigits.
		 */
		/*
		 * ilim1 is the maximum number of significant digits we want, based on k
		 * and ndigits, when it turns out that k was computed too high by one.
		 */
		...
		if (ilim >= 0 && ilim <= Quick_max && try_quick) {
			...
			ieps = 2; /* conservative */
			/*
			 * Divide d by 10^k, keeping track of the roundoff error and
			 * avoiding overflows.
			 */
			if (k > 0) {
				ds = tens[k & 0xf];
			...
			if (!fast_failed) {
				...
				if (leftright) {
					/*
					 * Use Steele & White method of only generating digits
					 * needed.
					 */
		...
		if (leftright) {
			if (mode < 2) {
				i = (denorm) ? be[0] + (Bias + (P - 1) - 1 + 1)
						: 1 + P - bbits[0];
				/*
				 * i is 1 plus the number of trailing zero bits in d's
				 * significand. Thus, (2^m2 * 5^m5) / (2^(s2+i) * 5^s5) = (1/2
				 * lsb of d)/10^k.
				 */
			} else {
				j = ilim - 1;
			...
			/*
			 * (mhi * 2^m2 * 5^m5) / (2^s2 * 5^s5) = one-half of last printed
			 * (when mode >= 2) or input (when mode < 2) significant digit,
			 * divided by 10^k.
			 */
		}
		/*
		 * We still have d/10^k = (b * 2^b2 * 5^b5) / (2^s2 * 5^s5). Reduce
		 * common factors in b2, m2, and s2 without changing the equalities.
		 */
		if (m2 > 0 && s2 > 0) {
			i = (m2 < s2) ? m2 : s2;
			b2 -= i;
		...
		/*
		 * Now we have d/10^k = (b * 2^b2) / (2^s2 * 5^s5) and (mhi * 2^m2) /
		 * (2^s2 * 5^s5) = one-half of last printed or input significant digit,
		 * divided by 10^k.
		 */
		...
		if (s5 > 0)
			S = pow5mult(S, s5);
		/*
		 * Now we have d/10^k = (b * 2^b2) / (S * 2^s2) and (mhi * 2^m2) / (S *
		 * 2^s2) = one-half of last printed or input significant digit, divided
		 * by 10^k.
		 */
		...
		if (mode < 2) {
			if ((word1(d) == 0) && ((word0(d) & Bndry_mask) == 0)
				 ...
				 * The special case. Here we want to be within a quarter of the
				 * last input significant digit instead of one half of it when
				 * the decimal output string's value is less than d.
				 */
				b2 += Log2P;
				s2 += Log2P;
				spec_case = true;
		...
		if ((i = (((s5 != 0) ? 32 - hi0bits(S_hiWord) : 1) + s2) & 0x1f) != 0)
			i = 32 - i;
		/*
		 * i is the number of leading zero bits in the most significant word of
		 * S*2^s2.
		 */
		if (i > 4) {
			i -= 4;
		...
		/*
		 * Now S*2^s2 has exactly four leading zero bits in its most significant
		 * word.
		 */
		if (b2 > 0)
			b = b.shiftLeft(b2);
		if (s2 > 0)
			S = S.shiftLeft(s2);
		/*
		 * Now we have d/10^k = b/S and (mhi * 2^m2) / S = maximum acceptable
		 * error, divided by 10^k.
		 */
		if (k_check) {
			if (b.compareTo(S) < 0) {
				k--;
				b = b.multiply(
						BigInteger.valueOf(10)); /* we botched the k estimate */
				if (leftright)
		...
		if (ilim <= 0 && mode > 2) {
			/*
			 * We're doing fixed-mode output and d is less than the minimum
			 * nonzero output in this mode. Output either zero or the minimum
			 * nonzero output depending on which is closer to d.
			 */
			if ((ilim < 0)
					|| ((i = b.compareTo(
							S = S.multiply(BigInteger.valueOf(5)))) < 0)
					|| ((i == 0 && !biasUp))) {
				/*
				 * Always emit at least one digit. If the number appears to be
				 * zero using the current mode, then emit one '0' digit and set
				 * decpt to 1.
				 */
				/*
		...
		if (leftright) {
			...
			/*
			 * Compute mlo -- check for special case that d is a normalized
			 * power of 2.
			 */
			...
			/*
			 * mlo/S = maximum acceptable error, divided by 10^k, if the output
			 * is less than d.
			 */
			/*
			 * mhi/S = maximum acceptable error, divided by 10^k, if the output
			 * is greater than d.
			 */
			...
			for (i = 1;; i++) {
				...
				/*
				 * Do we yet have the shortest decimal string that will round to
				 * d?
				 */
				j = b.compareTo(mlo);
				...
				if ((j < 0)
						|| ((j == 0) && (mode == 0) && ((word1(d) & 1) == 0))) {
					if (j1 > 0) {
						/*
						 * Either dig or dig+1 would work here as the least
						 * significant decimal digit. Use whichever would
						 * produce a decimal value closer to d.
						 */
						b = b.shiftLeft(1);
						j1 = b.compareTo(S);
						if (((j1 > 0)
	...
	static void JS_dtostr(StringBuilder buffer, int mode, int precision,
			double d) {
		int decPt; /*
					 * Position of decimal point relative to first digit returned
					 * by JS_dtoa
					 */
		...
		if (mode == DTOSTR_FIXED && (d >= 1e21 || d <= -1e21))
			mode = DTOSTR_STANDARD; /*
									 * Change mode here rather than below
									 * because the buffer may not be large
									 * enough to hold a large integer.
									 */

		decPt = JS_dtoa(d, dtoaModes[mode], mode >= DTOSTR_FIXED, precision,
				sign, buffer);
		nDigits = buffer.length();

		/*
		 * If Infinity, -Infinity, or NaN, return the string regardless of the
		 * mode.
		 */
		if (decPt != 9999) {
			boolean exponentialNotation = false;
			int minNDigits = 0; /*
								 * Minimum number of significand digits required
								 * by mode and precision
								 */
			int p;
			...
			/*
			 * If the number has fewer than minNDigits, pad it with zeros at the
			 * end
			 */
		...
		if (sign[0] && !(word0(d) == Sign_bit && word1(d) == 0)
				&& !((word0(d) & Exp_mask) == Exp_mask && ((word1(d) != 0)
						|| ((word0(d) & Frac_mask) != 0)))) {
			buffer.insert(0, '-');
		}
	}
