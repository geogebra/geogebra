...
public final class IRFactory extends Parser {
	...
	private Node transformArrayComp(ArrayComprehension node) {
		...
		try {
			...
			Node init = new Node(Token.EXPR_VOID, createAssignment(Token.ASSIGN,
		...
		try {
			...
			if (destructuring != null) {
				body.addChildToFront(
						new Node(Token.EXPR_VOID, destructuring, lineno));
			}
	...
	private Node transformReturn(ReturnStatement node) {
		boolean expClosure = Boolean.TRUE
				.equals(node.getProp(Node.EXPRESSION_CLOSURE_PROP));
		if (expClosure) {
			decompiler.addName(" ");
		} else {
		...
		for (VariableInitializer var : vars) {
			...
			if (var.isDestructuring()) {
				if (right == null) { // TODO: should this ever happen?
					...
					Node d = createDestructuringAssignment(node.getType(), left,
							right);
	...
	private Node initFunction(FunctionNode fnNode, int functionIndex,
		...
		if (functionType == FunctionNode.FUNCTION_EXPRESSION) {
			...
			if (name != null && name.length() != 0
				...
				fnNode.putSymbol(
						new Symbol(Token.FUNCTION, name.getIdentifier()));
	...
	void decompile(AstNode node) {
		...
		default:
			Kit.codeBug(
					"unexpected token: " + Token.typeToName(node.getType()));
		}
	}
	...
	void decompileObjectLiteral(ObjectLiteral node) {
		...
		for (int i = 0; i < size; i++) {
			...
			boolean destructuringShorthand = Boolean.TRUE
					.equals(prop.getProp(Node.DESTRUCTURING_SHORTHAND));
