...
import org.geogebra.desktop.gui.TitleLabel;
...
public class SpringUtilities {
	...
	public static void printSizes(Component c) {
		App.debug("minimumSize = " + c.getMinimumSize());
		App.debug("preferredSize = " + c.getPreferredSize());
		App.debug("maximumSize = " + c.getMaximumSize());
	}
	...
	public static void makeGrid(Container parent, int rows, int cols,
		...
		try {
			...
			App.debug("The first argument to makeGrid must use SpringLayout.");
			return;
		}
	...
	public static void makeCompactGrid(Container parent, int rows, int cols,
		...
		try {
			...
			App.debug("The first argument to makeCompactGrid must use SpringLayout.");
			return;
		}
	...
	/**
	 * Create a compact grid like makeCompactGrid() but let certain components
	 * span across the whole line (e.g. TitleLabel, JSeperator).
	 * 
	 ...
	 * @param yPad
	 */
	public static void makeCompactGridColspan(Container parent, int rows,
		...
		for (int r = 0; r < rows; ++r) {
			for (int c = 0; c < cols; ++c) {
				...
				if (comp instanceof JSeparator || comp instanceof TitleLabel) {
					...
					constraints.setWidth(Spring.sum(
							layout.getConstraints(parent).getWidth(),
