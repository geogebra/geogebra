...
public class BigComplex {
	...
	public BigComplex subtract(BigComplex b,MathContext mc){
		return new BigComplex(real.subtract(b.real,mc),imag.subtract(b.imag,mc));
	}
...
//		return new BigComplex(real.multiply(b.real,mc).subtract(imag.multiply(b.imag,mc),mc),
//				real.multiply(b.imag,mc).add(imag.multiply(b.real,mc),mc));
	...
	public BigComplex divide(BigComplex b,MathContext mc){
		...
		if (absReal.compareTo(absImag)>=0){
			...
			return new BigComplex(real.add(imag.multiply(dOverC,mc),mc).divide(div,mc),
					imag.subtract(real.multiply(dOverC,mc),mc).divide(div,mc));
		}else{
			BigDecimal cOverD=b.real.divide(b.imag,mc);
			BigDecimal div=b.real.multiply(cOverD,mc).add(b.imag,mc);
			return new BigComplex(real.multiply(cOverD,mc).add(imag,mc).divide(div,mc),
					imag.multiply(cOverD,mc).subtract(real,mc).divide(div,mc));
		}
//		BigDecimal div=b.real.multiply(b.real).add(b.imag.multiply(b.imag));
//		return new BigComplex(real.multiply(b.real).add(imag.multiply(b.imag).divide(div)),
//				real.multiply(b.imag).subtract(imag.multiply(b.real)).divide(div));
	...
	public BigDecimal abs(MathContext mc){
		...
		if (absReal.compareTo(BigDecimal.ONE)<0&&absImag.compareTo(BigDecimal.ONE)<0){
			return sqrt(absReal.multiply(absReal,mc).add(absImag.multiply(absImag,mc),mc),mc);
		}
		if (absReal.compareTo(absImag)>=0){
			BigDecimal q=imag.divide(real,mc);
			return absReal.multiply(sqrt(BigDecimal.ONE.add(q.multiply(q,mc),mc),mc),mc);
		}else{
			BigDecimal q=real.divide(imag,mc);
			return absImag.multiply(sqrt(BigDecimal.ONE.add(q.multiply(q,mc),mc),mc),mc);
		}
	}
	...
	public BigComplex sqrt(MathContext mc){
		...
		if (absReal.compareTo(absImag)>=0){
			...
			w=sqrt(absReal,mc).multiply(sqrt(BigDecimal.ONE.add(p,mc).divide(BigDecimal.valueOf(2),mc),mc),mc);
		}else{
			BigDecimal q=real.divide(imag,mc);
			BigDecimal p=sqrt(BigDecimal.ONE.add(q.multiply(q,mc),mc),mc);
			w=sqrt(absImag,mc).multiply(sqrt(q.abs(mc).add(p,mc).divide(BigDecimal.valueOf(2),mc),mc),mc);
		}
		if (w.compareTo(BigDecimal.ZERO)==0){
			return BigComplex.ZERO;
		}
		if (real.compareTo(BigDecimal.ZERO)>=0){
			return new BigComplex(w,imag.divide(BigDecimal.valueOf(2).multiply(w,mc),mc));			
		}else if (imag.compareTo(BigDecimal.ZERO)>=0){
			return new BigComplex(absImag.divide(BigDecimal.valueOf(2).multiply(w,mc),mc),w);
		}else{
			return new BigComplex(absImag.divide(BigDecimal.valueOf(2).multiply(w,mc),mc),w.negate());
		}
	}
	...
	public static BigDecimal sqrt(BigDecimal d,MathContext mc){
		...
		BigDecimal x=new BigDecimal(Math.sqrt(nDouble),mc); //initial approximation
		...
		while(i<10){
			BigDecimal s=BigDecimal.valueOf(3).subtract(x.multiply(x,mc).multiply(d,mc),mc).divide(BigDecimal.valueOf(2),mc);
			BigDecimal newDistOne=s.subtract(BigDecimal.ONE,mc).abs(mc);
			if (s.compareTo(BigDecimal.ONE)==0||(distOne!=null&&distOne.compareTo(newDistOne)<=0)){
				break;
			}else{
				x=x.multiply(s,mc);
			}
		...
		if (i==10){
//			Application.debug()
			Application.debug(String.format("s^2=%s,1/s=%s,s=%s",d,x,d.multiply(x,mc)));
		}
		return d.multiply(x,mc);
	}
	
	public String toString(){
		return String.format("%s + i %s", real.toPlainString(), imag.toPlainString());
	}
	...
	public static void main(String[] args){
		...
		Application.debug(sqrt(a,mc));
		a=new BigDecimal(2);
		Application.debug(sqrt(a,mc));
		a=new BigDecimal(Double.MAX_VALUE);
		a=a.multiply(a);
		Application.debug(sqrt(a,mc));
		BigComplex c=new BigComplex(BigDecimal.valueOf(3),BigDecimal.valueOf(4));
		Application.debug(c.abs(mc));
		Application.debug(c.sqrt(mc));
		...
		Application.debug(Arrays.deepToString(b.getRootsLaguerre(mc)));
		...
		b=new BigPolynomial(new double[]{-148.413, -469.075, -1062.1, -1287.92, -1145.84, -268.747, 223.29, 
				520.898, 111.839, 4.04776, -140.187, 14.33, -5.2737, 20.9335, 
				-8.59141, 1.},mc);
