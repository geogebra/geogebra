	...
	public GeoElement[] process(Command c) throws MyError {
		...
		case 1:
			if (allNumbers) {
				...
				GeoElement[] ret = { kernel.SumPoints(c.getLabel(), list) };
				return ret;
			} else if (allFunctions) {
				GeoElement[] ret = { kernel.SumFunctions(c.getLabel(), list) };
				return ret;
			} else if (allText) {
				GeoElement[] ret = { kernel.SumText(c.getLabel(), list) };
				return ret;
			} else {
		...
		case 2:
			if (arg[1].isGeoNumeric()) {
				...
				if (allNumbers) {
					...
					GeoElement[] ret = { kernel.SumFunctions(c.getLabel(),
		...
		default:
			...
			if (arg[0].isNumberValue()) {
				...
				GeoList wrapList = wrapInList(kernel, arg, arg.length,
						GeoClass.POINT);
				if (wrapList != null) {
					GeoElement[] ret = { kernel.SumPoints(c.getLabel(),
							wrapList) };
					return ret;
				}
