...
public class EuclidianStyleBar extends JToolBar implements ActionListener, geogebra.common.euclidian.EuclidianStyleBar {
	...
	protected void createButtons() {
		...
		btnHideShowLabel = new MyToggleButton(
				app.getImageIcon("mode_showhidelabel_16.gif"), iconHeight) {
			...
			public void update(Object[] geos) {
				...
				if (mode == EuclidianConstants.MODE_PEN) {
					...
					if (geosOK) {
						...
						int index = this.getColorIndex(geogebra.awt.Color
		...
		btnBgColor = new ColorPopupMenuButton(app, bgColorIconSize,
			...
			public void update(Object[] geos) {
				...
				if (geosOK) {
					...
					// find the geoColor in the table and select it
					int index = getColorIndex(geogebra.awt.Color
							.getAwtColor(geoColor));
					setSelectedIndex(index);
					setDefaultColor(alpha, geoColor);
					...
					if (index == -1) {
						this.setIcon(GeoGebraIcon.createColorSwatchIcon(alpha,
								bgColorIconSize,
								geogebra.awt.Color.getAwtColor(geoColor), null));
					}
				}
			}
		};
			...
			public void update(Object[] geos) {
				...
				if (geosOK) {
					GeoElement geo = ((GeoElement) geos[0])
							.getGeoElementForPropertiesDialog();
					geoColor = geogebra.awt.Color.getAwtColor(geo
							.getObjectColor());
					updateColorTable();
	...
	private void applyColor(ArrayList<GeoElement> geos) {
		...
		Color color = geogebra.awt.Color.getAwtColor(btnColor
				.getSelectedColor());
		...
		for (int i = 0; i < geos.size(); i++) {
			...
			if (!(geo.getGeoElementForPropertiesDialog() instanceof GeoImage || geo
				...
				if ((geogebra.awt.Color.getAwtColor(geo.getObjectColor()) != color || geo
						.getAlphaValue() != alpha)) {
	...
	private void applyBgColor(ArrayList<GeoElement> geos) {
		...
		Color color = geogebra.awt.Color.getAwtColor(btnBgColor
				.getSelectedColor());
