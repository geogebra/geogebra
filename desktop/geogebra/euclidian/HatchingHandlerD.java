...
public class HatchingHandlerD extends geogebra.common.euclidian.HatchingHandler{
	...
	private static BufferedImage getSeamlessTexture(int n, int pixelsPerPoint) {
		...
		for (double i = 0; i < n; i++) {
			for (double j = 0; j < n; j++) {
				...
				if (i == 0) {
					...
					if (j == 0)
						delaunayPlace(dt, points, new Pnt(x + n, y + n));
					else if (j == n - 1)
						delaunayPlace(dt, points, new Pnt(x + n, y - n));
				} else if (i == n - 1) {
					delaunayPlace(dt, points, new Pnt(x - n, y));
		...
		geogebra.common.awt.GBasicStroke bs = geogebra.common.factories.AwtFactory.prototype.newMyBasicStroke(0.0f);
		g2d.setStroke(geogebra.awt.GBasicStrokeD.getAwtStroke(bs));
		...
		for (Triangle triangle : dt)
			for (Pnt site : triangle) {
				...
				for (Triangle tri : list) {
					...
					// if (all.contains(p.coord(0) * pixelsPerPoint, p.coord(1)
					// * pixelsPerPoint)) {
					// pointInside = p;
					// }
					if (gp.getCurrentPoint() == null) {
						gp.moveTo((float) p.coord(0) * pixelsPerPoint,
								((float) p.coord(1) * pixelsPerPoint));
					} else {
						gp.lineTo((float) p.coord(0) * pixelsPerPoint,
								((float) p.coord(1) * pixelsPerPoint));
					}
					sb.append('(');
					sb.append(p.coord(0));
				...
				if (!intersection.isEmpty()) {
					...
					// Application.debug("left: testing point at height "+rec.y
					// + rec.height / 2);
					int col = image.getRGB(width - 1, rec.y + rec.height / 2); // test
																				// point
																				// on
																				// right
																				// edge
				...
				if (color == null) {
					...
					if (!intersection.isEmpty()) {
						...
						// Application.debug("right: testing point at height "+rec.y
						// + rec.height / 2);
						int col = image.getRGB(0, rec.y + rec.height / 2); // test
																			// point
																			// on
																			// left
																			// edge
				...
				if (color == null) {
					...
					if (!intersection.isEmpty()) {
						...
						// Application.debug("top: testing point at width "+rec.x
						// + rec.width / 2);
						int col = image.getRGB(rec.x + rec.width / 2, 0); // test
																			// point
																			// on
																			// bottom
																			// edge
				...
				if (color == null) {
					...
					if (!intersection.isEmpty()) {
						...
						// Application.debug("bottom: testing point at width "+rec.x
						// + rec.width / 2);
						int col = image.getRGB(rec.x + rec.width / 2,
								height - 1); // test point on top edge
						// Application.debug(col);
						if (col != 0)
							color = new Color(col);
					}
				...
				if (colCorner == 0)
					colCorner = image.getRGB(0, height - 1);
				if (colCorner == 0)
					colCorner = image.getRGB(width - 1, height - 1);
				if (colCorner == 0)
					colCorner = image.getRGB(width - 1, 0);

				// corners
				if (color == null && colCorner != 0) {
					if (gp.contains(topRight) || gp.contains(topLeft)
				...
				/*
				 * 
				 * if (gp.intersects(left)) { int col = image.getRGB(width - 1,
				 * (int)pointInside.coord(1) * pixelsPerPoint); // test point on
				 * right edge Application.debug(col); if (col != 0) color = new
				 * Color(col); } if (color == null && gp.intersects(right)) {
				 * int col = image.getRGB(0, (int)pointInside.coord(1) *
				 * pixelsPerPoint); // test point on left edge
				 * Application.debug(col); if (col != 0) color = new Color(col);
				 * } if (color == null && gp.intersects(top)) { int col =
				 * image.getRGB((int)pointInside.coord(0) * pixelsPerPoint, 0);
				 * // test point on bottom edge Application.debug(col); if (col
				 * != 0) color = new Color(col); } if (color == null &&
				 * gp.intersects(bottom)) { int col =
				 * image.getRGB((int)pointInside.coord(0) * pixelsPerPoint,
				 * height - 1); // test point on top edge
				 * Application.debug(col); if (col != 0) color = new Color(col);
				 * }
				 */
				...
				if (color != null)
					...
					g2d.setColor(new Color((float) Math.random(), (float) Math
							.random(), (float) Math.random(), 1.0f));
				...
				while (it.hasNext()) {
					Pnt ppp = it.next();
					if (gp.contains(new Point2D.Double(ppp.coord(0)
							* pixelsPerPoint, ppp.coord(1) * pixelsPerPoint)))
						cent = ppp;
				}
				...
				if (cent != null) {
					...
					/*
					 * Point2D center = new Point2D.Double(cent.coord(0)*
					 * pixelsPerPoint, cent.coord(1)* pixelsPerPoint); //float
					 * radius = 25; //Point2D focus = new
					 * Point2D.Double(Math.random() * width, Math.random() *
					 * height); float[] dist = {0.0f, 1.0f}; Color[] colors =
					 * {Color.BLACK, Color.WHITE}; RadialGradientPaint pp = new
					 * RadialGradientPaint(center, (float)(radius *
					 * pixelsPerPoint * 1.01), dist, colors,
					 ...
					 * g2d.setClip(gp);
					 * 
					 * for (int r = (int)(radius* pixelsPerPoint) ; r >= 0 ;
					 * r--) { g2d.setColor(new Color((int)(r * 255 /
					 * radius),(int)(r * 255 / radius),(int)(r * 255 /
					 * radius))); //g2d.setColor(new
					 * Color((float)Math.random(),(
					 * float)Math.random(),(float)Math.random(), 1.0f));
					 * g2d.fillOval((int)(cent.coord(0)* pixelsPerPoint - r/2),
					 * (int)(cent.coord(1)* pixelsPerPoint - r/2), r, r); }
					 */
				...
				sb.append("]\n"); // remove ','
				g2d.setStroke(EuclidianStaticD.getDefaultStrokeAwt());
				// System.err.println(sb.toString());
				// draw(vertices, withFill? getColor(site) : null);
				// if (withSites) draw(site);
			}
		/*
		 * for (Simplex<Pnt> triangle: dt) for (Simplex<Pnt> other:
		 * dt.neighbors(triangle)) { Pnt p =
		 * Pnt.circumcenter(triangle.toArray(new Pnt[0])); Pnt q =
		 * Pnt.circumcenter(other.toArray(new Pnt[0])); //draw(p,q);
		 * System.err.println
		 * ("Segment[("+p.coord(0)+","+p.coord(1)+"),("+q.coord
		 * (0)+","+q.coord(1)+")]"); }
		 */
