...
public class GuiManager {
   ...
   * applet. - Active the glass pane if the application is changing from applet
   * to frame mode.
   */
  ...
  public EuclidianViewInterface getActiveEuclidianView() {
    ...
    EuclidianDockPanelAbstract focusedEuclidianPanel = layout.getDockManager()
   ...
   * constProtocol = new ConstructionProtocolView(app); } return constProtocol;
   * }
   */
  ...
  /**
   * Toggles the popup menu for geo at the position p in the coordinate space of
    ...
    try {
      ...
      // TODO remove deprecated method
        ...
        reader = new InputStreamReader(
            (InputStream) transfer.getTransferData(DataFlavor.plainTextFlavor),
            "UNICODE");
  ...
  /**
   * /** Tries to gets an image from a transferable object or the clipboard (if
   * transfer is null). If an image is found, then it is loaded and stored in
   * this application's imageManager.
   * 
   * @param transfer
  ...
  /**
   * Loads and stores an image file is in this application's imageManager. If a
   * null image file is passed, then a file dialog is opened to choose a file.
   * 
   * @return fileName of image stored in imageManager
   */
  ...
  public boolean saveCurrentFile() {
    ...
    int returnVal = JOptionPane.showOptionDialog(comp,
        app.getMenu("DoYouWantToSaveYourChanges"), app.getMenu("CloseFile"),
      ...
      if (returnVal == JFileChooser.APPROVE_OPTION) {
        ...
        if (promptOverwrite && file.exists()) {
          ...
          int n = JOptionPane.showOptionDialog(app.getMainComponent(),
              ...
              app.getPlain("Question"), JOptionPane.DEFAULT_OPTION,
           ...
           * JOptionPane.QUESTION_MESSAGE); done = (n ==
    ...
    if (files != null) {
      ...
      for (int i = 0; i < files.length; i++) {
        ...
        if (!file.exists()) {
          ...
          if (extension.equals(Application.FILE_EXT_GEOGEBRA) && !file.exists()) {
            file = addExtension(removeExtension(file), Application.FILE_EXT_HTM);
          }
   ...
   * method for component transfer handlers that need to pass potential ggb file
   * drops on to the top level drop handler.
   * 
   * @param t
   * @return
   */
  ...
  public void openToolHelp(String page) {
    ...
    int n = JOptionPane.showOptionDialog(app.getMainComponent(),
        ...
        JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE, null, // do not
                                                                       // use a
                                                                       // custom
                                                                       // Icon
  ...
  private URL getHelpURL(Help type, String pageName) {
    ...
    try {
      ...
      File file = new File(Application.WINDOWS ? offlineStr.replaceAll(
          "[/\\\\]+", "\\" + "\\") : offlineStr); // replace slashes with
                                                  // backslashes
   ...
   * 
   * if (handwritingRecognition == null) { handwritingRecognition = new
   * HandwritingRecognitionTool(app); } handwritingRecognition.setVisible(show);
  ...
  public static void setLabelsRecursive(Container c) {
    ...
    for (Component com : components) {
      ...
      /*
       * for debugging, to show classes that might benefit from implementing
       * SetLabels if (com instanceof JPanel && !(com instanceof SetLabels)
