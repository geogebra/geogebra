...
{	
	...
	protected void initMenu() {
		...
		if(!isEmptySelection()){
			...
			item.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					GeoElement newGeo = cp.createPointGeoList(selectedCellRanges, false, true,true,true,true);
		...
		if(selectionType != MyTable.ROW_SELECT){
			...
			cbItem.setSelected(app.getTraceManager().isTraceColumn(table.minSelectionColumn));
		...
		item.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				app.getDialogManager().showOptionsDialog(OptionsDialog.TAB_SPREADSHEET);
			}
		});	 
		addItem(item);	
