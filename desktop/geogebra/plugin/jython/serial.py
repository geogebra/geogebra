...
# modified for GeoGebra/Jython
# replaced all javax.comm with gnu.io and tidied to just 2 imports:
...
class Serial(serialutil.FileLike):
    def __init__(self,
        ...
        if bytesize == FIVEBITS:
            self.databits = SerialPort.DATABITS_5
        elif bytesize == SIXBITS:
            self.databits = SerialPort.DATABITS_6
        elif bytesize == SEVENBITS:
            self.databits = SerialPort.DATABITS_7
        elif bytesize == EIGHTBITS:
            self.databits = SerialPort.DATABITS_8
        else:
            raise ValueError, "unsupported bytesize"
        
        if stopbits == STOPBITS_ONE:
            self.jstopbits = SerialPort.STOPBITS_1
        elif stopbits == STOPBITS_ONE_HALVE:
            self.jstopbits = SerialPort.STOPBITS_1_5
        elif stopbits == STOPBITS_TWO:
            self.jstopbits = SerialPort.STOPBITS_2
        else:
            raise ValueError, "unsupported number of stopbits"

        if parity == PARITY_NONE:
            self.jparity = SerialPort.PARITY_NONE
        elif parity == PARITY_EVEN:
            self.jparity = SerialPort.PARITY_EVEN
        elif parity == PARITY_ODD:
            self.jparity = SerialPort.PARITY_ODD
        elif parity == PARITY_MARK:
            self.jparity = SerialPort.PARITY_MARK
        elif parity == PARITY_SPACE:
            self.jparity = SerialPort.PARITY_SPACE
        else:
            raise ValueError, "unsupported parity type"

        jflowin = jflowout = 0
        if rtscts:
            jflowin = jflowin | SerialPort.FLOWCONTROL_RTSCTS_IN 
            jflowout = jflowout | SerialPort.FLOWCONTROL_RTSCTS_OUT
        if xonxoff:
            jflowin = jflowin | SerialPort.FLOWCONTROL_XONXOFF_IN
            jflowout = jflowout | SerialPort.FLOWCONTROL_XONXOFF_OUT
