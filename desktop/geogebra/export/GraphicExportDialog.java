	...
	final private boolean exportEPS(boolean exportToClipboard) {
		...
		file =
			app.getGuiManager().showSaveDialog(
				...
				app.getPlain("eps") + " " + app.getMenu("Files"), true, false);
		if (file == null)
			return false;
		try {		
	...
	final private boolean exportEMF(boolean exportToClipboard, boolean useEMFplus) {
		...
		file =
			app.getGuiManager().showSaveDialog(
				...
				app.getPlain("emf") + " " + app.getMenu("Files"), true, false);
		//  Michael Borcherds 2008-03-02 END
		...
		try {					   
			...
			g.startExport();	
			((EuclidianView)app.getActiveEuclidianView()).exportPaint(g, exportScale);
	...
	final private boolean exportPDF(boolean exportToClipboard) {
		...
		file =
			app.getGuiManager().showSaveDialog(
				...
				app.getPlain("pdf") + " " + app.getMenu("Files"), true, false);
	...
	final private boolean exportSVG(boolean exportToClipboard) {
		...
		file =
			app.getGuiManager().showSaveDialog(
				...
				app.getPlain("svg") + " " + app.getMenu("Files"), true, false);
