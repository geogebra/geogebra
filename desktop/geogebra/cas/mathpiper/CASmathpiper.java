...
public class CASmathpiper extends CASgeneric {
	...
	private synchronized Interpreter getMathPiper() {
		if (ggbMathPiper == null) {
			ggbMathPiper = org.mathpiper.interpreters.Interpreters
					.newSynchronousInterpreter();
	...
	/**
	 * Evaluates a valid expression in GeoGebraCAS syntax and returns the
	 * resulting String in GeoGebra notation.
	 * 
	 * @param casInput
	...
	/**
	 * Tries to parse a given MathPiper string and returns a String in GeoGebra
	 * syntax.
	 * 
	 * @param MathPiperString
	 *            String in MP syntax.
	...
	final synchronized public String evaluateMathPiper(String exp) {
		try {
			...
			// timeout needed for eg
			// Limit((Sin(1/x)*x^2-x*Cos(1/x))/x^2,-Infinity)
			...
			evaluationResponse = mathpiper.evaluate(exp);
			timer.cancel();
			...
			// convert MathPiper's scientific notation from e.g. 3.24e-4 to
			// 3.2E-4
	...
	private synchronized boolean initMyMathPiperFunctions() {
		...
		ggbMathPiper
				.evaluate("RandomNormal(mu,sigma) := N(Cos(2*Pi*Random())*Sqrt(-2*LogN(Random()))*Sqrt(sigma)+mu);");
		...
		ggbMathPiper
				.evaluate("Simplify(x_NotEqu == y_NotEqu)  <-- Simplify(x) == Simplify(y);");
		ggbMathPiper
				.evaluate("Factor(x_NotEqu == y_NotEqu)  <-- Factor(x) == Factor(y);");
		ggbMathPiper
				.evaluate("Expand(x_NotEqu == y_NotEqu)  <-- Expand(x) == Expand(y);");
		ggbMathPiper
				.evaluate("ExpandBrackets(x_NotEqu == y_NotEqu)  <-- ExpandBrackets(x) == ExpandBrackets(y);");
		ggbMathPiper
				.evaluate("Sqrt(x_NotEqu == y_NotEqu)  <-- Sqrt(x) == Sqrt(y);");
		ggbMathPiper
				.evaluate("Exp(x_NotEqu == y_NotEqu)  <-- Exp(x) == Exp(y);");
		ggbMathPiper.evaluate("Ln(x_NotEqu == y_NotEqu)  <-- Ln(x) == Ln(y);");

		// arithmetic for equations and scalars
		ggbMathPiper
				.evaluate("(x_NotEqu == y_NotEqu) + z_NotEqu <-- x + z == y + z;");
		ggbMathPiper
				.evaluate("z_NotEqu + (x_NotEqu == y_NotEqu) <-- z + x == z + y;");
		ggbMathPiper
				.evaluate("(x_NotEqu == y_NotEqu) - z_NotEqu <-- x - z == y - z;");
		ggbMathPiper
				.evaluate("z_NotEqu - (x_NotEqu == y_NotEqu) <-- z - x == z - y;");
		...
		ggbMathPiper
				.evaluate("(a_NotEqu == b_NotEqu) + (c_NotEqu == d_NotEqu) <-- a + c == b + d;");
		ggbMathPiper
				.evaluate("(a_NotEqu == b_NotEqu) - (c_NotEqu == d_NotEqu) <-- a - c == b - d;");
		ggbMathPiper
				.evaluate("(a_NotEqu == b_NotEqu) * (c_NotEqu == d_NotEqu) <-- a * c == b * d;");
		...
		// access functions for elements of a vector (for the "if" part, see
		// #556)
