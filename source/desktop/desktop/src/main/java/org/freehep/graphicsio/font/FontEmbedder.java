 ...
 * {@code writeGlyph} method which is called for all defined (up to 256)
 * characters and the notdefined character. These method calls are bracketed by
 * {@code openGlyphs()} and {@code closeGlyph()}. All invocations of methods
 * that are abstract in this class succeed the method calls of the superclass
 * {@code FontIncluder} (especially {@code closeIncludeFont()}!) All of these
 * calls are again succeeded by {@code closeEmbedFont}. <br>
 ...
 * <li>{@code writeWidths}
 * <li>{@code openGlyphs}
 * <li>loop over all glyphs: {@code openGlyphs}
 * <li>{@code closeGlyphs}
...
public abstract class FontEmbedder extends FontIncluder {
	...
	/**
	 * Writes a single glyph to the file. A null value for {@code glyphMetrics}
	 * indicates the undefined character. In this case the value of
	 * {@code unicodeName} equals the value of {@code NOTDEF} (=
	 * {@code .notdef}).
	 *
	 * @param unicodeName
