...
public class PrintColor extends Color {
	...
	private static void testColorValueRange(float asGray) {
		...
		if (rangeError == true) {
			throw new IllegalArgumentException(
					"PrintColor parameter outside of expected range:" + badComponentString);
		}
	}
	...
	public PrintColor(
			float red, float green, float blue, float alpha, float asGray, boolean asBlack) {
	...
	public PrintColor getColor(int mode) {
		...
		switch (mode) {
			case COLOR:
				return this;
			case GRAYSCALE:
				return new PrintColor(
						getAsGray(), getAsGray(), getAsGray(), getAlpha() / 255.0f, getAsGray(), getAsBlack());
	...
	public static Color getDefaultColor(int index) {
		if ((index < 0) || (index >= defaultColors.length)) {
			throw new IllegalArgumentException(
					"PrintColor.getDefaultColor index outside of expected range: " + index);
		}
	...
	public static PrintColor invert(Color color) {
		...
		return new PrintColor(
				new Color(printColor.getRGB() ^ 0x00808080),
