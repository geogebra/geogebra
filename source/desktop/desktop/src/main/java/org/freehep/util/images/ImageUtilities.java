...
public class ImageUtilities {
	...
	public static BufferedImage createBufferedImage(
			RenderedImage image, ImageObserver observer, Color bkg) {
	...
	public static byte[] getBytes(
			Image image, Color bkg, String code, int pad, ImageObserver observer) {
			...
			for (int x = 0; x < width; x++) {
				...
				for (int i = 0; i < code.length(); i++) {
					switch (code.charAt(i)) {
						...
						default:
							Log.debug(ImageUtilities.class + ": Invalid code in '" + code + "'");
							break;
					}
					index++;
				}
