...
public class GDIPlusObject extends EMFPlusTag {
	...
	protected static final int INVALID = 0x000;
	protected static final int BRUSH = 0x100;
	protected static final int PEN = 0x200;
	protected static final int PATH = 0x300;
	protected static final int REGION = 0x400;
	protected static final int IMAGE = 0x500;
	protected static final int FONT = 0x600;
	protected static final int STRING_FORMAT = 0x700;
	protected static final int IMAGE_ATTRIBUTES = 0x800;
	protected static final int CUSTOM_LINE_CAP = 0x900;
	...
	protected static final int FILL_MODE_ALTERNATE = 0x0000;
	protected static final int FILL_MODE_WINDING = 0x2000;
	...
	public GDIPlusObject(int index, Stroke stroke, Paint brush) {
		...
		if (!(stroke instanceof BasicStroke)) {
			throw new IllegalArgumentException(
					getClass() + ": can only handle Stroke of class BasicStroke");
		}
	...
	public String toString() {
		...
		switch (type) {
			...
			case PATH:
				...
				sb.append("\n  n: " + path.length);
				for (int i = 0; i < path.length; i++) {
					sb.append("\n  0x" + Integer.toHexString(path[i].getType()) + " (" + path[i].getX() + ", "
							+ path[i].getY() + ")");
				}
				break;
			default:
	...
	private static void writeBrush(EMFOutputStream emf, Paint brush) throws IOException {
		emf.writeUINT(0xDBC01001);
		if (brush instanceof Color) {
			...
			emf.writeUINT(
					paint.isCyclic() ? WRAP_MODE_TYLE_FLIP_XY : WRAP_MODE_TYLE_FLIP_Y); // NOTE: check
			...
			AffineTransform transform = new AffineTransform(scale, 0, 0, scale, dx / 2 + x1, dy / 2 + y1);
			...
			System.err.println(
					"No Brush for paint of class: " + brush.getClass() + " defaulting to black");
			emf.writeUINT(BRUSH_TYPE_SOLID_COLOR);
			emf.writeCOLOR(Color.BLACK);
		}
	}

	public static void writeTransform(EMFOutputStream emf, AffineTransform transform)
			throws IOException {
	...
	private static void writeImage(EMFOutputStream emf, RenderedImage image) throws IOException {
		emf.writeUINT(0xDBC01001);
