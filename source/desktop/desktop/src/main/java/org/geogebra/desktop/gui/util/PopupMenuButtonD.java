...
public class PopupMenuButtonD extends JButton implements ChangeListener {
	 ...
	 */
	public PopupMenuButtonD(
			AppD app, Object[] data, int rows, int columns, Dimension iconSize, SelectionTable mode) {
	...
	public PopupMenuButtonD(
		...
		myPopup.setBorder(BorderFactory.createCompoundBorder(
				BorderFactory.createLineBorder(Color.GRAY), BorderFactory.createEmptyBorder(3, 3, 3, 3)));
		...
		if (hasTable) {
			...
			myTable = new SelectionTableD(app, data, rows, columns, iconSize, mode);
			setSelectedIndex(0);
	...
	protected void onMousePressed(int x) {
		...
		final int clicDownArrowWidth =
				(int) Math.round(CLICK_DOWN_ARROW_WIDTH * (app.getScaledIconSize() / 16.0));
		// trigger popup
		// default: trigger only when the mouse is over the right side
		// of the button
		// if isStandardButton: pressing anywhere triggers the popup
		if (isStandardButton || x >= getWidth() - clicDownArrowWidth && x <= getWidth()) {
			if (hasTable) {
				myTable.updateFonts();
			}
			if (isDownwardPopup) {
	...
	protected void processMouseEvent(MouseEvent e) {
		if (e.getID() == MouseEvent.MOUSE_RELEASED) {
			...
			if (isStandardButton
					|| e.getX() >= getWidth() - app.getScaledIconSize() && e.getX() <= getWidth()) {
				return;
			}
		}
	...
	public void setIcon(Icon icon0) {
		...
		if (icon != null) {
			super.setIcon(GeoGebraIconD.joinIcons(icon, app.getScaledIcon(GuiResourcesD.TRIANGLE_DOWN)));
		} else {
