...
public class ContextMenuGraphicsWindow3DD extends ContextMenuGraphicsWindowD {
	...
	public ContextMenuGraphicsWindow3DD(AppD app) {
		...
		setTitle("<html>" + app.getLocalization().getMenu("GraphicsView3D") + "</html>");
	...
	protected void addAxesAndGridCheckBoxes() {
		...
		JCheckBoxMenuItem cbShowAxes =
				new JCheckBoxMenuItem(((GuiManager3D) app.getGuiManager()).getShowAxes3DAction());
		...
		JCheckBoxMenuItem cbShowGrid =
				new JCheckBoxMenuItem(((GuiManager3D) app.getGuiManager()).getShowGrid3DAction());
		...
		JCheckBoxMenuItem cbShowPlane =
				new JCheckBoxMenuItem(((GuiManager3D) app.getGuiManager()).getShowPlaneAction());
	...
	protected void showOptionsDialog() {
		...
		((GuiManagerD) app.getGuiManager())
				.setFocusedPanel(((App3D) app).getEuclidianView3D().getViewID(), true);
	}
}
