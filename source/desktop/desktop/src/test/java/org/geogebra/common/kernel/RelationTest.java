...
class RelationTest extends BaseUnitTest {
	...
	void moreButtonShouldShowNDGS() {
		...
		assertThat(rel.getRows()[0].getInfo(), containsString("collinear"));
		assertThat(
				rel.getExpandedRow(0).getInfo(),
				allOf(containsString("under the condition"), containsString("are not equal")));
	}

	@Test
	void moreButtonShouldShowNDGSPath() {
		Relation rel = new Relation(getApp(), f, C, null, null);
		assertThat(rel.getRows()[0].getInfo(), containsString("lies on"));
		assertThat(
				rel.getExpandedRow(0).getInfo(),
				allOf(containsString("under the condition"), containsString("are not equal")));
	}
	...
	void relationOrdering() {
		...
		assertThat(rows[0].getInfo(), containsString("perpendicular"));
		assertThat(rows[1].getInfo(), containsString("length"));

		getApp().setLocale(new Locale("es"));
		rows = new Relation(getApp(), a, b, null, null).getRows();
		assertThat(rows[0].getInfo(), containsString("perpendicular"));
		assertThat(rows[1].getInfo(), containsString("longitud"));
	}
}
