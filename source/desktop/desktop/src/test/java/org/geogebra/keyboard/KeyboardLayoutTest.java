...
class KeyboardLayoutTest {
	...
	void testSpecialTabWithTemplateButtons() {
		KeyboardFactory kbf =
				new DefaultKeyboardFactory(new DefaultKeyboardFactory.BaseTemplateKeyProvider());
		KeyboardModel kb = kbf.createSpecialSymbolsKeyboard().getModel();
		StringBuilder actions = new StringBuilder();
		StringBuilder resources = new StringBuilder();
		for (Row row : kb.getRows()) {
	...
	void letterTabTest() {
		...
		KeyboardRowDefinitionProvider latinProvider = new KeyboardRowDefinitionProvider(localization);
		for (Language lang : Language.values()) {
			localization.setLocale(localization.convertToLocale(lang));
			String[] rows = latinProvider.getLowerKeys();
			List<Integer> lengths = Arrays.stream(rows).map(String::length).collect(Collectors.toList());
