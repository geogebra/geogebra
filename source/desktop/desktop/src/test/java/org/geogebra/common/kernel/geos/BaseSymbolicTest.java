...
public class BaseSymbolicTest {
	...
	void setup() {
		...
		kernel
				.getGeoGebraCAS()
				.evaluateGeoGebraCAS("1+1", null, StringTemplate.defaultTemplate, app.getKernel());
		undoManager = kernel.getConstruction().getUndoManager();
	}

	protected void t(String input, String... expected) {
		AlgebraTestHelper.checkSyntaxSingle(input, expected, ap, StringTemplate.testTemplate);
	}

	protected void t(String input, Matcher<String> matcher) {
		AlgebraTestHelper.checkSyntaxSingle(
				input, Collections.singletonList(matcher), ap, StringTemplate.testTemplate);
	}

	protected void tn(String input, String... expected) {
		AlgebraTestHelper.checkSyntaxSingle(input, expected, ap, StringTemplate.testNumeric);
	}
	...
	protected void t(String input, EvalInfo info, String... expected) {
		GeoElementND result = ap.processAlgebraCommandNoExceptionHandling(
						input, false, TestErrorHandler.INSTANCE, info, null)[0];
		AlgebraTestHelper.assertOneOf(result, expected, StringTemplate.testTemplate);
	}

	protected <T extends GeoElement> T add(String text) {
		GeoElementND[] geos =
				ap.processAlgebraCommandNoExceptionHandling(text, false, ErrorHelper.silent(), false, null);
