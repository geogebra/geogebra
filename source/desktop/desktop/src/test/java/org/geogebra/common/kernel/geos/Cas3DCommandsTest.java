...
class Cas3DCommandsTest extends BaseSymbolicTest {
	...
	void testDistancePointAndLine3D() {
		...
		ArrayList<ExpressionNode> args = new ArrayList<>(Arrays.asList(point3D.wrap(), line3D.wrap()));

		assertEquals(
				"[[[ggbdisans:=0/0],[ggbdisarg01:=point(6,7,-3)],[ggbdisarg11:=point("
				...
				geoGebraCAS.getCASCommand(
						"Distance", args, false, StringTemplate.giacTemplate, SymbolicMode.NONE));
	}
	...
	void testDistanceWithVector() {
		...
		ArrayList<ExpressionNode> args = new ArrayList<>(Arrays.asList(vec1.wrap(), vec2.wrap()));
		String distCommand = geoGebraCAS.getCASCommand(
				"Distance", args, false, StringTemplate.giacTemplate, SymbolicMode.NONE);
