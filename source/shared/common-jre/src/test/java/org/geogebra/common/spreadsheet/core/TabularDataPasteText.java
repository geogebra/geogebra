...
public class TabularDataPasteText implements TabularDataPasteInterface<String> {
	...
	public void pasteInternal(
			TabularData<String> tabularData,
			TabularClipboard<String> clipboard,
		...
		for (int row = 0; row < clipboard.numberOfRows(); row++) {
			for (int column = 0; column < clipboard.numberOfColumns(); column++) {
				tabularData.setContent(
						destination.getFromRow() + row,
	...
	public void pasteExternal(
			TabularData<String> tabularData, String[][] clipboardContent, TabularRange destination) {
