...
class AlgoListElementTest extends BaseUnitTest {
	...
	private void shouldHaveType(GeoClass geoClass) {
		assertThat(output[0], hasProperty("type", GeoElement::getGeoClassType, geoClass));
	}
	...
	private AlgoListElementTest at(int... indexes) {
		...
		AlgoListElement algo = new AlgoListElement(getConstruction(), list, nums, false);
		output = algo.getOutput();
		return this;
	}
	...
	void testNonExistingIndexInPointMatrix() {
		withList("{{(1,1),(0,0)},{(0,0),(4,4)}}").at(2, 2).shouldHaveType(GeoClass.POINT);
	}
