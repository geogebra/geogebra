...
class FunctionSamplerTest extends BaseUnitTest {
	...
	void testExtend() {
		...
		FunctionSampler sampler = PlotterUtils.newSampler(
				function, range, 5, new EuclidianViewBoundsImp(getApp().getActiveEuclidianView()));
		assertEquals(6, sampler.tuples().count());

		sampler.extend(new Interval(1000, 1005));
		assertEquals(Arrays.asList(999, 1000, 1001, 1002, 1003, 1004), getX(sampler.tuples()));

		sampler.extend(new Interval(1002, 1007));
		assertEquals(
				Arrays.asList(999, 1000, 1001, 1002, 1003, 1004, 1005, 1006), getX(sampler.tuples()));

		sampler.extend(new Interval(1001, 1016));
		assertEquals(Arrays.asList(999, 1002, 1005, 1008, 1011, 1014), getX(sampler.tuples()));
	}

	private List<Integer> getX(IntervalTupleList tuples) {
		return tuples.stream()
				.map(t -> (int) connectedInterval(t.xSet()).getLow())
