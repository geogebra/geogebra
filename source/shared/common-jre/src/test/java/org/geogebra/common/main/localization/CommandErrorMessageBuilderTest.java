...
class CommandErrorMessageBuilderTest extends BaseUnitTest {
	...
	void setupTest() {
		...
		Mockito.when(localization.getCommandSyntax(Mockito.anyString()))
				.then(invocation -> invocation.getArgument(0) + ".Syntax");
	...
	void testBuildArgumentNumberError() {
		...
		assertThat(
				"It contains the illegal argument number", message, containsString(Integer.toString(1)));
		assertThat("It contains the command name", message, containsString("Command Cmd"));
	...
	void testBuildArgumentError() {
		...
		Mockito.when(value.toString(Mockito.any(StringTemplate.class)))
				.then(invocation -> expressionValue);

		builder.setShowingSyntax(true);
		String message = builder.buildArgumentError("Cmd", value);
		assertThat("It contains the command syntax", message, containsString("Cmd.Syntax"));
		assertThat("It contains the expression value", message, containsString(expressionValue));
		assertThat("It contains the command name", message, containsString("Command Cmd"));
