...
public class EditorTypingTest {
	...
	public void shouldNotSerializeMixedNumberIfDisabled() {
		SyntaxAdapterImpl syntaxAdapter = new SyntaxAdapterImpl(app.getKernel());
		syntaxAdapter.applyRestrictions(Set.of(ExamFeatureRestriction.DISABLE_MIXED_NUMBERS),
				ExamType.WTR);
		checker.setModifiers(KeyEvent.CTRL_MASK).typeKey(JavaKeyCodes.VK_M).setModifiers(0)
				.type("1")
				.right(1).type("2").right(1).type("3")
				.checkGGBMath("1 * 2 / 3", syntaxAdapter);
	}
