...
class InternalClipboardTest extends BaseAppTestSetup {
	...
	void clipboardItemsShouldBeSorted() {
		...
		String clipboard =
				InternalClipboard.getTextToSave(getApp(), Collections.singletonList(s), txt -> txt);
		String labels = clipboard.split("\n")[0];
		assertEquals("@A @B @s", labels.trim().replace(CopyPaste.labelPrefix, "@"));
		EventAccumulator acu = new EventAccumulator();
		getApp().getEventDispatcher().addEventListener(acu);
		InternalClipboard.pasteGeoGebraXMLInternal(
				getApp(), Arrays.asList(labels.split(" ")), clipboard.substring(clipboard.indexOf("\n")));
		assertEquals(
				"PASTE_ELMS_COMPLETE [A_{1} = (0, 0), B_{1} = (1, 1), s_{1} = 1.41]",
	...
	void pastedElementsShouldBeCentered(String input, String centered) {
		...
		String clipboard = InternalClipboard.getTextToSave(getApp(), List.of(s), txt -> txt);
		String labels = clipboard.split("\n")[0];
		InternalClipboard.pasteGeoGebraXMLInternal(
				getApp(), Arrays.asList(labels.split(" ")), clipboard.substring(clipboard.indexOf("\n")));
		assertArrayEquals(new String[] {"s", "s_{1}"}, getApp().getGgbApi().getAllObjectNames());
		assertEquals(
				centered, getKernel().lookupLabel("s_{1}").getDefinition(StringTemplate.testTemplate));
	}

	@Test
	void pastingShouldNotCreateDuplicatedOrdering() {
		setupNotesApp();
		GeoElement polygon = evaluateGeoElement("poly = Polygon((-4, 2), (-1, 2), (-1, 0), (4, 0))");
