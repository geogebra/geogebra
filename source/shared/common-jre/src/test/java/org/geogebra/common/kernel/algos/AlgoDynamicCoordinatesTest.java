...
class AlgoDynamicCoordinatesTest extends BaseUnitTest {
	...
	void testDynamicCoordsCompute() {
		...
		AlgoIntersectLineConic intersectionAlgo =
				new AlgoIntersectLineConic(cons, j.getRay(), e.getCircle());
		GeoPoint K = (GeoPoint) intersectionAlgo.getOutput(0);
		K.setLabel("K");
		GeoElementND[] dynCoords = cons.getKernel()
				.getAlgebraProcessor()
				.processAlgebraCommand("DynamicCoordinates(J, x(K), y(K))", false);
		I.setCoords(10, -6, 1);
