...
class DefaultPropertiesFactoryTest extends BaseAppTestSetup {
	...
	void testPropertiesGraphingWeb() {
		...
		List<PropertiesArray> props = new DefaultPropertiesFactory()
				.createProperties(graphingApp, graphingApp.getLocalization(), null);
		...
		assertEquals(
				List.of(
						...
						"Angle Unit",
						"Font Size",
						"ActionablePropertyCollection"),
				getNames(props.get(0)));
		assertEquals(List.of("Display", "Auxiliary Objects"), getNames(props.get(1)));
		assertEquals(
				List.of("Grid", "Axes", "Dimensions", "xAxis", "yAxis", "Advanced"),
