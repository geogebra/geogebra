...
final class SpreadsheetTestHelpers {
	...
	static void simulateCellMouseClick(
			SpreadsheetController controller, int row, int column, int nrClicks) {
	...
	static Point getCellCenter(SpreadsheetController controller, int row, int column) {
		...
		if (row == -1 && column >= 0) {
			...
			Rectangle cellBounds = layout.getBounds(0, column);
			return new Point(
					layout.getRowHeaderWidth() + cellBounds.getMidX(), layout.getColumnHeaderHeight() / 2);
		}
		if (column == -1 && row >= 0) {
			// entire row, return center of row header
			Rectangle cellBounds = layout.getBounds(row, 0);
			return new Point(
					layout.getRowHeaderWidth() / 2, layout.getColumnHeaderHeight() + cellBounds.getMidY());
		}
		Rectangle cellBounds = layout
				.getBounds(row, column)
