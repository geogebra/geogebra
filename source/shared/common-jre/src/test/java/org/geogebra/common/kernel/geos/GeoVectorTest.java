...
class GeoVectorTest extends BaseUnitTest {
	...
	@Test
	void testLatexDefinitionForIndependent() {
		GeoVector vector = addAvInput("v = (1, 2)");
		assertThat(
	...
	@Test
	void testLatexDefinitionForDependent() {
		addAvInput("a = 1");
		GeoVector vector = addAvInput("v = (a, 2)");
		assertThat(
				vector.getDefinition(StringTemplate.latexTemplate),
	...
	@Test
	void testDefaultHeadShouldNotAppearInXml() {
		GeoVector vector = addAvInput("v = (1, 2)");
		assertThat(vector.getXML(), not(containsString("\t<headStyle")));
	}

	@Test
	void testGetXmlWithArrowHead() {
		GeoVector vector = addAvInput("v = (1, 2)");
	...
	@Test
	@Issue("APPS-1470")
	void elementShouldNotBeConvertedToVectorIfLocalVariableExists() {
