...
class EuclidianViewBoundsRWSCMockTest {
	private final EuclidianViewBoundsRWSCMock bounds =
			new EuclidianViewBoundsRWSCMock(-5, 5, -5, 5, 800, 600);

	@ParameterizedTest
	@CsvSource({"-5, 0", "-2.5, 200", "0, 400", "2.5, 600", "5, 800"})
	void testToScreenCoordsX(double xRW, double x) {
		assertEquals(x, bounds.toScreenCoordXd(xRW));
	}

	@ParameterizedTest
	@CsvSource({"0, -5", "200, -2.5", "400, 0", "600, 2.5", "800, 5"})
	void testRealWorldCoordsX(double x, double xRW) {
		assertEquals(xRW, bounds.toRealWorldCoordX(x));
	}

	@ParameterizedTest
	@CsvSource({"-5, 0", "-2.5, 150", "0, 300", "2.5, 450", "5, 600"})
	void testToScreenCoordsY(double yRW, double y) {
		assertEquals(y, bounds.toScreenCoordYd(yRW));
	}

	@ParameterizedTest
	@CsvSource({"0, -5", "150, -2.5", "300, 0", "450, 2.5", "600, 5"})
