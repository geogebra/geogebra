...
class IntegralEditorTests {
	...
	void testIntegralSerialization(
			Tag tag,
			String integrand,
			String variable,
			String lowerLimit,
			String upperLimit,
			boolean autoDefaultVariable,
			boolean limitsVisible,
			String expected)
	...
	void testInitialInsertedIntegralStructureFromSyntax(
			String syntax, Tag tag, String variable, boolean autoDefaultVariable, boolean limitsVisible) {
		...
		assertEquals(
				variable,
				new GeoGebraSerializer(null)
						.serialize(integral.getChild(IntegralHelper.VARIABLE), new StringBuilder())
						.toString());
	...
	void testInitialInsertedCasIntegralStructureFromSyntax(
			String syntax, Tag tag, String variable, boolean autoDefaultVariable, boolean limitsVisible) {
	...
	void testIntegralWithEvaluateFromSuggestionStaysGenericCommand() {
		KeyboardInputAdapter.onKeyboardInput(
				checker.getMathField().getInternal(),
	...
	void testExcludedIntegralSyntaxDoesNotStartWithEmptyArgument() {
		checker
				.convertFormulaForAV("a=Integral(x,1,2,true)")
	...
	void testCasIntegralFromSuggestionCreatesInlineIntegral() {
		KeyboardInputAdapter.onKeyboardInput(
				casChecker.getMathField().getInternal(),
	...
	void testIntegralWithEmptyVariableFromSuggestionEvaluatesWithDefaultVariable() {
		KeyboardInputAdapter.onKeyboardInput(
				checker.getMathField().getInternal(), "Integral( <Function>, <Variable> )");
		checker.type("x");
		assertEquals(
				"Integral(x)",
				new GeoGebraSerializer(null)
	...
	void testVisitedEmptyVariableFromSuggestionEvaluatesWithDefaultVariable() {
		...
		checker
				.right(1)
				.checkCaret(VARIABLE_START)
				.left(1)
				.checkCaret(INTEGRAND_START)
				.type("x");
		assertEquals(
				"Integral(x)",
				new GeoGebraSerializer(null)
	...
	void testLocalizedIntegralWithIntegrandAndVariableFromSuggestionCreatesInlineIntegral() {
		KeyboardInputAdapter.onKeyboardInput(
				localizedChecker.getMathField().getInternal(), "Integrale( <Funzione>, <Variabile> )");
	...
	void testLocalizedIntegralWithEvaluateFromSuggestionStaysGenericCommand() {
		KeyboardInputAdapter.onKeyboardInput(
				localizedChecker.getMathField().getInternal(),
	...
	void testLocalizedKeyboardIntegralCommandCreatesInlineIntegral() {
		KeyboardInputAdapter.onCommandInput(localizedChecker.getMathField().getInternal(), "Integrale");
	...
	void testEnglishKeyboardIntegralCommandWorksWithLocalizedCommands() {
		KeyboardInputAdapter.onCommandInput(localizedChecker.getMathField().getInternal(), "Integral");
		localizedChecker.checkRaw("SequenceNode[FnINTEGRAL["
				+ "SequenceNode[], SequenceNode[], SequenceNode[], SequenceNode[x]]]");
	}
	...
	void testParserPreservesSupportedAndUnsupportedIntegralCommands(String input, String expected)
			throws ParseException {
		assertEquals(
				expected,
				new GeoGebraSerializer(null).serialize(new Parser(new TemplateCatalog()).parse(input)));
	}

	@Test
	void testFourArgumentIntegralWithBooleanFlagRemainsGenericCommand() throws ParseException {
		FunctionNode command = (FunctionNode) new Parser(new TemplateCatalog())
				.parse("Integral(f,a,b,true)")
				.getRootNode()
				.getChild(0);
	...
	void testTeXSerializerHidesIndefiniteIntegralLimits() throws ParseException {
		assertEquals(
				"\\int{}f\\,\\mathrm{d}x",
				new TeXSerializer().serialize(new Parser(new TemplateCatalog()).parse("Integral(f)")));
	}

	@Test
	void testTeXSerializerShowsDefiniteIntegralLimits() throws ParseException {
		assertEquals(
				"\\int\\limits_{0}^{1}f\\,\\mathrm{d}x",
				new TeXSerializer().serialize(new Parser(new TemplateCatalog()).parse("Integral(f,0,1)")));
	}

	@Test
	void testTeXSerializerUsesFunctionVariableForDefiniteIntegral() throws ParseException {
		assertEquals(
				"\\int\\limits_{1}^{2}{{{\\mathrm{cos}}\\left(t\\right)}}\\,\\mathrm{d}t",
				new TeXSerializer()
						.serialize(new Parser(new TemplateCatalog()).parse("Integral(cos(t),1,2)")));
	}
	...
	@Test
	void testFocusingEmptyLimitRevealsLimitsWithInvisiblePlaceholder() throws ParseException {
		TemplateCatalog catalog = new TemplateCatalog();
		Parser parser = new Parser(catalog);
		Formula formula = new Formula(catalog, new SequenceNode());
	...
	void testRightArrowNavigatesIntegralPrimaryFields() {
		checker
				.parse("Integral(f)")
				.typeKey(VK_HOME)
				.checkCaret(BEFORE_INTEGRAL)
				.right(1)
				.checkCaret(UPPER_LIMIT_START)
				.right(1)
				.checkCaret(INTEGRAND_START)
				.right(1)
				.checkCaret(INTEGRAND_END)
				.right(1)
				.checkCaret(VARIABLE_START)
				.right(1)
				.checkCaret(VARIABLE_END)
				.right(1)
				.checkCaret(AFTER_INTEGRAL);
	}
	...
	void testLeftArrowFromIntegrandStartMovesAfterUpperLimit() {
		checker
				.parse("Integral(f,t,0,1)")
				.typeKey(VK_HOME)
				.checkCaret(BEFORE_INTEGRAL)
				.right(3)
				.checkCaret(INTEGRAND_START)
				.left(1)
				.checkCaret(UPPER_LIMIT_END);
	}
	...
	void testBackspaceNavigatesTemplateWithDefaultVariableAfterCaret() {
		KeyboardInputAdapter.onCommandInput(checker.getMathField().getInternal(), "Integral");
		checker.right(3).checkCaret(AFTER_INTEGRAL).typeKey(VK_BACK_SPACE).checkCaret(VARIABLE_END);
	}
	...
	private void clickAtPath(Integer... path) {
		...
		mathField
				.getMathFieldController()
				.updateCursorPosition(
						mathField.getFormula(),
		...
		checker.click((int) Math.floor(CursorBox.startX) + 1, (int) Math.floor(CursorBox.startY) + 1);
	}

	private static FunctionNode getInsertedFunction(EditorChecker checker) {
		return (FunctionNode)
				checker.getMathField().getInternal().getFormula().getRootNode().getChild(0);
	}
}
