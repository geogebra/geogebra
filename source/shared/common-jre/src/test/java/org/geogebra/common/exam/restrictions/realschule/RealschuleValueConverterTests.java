...
class RealschuleValueConverterTests extends BaseAppTestSetup {
	private final ToStringConverter converter =
			new ProtectiveGeoElementValueConverter(new RealschuleAlgebraOutputFilter());
	...
	void testLabelRestrictions() throws NotApplicablePropertyException {
		...
		captionStyleProperty.setValue(GeoElementND.LABEL_VALUE);
		assertEquals(
				implicitCurve.toValueString(defaultTemplate),
				converter.toLabelAndDescription(implicitCurve));
		captionStyleProperty.setValue(GeoElementND.LABEL_NAME_VALUE);
		assertEquals(
				implicitCurve.toString(defaultTemplate), converter.toLabelAndDescription(implicitCurve));
		captionStyleProperty.setValue(GeoElementND.LABEL_NAME);
		assertEquals(
		...
		captionStyleProperty.setValue(GeoElementND.LABEL_VALUE);
		assertEquals(fitPoly.toValueString(defaultTemplate), converter.toLabelAndDescription(fitPoly));
		captionStyleProperty.setValue(GeoElementND.LABEL_NAME_VALUE);
		...
		captionStyleProperty.setValue(GeoElementND.LABEL_VALUE);
		assertEquals(
				fitGrowth.toValueString(defaultTemplate), converter.toLabelAndDescription(fitGrowth));
		captionStyleProperty.setValue(GeoElementND.LABEL_NAME_VALUE);
		assertEquals(fitGrowth.toString(defaultTemplate), converter.toLabelAndDescription(fitGrowth));
		captionStyleProperty.setValue(GeoElementND.LABEL_NAME);
		assertEquals(fitGrowth.getLabel(defaultTemplate), converter.toLabelAndDescription(fitGrowth));
		captionStyleProperty.setValue(GeoElementND.LABEL_CAPTION);
		assertEquals(fitGrowth.getCaption(defaultTemplate), converter.toLabelAndDescription(fitGrowth));

		GeoElement fitPow = evaluateGeoElement("FitPow((1,2),(3,4))");
		captionStyleProperty = new CaptionStyleProperty(localization, fitPow);
		captionStyleProperty.setValue(GeoElementND.LABEL_VALUE);
		assertEquals(fitPow.toValueString(defaultTemplate), converter.toLabelAndDescription(fitPow));
		captionStyleProperty.setValue(GeoElementND.LABEL_NAME_VALUE);
		assertEquals(fitPow.toString(defaultTemplate), converter.toLabelAndDescription(fitPow));
		captionStyleProperty.setValue(GeoElementND.LABEL_NAME);
		assertEquals(fitPow.getLabel(defaultTemplate), converter.toLabelAndDescription(fitPow));
		captionStyleProperty.setValue(GeoElementND.LABEL_CAPTION);
		assertEquals(fitPow.getCaption(defaultTemplate), converter.toLabelAndDescription(fitPow));
	}
}
