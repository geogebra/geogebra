...
public class Korean {
	...
	private static void appendKoreanChar(StringBuilder ret, char lead, char vowel, char tail) {
	...
	public static char[] checkMerge(char lastChar, char newChar) {
		...
		if (Korean.isKoreanLeadPlusVowelChar(lastChar) && Korean.isKoreanTailChar(newChar, true)) {
		...
		if (Korean.isKoreanLeadChar(lastChar, true) && Korean.isKoreanVowelChar(newChar, true)) {
		...
		if (lastCharFlat.length() == 3 && Korean.isKoreanVowelChar(newChar, true)) {
			...
			String unmerged = unmergeDoubleCharacterToLeadTail(lastCharFlat.charAt(2));

			newLastChar = Korean.unflattenKorean(lastCharFlat.substring(0, 2) + "" + unmerged.charAt(0))
					.charAt(0);

			newNewChar = Korean.unflattenKorean(Korean.tailToLead(unmerged.charAt(1)) + "" + newChar)
					.charAt(0);
			ret[0] = newLastChar;
			ret[1] = newNewChar;
			return ret;
		...
		if (lastCharFlat.length() == 3 && Korean.isKoreanTailChar(newChar, true)) {
			...
			String doubleCheck = Korean.mergeDoubleCharacters(lastChar2 + "" + newChar1);

			if (doubleCheck.length() == 1) {
				newChar1 = Korean.unflattenKorean(lastCharFlat.substring(0, 2) + "" + doubleCheck)
						.charAt(0);
		...
		if (Korean.isKoreanLeadPlusVowelChar(lastChar) && Korean.isKoreanVowelChar(newChar, true)) {

			char lastChar1 = lastCharFlat.charAt(1);

			// if this is length 1, merge succeeded
			String doubleCheck = Korean.mergeDoubleCharacters(
					lastChar1 + "" + Korean.convertFromCompatibilityJamo(newChar, true));

			if (doubleCheck.length() == 1) {
				char newChar1 =
						Korean.unflattenKorean(lastCharFlat.charAt(0) + "" + doubleCheck).charAt(0);
	...
	public static boolean isSingleKoreanChar(char ch) {
		return isKoreanLeadChar(ch, true) || isKoreanVowelChar(ch, true) || isKoreanTailChar(ch, true);
	}
}
