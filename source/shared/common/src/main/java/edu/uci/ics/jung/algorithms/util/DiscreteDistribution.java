...
public class DiscreteDistribution {
	...
	/**
	 * Returns the Kullback-Leibler divergence between the two specified
	 * distributions, which must have the same number of elements. This is
	 * defined as the sum over all <code>i</code> of
	...
	/**
	 * Returns the squared difference between the two specified distributions,
	 * which must have the same number of elements. This is defined as the sum
	 * over all <code>i</code> of the square of
	...
	/**
	 * Returns the cosine distance between the two specified distributions,
	 * which must have the same number of elements. The distributions are
	 ...
	 * <code>vw / (Math.sqrt(v) * Math.sqrt(w))</code>.
	 */
	...
	/**
	 * Returns the entropy of this distribution. High entropy indicates that the
	 * distribution is close to uniform; low entropy indicates that the
	 * distribution is close to a Dirac delta (i.e., if the probability mass is
	 * concentrated at a single point, this method returns 0). Entropy is
	 * defined as the sum over all <code>i</code> of
	...
	/**
	 * Throws an <code>IllegalArgumentException</code> if the two arrays are not
	 * of the same length.
	 */
	 ...
	 * array, so that the values sum to 1 (i.e., can be treated as
	...
	/**
	 * Returns the mean of the specified array of distributions, represented as
	 * normalized arrays of <code>double</code> values. Will throw an
	 * "index out of bounds" exception if the distribution arrays are not all of
	 * the same length.
	 */
