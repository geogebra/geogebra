...
public class Derivative {
	...
	public static ExpressionNode get(
		...
		switch (operation) {
			...
			case DIVIDE:
				...
				if (right.isExpressionNode()
					...
					return wrap(left.derivative(fv, kernel0))
							.multiply(rl)
							.subtract(wrap(rl.derivative(fv, kernel0)).multiply(left).multiply(rr))
			...
			case MULTIPLY:
				...
				ExpressionNode scalarExpanded =
						VectorArithmetic.expandScalarProduct(kernel0, left, right, operation);
			...
			case SEC:
				return new ExpressionNode(kernel0, left, Operation.SEC, null)
						.multiply(new ExpressionNode(kernel0, left, Operation.TAN, null))
						.multiply(left.derivative(fv, kernel0));
			case CSC:
				return new ExpressionNode(kernel0, left, Operation.CSC, null)
						.multiply(new ExpressionNode(kernel0, left, Operation.COT, null))
						.multiply(left.derivative(fv, kernel0))
						.multiply(-1);
			case COT:
				return new ExpressionNode(kernel0, left, Operation.CSC, null)
			...
			case SECH:
				return new ExpressionNode(kernel0, left, Operation.SECH, null)
						.multiply(new ExpressionNode(kernel0, left, Operation.TANH, null))
						.multiply(left.derivative(fv, kernel0))
						.multiply(-1);
			case CSCH:
				return new ExpressionNode(kernel0, left, Operation.CSCH, null)
						.multiply(new ExpressionNode(kernel0, left, Operation.COTH, null))
						.multiply(left.derivative(fv, kernel0))
						.multiply(-1);
			case COTH:
				return new ExpressionNode(kernel0, left, Operation.CSCH, null)
			...
			case ARCTAND:
			case ARCTAN:
				return wrap(left.derivative(fv, kernel0)).divide(wrap(left).square().plus(1));
			...
			case ACOSH:
				...
				return wrap(left.derivative(fv, kernel0))
						.divide(wrap(left).plus(1).sqrt().multiply(wrap(left).subtract(1).sqrt()));
			case ATANH:
				return wrap(left.derivative(fv, kernel0)).divide(wrap(left).square().subtractR(1));
			...
			case SI:
				return wrap(left.derivative(fv, kernel0)).multiply(wrap(left).sin().divide(left));

			case CI:
				return wrap(left.derivative(fv, kernel0)).multiply(wrap(left).cos().divide(left));

			case EI:
				return wrap(left.derivative(fv, kernel0)).multiply(wrap(left).exp().divide(left));

			case ERF:
				return wrap(left.derivative(fv, kernel0))
						.multiply(wrap(kernel0, 2))
						.divide(wrap(left).square().exp().multiply(wrap(kernel0, Math.PI).sqrt()));
			...
			case POLYGAMMA:
				if (left.isNumberValue() && !left.contains(fv)) {
					...
					return wrap(right.derivative(fv, kernel0)).multiply(wrap(right).polygamma(n + 1));
				}
			...
			case IF_ELSE:
				...
				return new ExpressionNode(kernel0, np, Operation.IF_ELSE, right.derivative(fv, kernel0));

			case IF:
			case IF_SHORT:
				return new ExpressionNode(kernel0, left, Operation.IF, right.derivative(fv, kernel0));

			case IF_LIST:
				MyList rtDiff = new MyList(kernel0);
				MyList rt = (MyList) right;
				for (int i = 0; i < rt.size(); i++) {
					rtDiff.addListElement(rt.get(i).derivative(fv, kernel0));
				}
			...
			case LOGB:
				...
				return log(right, kernel0).divide(log(left, kernel0)).derivative(fv, kernel0);
			...
			case CBRT:
				...
				// correct domain
				return wrap(left.derivative(fv, kernel0))
						.divideSimplify(wrap(left).square().cbrt())
						.divide(3);
			...
			case FUNCTION:
				if (left instanceof GeoFunction) {
					...
					GeoFunction geoFun = new GeoFunction(kernel0.getConstruction(), fun2);

					return new ExpressionNode(kernel0, geoFun, Operation.FUNCTION, right)
							.multiply(right.derivative(fv, kernel0));
				}
				break;
			case ARCTAN2:
				// (((-f(x)) * g'(x)) + (f'(x) * g(x))) / (f(x)^(2) + g(x)^(2))
				ExpressionNode numerator = left.derivative(fv, kernel0)
						.wrap()
						.multiply(right)
						.wrap()
						.subtract(right.derivative(fv, kernel0).wrap().multiply(left).wrap());
				return numerator.divide(left.wrap().power(2).wrap().plus(right.wrap().power(2)));
			...
			case FACTORIAL:
				// x! -> psi(x+1) * x!
				return new ExpressionNode(kernel0, left.wrap().plus(1), Operation.PSI, null)
						.multiply(new ExpressionNode(kernel0, left, Operation.FACTORIAL, null))
						.multiply(left.derivative(fv, kernel0));

			case GAMMA:
				// gamma(x) -> gamma(x) psi(x)
				return new ExpressionNode(kernel0, left, Operation.PSI, null)
						.multiply(new ExpressionNode(kernel0, left, Operation.GAMMA, null))
						.multiply(left.derivative(fv, kernel0));
		...
		Log.error("unhandled operation in derivative() (no CAS version): " + operation);

		// undefined
		return wrap(kernel0, Double.NaN);
	}

	private static ExpressionNode coordDerivative(
			ExpressionValue left, int i, FunctionVariable fv, Kernel kernel0) {
	...
	private static ExpressionNode derivativePower(
			ExpressionValue left, ExpressionValue right, FunctionVariable fv, Kernel kernel0) {
		if (right.isNumberValue() && !right.contains(fv)) {
			...
			if (right.isConstant()) {
				ret = derivativeConstantPower(left, right, fv, kernel0);
			}
			if (ret != null) {
				return ret;
			}
			...
			return wrap(left)
					.power(wrap(right).subtract(1))
					.multiply(left.derivative(fv, kernel0))
					.multiply(right);
		}
		ExpressionNode scalarExpanded =
				VectorArithmetic.expandScalarProduct(kernel0, left, right, Operation.POWER);
		if (scalarExpanded != null) {
			return scalarExpanded.derivative(fv, kernel0);
		}
		return wrap(left)
				...
				.multiply(wrap(right.derivative(fv, kernel0))
						.multiply(wrap(left).ln())
						.plus(wrap(right).multiply(left.derivative(fv, kernel0)).divideSimplify(left)));
	}

	private static ExpressionNode derivativeConstantPower(
			ExpressionValue left, ExpressionValue right, FunctionVariable fv, Kernel kernel0) {
		double rightDouble = right.evaluateDouble();
		if (DoubleUtil.isEqual(rightDouble, 2)) {
			return wrap(left).multiply(left.derivative(fv, kernel0)).multiply(right);
		}
		// not an integer, convert to x^(a/b)
		if (!DoubleUtil.isInteger(rightDouble)) {

			double[] fraction =
					AlgoFractionText.decimalToFraction(rightDouble, Kernel.STANDARD_PRECISION);
			...
			// a/b-1 = (a-b)/b
			ExpressionNode newPower = wrap(new MyDouble(kernel0, a - b)).divide(new MyDouble(kernel0, b));
