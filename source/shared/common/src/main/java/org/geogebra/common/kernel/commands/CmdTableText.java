...
public class CmdTableText extends CommandProcessor {
		...
		switch (n) {
			...
			case 3:
				if (arg[0].isGeoList() && arg[1].isGeoText() && arg[2] instanceof GeoNumberValue) {
					...
					if (first.size() == 0 || first.get(0).isGeoList()) {
						...
						GeoElement[] ret = {
							tableText(c.getLabel(), arg, first, (GeoText) arg[1], minWidthHeight)
						};
						return ret;
					}
			...
			default:
				...
				if (arg[arg.length - 1].isGeoText()) {
					...
					if (list != null) {
						GeoElement[] ret = {
							tableText(c.getLabel(), arg, list, (GeoText) arg[arg.length - 1], null)
						};
						return ret;
					}
					...
					if (arg[arg.length - amountNumerics - 1].isGeoText()) {
						...
						if (list != null) {
							GeoElement[] ret = {
								tableText(
										...
										arg,
										list,
										(GeoText) arg[arg.length - amountNumerics - 1],
										minWidthHeight)
							};
							return ret;
						}
						...
						if (list != null) {
							GeoElement[] ret = {tableText(c.getLabel(), arg, list, null, minWidthHeight)};
							return ret;
						}
					}
				} else {
	 ...
	 * @return table text
	 */
	public final GeoText tableText(
			String label, GeoElement[] arg, GeoList list, GeoText args, GeoNumberValue[] minWidthHeight) {
