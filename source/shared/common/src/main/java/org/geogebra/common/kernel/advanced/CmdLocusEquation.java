...
public class CmdLocusEquation extends CommandProcessor {
	...
	public GeoElement[] process(Command c, EvalInfo info) throws MyError {
				...
				if ((ok[0] = arg[0].isGeoPoint()) && (ok[1] = arg[1].isGeoPoint())) {
					...
					if ((ok[0] = arg[0] instanceof BooleanValue)
						...
						return new GeoElement[] {
							locusEquation(c.getLabel(), implicitLocus, movingPoint).toGeoElement()
						};
					} // else
		...
		return new GeoElement[] {
			locusEquation(c.getLabel(), locusPoint, movingPoint).toGeoElement()
		};
	}
	 ...
	 */
	public final GeoImplicit locusEquation(String label, GeoPoint locusPoint, GeoPoint movingPoint) {
		if (movingPoint.getPath() == null
				|| locusPoint.getPath() != null
	 ...
	 * @return implicit locus curve
	 */
	public final GeoImplicit locusEquation(
			String label, GeoElement implicitLocus, GeoPoint movingPoint) {
		AlgoLocusEquation algo = new AlgoLocusEquation(cons, implicitLocus, movingPoint);
		GeoImplicit poly = algo.getPoly();
