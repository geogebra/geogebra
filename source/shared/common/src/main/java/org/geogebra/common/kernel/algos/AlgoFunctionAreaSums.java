...
public abstract class AlgoFunctionAreaSums extends AlgoElement implements DrawInformationAlgo {
	 ...
	 */
	public AlgoFunctionAreaSums(
			GeoFunctionable f, GeoNumberValue a, GeoNumberValue b, GeoNumberValue n, GeoNumberValue d) {
	 ...
	 */
	public AlgoFunctionAreaSums(
			Construction cons, String label, GeoList list1, GeoList list2, boolean right) {
	 ...
	 *            right histogram?
	 */
	public AlgoFunctionAreaSums(Construction cons, GeoList list1, GeoList list2, boolean right) {
	...
	public AlgoFunctionAreaSums(
		...
		this(cons, isCumulative, list1, list2, list3, useDensity, density, right);
	 ...
	 */
	public AlgoFunctionAreaSums(
			Construction cons, String label, GeoNumberValue p1, GeoBoolean isCumulative, SumType type) {
	...
	private void compute(boolean onlyZoom) {
		...
		switch (type) {
			...
			case UPPERSUM:
				if (f == null
						|| !(f.isDefined() && ageo.isDefined() && bgeo.isDefined() && ngeo.isDefined())) {
					sum.setUndefined();
					return;
				}
				...
				double visibleMin = Math.max(Math.min(ad, bd), kernel.getViewsXMin(sum));

				double visibleMax = Math.min(Math.max(ad, bd), kernel.getViewsXMax(sum));

				// subsample every 5 pixels
				double noOfSamples =
						kernel.getApplication().countPixels(visibleMin, visibleMax) / SAMPLE_PIXELS;

				double subStep = Math.abs(visibleMax - visibleMin) / noOfSamples;
				boolean doSubSamples = !DoubleUtil.isZero(subStep) && Math.abs(STEP) > subStep;
				...
				for (int i = 0; i < upperBound; i++) { // N+1 for trapezoids
					...
					if (type == SumType.RECTANGLESUM) {
						...
						if ((dd >= 0) && (dd <= 1)) {
							...
							double xVal = Math.min(rightBorder, leftBorder[i] + dd * STEP);
			...
			case BARCHART_BERNOULLI:
				...
				boolean cumulative = isCumulative != null && ((GeoBoolean) isCumulative).getBoolean();
				yval[0] = 1 - p;
	...
	private boolean functionDefined(double ad, double bd) {
		...
		for (double temp = ad;
				(interval > 0 && temp < bd) || (interval < 0 && temp > bd);
				temp += interval) {
