		...
		switch (n) {
			case 1:
				if (arg[0].isGeoList()) {
					...
					AlgoLastFunction algo = new AlgoLastFunction(cons, c.getLabel(), (GeoFunction) arg[0]);
			...
			case 2:
				...
				if (list && arg[1].isGeoNumeric()) {
					GeoElement[] ret = {last(c.getLabel(), (GeoList) arg[0], (GeoNumeric) arg[1])};
					return ret;
				} else if (text && arg[1].isGeoNumeric()) {
					GeoElement[] ret = {last(c.getLabel(), (GeoText) arg[0], (GeoNumeric) arg[1])};
					return ret;
				} else {
					throw argErr(c, (list && text) ? arg[1] : arg[0]);
				}
