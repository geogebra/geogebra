...
public class CmdDifference extends CommandProcessor {
	...
	public GeoElement[] process(Command c, EvalInfo info)
		...
		switch (argumentNo) {
			case 2:
				if ((ok[0] = arg[0] instanceof GeoPolygon) && (ok[1] = arg[1] instanceof GeoPolygon)) {
					if (arg[0].isGeoElement3D() && arg[1].isGeoElement3D()) {
						return difference3D(c.getLabels(), (GeoPolygon) arg[0], (GeoPolygon) arg[1]);
					}
					return difference(c.getLabels(), (GeoPolygon) arg[0], (GeoPolygon) arg[1]);
				}
			...
			case 3:
				if ((ok[0] = arg[0] instanceof GeoPolygon)
					...
					if (arg[0].isGeoElement3D() && arg[1].isGeoElement3D()) {
						return difference3D(
								c.getLabels(), (GeoPolygon) arg[0], (GeoPolygon) arg[1], (GeoBoolean) arg[2]);
					}
					return difference(
							c.getLabels(), (GeoPolygon) arg[0], (GeoPolygon) arg[1], (GeoBoolean) arg[2]);
				}
	 ...
	 */
	protected GeoElement[] difference(String[] labels, GeoPolygon poly1, GeoPolygon poly2) {
	 ...
	 * @return resulting polygons
	 */
	protected GeoElement[] difference(
			String[] labels, GeoPolygon poly1, GeoPolygon poly2, GeoBoolean exclusive) {
	 ...
	 * @return resulting polygons
	 */
	protected GeoElement[] difference3D(
			String[] labels, GeoPolygon poly1, GeoPolygon poly2, GeoBoolean exclusive) {
		return kernel.getManager3D().differencePolygons(labels, poly1, poly2, exclusive);
	}
}
