...
public class DrawAngle extends Drawable implements Previewable {
	 ...
	 */
	public final Coords getCoordsInView(GeoPointND p) {
			...
			switch (view.getRightAngleStyle()) {
				...
				case EuclidianStyleConstants.RIGHT_ANGLE_STYLE_SQUARE:
					...
					square.lineTo(
							...
							coords[1] - length * Math.sin(angSt) * view.getScaleRatio());
					square.lineTo(
							coords[0] + arcSize * Math.cos(angSt + Kernel.PI_HALF / 2),
							coords[1] - arcSize * Math.sin(angSt + Kernel.PI_HALF / 2) * view.getScaleRatio());
					square.lineTo(
							coords[0] + length * Math.cos(angSt + Kernel.PI_HALF),
							coords[1] - length * Math.sin(angSt + Kernel.PI_HALF) * view.getScaleRatio());
				...
				case EuclidianStyleConstants.RIGHT_ANGLE_STYLE_L:
					...
					square.moveTo(
							coords[0]
									+ length * Math.cos(angSt)
									+ offset * Math.cos(angSt)
									+ offset * Math.cos(angSt + Kernel.PI_HALF),
							coords[1]
									- length * Math.sin(angSt) * view.getScaleRatio()
									- offset * Math.sin(angSt)
									- offset * Math.sin(angSt + Kernel.PI_HALF));
					square.lineTo(
							coords[0] + offset * Math.cos(angSt) + offset * Math.cos(angSt + Kernel.PI_HALF),
							coords[1] - offset * Math.sin(angSt) - offset * Math.sin(angSt + Kernel.PI_HALF));
					square.lineTo(
							coords[0]
									...
									+ offset * Math.cos(angSt + Kernel.PI_HALF),
							coords[1]
									- length * Math.sin(angSt + Kernel.PI_HALF) * view.getScaleRatio()
									- offset * Math.sin(angSt)
									- offset * Math.sin(angSt + Kernel.PI_HALF));
					shape = square; // FIXME
				...
				case EuclidianStyleConstants.RIGHT_ANGLE_STYLE_DOT:
					...
					view.toScreenCoords(coords);
					dot90degree.setFrame(
							coords[0] - geo.getLineThickness(),
							coords[1] - geo.getLineThickness(),
							diameter,
							diameter);
		...
		// STANDARE case: draw arc with possible decoration
			...
			switch (geo.getDecorationType()) {
				...
				case GeoElementND.DECORATION_ANGLE_ARROW_CLOCKWISE:
					...
					size = size * 0.9d;
					p2[0] = p1[0] + (1 * n[0] + 3 * v[0]) * size * scale;
					p2[1] = p1[1] + (1 * n[1] + 3 * v[1]) * size * scale;

					double[] p3 = new double[2]; // arrow vertex 2
					p3[0] = p1[0] + (-1 * n[0] + 3 * v[0]) * size * scale;
					p3[1] = p1[1] + (-1 * n[1] + 3 * v[1]) * size * scale;
	...
	private void initPreview() {
		...
		AlgoAnglePoints algoPreview =
				new AlgoAnglePoints(cons, previewTempPoints[0], previewTempPoints[1], previewTempPoints[2]);
