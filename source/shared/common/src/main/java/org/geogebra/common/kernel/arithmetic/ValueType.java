...
public enum ValueType implements ExpressionValueType {
	 ...
	 */
	public static ExpressionValueType resolve(
			Operation op, ExpressionValue left, ExpressionValue right) {
		...
		switch (op) {
			...
			case MULTIPLY:
				...
				if ((rightType == ValueType.NONCOMPLEX2D || rightType == ValueType.VECTOR3D)
						&& (leftType == ValueType.NONCOMPLEX2D || leftType == ValueType.VECTOR3D)) {
			...
			case ROUND2:
			case ARCTAN2:
				return ValueType.COMPLEX.check(left, checkList(left, checkList(right, ValueType.NUMBER)));
			...
			case LOG10:
			case LOG2:
				return ValueType.COMPLEX.check(left, checkList(left, ValueType.NUMBER));
			case FUNCTION:
				return ValueType.COMPLEX.check(right, checkList(right, ValueType.NUMBER));
			case FUNCTION_NVAR:
			case FREEHAND:
			case DATA:
			case ARBCONST:
			...
			case MATRIXTOVECTOR:
				...
				return list.size() == 3 ? ValueType.VECTOR3D : ValueType.NONCOMPLEX2D;
			...
			case GREATER:
			case GREATER_EQUAL:
				return checkList(left, checkList(right, ValueType.BOOLEAN));
			case LAMBERTW:
			case POLYGAMMA:
				break;
			case POWER:
	...
	private static ExpressionValueType plusMinusType(ExpressionValue left, ExpressionValue right) {
		if (right.evaluatesToList()) {
			return right.getValueType();
		}
		if (right.getValueType() == ValueType.VECTOR3D || left.getValueType() == ValueType.VECTOR3D) {
			return ValueType.VECTOR3D;
		}
	...
	private static ExpressionValueType checkList(ExpressionValue arg, ExpressionValueType fallback) {
