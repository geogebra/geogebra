...
public class ExpressionNode extends ValidExpression
	 ...
	 */
	public ExpressionNode(
			Kernel kernel, ExpressionValue left, Operation operation, ExpressionValue right) {
	 ...
	 *            left subexpression
	 */
	public final void setLeft(ExpressionValue l) {
		left = l;
	 ...
	 */
	public final void setRight(ExpressionValue r) {
		right = r;
		if (right != null) {
	 ...
	 */
	public final void simplifyLeaves() {
		if (left.isExpressionNode()) {
	...
	private void doResolveVariables(EvalInfo info) {
		...
		if (left.isVariable()) {
			...
			if (operation == Operation.POWER || operation == Operation.FACTORIAL) {
	...
	private static boolean isSameVar(ExpressionValue a, ExpressionValue b) {
		return a.unwrap() instanceof FunctionVariable
				...
				&& a.toString(StringTemplate.xmlTemplate).equals(b.toString(StringTemplate.xmlTemplate));
	}
	 ...
	 */
	public final boolean includesDivisionBy(ExpressionValue val) {
		...
		if (operation == Operation.DIVIDE) {
			...
			if (left.isExpressionNode() && ((ExpressionNode) left).includesDivisionBy(val)) {
				return true;
			}
		} else {
			if (left.isExpressionNode() && ((ExpressionNode) left).includesDivisionBy(val)) {
	...
	public final boolean includesNonContinuousIntegral() {
		...
		if (left.isExpressionNode() && ((ExpressionNode) left).includesNonContinuousIntegral()) {
			return true;
		}
		...
		if ((operation == Operation.FUNCTION || operation == Operation.FUNCTION_NVAR)
	...
	public final int replaceVariables(String varName, FunctionVariable fVar) {
		...
		if (left instanceof GeoDummyVariable) {
			if (varName.equals(left.toString(StringTemplate.defaultTemplate))) {
				left = fVar;
				replacements++;
			}
		...
		if (right != null) {
			if (right.isExpressionNode()) {
				...
				if (varName.equals(right.toString(StringTemplate.defaultTemplate))) {
					right = fVar;
					replacements++;
				}
	 ...
	 *            variable z
	 */
	public void replaceXYZnodes(FunctionVariable xVar, FunctionVariable yVar, FunctionVariable zVar) {
	...
	public void fixPowerFactorial(Operation multiplicativeOperation) {
		if (left.isExpressionNode()
				&& ((ExpressionNode) left).operation == multiplicativeOperation
				&& !((ExpressionNode) left).hasBrackets()) {
			right = new ExpressionNode(kernel, ((ExpressionNode) left).getRight(), operation, right);
	...
	private void fixPowerFactorialTrig() {
		if (isSingleArgumentFunction(left)) {
			...
			if (trigArg.isOperation(Operation.MULTIPLY)) {
				...
				left =
						trigArgExpr.getRight().wrap().apply(operation, right).multiply(trigArgExpr.getLeft());
	...
	private void fixMultiplyDeg() {
		...
		if (isSingleArgumentFunction(left)) {
			...
			left = new ExpressionNode(
					kernel, removeDeg(((ExpressionNode) left).getLeft()), Operation.MULTIPLY, right);
			unsetRight();
		}
	}
	...
	public final boolean containsGeoFunctionNVar() {
		if ((left instanceof GeoFunctionNVar) || (right instanceof GeoFunctionNVar)) {
			return true;
		}

		if ((left instanceof ExpressionNode) && ((ExpressionNode) left).containsGeoFunctionNVar()) {
			return true;
		}
		return (right instanceof ExpressionNode) && ((ExpressionNode) right).containsGeoFunctionNVar();
	}
	 ...
	 */
	protected final Polynomial makePolynomialTree(Equation equ, boolean keepFraction) {
		Polynomial lt;
		Polynomial rt = null;
		...
		if (operation == Operation.FUNCTION_NVAR) {
			if ((left instanceof FunctionalNVar) && (right instanceof MyList)) {
				return makePolynomialTreeFromFunctionNVar(
						((FunctionalNVar) left).getFunction(), equ, keepFraction);
			}
			...
			if (left instanceof GeoFunction) {
				...
				Function base = ((Functional) ((ExpressionNode) left).getLeft()).getFunction();
				int deg = (int) Math.ceil(((ExpressionNode) left).getRight().evaluateDouble());
				for (int i = 0; i < deg; i++) {
					base = new Function(
							((Functional) ((ExpressionNode) left).getLeft())
									.getFunction()
									.derivative(base.getFunctionVariable(), kernel)
									.wrap(),
		...
		if (!polynomialOperation(operation)) {
			...
			return new Polynomial(
					kernel, new Term(new ExpressionNode(kernel, left, operation, right), ""));
		}
		ExpressionNode scalarExpanded =
				VectorArithmetic.expandScalarProduct(kernel, left, right, operation);
		...
		if (right != null) {
			if (right.isExpressionNode()) {
				rt = ((ExpressionNode) right).makePolynomialTree(equ, keepFraction);
			} else if (right instanceof FunctionVariable) {
	...
	private Polynomial makePolynomialTreeFromFunctionNVar(
			FunctionNVar func, Equation equ, boolean keepFraction) {
		...
		return new Polynomial(kernel, new Term(new ExpressionNode(kernel, left, operation, right), ""));
	}

	private Polynomial makePolyTreeFromFunction(Function func, Equation equ, boolean keepFraction) {
		if (right instanceof ExpressionNode) {
			if (!equ.isFunctionDependent()) {
				equ.setFunctionDependent(((ExpressionNode) right).containsFunctionVariable());
			}
	 ...
	 */
	public final boolean isForceInequality() {
	 ...
	 */
	@Override
	public final void getVariables(Set<GeoElement> variables, SymbolicMode mode) {
		if (leaf) {
	...
	public static boolean isImaginaryUnit(ExpressionValue value) {
		return (value instanceof GeoVec2D) && ((GeoVec2D) value).isImaginaryUnit();
	}
	...
	public boolean containsMyStringBuffer() {
		...
		if ((left instanceof MyStringBuffer) || (right instanceof MyStringBuffer)) {
			return true;
		}
	...
	public String getCASstring(StringTemplate tpl, boolean symbolic) {
		...
		// STANDARD case: no leaf
			...
			if (operation == Operation.RANDOM) {
				...
				ret = ExpressionSerializer.operationToString(
						left, right, operation, leftStr, rightStr, true, tpl, kernel);
			} else {
				ret = ExpressionSerializer.operationToString(
						left, right, operation, leftStr, rightStr, !symbolic, tpl, kernel);
			}
		}
	...
	public static String getCasString(
			ExpressionValue expr, StringTemplate tpl, boolean symbolic, boolean shaveOffBrackets) {
		...
		} else if (expr.isGeoElement() && ((GeoElement) expr).getDefinition() != null) {
	...
	public final String toString(StringTemplate tpl) {
		...
		if (tpl.getStringType().equals(StringType.OGP) && expandForOGP(left)) {
		...
		return ExpressionSerializer.operationToString(
				left, right, operation, leftStr, rightStr, false, tpl, kernel);
	}
	...
	private boolean shaveBrackets() {
		return (operation == Operation.FUNCTION_NVAR
						...
						|| (operation == Operation.VEC_FUNCTION && left instanceof GeoSurfaceCartesianND))
	...
	public final String toLaTeXString(boolean symbolic, StringTemplate tpl) {
		...
		if (right != null) {
			...
			if (((operation == Operation.FUNCTION_NVAR) || (operation == Operation.ELEMENT_OF))
				...
				rightStr = ((MyList) right).toLaTeXStringNoBrackets(symbolic, tpl);
	 ...
	 */
	public static int opID(ExpressionValue ev) {
	...
	public static ExtendedBoolean isEqual(ExpressionValue ev1, ExpressionValue ev2) {
		...
		if (ev1 instanceof NumberValue && ev2 instanceof NumberValue) {
			ret =
					DoubleUtil.isEqual(ev1.evaluateDouble(), ev2.evaluateDouble(), Kernel.STANDARD_PRECISION);
		} else if (ev1 instanceof TextValue && ev2 instanceof TextValue) {
			ret = ev1.toValueString(StringTemplate.defaultTemplate)
					.equals(ev2.toValueString(StringTemplate.defaultTemplate));
		} else if (ev1 instanceof VectorNDValue && ev2 instanceof VectorNDValue) {
			ret = ((VectorNDValue) ev1).getVector().isEqual(((VectorNDValue) ev2).getVector());
		} else if (ev1 instanceof BooleanValue && ev2 instanceof BooleanValue) {
			ret = ((BooleanValue) ev1).getBoolean() == ((BooleanValue) ev2).getBoolean();
	 ...
	 */
	public static boolean isEqualString(ExpressionValue ev, double val, boolean symbolic) {
	...
	public boolean isTopLevelCommand(@NonNull String commandName) {
		return isLeaf()
				&& (left instanceof ValidExpression unwrapped)
	...
	public static boolean isConstantDouble(ExpressionValue ev, double v) {
		...
		return base instanceof MyDouble
				&& base.isConstant()
	...
	public ExpressionNode lessThanEqual(double d) {
		return new ExpressionNode(kernel, this, Operation.LESS_EQUAL, new MyDouble(kernel, d));
	}
	...
	public ExpressionNode betaRegularized(ExpressionValue v1, ExpressionValue v2) {
		return new ExpressionNode(
				kernel, new MyNumberPair(kernel, this, v1), Operation.BETA_INCOMPLETE_REGULARIZED, v2);
	}
	...
	public ExpressionNode plusR(double d) {
		...
		return new ExpressionNode(kernel, new MyDouble(kernel, d), Operation.PLUS, this);
	}
	...
	public ExpressionNode power(double d) {
		...
		} else if (DoubleUtil.isEqual(1, d) || isConstantDouble(this, 1) || isConstantDouble(this, 0)) {
			return this;
		}
		return new ExpressionNode(kernel, this, Operation.POWER, new MyDouble(kernel, d));
	}
	...
	public ExpressionNode subtractR(double d) {
		if (d == 0) {
			return new ExpressionNode(kernel, new MyDouble(kernel, -1), Operation.MULTIPLY, this);
		}
	...
	public ExpressionNode negation() {
		if (Operation.AND_INTERVAL.equals(operation)) {
			// unary, not binary
			return new ExpressionNode(
					kernel, left.wrap().negation(), Operation.OR, right.wrap().negation());
		}
		...
		if (toRoot && getOperation() == Operation.POWER && getRight().isExpressionNode()) {
			...
			if ((rightLeaf.getOperation() == Operation.DIVIDE) && rightLeaf.isConstant()) {
				if (rightLeaf.getRight().toString(StringTemplate.defaultTemplate).equals("2")) {
					...
					hit = true;
						...
						&& DoubleUtil.isInteger(rightLeaf.getRight().evaluateDouble())
						&& rightLeaf.getRight().evaluateDouble() <= maxRoot) {
					setOperation(Operation.NROOT);
					setRight(new MyDouble(kernel, rightLeaf.getRight().evaluateDouble()));
					hit = true;
				}
				if (hit) {
					didReplacement = true;
			...
			if (getOperation() == Operation.SQRT) {
				power = new ExpressionNode(
						kernel, new MyDouble(kernel, 1), Operation.DIVIDE, new MyDouble(kernel, 2));
				hit = true;
			} else if (getOperation() == Operation.CBRT) {
				power = new ExpressionNode(
						kernel, new MyDouble(kernel, 1), Operation.DIVIDE, new MyDouble(kernel, 3));
				hit = true;
			} else if (getOperation() == Operation.NROOT) {
				power = new ExpressionNode(kernel, new MyDouble(kernel, 1), Operation.DIVIDE, right);
				hit = true;
			}
			if (hit) {
				didReplacement = true;
	...
	private void replaceFractionalPower(ExpressionNode rightLeaf) {
		if (rightLeaf.getLeft().toString(StringTemplate.defaultTemplate).equals("1")) {
			...
			if (c < 0) {
				setRight(
						new ExpressionNode(kernel, getLeft().wrap().power(-c), getOperation(), getRight()));
	...
	public boolean hasCoords() {
		...
		return getLeft().hasCoords() || getRight().hasCoords() || operation == Operation.VEC_FUNCTION;
	}
	...
	public ExpressionNode integral(FunctionVariable fv, Kernel kernel0) {
		...
		switch (operation) {
				...
				if (left == fv && !right.contains(fv)) {
					...
					if (!Double.isNaN(index) && !Double.isInfinite(index)) {
						...
						if (DoubleUtil.isZero(index + 1)) {
							return new ExpressionNode(kernel0, left, Operation.LOG, null);
						}
			...
			case DIVIDE:
				...
				if (left.isNumberValue() && !left.contains(fv) && right == fv) {
					// eg 4/x
					return new ExpressionNode(kernel0, fv, Operation.LOG, null).multiply(left);
				}
				break;
			...
			case IF_ELSE:
				...
				return new ExpressionNode(kernel0, np, Operation.IF_ELSE, right.integral(fv, kernel0));

			case IF:
			case IF_SHORT:
				return new ExpressionNode(kernel0, left, Operation.IF, right.integral(fv, kernel0));
		...
		Log.warn("unhandled operation in integral() (no CAS version): " + operation.toString());
	...
	private ExpressionNode linearIntegral(int i, Operation op, FunctionVariable fv) {
		...
		if (!Double.isNaN(coeff)) {
			return new ExpressionNode(kernel, arg, op, null).multiplyR(i).divide(coeff);
		}

		coeff = getLinearCoefficientDiv(fv, arg);

		if (!Double.isNaN(coeff)) {
			return new ExpressionNode(kernel, arg, op, null).multiply(coeff).multiplyR(i);
		}
	...
	public ExpressionNode ifElse(ExpressionValue ifBranch, ExpressionValue elseBranch) {
		return new ExpressionNode(
				kernel, new MyNumberPair(kernel, this, ifBranch), Operation.IF_ELSE, elseBranch);
	}
	...
	public boolean containsFreeFunctionVariable(String name) {
		return checkForFreeVars(left, name)
				|| (operation == Operation.IF_LIST
						&& left instanceof MyList
						&& ((ValidExpression) left).containsFunctionVariable(name))
				|| (right != null && checkForFreeVars(right, name))
				|| ((operation == Operation.FUNCTION_NVAR
								|| operation == Operation.ELEMENT_OF
								|| operation == Operation.IF_LIST)
						&& right instanceof MyList
						&& ((ValidExpression) right).containsFunctionVariable(name));
	}
	...
	public boolean containsFreeFunctionVariableOtherThan(FunctionVariable[] vars) {
		return checkForFreeVars(left, vars)
				...
				|| ((operation == Operation.FUNCTION_NVAR || operation == Operation.ELEMENT_OF)
						&& right instanceof MyList
						&& ((ValidExpression) right).containsFunctionVariableOtherThan(vars));
	}
	...
	private boolean checkForFreeVars(ExpressionValue ev, FunctionVariable[] vars) {
		...
		if (ev instanceof ExpressionNode) {
			return ((ExpressionNode) ev).containsFreeFunctionVariableOtherThan(vars);
		}
	 ...
	 */
	public static boolean doesNotInclude(FunctionVariable[] vars, ExpressionValue ev) {
	...
	private boolean checkForFreeVars(ExpressionValue ev, String name) {
		if (ev instanceof FunctionVariable) {
			return name == null || name.equals(((FunctionVariable) ev).getSetVarString());
		}
		if (ev instanceof ExpressionNode) {
	...
	public boolean has2piPeriodicOperations() {
		return this.any(new Inspecting() {
			...
			public boolean check(ExpressionValue v) {
				return v.isExpressionNode() && is2piPeriodicOperation(((ExpressionNode) v).getOperation());
			}

			private boolean is2piPeriodicOperation(Operation op) {
				return op == Operation.SIN
						|| op == Operation.COS
						|| op == Operation.TAN
						|| op == Operation.COT
						|| op == Operation.SEC
	...
	public static ExpressionValue unaryMinus(Kernel kernel2, ExpressionValue f) {
		if (f instanceof MyDouble && f.isConstant() && !(f instanceof MyDoubleDegreesMinutesSeconds)) {
		...
		if (f.isOperation(Operation.MULTIPLY)) {
			...
			if (en.getRight() instanceof MySpecialDouble
				...
				return new ExpressionNode(
						kernel2, unaryMinus(kernel2, en.getLeft()), Operation.MULTIPLY, en.getRight());
			}
		}
	...
	public boolean isStringAddition() {
		...
		if (left instanceof ExpressionNode && ((ExpressionNode) left).isStringAddition()) {
			return true;
		}
		return right instanceof ExpressionNode && ((ExpressionNode) right).isStringAddition();
	}
	...
	public ArrayList<ExpressionNode> getFactorsWithoutPow() {
		...
		ArrayList<ExpressionNode> factorsWithoutPow = new ArrayList<>(factors.size());
		if (!factors.isEmpty()) {
			for (int i = 0; i < factors.size(); i++) {
				if (factors.get(i).getOperation().equals(Operation.POWER)
						&& (factors.get(i).getRight() instanceof MyDouble
								|| factors.get(i).getRight() instanceof MySpecialDouble)) {
	...
	public String toFractionString(StringTemplate tpl) {
		...
		return ((ExpressionNode) resolve).toFractionStringFlat(tpl, kernel.getLocalization());
	}
	...
	private String toFractionStringFlat(StringTemplate tpl, Localization locale) {
		...
		if (operation == Operation.DIVIDE) {
			...
			if (leftS.startsWith("-")) {
				return "-"
						+ tpl.divideString(
								left, right, leftS.substring(1), right.toValueString(tpl), true, locale);
			}
			return tpl.divideString(left, right, leftS, right.toValueString(tpl), true, locale);
		}
	...
	public boolean endsInNumber(boolean valueForm) {
		...
		if (operation == Operation.NO_OPERATION && left instanceof NumberValue) {
			return (left instanceof MyDouble && ((MyDouble) left).isDigits())
					|| (valueForm
							&& left.isGeoElement()
	...
	public boolean isProperFraction() {
		ExpressionValue unsigned = getUnsigned(this);
		return unsigned.isExpressionNode()
				&& ((ExpressionNode) unsigned).isUnsignedFraction()
	...
	private boolean isUnsignedFraction() {
		if (operation == Operation.DIVIDE) {
			...
			if (leftUnsigned instanceof MyDouble && right.unwrap() instanceof MyDouble) {
				double lt = left.evaluateDouble();
	...
	private ExpressionValue getUnsigned(ExpressionValue expr) {
		if (expr.isOperation(Operation.MULTIPLY)
				&& ExpressionNode.isConstantDouble(expr.wrap().getLeft(), -1)) {
	...
	public static boolean isDefined(ExpressionValue value) {
		...
		return !(def instanceof MyDouble && def.isConstant() && Double.isNaN(def.evaluateDouble()));
	}
