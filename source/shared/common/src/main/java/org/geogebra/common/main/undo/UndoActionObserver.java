...
public class UndoActionObserver implements PropertyValueObserver<Object> {
	...
	private static List<GeoElement> skipTextObjects(
			List<GeoElement> geos, Predicate<GeoElement> filter) {
		return geos.stream().filter(geo -> !filter.test(geo)).collect(Collectors.toList());
	}

	private static List<GeoInline> keepOnlyTextObjects(
			List<GeoElement> geos, Predicate<GeoElement> filter) {
		return geos.stream().filter(filter).map(geo -> (GeoInline) geo).collect(Collectors.toList());
	}
