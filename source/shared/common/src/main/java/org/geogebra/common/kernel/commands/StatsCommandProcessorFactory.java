...
public class StatsCommandProcessorFactory implements CommandProcessorFactory {
	...
	public CommandProcessor getProcessor(Commands command, Kernel kernel) {
		switch (command) {
			...
			case Cauchy:
			case Weibull:
				return new CmdRealDistribution2Params(
						kernel, ProbabilityCalculatorSettings.Dist.forCommand(command));
			case InverseNormal:
			case InverseBeta:
			case InverseGamma:
			...
			case InverseCauchy:
			case InverseWeibull:
				return new CmdInverseRealDistribution2Params(
						kernel, ProbabilityCalculatorSettings.Dist.forInverse(command));
			case TDistribution:
			case Exponential:
			case ChiSquared:
				return new CmdRealDistribution1Param(
						kernel, ProbabilityCalculatorSettings.Dist.forCommand(command));
			case InverseExponential:
			case InverseChiSquared:
			case InverseTDistribution:
				return new CmdInverseRealDistribution1Param(
						kernel, ProbabilityCalculatorSettings.Dist.forInverse(command));
			case HyperGeometric:
