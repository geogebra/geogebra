...
public class HatchingHandler {
	 ...
	 */
	public final GPaint getHatchingTexture(
			GBasicStroke defObjStroke,
			GColor color,
			GColor bgColor,
		...
		ExportType exportType =
				app.isHTML5Applet() && app.isExporting() ? app.getExportType() : ExportType.NONE;
		...
		bufferedImage = null;
		GGraphics2D g2d = createImage(
				bgColor, backgroundTransparency, xInt * exportScale, yInt * exportScale, exportType);
		g2d.setColor(color);
		GBasicStroke objStroke = defObjStroke;
		if (exportScale != 1) {
			objStroke = AwtFactory.getPrototype().newBasicStroke(objStroke.getLineWidth() * exportScale);
		}
		...
		switch (fillType) {
			...
			case BRICK:
				if (angle == 0
						|| DoubleUtil.isEqual(Math.PI, angle, 10E-8)
			...
			case SYMBOLS:
				...
				GFont font = app.getFontCanDisplay(symbol).deriveFont(GFont.PLAIN, (int) (dist * 2.5));
				g2d.setFont(font);
				GTextLayout t =
						AwtFactory.getPrototype().newTextLayout(symbol, font, g2d.getFontRenderContext());
				int tileSize = (int) (Math.round(t.getAscent() + t.getDescent()) / 3);
				g2d = createImage(bgColor, backgroundTransparency, tileSize, tileSize, exportType);
				g2d.setColor(color);
				g2d.setStroke(objStroke);
				g2d.setFont(app.getFontCanDisplay(symbol).deriveFont(GFont.PLAIN, 24 * exportScale));
		...
		if (bufferedImage == null) {
			return new VectorPatternPaint(
					...
					exportType == ExportType.SVG
							? VectorPatternPaint.VectorType.SVG
							: VectorPatternPaint.VectorType.PDF);
		} else {
			return AwtFactory.getPrototype()
					.newTexturePaint(
							bufferedImage.getSubimage(startX, startY, width, height),
							AwtFactory.getPrototype()
									.newRectangle(0, 0, width / exportScale, height / exportScale));
		}
	}

	private GGraphics2D createImage(
			GColor bgColor, double backgroundTransparency, int xInt, int yInt, ExportType exportType) {
		GGraphics2D g2d;
		if (exportType == ExportType.PDF_HTML5) {
			...
			bufferedImage = AwtFactory.getPrototype().newBufferedImage(xInt * 3, yInt * 3, 1);
		...
		if (backgroundTransparency > 0) {
			...
			if (exportType == ExportType.PDF_HTML5) {
				...
				g2d.setColor(GColor.newColor(
						base.getRed(), base.getGreen(), base.getBlue(), (int) (backgroundTransparency * 255f)));
			}
	...
	public void setTexture(
			GGraphics2D g3, MyImage image, GeoElementND fallback, GColor bgColor, double alpha) {
		...
		GRectangle tr =
				AwtFactory.getPrototype().newRectangle(0, 0, image.getWidth(), image.getHeight());

		GPaint tp;

		if (alpha < 1.0f) {
			GBufferedImage copy =
					AwtFactory.getPrototype().newBufferedImage(image.getWidth(), image.getHeight(), 1);
			...
			if (alpha > 0.0f) {
				...
				GAlphaComposite ac = AwtFactory.getPrototype().newAlphaComposite(alpha);
	...
	private static void drawDotted(double dist, GGraphics2D g2d, double size) {
		g2d.fill(AwtFactory.getPrototype().newEllipse2DDouble(dist, dist, size, size));
		g2d.fill(AwtFactory.getPrototype().newEllipse2DDouble(2 * dist, dist, size, size));
		g2d.fill(AwtFactory.getPrototype().newEllipse2DDouble(dist, 2 * dist, size, size));
		g2d.fill(AwtFactory.getPrototype().newEllipse2DDouble(2 * dist, 2 * dist, size, size));
	}
	...
	private void drawChessboard(double hatchDist, GGraphics2D g2d) {
		...
		g2d.fillRect(distInt + distInt / 2, distInt + distInt / 2, distInt, distInt);
	}
	...
	private static void drawHatching(double angle, double y, int xInt, int yInt, GGraphics2D g2d) {
		if (angle == 0) { // horizontal
