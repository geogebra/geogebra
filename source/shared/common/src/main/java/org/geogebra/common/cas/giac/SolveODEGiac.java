...
public class SolveODEGiac {
	...
	public static void add(BiConsumer<String, String> commandMap) {
						...
						"[solveodeans:="
								+ when(
										...
										when(
												...
												"normal(map(desolve(%0,x,y),x->y=x))", "normal(map(desolve(%0),x->y=x))"),
		...
		commandMap.accept("SolveODEPoint.2", "regroup(check_derivative(desolve(y'=%0,x,y,%1),%1))");
		...
		commandMap.accept(
				"SolveODE.5", // SolveODE[y''=x,y,x,A,{B}]
