...
public class CmdCommonDenominator extends CommandProcessor {
	...
	public final GeoElement[] process(Command c, EvalInfo info) throws MyError {
		...
		switch (n) {
			case 2:
				arg = resArgs(c, info);
				if ((ok[0] = arg[0] instanceof NumberValue) && (ok[1] = arg[1] instanceof GeoNumberValue)) {
					AlgoCommonDenominator algo = new AlgoCommonDenominator(
							cons, c.getLabel(), (GeoNumberValue) arg[0], (GeoNumberValue) arg[1]);
					GeoElement[] ret = {algo.getResult()};
					return ret;

				} else if ((ok[0] = arg[0].isGeoFunction()) && (ok[1] = arg[1].isGeoFunction())) {
					AlgoPolynomialCD algo =
							new AlgoPolynomialCD(cons, c.getLabel(), (GeoFunction) arg[0], (GeoFunction) arg[1]);
					GeoElement[] ret = {algo.getResult()};
					return ret;
				} else {
					if (!ok[0]) {
