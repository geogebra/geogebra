...
public class DrawLine extends SetDrawable implements Previewable {
	...
	public void update(CoordMatrix matrix) {
		...
		if (isVisible) {
			...
			setClippedLine(
					view.getMinXScreen(), view.getMinYScreen(), view.getMaxXScreen(), view.getMaxYScreen());
	...
	private void setClippedLine(double minx, double miny, double maxx, double maxy) {
	 ...
	 *
	 */
	public void drawStylePreview(GGraphics2D g2, int marginX, int marginY, int width) {
		updateStrokes(geo);
		g2.setStroke(objStroke);
			...
			switch (previewMode) {
				...
				case LINE:
					...
					this.coordsForMousePos.setCrossProduct4(
							view.getCoordsForView(startPoint.getInhomCoordsInD3()).projectInfDim(),
							new Coords(xRW, yRW, 1));

					((GeoLine) g)
							.setCoords(
									coordsForMousePos.getX(), coordsForMousePos.getY(), coordsForMousePos.getZ());
				...
				case PARALLEL:
					...
					GeoVec3D.cross(xRW, yRW, 1.0, linePreview.getY(), -linePreview.getX(), 0.0, (GeoLine) g);
					break;
				case PERPENDICULAR:
					if (functions.size() == 1) {
						linePreview = functions.get(0);
					...
					GeoVec3D.cross(xRW, yRW, 1.0, linePreview.getX(), linePreview.getY(), 0.0, (GeoLine) g);
					break;
				case PERPENDICULAR_BISECTOR:
				...
				case ANGLE_BISECTOR:
					...
					coordsForMousePos.setCrossProduct4(
							previewPoint2.getCoordsInD2(), startPoint.getCoordsInD2());
					g1.setCoords(
							coordsForMousePos.getX(), coordsForMousePos.getY(), coordsForMousePos.getZ());
					coordsForMousePos.setCrossProduct4(
							previewPoint2.getCoordsInD2(), new Coords(xRW, yRW, 1));
					h.setCoords(coordsForMousePos.getX(), coordsForMousePos.getY(), coordsForMousePos.getZ());
	 ...
	 */
	@Override
	public final boolean hit(int screenx, int screeny, int hitThreshold) {
		return line != null
				&& line.intersects(
						screenx - hitThreshold, screeny - hitThreshold, 2 * hitThreshold, 2 * hitThreshold);
	}
	...
	@Override
	public final void setGeoElement(GeoElement geo) {
		this.geo = geo;
	}
