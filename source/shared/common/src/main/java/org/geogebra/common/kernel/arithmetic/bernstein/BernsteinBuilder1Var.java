...
public class BernsteinBuilder1Var {
	...
	BernsteinPolynomial1D build(
			double[] powerCoeffs, int degree, char variable, double min, double max) {
	...
	private double bernsteinCoefficient(
			int i, int j, int degree, double min, double max, double[] lastValues) {
		if (i == 0 && j == 0) {
		...
		return binomial * a_nMinusI + min * lastValues[j] + max * lastValues[j - 1];
	}
}
