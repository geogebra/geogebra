...
public class Ggb2giac {
	...
	public static final String ELEMENT_2 =
	...
	public static Map<String, String> getMap() {
		...
		p("Append.2", "when(type(%0)==DOM_LIST,append(%0,%1),flatten1({%0,%1}))");
		...
		p("BinomialDist.3", "simplify(" + binom3Command + ")");
		...
		// 2 vars with eq in 2nd degree -> conic, 3 vars with eq in 2nd degree -> quadric, everything
		// else is a polynomial
		...
		p("CountIf.3", "[[ggbcount(%1):=begin when(%0,1,0);end],sum(map(%2,loc->ggbcount(loc)))][1]");
		p("CountIf.2", "[[ggbcount(x):=begin when(%0,1,0);end],sum(map(%1,loc->ggbcount(loc)))][1]");
		...
		p(
				"Evaluate.1",
				"[[[ggbevalarg:=0/0],[ggbevalarg:=%0],[ggbevalans:=regroup(normal(ggbevalarg))],[ggbevalans2:=regroup(ggbevalarg)]],when(length(\"\"+ggbevalans)<=length(\"\"+ggbevalans2),ggbevalans,ggbevalans2)][1]");
		...
		String extremumPrefix =
				"[[[ggbextremumfun:=when((%0)[0]==equal&&(%0)[1]=='y',(%0)[2],%0)],[ggbextvar:=when(size(lname(ggbextremumfun) intersect [x])==0,lname(ggbextremumfun)[0],x)],";
		String extremumAns =
				"[ggbextans:=extrema(when((%0)[0]==equal&&(%0)[1]=='y',(%0)[2],%0))]],map(ggbextans,it->point(it,normal(regroup(subst(ggbextremumfun,ggbextvar,it)))))][1]";
		p("Extremum.1", extremumPrefix + extremumAns);
		p("Extremum.3", extremumPrefix + "[assume(%1<x && %2>x)]," + extremumAns);
		...
		String iFactor =
				"[[ggbfacans:=%0],[if type(ggbfacans)==DOM_INT then ggbfacans:=ifactor(ggbfacans);"
		...
		String integralPart1 =
				"[[ggbintans:=0/0], [ggbintanssimplified:=0/0], [arg0:=REPLACEME0], [vars := lname(arg0)], [factored:=factors(arg0)],";
		...
		p(
				"Integral.3",
				(integralPart1 + integralPart2 + integralPart3)
						.replace("REPLACEME0", "%0")
						.replace("REPLACEME1", "x")
						.replace("REPLACEME2", "%1")
						.replace("REPLACEME3", "%2"));

		p(
				"Integral.4",
				(integralPart1 + integralPart3)
						.replace("REPLACEME0", "%0")
						.replace("REPLACEME1", "%1")
		...
		p(
				...
				(integralPart1 + integralPart3)
						.replace("REPLACEME0", "%0-(%1)")
						.replace("REPLACEME1", "%2")
						.replace("REPLACEME2", "%3")
		...
		p(
				...
				"[[ggbinterans:=0/0]," + "[ggbinarg0:=%0],"
						...
						// Helper functions used to shorten the String we send to giac
						+ "[ggb_is_cas_3d_line(arg1):=when((arg1)[0]==equal&&(arg1)[1]=='ggbtmpvarX',true,false)],"
						+ "[ggb_is_3d_line(arg1):=when((xcoord(arg1))[0]==equal&&string((xcoord(arg1))[1])==string(X),true,false)],"
						+ "[ggb_extract_point(arg1):=point(xcoord(arg1),ycoord(arg1),zcoord(arg1))],"
						+ "[ggb_add_points(p1,p2):=point(xcoord(p1)+xcoord(p2),ycoord(p1)+ycoord(p2),zcoord(p1)+zcoord(p2))],"
						+ "[ggb_line_eval(arg1,uval):=point(re(subst(parameq(line(arg1),u),u=uval)),"
						+ "im(subst(parameq(line(arg1),u),u=uval)),0)],"
						+ "[ggb_line_for_3d_line(arg1):=line(point("
						+ "(arg1)[0][2],(arg1)[1][2],(arg1)[2][2][1]),"
						+ "point((arg1[2][2][2])[2][0]+(arg1)[0][2],"
						+ "(arg1[2][2][2])[2][1]+ (arg1)[1][2],"
						+ "(arg1[2][2][2])[2][2]+(arg1)[2][2][1]))],"
						+ "[ggb_line_from_cas_3d_line(arg1):=line(point("
						+ "(arg1)[2][1][0][1],"
						+ "(arg1)[2][1][1][1],"
						+ "(arg1)[2][1][2][1]),"
						+ "point(coeffs((arg)[2][1][0],ggbtmpvar"
						+ Unicode.lambda + ",1)+(arg1)[2][1][0][1],"
						+ "coeffs((arg)[2][1][1],ggbtmpvar"
						+ Unicode.lambda + ",1)+(arg1)[2][1][1][1],"
						+ "coeffs((arg)[2][1][2],ggbtmpvar"
						+ Unicode.lambda + ",1)+(arg1)[2][1][2][1]))],"
						+ "[ggb_line_from_3d_line(arg1):=line(point("
						+ "expr(arg1)[0][2][1],"
						+ "expr(arg1)[1][2][1],"
						+ "expr(arg1)[2][2][1]),"
						+ "point(coeff(expr(arg1)[0][2]," + Unicode.lambda + ",1)"
						+ "+expr(arg1)[0][2][1],"
						+ "coeff(expr(arg1)[1][2]," + Unicode.lambda + ",1)"
						+ "+expr(arg1)[1][2][1],"
						+ "coeff(expr(arg1)[2][2]," + Unicode.lambda + ",1)"
						+ "+expr(arg1)[2][2][1]))],"
						...
						+ "ggb_extract_point((line_inter(ggbinarg0,ggbinarg1))[1][0]),"
						+ "\"+" + Unicode.lambda + "*\","
						+ "ggb_extract_point((line_inter(ggbinarg0,ggbinarg1))[1][1]-(line_inter(ggbinarg0,ggbinarg1))[1][0])"
						+ ")))),"

						// Intersect[Plane,Cas3dLine]
						+ "when(ggb_is_cas_3d_line(ggbinarg1),"
						+ "inter(ggbinarg0,"
						+ "ggb_line_from_cas_3d_line(ggbinarg1)"
						+ ")[0],"
						...
						+ "line_inter(line("
						+ "ggb_extract_point((ggbinarg3)[2][1])"
						+ ","
						+ "ggb_add_points(ggbinarg3[2][2][2],(ggbinarg3)[2][1])"
						+ "),ggbinarg0),"

						// Intersect[Plane,3dLine]
						+ "when(ggb_is_3d_line(ggbinarg1),"
						+ "line_inter(ggbinarg0,"
						+ "ggb_line_for_3d_line(ggbinarg1)"
						+ "),"
						...
						+ "line_inter(ggbinarg0,"
						+ "line("
						+ "ggb_line_eval(ggbinarg1,1),"
						+ "ggb_line_eval(ggbinarg1,2)"
						+ ")),?))))),"
						...
						+ "when((ggbinarg1)[0]=='pnt',"
						+ "line_inter(line("
						+ "ggb_extract_point((ggbinarg2)[2][1])"
						+ ","
						+ "ggb_add_points(ggbinarg2[2][2][2],(ggbinarg2)[2][1])"
						+ "),ggbinarg1),"

						// Intersect[Cmd3dLine,Cas3dLine]
						+ "when(ggb_is_cas_3d_line(ggbinarg1),"
						+ "inter("
						+ "ggb_line_from_3d_line(ggbinarg0),"
						+ "ggb_line_from_cas_3d_line(ggbinarg1)"
						+ ")[0],"
						...
						+ "when(ggb_is_3d_line(ggbinarg0),"
						+ "when((ggbinarg1)[0]=='pnt',"
						+ "line_inter("
						+ "ggb_line_for_3d_line(ggbinarg0)"
						+ ",ggbinarg1),"
						...
						+ "ggb_line_for_3d_line(ggbinarg0)"
						+ ","
						+ "ggb_line_from_3d_line(ggbinarg1)"
						+ "),"
						...
						+ "when(ggb_is_cas_3d_line(ggbinarg0),"
						+ "when(ggb_is_cas_3d_line(ggbinarg1),"
						+ "inter("
						+ "ggb_line_from_cas_3d_line(ggbinarg0),"
						+ "ggb_line_from_cas_3d_line(ggbinarg1))[0],"
						...
						+ "ggb_line_from_cas_3d_line(ggbinarg0)"
						+ ","
						+ "ggb_line_from_3d_line(ggbinarg1))[0],"
						...
						+ "line_inter(line("
						+ "ggb_line_eval(ggbinarg0,1),"
						+ "ggb_line_eval(ggbinarg0,2)"
						+ "),ggbinarg1),"
		...
		p(
				"Length.1",
				"[[ggbv:=%0],regroup(when(type(ggbv)==DOM_LIST&&subtype(ggbv)!=" + GGBVECT_TYPE
						+ ",size(ggbv),when(real(ggbv)==ggbv&&subtype(ggbv)!=" + GGBVECT_TYPE
						+ ",?,l2norm(ggbv))))][1]");
		...
		p(
				"Limit.2",
				"[[ggblimvans:=0/0],[ggblimvans:=limit(%0,%1)],[ggblimvans:=when(ggblimvans==inf||ggblimvans==-inf||ggblimvans==undef,ggblimvans,regroup(ggblimvans))],ggblimvans][3]");
		p(
				"Limit.3",
				"[[ggblimvans:=0/0],[ggblimvans:=limit(%0,%1,%2)],[ggblimvans:=when(ggblimvans==inf||ggblimvans==-inf||ggblimvans==undef,ggblimvans,regroup(ggblimvans))],ggblimvans][3]");
		p(
				"LimitAbove.2",
				"[[ggblimvans:=0/0],[ggblimvans:=limit(%0,x,%1,1)],[ggblimvans:=when(ggblimvans==inf||ggblimvans==-inf||ggblimvans==undef,ggblimvans,regroup(ggblimvans))],ggblimvans][3]");
		p(
				"LimitAbove.3",
				"[[ggblimvans:=0/0],[ggblimvans:=limit(%0,%1,%2,1)],[ggblimvans:=when(ggblimvans==inf||ggblimvans==-inf||ggblimvans==undef,ggblimvans,regroup(ggblimvans))],ggblimvans][3]");
		p(
				"LimitBelow.2",
				"[[ggblimvans:=0/0],[ggblimvans:=limit(%0,x,%1,-1)],[ggblimvans:=when(ggblimvans==inf||ggblimvans==-inf||ggblimvans==undef,ggblimvans,regroup(ggblimvans))],ggblimvans][3]");
		p(
		...
		p(
				"Normal.4",
				"[[[ggbnormarg0:=%0],[ggbnormarg1:=%1],[ggbnormarg2:=%2], [ggbnormarg3:=%3]],if (ggbnormarg3+\"\")==\"true\" then normald_cdf(ggbnormarg0,ggbnormarg1,ggbnormarg2) "
						+ "elif (ggbnormarg3+\"\")==\"false\" then (1/sqrt(2*pi*((ggbnormarg1)^2))) * exp(-((ggbnormarg2-(ggbnormarg0))^2)/(2*((ggbnormarg1)^2)))"
						+ "else normald_cdf(ggbnormarg0,ggbnormarg1,ggbnormarg2,ggbnormarg3) fi][1]");
		...
		p(
				...
				"ggbsort([[ggbnsans:=(%%0)],[ggbnsans:=when(type(ggbnsans)==DOM_LIST,"
						...
						// regroup() added for
						// NSolve[BC^2=4^2+3^2-2*4*3*cos(50deg)]
		...
		String numericInit =
				"[[ggbnumans:=0/0],[ggbnumans:=%0],when(dim(lname(ggbnumans))==0||count_eq(unicode0176u,lname(ggbnumans))>0,";
		p(
				"Numeric.1",
				numericInit
						+
						...
						+ "," +
						// #4537
						"normal(evalf(regroup(ggbnumans),13))"
						+ ")][2]");
		...
		p(
				"Q1.1",
				"[[ggblist:=sort(%0)],[ggbmedian:=median(ggblist)], [ggbsublist:=at(ggblist,0..floor(length(ggblist)/2)-1)], when(length(ggblist)<2,undef, median(ggbsublist))][3]");

		// return the number q3 such that three-fourths of the list numbers fall at or below q3.
		p(
				"Q3.1",
				"[[ggblist:=sort(%0)],[ggbmedian:=median(ggblist)], [ggbsublist:=at(ggblist, (ceil(length(ggblist)/2))..(length(ggblist)-1))], when(length(ggblist)<2,undef, median(ggbsublist))][3]");
		...
		p(
				"ScientificText.1",
				" [[[ggbstinput:=%0],[ggbstans:=0/0],[ggbstabsans:=abs(ggbstinput)],[ggbstpower:=evalf(floor(log10(ggbstabsans)))],"
		...
		// lname(0/0)[0] used instead of ? because ? is short-circuiting some commands
		p(
				"Simplify.1",
		...
		p("Sum.4", "expand(sum(quote(%0),%1,%2,%3))");

		p("Product.4", "expand(product(quote(%0),%1,%2,%3))");
		...
		p(
				"ToComplex.1",
				"[[ggbtcans:=0/0],[ggbtcans:=%0],[ggbtype:=type(evalf(ggbtcans))],"
						+
						// ToComplex[3.1]
		...
		p(
				"TrigExpand.1",
				"[[[ggbsimparg0:=%0],[ggbsimpans:=0/0],[ggbsimpans:=tan2sincos(trigexpand(ggbsimparg0))],[ggbsimpans2:=tlin(ggbsimparg0)]],"
		...
		// evalCmd=""/>
		String centerMidpoint1 =
				"[[ggbmidarg0:=%0],[ggbcoordarg0:=when(count_eq(z,lname(ggbmidarg0))==0,"
		...
		p(
				"OrthogonalLine.2",
				"equation(perpendicular(%0,when(subtype(%1)==" + GGBVECT_TYPE + ",%1,line(%1))))");
		...
		p(
				"Vector.2",
				"when(is_3dpoint(%0)||is_3dpoint(%1)," +
						// 3D points
		...
		p(
				"ExtendedGCD.2",
				"when((type(%0)==DOM_INT)&&(type(%1)==DOM_INT), "
						+ "iegcd(%0,%1), "
		...
		p(
				"LUDecomposition.1",
				"[[luarg:=%0], [lu0:=lu(luarg)], [lu1:=lu0[0]],[lu2:=lu0[1]],[lu3:=lu0[2]],[permu2mat(lu1),lu2,lu3]][-1]");
	...
	private static void pOptimize(String optimize, String optimizeGiac, String optimizeGiacFun) {
		...
		p(
				optimize + ".2",
				"[[ggbparam1:=%0],[ggbparam2:=%1],"
						+ "when(type(ggbparam1)==DOM_LIST&&type(ggbparam2)==DOM_LIST,"
						+ optimizeGiac
						+ "(map(remove(x->(x[1]<=0), zip((x,y)->[x,y], ggbparam1, ggbparam2)), x->x[0])),"
						+ optimizeGiac + "(ggbparam1,ggbparam2))][2]");
		p(
				optimize + ".3",
				"[[ggbfun:=%0],[ggbinta:=%1],[ggbintb:=%2],"
