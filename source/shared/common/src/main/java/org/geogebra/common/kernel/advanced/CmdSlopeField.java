...
public class CmdSlopeField extends CommandProcessor {
		...
		switch (n) {
			case 1:
				if (arg[0] instanceof Evaluate2Var) {
					GeoElement[] ret = {
						slopeField(c.getLabel(), (Evaluate2Var) arg[0], null, null, null, null, null, null)
					};
					return ret;
				}
			...
			case 2:
				if ((ok[0] = arg[0] instanceof Evaluate2Var) && (ok[1] = arg[1].isGeoNumeric())) {
					GeoElement[] ret = {
						slopeField(
								c.getLabel(),
								(Evaluate2Var) arg[0],
	...
	private GeoLocus slopeField(
		...
		AlgoSlopeField algo =
				new AlgoSlopeField(cons, label, func, n, lengthRatio, minX, minY, maxX, maxY);
