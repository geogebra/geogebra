...
public class GeoIntervalUtil {
	 ...
	 *            bounds as doubles
	 */
	public static void updateBoundaries(ExpressionNode en, double[] leftRightDouble) {
	...
	private static void doUpdateBoundaries(ExpressionNode en, double[] leftRightDouble) {
		Operation op = en.getOperation();
		if (op.equals(Operation.AND) || op.equals(Operation.AND_INTERVAL)) {
		...
		} else if (op.equals(Operation.LESS) || op.equals(Operation.LESS_EQUAL)) {
			// x < c
			updateBoundariesSingle(en.getLeft(), en.getRight(), leftRightDouble);
		} else if (op.equals(Operation.GREATER) || op.equals(Operation.GREATER_EQUAL)) {
	...
	private static void updateBoundariesSingle(
			ExpressionValue smaller, ExpressionValue greater, double[] leftRightDouble) {
		if (smaller instanceof FunctionVariable && isConstant(greater)) {
			leftRightDouble[1] = Math.min(leftRightDouble[1], greater.evaluateDouble());
		} else if (greater instanceof FunctionVariable && isConstant(smaller)) {
