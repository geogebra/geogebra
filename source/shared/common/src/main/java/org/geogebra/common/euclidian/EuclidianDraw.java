...
public class EuclidianDraw {
	...
	public static DrawableND newDrawable(EuclidianView ev, GeoElementND geo) {
		...
		switch (geo.getGeoClassType()) {
			...
			case PENSTROKE:
				d = new DrawLocus(ev, ((GeoLocusNDInterface) geo).getLocus(), CoordSys.XOY);
				d.update();
				break;
			case SHAPE_STADIUM:
			...
			case NUMERIC:
				...
				if (d != null) {
					if (!geo.isColorSet()) {
						...
						if (geo.isIndependent()) {
							GColor col =
									consDef.getDefaultGeo(ConstructionDefaults.DEFAULT_NUMBER).getObjectColor();
							geo.setObjColor(col);
							geo.setLineThickness(GeoNumeric.DEFAULT_SLIDER_THICKNESS);
							geo.setLineType(EuclidianStyleConstants.LINE_TYPE_FULL);
						} else {
							GColor col =
									consDef.getDefaultGeo(ConstructionDefaults.DEFAULT_POLYGON).getObjectColor();
							geo.setObjColor(col);
						}
					}
				}
			...
			case FUNCTION:
				if (((GeoFunction) geo).isBooleanFunction()) {
					...
					d = ev.getCompanion().newDrawParametricCurve((ParametricCurve) geo);
					d.update();
				}
				break;
			case FORMULA:
