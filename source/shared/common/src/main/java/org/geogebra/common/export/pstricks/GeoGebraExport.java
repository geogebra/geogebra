...
public abstract class GeoGebraExport {
	...
	protected static final double PRECISION_XRANGE_FUNCTION = 0.00001;
	...
	public void refreshSelectionRectangle() {
		...
		GRectangle rec = AwtFactory.getPrototype().newRectangle(x1, y2, x2 - x1, y1 - y2);
	...
	protected void drawGeoElement(GeoElement g, boolean fromGeoList, boolean trimmedInter) {
		if (g.isGeoList()) {
			...
			// To draw Inequalities
				...
				Log.debug("Export: unsupported GeoElement " + g.getGeoClassType() + " " + g.isDrawable());
			}
		}
	}
	...
	protected void drawBarChartOrHistogram(GeoNumeric g) {
		...
		if (g.getParentAlgorithm() instanceof AlgoFunctionAreaSums) {
			AlgoFunctionAreaSums algo = (AlgoFunctionAreaSums) g.getParentAlgorithm();
			drawHistogramOrBarChartBox(
					algo.getValues(), algo.getLeftBorder(), algo.getValues().length - 1, 0, g);
		} else if (g.getParentAlgorithm() instanceof AlgoBarChart) {
			AlgoBarChart algo = (AlgoBarChart) g.getParentAlgorithm();
			drawHistogramOrBarChartBox(
					algo.getValues(), algo.getLeftBorder(), algo.getValues().length, algo.getWidth(), g);
		}
	}

	protected boolean isSinglePointConic(GeoElementND geo) {
		if (geo.isGeoConic()) {
			if (((GeoConicND) geo).getType() == GeoConicNDConstants.CONIC_SINGLE_POINT) {
				return true;
			}
		}
		return false;
	...
	protected boolean isDoubleLineConic(GeoElementND geo) {
		if (geo.isGeoConic()) {
			if (((GeoConicND) geo).getType() == GeoConicNDConstants.CONIC_DOUBLE_LINE
					|| ((GeoConicND) geo).getType() == GeoConicNDConstants.CONIC_INTERSECTING_LINES
					|| ((GeoConicND) geo).getType() == GeoConicNDConstants.CONIC_PARALLEL_LINES) {
				return true;
			}
		}
		return false;
	...
	protected boolean isEmpty(GeoElementND geo) {
		if (geo.isGeoConic()) {
			if (((GeoConicND) geo).getType() == GeoConicNDConstants.CONIC_EMPTY) {
				return true;
			}
		}
		return false;
	 ...
	 *            The point to export
	 */
	protected abstract void drawGeoPoint(GeoPointND geo);
	 ...
	 *            The line to export
	 */
	protected abstract void drawGeoLine(GeoLine geo);
	 ...
	 *            The ray to export
	 */
	protected abstract void drawGeoRay(GeoRayND geo);
	 ...
	 */
	protected abstract void drawGeoSegment(GeoSegmentND geo);
	 ...
	 */
	protected abstract void drawPolygon(GeoPolygon geo);
	 ...
	 */
	protected abstract void drawSlider(GeoNumeric geo);

	protected abstract void drawSlope(GeoNumeric geo);
	 ...
	 */
	protected abstract void drawIntegral(GeoNumeric geo);
	 ...
	 *            The object to export
	 */
	protected abstract void drawIntegralFunctions(GeoNumeric geo);
	 ...
	 *            The object to export
	 */
	protected abstract void drawSumUpperLower(GeoNumeric geo);
	 ...
	 *            The object to export
	 */
	protected abstract void drawSumTrapezoidal(GeoNumeric geo);
	 ...
	 */
	protected abstract void drawBoxPlot(GeoNumeric geo);
	 ...
	 *            The angle to export
	 */
	protected abstract void drawAngle(GeoAngle geo);
	 ...
	 */
	protected abstract void drawGeoVector(GeoVector geo);
	 ...
	 *            The conic to export
	 */
	protected abstract void drawGeoConic(GeoConicND geo);
	 ...
	 */
	protected abstract void drawGeoConicPart(GeoConicPart geo);
	 ...
	 */
	protected abstract void drawLabel(GeoElementND geo, DrawableND drawGeo);
	 ...
	 */
	protected abstract void drawFunction(GeoFunction geo);
	 ...
	 *            The function to export
	 */
	protected abstract void drawImplicitPoly(GeoImplicit geo);
	 ...
	 *            The function to export
	 */
	protected abstract void drawSingleCurveCartesian(GeoCurveCartesian geo, boolean transparency);
	 ...
	 *            The text to export
	 */
	protected abstract void drawText(GeoText geo);
	 ...
	 *            The locus to export
	 */
	protected abstract void drawLocus(GeoLocus geo);
	 ...
	 *            geo Object
	 */
	protected abstract void drawLine(double x1, double y1, double x2, double y2, GeoElementND geo);
	 ...
	 *            radius
	 */
	protected abstract void drawArc(
			GeoAngle geo, double[] vertex, double angSt, double angEnd, double r);
	 ...
	 *            angle
	 */
	protected abstract void drawTick(GeoAngle geo, double[] vertex, double angleTick);

	protected abstract void drawArrowArc(
			GeoAngle geo, double[] vertex, double angSt, double angEnd, double r, boolean clockwise);
	 ...
	 */
	protected abstract void colorCode(GColor color, StringBuilder sb);
	 ...
	 */
	protected abstract void drawPolyLine(GeoPolyLine geo);
	 ...
	 *            For visual style
	 */
	protected abstract void drawHistogramOrBarChartBox(
			double[] values, double[] leftBorder, int length, double width, GeoNumeric g);
	...
	protected abstract boolean fillSpline(GeoCurveCartesian[] curves);
	...
	protected void mark(double[] A, double[] B, int deco, GeoElementND geo) {
		...
		switch (deco) {
			...
			case GeoElementND.DECORATION_SEGMENT_TWO_ARROWS:
				...
				y1 = euclidianView.toRealWorldCoordY(midY - 2 * arrowlength * vy);
				x2 =
						euclidianView.toRealWorldCoordX(midX - 2 * arrowlength * vx + arrowlength * (-nx + vx));
				y2 =
						euclidianView.toRealWorldCoordY(midY - 2 * arrowlength * vy + arrowlength * (-ny + vy));
				drawLine(x1, y1, x2, y2, geo);
			...
			case GeoElementND.DECORATION_SEGMENT_THREE_ARROWS:
				...
				y1 = euclidianView.toRealWorldCoordY(midY - 3 * arrowlength * vy);
				x2 =
						euclidianView.toRealWorldCoordX(midX - 3 * arrowlength * vx + arrowlength * (-nx + vx));
				y2 =
						euclidianView.toRealWorldCoordY(midY - 3 * arrowlength * vy + arrowlength * (-ny + vy));
				drawLine(x1, y1, x2, y2, geo);
				break;
		}
	}
	 ...
	 *            Angle End
	 */
	protected void markAngle(GeoAngle geo, double r0, double[] vertex, double angSt, double angEnd) {
		double rdiff;
		double r = r0;
	...
	protected void drawAllElements() {
		boolean increment =
				euclidianView.getShowGrid() || euclidianView.getShowXaxis() || euclidianView.getShowYaxis();
	 ...
	 */
	protected final StringBuilder drawNoLatexFunction(
			CurveEvaluable geo, double xRangeMax, double xRangeMin, int point, String template) {
	...
	private void drawCurveCartesian(GeoElementND geo) {
		if (!isLatexFunction(geo.toValueString(StringTemplate.noLocalDefault))) {
			...
			if (f.getFunctionExpression().getOperation() == Operation.IF_LIST) {
				...
				ListValue exr = (ListValue) f.getFunctionExpression().getRight();
				double[] paramValues = new double[exl.size() + 2];
				paramValues[0] = 0;
				for (int i = 0; i < exl.size(); i++) {
					paramValues[i + 1] = exl.get(i).wrap().getRight().evaluateDouble();
				}
				...
				StringBuilder lines = drawNoLatexFunction(
						curve, curve.getMaxParameter(), curve.getMinParameter(), 400, getLineTemplate(geo));
				code.append(lines);
			}
		} else {
	...
	private Function asFunction(ExpressionValue listElement) {
		return new Function(listElement.deepCopy(kernel).wrap(), new FunctionVariable(kernel, "t"));
	}
	...
	protected static void renameFunc(StringBuilder sb, String nameFunc, String nameNew) {
	...
	protected int getWidth(String st, StringBuilder sb, GFont font) {
		...
		for (int i = 0; i < lines.length; i++) {
			String line = lines[i];
			width =
					Math.max(width, (int) Math.ceil(StringUtil.getPrototype().estimateLength(line, font)));
			sb.append(line);
			if (i < lines.length - 1) {
				sb.append(" \\\\ ");
			}
