...
public class VertexShader {
	...
	private static final String vertexHeaderDesktop = "#if __VERSION__ >= 130 "
	...
	private static final String inUniform = "\n"
			+ "uniform mat4 matrix;\n"
			+ "uniform vec3 lightPosition;\n"
			+ "uniform vec4 eyePosition;\n"
			+ "uniform vec2 ambientDiffuse;\n"
	...
	private static final String light = "  if (enableLight == 1){// color with light\n"
			+ "    float factor = dot(n, lightPosition);\n"
			+ "    factor = float(culling) * factor;\n"
			+ "    factor = max(0.0, factor);\n"
			+ "    float ambient = ambientDiffuse[0];\n"
			+ "    float diffuse = ambientDiffuse[1];\n"
			...
			+ "    varying_Color.rgb = (ambient + diffuse * factor) * c.rgb;\n"
			+ "    varying_Color.a = c.a;\n"
			+ "  }else{ //no light\n"
	...
	private static final String depthToColorString = "  float gray = gl_Position.z + 0.5;\n"
			+ "  varying_Color.r = gray;\n"
			+ "  varying_Color.g = gray;\n"
			+ "  varying_Color.b = gray;\n"
			+ "  coordTexture = attribute_Texture;\n";

	private static final String shiny_packed = inUniform
			+ "\n"
			...
			+ "  }\n"
			+ "\n"
			+ (Renderer.TEST_DRAW_DEPTH_TO_COLOR ? depthToColorString : light)
			+ "\n"
			+ "  coordTexture = attribute_Texture;\n"
			+ "}";
