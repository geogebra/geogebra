...
public class CmdKeepIf extends CommandProcessor {
		...
		switch (n) {
			case 3:
				...
				try {
					if (!arg1Str.equals(kernel.getParser().parseLabel(arg1Str))) {
						throw argErr(c, new MyStringBuffer(kernel, arg1Str));
					}
				} catch (ParseException e) {
					throw argErr(c.getName(), new MyStringBuffer(kernel, arg1Str), e);
				}
			...
			case 2:
				...
				if ((ok[0] = args[0] instanceof GeoFunction) && (ok[1] = args[1].isGeoList())) {
					...
					if ((ok[0] = booleanFun.isBooleanFunction() || !booleanFun.isDefined())
	 ...
	 * @return result of the command
	 */
	protected GeoElement[] getResult3(
			ValidExpression c, GeoBoolean arg, GeoElement[] vars, GeoList[] over) {
