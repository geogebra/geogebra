...
final class SpreadsheetRenderer {
	...
	private static final GBasicStroke gridStroke = AwtFactory.getPrototype().newBasicStroke(1);
	private static final GBasicStroke dashedGridStroke = EuclidianStatic.getStroke(
			gridStroke.getLineWidth(), EuclidianStyleConstants.LINE_TYPE_DASHED_SHORT);
	private static final int BORDER_THICKNESS = 2;
	private static final GBasicStroke borderStroke =
			AwtFactory.getPrototype().newBasicStroke(BORDER_THICKNESS);
	private final SpreadsheetStyling styling;
	private final TabularData<?> tabularData;
	private static final int ERROR_TRIANGLE_WIDTH = 10;
	private static final int TEXT_PADDING = 10;
	private final StringRenderer errorRenderer = new StringRenderer();

	private static final int[] REFERENCE_COLOR_RGB_VALUES = {
		0x6557d2, 0xe0bf00, 0x3bb4a6, 0xda6a9d, 0x3b1c32, 0xff8c70
	};
	private static final List<GColor> REFERENCE_COLORS = Arrays.stream(REFERENCE_COLOR_RGB_VALUES)
			.mapToObj(GColor::newColorRGB)
			.collect(Collectors.toList());
	private static final int[] REFERENCE_STROKE_INDICES = {0, 1, 2};
	private static final List<GBasicStroke> referenceStrokes = Arrays.stream(REFERENCE_STROKE_INDICES)
			.mapToObj(index -> AwtFactory.getPrototype()
					.newBasicStroke(
							2,
	...
	SpreadsheetRenderer(
			@NonNull TableLayout layout,
			@NonNull CellRenderableFactory converter,
			@NonNull SpreadsheetStyling styling,
			@NonNull TabularData<?> tabularData) {
	...
	void drawCell(
			int row, int column, GGraphics2D graphics, @Nullable Object content, boolean hasError) {
	...
	private void drawCellBorderIfNeeded(
			int row, int column, GGraphics2D graphics, Rectangle cellBounds) {
	...
	void drawErrorCell(
		...
		if (bounds != null) {
			drawVisibleBorders(
					graphics, bounds, leftCropped, topCropped, leftCropped + width, topCropped + height);
		}
	...
	private void drawErrorString(
			GGraphics2D graphics, double left, double top, double width, double height) {
		graphics.setColor(SpreadsheetStyling.getDefaultTextColor());
		graphics.setClip(left, top, width, height);
		errorRenderer.draw(
				tabularData.getErrorString(),
				converter.getFontSize(),
				GFont.ITALIC,
				TEXT_PADDING,
				graphics,
	...
	void drawRowBorder(int row, GGraphics2D graphics) {
		...
		graphics.drawStraightLine(
				0,
				layout.getMinY(row),
				styling.isShowGrid() ? layout.getTotalWidth() : layout.getRowHeaderWidth(),
				layout.getMinY(row));
	}

	private void ensureHeaders(
			List<SelfRenderable> rowHeaders, int row, Function<Integer, String> nameProvider) {
		for (int i = rowHeaders.size(); i <= row; i++) {
			rowHeaders.add(new SelfRenderable(
					stringRenderer,
					converter.getFontSize(),
					GFont.PLAIN,
					CellFormat.ALIGN_CENTER,
					nameProvider.apply(i)));
		}
	}
	...
	void drawColumnBorder(int column, GGraphics2D graphics) {
		graphics.setStroke(gridStroke);
		graphics.drawStraightLine(
				layout.getMinX(column),
				0,
				layout.getMinX(column),
				styling.isShowGrid() ? layout.getTotalHeight() : layout.getColumnHeaderHeight());
	}

	void drawColumnHeader(int column, GGraphics2D graphics, Function<Integer, String> nameProvider) {
	...
	void drawSelection(TabularRange selection, GGraphics2D graphics, Rectangle viewport) {
		...
		if (bounds != null) {
			...
			fillRect(graphics, bounds.getMinX(), bounds.getMinY(), bounds.getWidth(), bounds.getHeight());
		}
	}

	void drawSelectionBorder(
			TabularRange selection,
			GGraphics2D graphics,
			Rectangle viewport,
		...
		if (bounds != null) {
			...
			graphics.setColor(
					dashed ? styling.getDashedSelectionBorderColor() : styling.getSelectionBorderColor());
	...
	private void drawRectangleWithStraightLines(
			GGraphics2D graphics, double minX, double minY, double maxX, double maxY) {
		graphics.drawStraightLine(minX, minY, maxX, minY);
		graphics.drawStraightLine(minX, maxY, maxX, maxY);
		graphics.drawStraightLine(minX, minY, minX, maxY);
		graphics.drawStraightLine(maxX, minY, maxX, maxY);
	...
	private void drawVisibleBorders(
			GGraphics2D graphics, Rectangle bounds, double minX, double minY, double maxX, double maxY) {
	...
	void drawSelectionHeader(Selection selection, GGraphics2D graphics, Rectangle viewport) {
		...
		graphics.setColor(
				range.getMinColumn() == -1 ? styling.getSelectionHeaderColor() : styling.getGridColor());
		...
		graphics.setColor(
				range.getMinRow() == -1 ? styling.getSelectionHeaderColor() : styling.getGridColor());
	...
	void drawDraggingDot(Point location, GGraphics2D graphics) {
		...
		fillRect(graphics, location.x - dotSize, location.y - dotSize, dotSize * 2, dotSize * 2);
		graphics.setStroke(gridStroke);
		graphics.setColor(GColor.WHITE);
		drawRectangleWithStraightLines(
				graphics,
				location.x - dotSize,
				location.y - dotSize,
				location.x + dotSize,
	...
	void drawEditorBorder(Rectangle bounds, GGraphics2D graphics) {
		...
		graphics.setStroke(borderStroke);
		drawRectangleWithStraightLines(
				graphics, bounds.getMinX(), bounds.getMinY(), bounds.getMaxX(), bounds.getMaxY());
	}
	 ...
	 */
	void drawReference(
			@NonNull SpreadsheetReference reference,
			int referenceIndex,
			boolean filled,
			GGraphics2D graphics,
			Rectangle viewport) {
		drawReference(
				...
				referenceStrokes.get(referenceIndex % referenceStrokes.size()),
				graphics,
				viewport);
	}

	void drawStatisticsReference(
			@NonNull SpreadsheetReference reference,
			boolean focused,
			GGraphics2D graphics,
			Rectangle viewport) {
		drawReference(
				reference,
				focused,
				focused ? PURPLE_600 : NEUTRAL_500,
				referenceStrokes.get(0),
				graphics,
				viewport);
	}
	...
	private void drawReference(
		...
		if (minX < bounds.getMaxX() && minY < bounds.getMaxY()) {
			drawVisibleBorders(graphics, bounds, minX, minY, bounds.getMaxX(), bounds.getMaxY());
		}
		graphics.setStroke(gridStroke);
	}
	...
	private @Nullable TabularRange getVisibleTabularRange(SpreadsheetReference reference) {
		...
		int maxColumn =
				Math.min(layout.numberOfColumns() - 1, Math.max(from.columnIndex, to.columnIndex));
		return minRow <= maxRow && minColumn <= maxColumn
				? new TabularRange(minRow, minColumn, maxRow, maxColumn)
				: null;
	}
	...
	void fillRect(GGraphics2D graphics, double x, double y, double width, double height) {
		graphics.fillRect((int) Math.round(x), (int) Math.round(y), (int) Math.round(width), (int)
				Math.round(height));
	}
