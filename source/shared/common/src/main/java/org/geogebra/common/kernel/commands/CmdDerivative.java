...
public final class CmdDerivative extends CommandProcessor {
			...
			case 2:
				...
				try {
					...
					if ((arg[0].isGeoFunction() || arg[0].isGeoCurveCartesian())
						...
						GeoElement[] ret = {derivative(label, f, null, (GeoNumberValue) arg[1], info)};
						return ret;
					}
				} catch (Throwable t) {
					Log.debug(t);
	...
	static String getDerivLabel(GeoElementND geo, int order) {
		...
		if (geo.isLabelSet()) {
			StringBuilder labelBuilder = new StringBuilder(geo.getLabel(StringTemplate.defaultTemplate));
			...
			if (geo.getParentAlgorithm() instanceof AlgoDependentGeoCopy) {
				return getDerivLabel(geo.getParentAlgorithm().getInput(0), order);
			}
		}
	 ...
	 */
	public GeoElement derivative(
			String label, CasEvaluableFunction f, GeoNumeric var, GeoNumberValue n, EvalInfo info) {
		boolean numeric =
				cmd == Commands.NDerivative || !app.getSettings().getCasSettings().isEnabled();
