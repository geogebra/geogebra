...
public class PascalDistribution extends CachingDiscreteDistribution {
	...
	protected DiscreteProbability createProbability(DistributionParameters parameters) {
		...
		AlgoInversePascal n2 = new AlgoInversePascal(cons, nGeo, pGeo, new GeoNumeric(cons, nearlyOne));
		cons.removeFromConstructionList(n2);
		GeoElementND n2Geo = n2.getOutput(0);

		AlgoSequenceRange algoSeq =
				new AlgoSequenceRange(cons, new GeoNumeric(cons, 0.0), (GeoNumberValue) n2Geo, null);
		...
		ExpressionNode nPlusOne =
				new ExpressionNode(kernel, n2Geo, Operation.PLUS, new MyDouble(kernel, 1.0));
