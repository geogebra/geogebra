...
public abstract class CopyPasteCut {
	 ...
	 */
	public abstract void copy(int column1, int row1, int column2, int row2, boolean skipGeoCopy);
	...
	public boolean paste(TabularRange range) {
		return paste(range.getMinColumn(), range.getMinRow(), range.getMaxColumn(), range.getMaxRow());
	}
	 ...
	 * @return true if successful
	 */
	public abstract boolean paste(int column1, int row1, int column2, int row2);
	 ...
	 * @return true if successful
	 */
	public boolean pasteInternalMultiple(int column1, int row1, int column2, int row2) {
		boolean succ = true;
	 ...
	 * @return true if successful
	 */
	public boolean pasteInternal(int column1, int row1, int maxColumn, int maxRow) {
		...
		try {
			for (int x = x1; x <= x2; ++x) {
				...
				for (int y = y1; y <= y2; ++y) {
					...
					boolean inSource =
							x + x3 - x1 <= x2 && x + x3 - x1 >= x1 && y + y3 - y1 <= y2 && y + y3 - y1 >= y1;

					if (relX + column1 <= maxColumn
							&& relY + row1 <= maxRow // ) {
							// //
							// check
							// not
						...
						if (values1[relX][relY] != null) {
							...
							constructionIndexes[count] = new Record(
									values1[relX][relY].getConstructionIndex(), relX, relY, x3 - x1, y3 - y1);
							count++;
						}
						// values2[relX][relY] =
			...
			for (int i = 0; i < count; i++) {
				...
				int relX = r.getx1();
				int relY = r.gety1();
				values2[relX][relY] = relativeCopy.doCopyNoStoringUndoInfo0(
						values1[relX][relY], values2[relX][relY], r.getx2(), r.gety2());
			}
	...
	protected boolean pasteExternalMultiple(String[][] data, TabularRange destination) {
		...
		boolean success = tiledRange != null && adapter.pasteExternalMultiple(data, tiledRange);
	...
	public static boolean delete(
			App app, int column1, int row1, int column2, int row2, SelectionType selectionType) {
		boolean succ = false;
		TreeSet<GeoElement> toRemove = new TreeSet<>();
		for (int column = column1; column <= column2; ++column) {
			for (int row = row1; row <= row2; ++row) {
				GeoElement value0 = RelativeCopy.getValue(app.getSpreadsheetTableModel(), column, row);
	...
	public String copyString(int minColumn, int minRow, int maxColumn, int maxRow) {
		...
		for (int row = minRow; row <= maxRow; ++row) {
			for (int column = minColumn; column <= maxColumn; ++column) {
				...
				if (value != null) {
					String valueString = value.toValueString(preciseTemplate);
	...
	private String removeTrailingZeros(String valueString) {
		int index = valueString.indexOf(app.getKernel().getLocalization().getDecimalPoint());
		if (index > -1) {
