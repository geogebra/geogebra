		...
		switch (n) {
			case 1:
				if (arg[0] instanceof GeoText) {
					AlgoParseToNumberOrFunction algo = new AlgoParseToNumberOrFunction(
							cons, (GeoText) arg[0], null, Commands.ParseToNumber, c.getLabel());
			...
			case 2:
				if ((ok = arg[0].isGeoNumeric()) && arg[1].isGeoText()) {
					...
					try {
						kernel.getAlgebraProcessor().evaluateToDouble(str, true, num);
						num.updateCascade();
					} catch (Exception e) {
						num.setUndefined();
						num.updateCascade();
