...
public class DrawConicPart extends Drawable implements Previewable {
	...
	private void initConicPart(GeoConicPartND initConicPart) {
		...
		// arc or sector?
		closure = initConicPart.getConicPartType() == GeoConicNDConstants.CONIC_PART_SECTOR
				? GArc2D.PIE
				: GArc2D.OPEN;
	}
	...
	public final void update() {
		...
		if (isVisible) {
			...
			drawAndUpdateTraceIfNeeded(((Traceable) conicPart).getTrace());
		}
	}
	...
	private void updateEllipse() {
		...
		if (xradius < BIG_RADIUS && yradius < BIG_RADIUS) {
			...
			shape =
					ClipShape.clipToRect(arc, transform, -1, -1, view.getWidth() + 2, view.getHeight() + 2);
		}
	...
	private void initPreview() {
		...
		switch (previewMode) {
			...
			case EuclidianConstants.MODE_SEMICIRCLE:
				AlgoSemicircle alg = new AlgoSemicircle(cons, previewTempPoints[0], previewTempPoints[1]);
			...
			case EuclidianConstants.MODE_CIRCLE_SECTOR_THREE_POINTS:
				...
				AlgoConicPartCircle algo = new AlgoConicPartCircle(
						cons, previewTempPoints[0], previewTempPoints[1], previewTempPoints[2], arcMode);
	...
	public final void updateMousePos(double xRW, double yRW) {
		if (isVisible) {
			...
			if (prevPoints.size() == 2) {
				if (DoubleUtil.isEqual(prevPoints.get(0).getInhomX(), xRW)
						&& DoubleUtil.isEqual(previewTempPoints[0].getInhomY(), yRW)) {
					isVisible = false;
					return;
				}
				if (DoubleUtil.isEqual(prevPoints.get(1).getInhomX(), xRW)
						&& DoubleUtil.isEqual(previewTempPoints[1].getInhomY(), yRW)) {
					isVisible = false;
					return;
				}
			}
