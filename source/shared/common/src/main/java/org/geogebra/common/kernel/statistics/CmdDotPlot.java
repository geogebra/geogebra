		...
		switch (n) {
			...
			case 2:
				if ((ok[0] = arg[0].isGeoList()) && (ok[1] = arg[1].isGeoNumeric())) {
					AlgoDotPlot algo =
							new AlgoDotPlot(cons, c.getLabel(), (GeoList) arg[0], (GeoNumeric) arg[1]);
					GeoElement[] ret = {algo.getResult()};
					return ret;

				} else if ((ok[0] = arg[0].isGeoList()) && (ok[1] = arg[1].isGeoBoolean())) {
					AlgoDotPlot algo = new AlgoDotPlot(cons, (GeoList) arg[0], (GeoBoolean) arg[1], null);
			...
			case 3:
				if ((ok[0] = arg[0].isGeoList())
					...
					AlgoDotPlot algo =
							new AlgoDotPlot(cons, (GeoList) arg[0], (GeoBoolean) arg[1], (GeoNumeric) arg[2]);
					algo.getResult().setLabel(c.getLabel());
					GeoElement[] ret = {algo.getResult()};
					return ret;
				} else if (!ok[0]) {
