					...
					if (geo.isGeoLocus()) {
						...
						for (int i = 0; i < length; i++) {
							...
							try {
								...
								// command, so delete
								removePossibleGeo(GeoElementSpreadsheet.getSpreadsheetCellName(minCol, row));
								removePossibleGeo(GeoElementSpreadsheet.getSpreadsheetCellName(minCol + 1, row));

								MyPoint p = al.get(i);

								kernel
										.getGeoElementSpreadsheet()
										.setSpreadsheetCell(app, row, minCol, new GeoNumeric(cons, p.x));
								kernel
										.getGeoElementSpreadsheet()
										.setSpreadsheetCell(app, row, minCol + 1, new GeoNumeric(cons, p.y));
							} catch (Exception e) {
						...
						for (int row = minRow; row <= maxRow; row++) {
							for (int col = minCol; col <= maxCol; col++) {
								try {
									...
									removePossibleGeo(GeoElementSpreadsheet.getSpreadsheetCellName(col, row));
									...
									relativeCopy.doCopyNoStoringUndoInfo0(
											geo, null, col - minCol, row - minRow, minRow, minCol);
						...
						for (int row = minRow; row <= maxRow; row++) {
							...
							for (int col = minCol; col <= maxCol; col++) {
								try {
									...
									kernel
											.getGeoElementSpreadsheet()
											.setSpreadsheetCell(app, row, col, rowList.get(countY % rowList.size()));
									countY++;
						...
						for (int row = minRow; row <= maxRow; row++) {
							for (int col = minCol; col <= maxCol; col++) {
								try {
									...
									kernel
											.getGeoElementSpreadsheet()
											.setSpreadsheetCell(app, row, col, list.get(count % list.size()));
									count++;
				...
				// arg[0] not list
				{
					if (GeoElementSpreadsheet.hasSpreadsheetLabel(arg[0])) {
						...
						MatchResult matcher = GeoElementSpreadsheet.spreadsheetPattern.exec(
								arg[0].getLabel(StringTemplate.defaultTemplate));
						...
						if (list.isMatrix()) {
								...
								for (int r = 0; r < rows; r++) {
									...
									for (int c1 = 0; c1 < cols; c1++) {
										kernel
												.getGeoElementSpreadsheet()
												.setSpreadsheetCell(
														app, row + r, column + c1, rowList.get(c1).copy());
									}
								}
							} catch (Exception e) {
								app.setScrollToShow(true);
							...
							for (int i = list.size() - 1; i >= 0; i--) {
								try {
									kernel
											.getGeoElementSpreadsheet()
											.setSpreadsheetCell(app, row, column + i, list.get(i).copy());
								} catch (Exception e) {
									app.setScrollToShow(true);
									throw argErr(c.getName(), arg[1], e);
								}
