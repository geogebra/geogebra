...
public class ExpressionSerializer implements ExpressionNodeConstants {
	 ...
	 */
	public static String operationToString(
		...
		switch (operation) {
			case NO_OPERATION:
				return leftStr;
			case NOT:
				return tpl.notString(left, leftStr, loc);
			...
			case IS_SUBSET_OF_STRICT:
				if (stringType.equals(StringType.CONTENT_MATHML)) {
					...
					tpl.infixBinary(sb, left, right, operation, leftStr, rightStr, tpl.strictSubsetSign());
				}
				break;
			...
			case VECTORPRODUCT:
				...
				break;
			case PLUS:
				return tpl.plusString(left, right, leftStr, rightStr, valueForm, loc);
			case INVISIBLE_PLUS:
			...
			case DIVIDE:
				return tpl.divideString(left, right, leftStr, rightStr, valueForm, loc);

			case POWER:
				return tpl.powerString(left, right, leftStr, rightStr, valueForm, loc);
				...
				switch (stringType) {
					...
					default:
						if ((leftStr.charAt(0) != '-')
							...
							tpl.appendWithBrackets(sb, leftStr, loc);
						}
						sb.append('!');
						break;
				}
			...
			case ARCTAN2:
				twoVar(
						sb, leftStr, rightStr, "atan2", "<arctan/>", "ATAN2", "atan2", tpl, kernel, true, loc);
				break;
			case ARCTAN2D:
				twoVar(
						sb,
				...
				break;
			case NCR:
				twoVar(sb, leftStr, rightStr, "nCr", "<ncr/>", "NCR", "nCr", tpl, kernel, false, loc);
				break;
			case NPR:
				twoVar(sb, leftStr, rightStr, "nPr", "<npr/>", "NPR", "nPr", tpl, kernel, false, loc);
				break;
			case DOT:
				if (stringType.isGiac()) {
					sb.append(kernel
							.getGeoGebraCAS()
							.translateCommandSignature("Dot.2")
							.replace("%0", leftStr)
							.replace("%1", rightStr));
				} else {
					twoVar(
			...
			case ZETA:
				...
				sb.append(leftStr);
				sb.append(tpl.rightBracket(loc));
				break;
			case CI:
				switch (stringType) {
					case LATEX:
			...
			case LOGB:
				switch (stringType) {
					case CONTENT_MATHML:
						MathmlTemplate.mathml(
								sb, "<log/>", "<logbase>", leftStr, "</logbase>", "", rightStr, "");
						break;
					case LATEX:
						sb.append("\\log_{");
						sb.append(leftStr);
			...
			case PLUSMINUS:
				...
				tpl.append(sb, leftStr, left, Operation.PLUSMINUS);
				sb.append(Unicode.PLUSMINUS);
				if (right.isLeaf() || (ExpressionNode.opID(right) >= Operation.VECTORPRODUCT.ordinal())) {
					sb.append(rightStr);
				} else {
					tpl.appendWithBrackets(sb, rightStr, loc);
				}
			...
			case CBRT:
				switch (stringType) {
					case SCREEN_READER_ASCII:
						sb.append(ScreenReader.nroot(leftStr, "3", loc));
						break;
					case CONTENT_MATHML:
			...
			// $FALL-THROUGH$
			case FUNCTION:
				if (stringType.isGiac() && right instanceof ListValue list && left instanceof Function) {
				...
				if (left instanceof GeoFunction geo) {
					...
					switch (en.getOperation()) {
						...
						case DERIVATIVE:
							...
							appendUserFunction(sb, leftStr, rightStr, tpl, loc);
							break;
						default:
							appendUserFunction(sb, leftStr, rightStr, tpl, loc);
							break;
			...
			case ELEMENT_OF:
				if (tpl.hasCASType() && right instanceof MyList list) {
				...
				appendFunctionNVar(sb, left, leftStr, rightStr, tpl, loc);
				break;
			case FUNCTION_NVAR:
				if (valueForm) {
					...
					if ((left instanceof FunctionalNVar) && (right instanceof MyList)) {
						...
						for (int i = 0; (i < func.getVarNumber()) && (i < ((MyList) right).size()); i++) {
							en.replace(func.getFunctionVariables()[i], ((MyList) right).get(i));
						}
			...
			case IF:
				if (stringType.isGiac()) {
					...
					sb.append(", ");
					sb.append(rightStr);
					sb.append(tpl.rightCommandBracket(loc));
				}
				break;
			case IF_ELSE:
				if (stringType.isGiac()) {
			...
			case IF_LIST:
				...
				for (int i = 0; i < cond.size(); i++) {
					...
					sb.append(valueForm ? cond.get(i).toValueString(tpl) : cond.get(i).toString(tpl));
					sb.append(", ");
					sb.append(valueForm ? fn.get(i).toValueString(tpl) : fn.get(i).toString(tpl));
				}
				if (fn.size() > cond.size()) {
					sb.append(", ");
					sb.append(
							valueForm
									? fn.get(fn.size() - 1).toValueString(tpl)
									: fn.get(fn.size() - 1).toString(tpl));
				}

				sb.append(stringType.isGiac() ? ")" : tpl.rightCommandBracket(loc));
	...
	private static void appendFunctionLabel(
			GeoFunction geo,
			StringBuilder sb,
			StringTemplate tpl,
			String leftStr,
			...
			switch (tpl.getStringType()) {
				case SCREEN_READER_ASCII:
					if (altText == null) {
						...
						sb.append(loc.getMenuDefault(altText, altText.replace("altText.", "")));
					}
	...
	private static void appendUserFunction(
			StringBuilder sb, String leftStr, String rightStr, StringTemplate tpl, Localization loc) {
		sb.append(leftStr);
		tpl.appendWithBrackets(sb, rightStr, loc);
	}
	...
	private static void appendFunctionNVar(
			StringBuilder sb,
			ExpressionValue left,
			String leftStr,
			String rightStr,
