...
public class CmdParseToFunction extends CommandProcessor {
				...
				if ((ok = arg[0].isGeoFunction() || arg[0].isGeoFunctionNVar()) && arg[1].isGeoText()) {
					...
					try {
						GeoElement parsed = arg[0].isGeoFunction()
								? kernel.getAlgebraProcessor().evaluateToFunction(str, true)
								: kernel.getAlgebraProcessor().evaluateToFunctionNVar(str, true, false);
						fun.set(parsed);
						fun.updateCascade();
					} catch (Exception e) {
	...
	private GeoElement[] getParseAlgoResult(GeoText text, Command c, GeoList vars) {
		AlgoParseToNumberOrFunction parseAlgo =
				new AlgoParseToNumberOrFunction(cons, text, vars, Commands.ParseToFunction, c.getLabel());
