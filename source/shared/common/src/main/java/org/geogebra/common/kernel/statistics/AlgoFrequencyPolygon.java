...
public class AlgoFrequencyPolygon extends AlgoElement {
	 ...
	 */
	public AlgoFrequencyPolygon(Construction cons, GeoList list1, GeoList list2) {
	...
	protected void setInputOutput() {
		...
		algoHistogram =
				new AlgoHistogram(cons, isCumulative, list1, list2, list3, useDensity, density, right);
	...
	public final void compute() {
		...
		if (doCumulative) {
			...
			for (int i = 0; i < yValue.length - 1; i++) {
				points[i + 1] = new GeoPoint(cons, leftBorder[i + 1], yValue[i], 1.0);
			}
		} else {
			...
			for (int i = 0; i < yValue.length - 1; i++) {
				...
				points[i + 1] = new GeoPoint(cons, midpoint, yValue[i], 1.0);
			}
			midpoint = 1.5 * leftBorder[yValue.length - 1] - .5 * leftBorder[yValue.length - 2];
			points[yValue.length] = new GeoPoint(cons, midpoint, 0.0, 1.0);
		}
