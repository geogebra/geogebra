...
public class CmdZoomIn extends CmdScripting {
	...
	protected final GeoElement[] perform(Command c) throws MyError {
		...
		switch (n) {
			...
			case 6:
				...
				EuclidianSettings3D evs3D = app.getEuclidianView3D().getSettings();
				if (evs3D != null) {
				...
				kernel
						.getApplication()
						.getGgbApi()
						.setCoordSystem(xMin, xMax, yMin, yMax, zMin, zMax, false);
	 ...
	 */
	protected static GeoElement[] zoomIn2(
			GeoElement[] arg, Command c, double factor, CmdScripting proc) {

		boolean ok0;
		if ((ok0 = arg[0].isGeoNumeric()) && arg[1].isGeoPoint()) {

			EuclidianViewInterfaceCommon ev = proc.getApp().getActiveEuclidianView();
			Coords coords = ev.getCoordsForView(((GeoPointND) arg[1]).getCoordsInD3());
