		...
		switch (n) {
			...
			case 2:
				...
				if (list && arg[1].isGeoNumeric()) {
					GeoElement[] ret = {first(c.getLabel(), (GeoList) arg[0], (GeoNumeric) arg[1])};
					return ret;
				} else if (string && arg[1].isGeoNumeric()) {
					GeoElement[] ret = {first(c.getLabel(), (GeoText) arg[0], (GeoNumeric) arg[1])};
					return ret;
				} else if (locus && arg[1].isGeoNumeric()) {

					AlgoFirstLocus algo =
							new AlgoFirstLocus(cons, c.getLabel(), (GeoLocusable) arg[0], (GeoNumeric) arg[1]);

					GeoElement[] ret = {algo.getResult()};
					return ret;
				} else {
					throw argErr(c, (list && string) ? arg[1] : arg[0]);
				}
