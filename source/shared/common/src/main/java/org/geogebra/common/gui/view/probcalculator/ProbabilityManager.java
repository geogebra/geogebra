...
public class ProbabilityManager {
	...
	public HashMap<Dist, String[]> getDistributionParameterTransKeys() {
		if (distributionParameterTransKeys == null) {
			...
			distributionParameterTransKeys.put(
					ProbabilityCalculatorSettings.Dist.STUDENT, new String[] {"DegreesOfFreedom.short"});
			distributionParameterTransKeys.put(
					ProbabilityCalculatorSettings.Dist.CHISQUARE, new String[] {"DegreesOfFreedom.short"});
			distributionParameterTransKeys.put(
					ProbabilityCalculatorSettings.Dist.F,
					new String[] {"DegreesOfFreedom1.short", "DegreesOfFreedom2.short"});
			distributionParameterTransKeys.put(
					ProbabilityCalculatorSettings.Dist.EXPONENTIAL, new String[] {Unicode.lambda + ""});
			distributionParameterTransKeys.put(
			...
			distributionParameterTransKeys.put(
					...
					new String[] {"Binomial.number", "Binomial.probability"});
			distributionParameterTransKeys.put(
					ProbabilityCalculatorSettings.Dist.POISSON, new String[] {"Mean.short"});
			distributionParameterTransKeys.put(
					ProbabilityCalculatorSettings.Dist.HYPERGEOMETRIC, new String[] {
	 ...
	 */
	protected static AlgoDistribution getInverseCommand(
			Dist dist,
			Construction cons,
			GeoNumberValue param1,
			GeoNumberValue param2,
			GeoNumberValue param3,
			GeoNumberValue x) {
		...
		switch (dist) {
			...
			case BETA:
			case CAUCHY:
				ret = new AlgoInverseRealDistribution2Params(cons, param1, param2, x, dist);
				break;
			case LOGNORMAL:
			...
			case CHISQUARE:
			case EXPONENTIAL:
				ret = new AlgoInverseRealDistribution1Param(cons, param1, x, dist);
				break;
			case BINOMIAL:
			...
			case POISSON:
				...
				break;
			case HYPERGEOMETRIC:
				ret = new AlgoInverseHyperGeometric(cons, param1, param2, param3, x);
				break;
		}
	 ...
	 * @return AlgoDistribution
	 */
	protected static AlgoDistribution getCommand(
			Dist dist,
			Construction cons,
			GeoNumberValue param1,
			GeoNumberValue param2,
		...
		switch (dist) {
			...
			case WEIBULL:
				...
				break;
			case BINOMIAL:
				ret = new AlgoBinomialDist(cons, param1, param2, x, new GeoBoolean(cons, isCumulative));
				break;
			case PASCAL:
				ret = new AlgoPascal(cons, param1, param2, x, new GeoBoolean(cons, isCumulative));
				break;
			case POISSON:
				ret = new AlgoPoisson(cons, param1, x, new GeoBoolean(cons, isCumulative));
				break;
			case HYPERGEOMETRIC:
				ret = new AlgoHyperGeometric(
						cons, param1, param2, param3, x, new GeoBoolean(cons, isCumulative));
				break;
			case LOGNORMAL:
	 ...
	 */
	public double[] getPlotDimensions(
			Dist selectedDist, GeoNumberValue[] params, GeoFunction densityCurve, boolean isCumulative) {
		...
		switch (selectedDist) {
			...
			case LOGNORMAL:
				...
				double var = (Math.exp(varParam) - 1) * Math.exp(2 * meanParam + varParam);
				sigma = Math.sqrt(var);
			...
			case PASCAL:
				...
				if (params[0].getDouble() > 1) {
					mode = Math.floor(
							(1 - params[1].getDouble()) * (params[0].getDouble() - 1) / params[1].getDouble());
				}
			...
			case HYPERGEOMETRIC:
				...
				mode = Math.floor((params[1].getDouble() + 1)
						* (params[2].getDouble() + 1)
	...
	private static double getContXMax(
			GeoFunction densityCurve, double startX, double stepX, double yMinimum) {
	...
	public Double[] getDistributionMeasures(Dist selectedDist, GeoNumberValue[] params) {
		...
		switch (selectedDist) {
			...
			case WEIBULL:
				...
				variance = scale * scale * MyMath2.gamma(1 + 2 / shape) - mean * mean;
				sigma = Math.sqrt(variance);
	 ...
	 */
	public AlgoDistribution getDistributionAlgorithm(
			GeoNumeric value, GeoNumberValue[] params, Dist distType, boolean isCumulative) {
		...
		AlgoDistribution algo = getCommand(distType, cons, param1, param2, param3, value, isCumulative);
	...
	public double probability(
			double value, GeoNumberValue[] params, Dist distType, boolean isCumulative) {
	 ...
	 */
	public double intervalProbability(
			double low, double high, Dist distType, GeoNumberValue[] params, int probMode) {
		if (probMode == ProbabilityCalculatorView.PROB_LEFT) {
			...
			return rightProbability(high, params, distType) - probability(low, params, distType, true);
			...
			return probability(high, params, distType, true) - probability(low, params, distType, true);
		}
	}
	 ...
	 */
	public double inverseProbability(Dist distType, double prob, GeoNumberValue[] params) {
		...
		AlgoDistribution algo =
				getInverseCommand(distType, cons, param1, param2, param3, new GeoNumeric(cons, prob));
