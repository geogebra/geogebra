 ...
 *
 */
public class DrawSurfaceOfRevolution extends Drawable3DSurfaces implements Previewable {
	 ...
	 *            selected functions
	 */
	public DrawSurfaceOfRevolution(EuclidianView3D a_view3d, ArrayList<Path> selectedFunctions) {
		super(a_view3d);
	...
	public void updatePreview() {
		if (previewSurface == null
			...
			previewSurface =
					kernel.getManager3D().surfaceOfRevolution(selectedFunctions.get(0), angle, null);
			...
			((EuclidianController3D) getView3D().getEuclidianController())
					.setHandledGeo(previewSurface, selectedFunctions.get(0).toGeoElement());
		}
	}
	...
	public void createAngle() {
		...
		if (angle != null && angle.getValue() > RotationConverter.SNAP_PRECISION) {
