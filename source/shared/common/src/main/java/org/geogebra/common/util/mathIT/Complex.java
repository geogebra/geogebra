...
public class Complex {
	...
	public Complex divide(Complex other) {
		return new Complex(
				divide(new double[] {this.z[0], this.z[1]}, new double[] {other.z[0], other.z[1]}));
	}
	...
	public static double[] gamma(double[] z) {
		if (z[0] < 0) { // Weierstrass form:
			double[] w = divide(ONE_, multiply(z, power(Math.E, multiply(MyDouble.EULER_GAMMA, z))));
	...
	public static double[] sin(double[] z) {
		return new double[] {Math.sin(z[0]) * cosh(z[1]), Math.cos(z[0]) * sinh(z[1])};
	}
	 ...
	 */
	public Complex sin() {
		return new Complex(Math.sin(z[0]) * cosh(z[1]), Math.cos(z[0]) * sinh(z[1]));
	}
	...
	public static String toString(double[] z) {
		NumberFormatAdapter nf = FormatFactory.getPrototype().getNumberFormat(13);
