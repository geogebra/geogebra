...
public class AlgoIntersectSegmentConicRegion extends AlgoIntersect {
	 ...
	 *            input conic
	 */
	public AlgoIntersectSegmentConicRegion(
			Construction cons, String[] labels, GeoSegment segment, GeoConic conic) {
	 ...
	 *            input conic
	 */
	public AlgoIntersectSegmentConicRegion(Construction cons, GeoSegment segment, GeoConic conic) {
		super(cons);
	...
	public void compute() {
		...
		AlgoIntersectSegmentConicRegion.intersectSegmentConic(
				getSegment(), getConic(), intersectPoints);
		...
		if (getConic().getType() == GeoConicNDConstants.CONIC_CIRCLE && getConic().isLimitedPath()) {
			...
			AlgoIntersectSegmentConicRegion.intersectSegmentConicClosureSegments(
					getSegment(), getConic(), closureSegments, closureIntersect, intersectPoints);
		}
		...
		switch (numberOfLineParts) {
			...
			case 3: {
				...
				for (int i = 0; i < 3; i++) {
					if (isPartValid[i]) {
						outputSegments.getElement(count).set(pnt[order[i]], pnt[order[i + 1]], getSegment());
						count++;
					}
				}
	 ...
	 *            GeoPoint[2]
	 */
	public static void intersectSegmentConic(
			GeoSegment segment, GeoConic conic, GeoPoint[] intersectPoints) {
		/*
		 * calculate and assign intersection point between segment(as line) and
		 * conic(full conic)
		 */
		AlgoIntersectLineConic.intersectLineConic(
				segment, conic, intersectPoints, Kernel.MIN_PRECISION);
		...
		for (int i = 0; i < 2; i++) {
			if (intersectPoints[i].isDefined()) {
				...
				if (!(segment.isOnPath(intersectPoints[i], Kernel.MIN_PRECISION)
						&& conic.isOnPath(intersectPoints[i], Kernel.MIN_PRECISION))) {
	 ...
	 *            GeoSegment[2]
	 */
	public static void calcConicClosure(
			GeoConic conic, GeoPoint midPoint, GeoPoint[] endPoints, GeoSegment[] closureSegments) {

		if (conic.isLimitedPath()) {

			double startParameter = ((GeoConicPartND) conic).getParameterStart();
			...
			switch (((GeoConicPartND) conic).getConicPartType()) {
				case GeoConicNDConstants.CONIC_PART_ARC:
					GeoVec3D.lineThroughPoints(endPoints[0], endPoints[1], closureSegments[0]);
					closureSegments[0].setPoints(endPoints[0], endPoints[1]);
					closureSegments[1].setCoords(Double.NaN, Double.NaN, Double.NaN);
					break;
				case GeoConicNDConstants.CONIC_PART_SECTOR:
					GeoVec3D.lineThroughPoints(midPoint, endPoints[0], closureSegments[0]);
					closureSegments[0].setPoints(midPoint, endPoints[0]);
					GeoVec3D.lineThroughPoints(midPoint, endPoints[1], closureSegments[1]);
					closureSegments[1].setPoints(midPoint, endPoints[1]);
					break;
				default:
					for (int i = 0; i < 2; i++) {
						closureSegments[i].setCoords(Double.NaN, Double.NaN, Double.NaN);
					}
			}
	 ...
	 *            segment GeoPoint[2]
	 */
	public static void intersectSegmentConicClosureSegments(
			GeoSegment segment,
			GeoConic conic,
			GeoSegment[] closureSegments,
			GeoPoint[] closureIntersect,
		...
		switch (((GeoConicPartND) conic).getConicPartType()) {
			case GeoConicNDConstants.CONIC_PART_ARC:
				if (!Double.isNaN(closureSegments[0].getX())) {
					...
					if (closureIntersect[0].isDefined()) {
						if (!(DoubleUtil.isZero(closureSegments[0].distance(closureIntersect[0]))
								&& DoubleUtil.isZero(segment.distance(closureIntersect[0])))) {
					...
					if (closureIntersect[0].isDefined()) {
						for (int i = 0; i < 2; i++) {
							if (DoubleUtil.isZero(closureIntersect[0].distanceSqr(intersectPoints[i]))) {
								closureIntersect[0].setUndefined();
							}
						}
					}
				...
				break;
			case GeoConicNDConstants.CONIC_PART_SECTOR:
				if (!Double.isNaN(closureSegments[0].getX()) && !Double.isNaN(closureSegments[1].getX())) {

					for (int i = 0; i < closureIntersect.length; i++) {
						// calc intersection points
						GeoVec3D.cross(closureSegments[i], segment, closureIntersect[i]);

						// checks whether the cross product(intersection point) is
						// actually an intersection point
						if (closureIntersect[i].isDefined()) {
							if (!(DoubleUtil.isZero(closureSegments[i].distance(closureIntersect[i]))
									&& DoubleUtil.isZero(segment.distance(closureIntersect[i])))) {
						...
						if (closureIntersect[i].isDefined()) {
							for (int j = 0; j < 2; j++) {
								if (DoubleUtil.isZero(closureIntersect[i].distanceSqr(intersectPoints[j]))) {
									closureIntersect[i].setUndefined();
								}
							}
						}
