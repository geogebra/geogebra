...
public class AlgoIsTangent extends AlgoElement implements SymbolicParametersBotanaAlgoAre {
	 ...
	 *            the second object
	 */
	public AlgoIsTangent(Construction cons, GeoLine inputElement1, GeoConic inputElement2) {
		super(cons);
	 ...
	 *            the second object
	 */
	public AlgoIsTangent(
			Construction cons, String label, GeoLine inputElement1, GeoConic inputElement2) {
	...
	public final void compute() {
		...
		if (l.isDefinedTangent(c)) {
			...
			int type = AlgoIntersectLineConic.intersectLineConic(l, c, points, Kernel.STANDARD_PRECISION);
	...
	public PPolynomial[][] getBotanaPolynomials() throws NoSymbolicParametersException {
		...
		if (c.isCircle()) {
			...
			botanaPolynomials[0][1] =
					PPolynomial.perpendicular(foot[0], foot[1], lv[0], lv[1], foot[0], foot[1], cv[0], cv[1]);
			botanaPolynomials[0][2] =
					PPolynomial.equidistant(foot[0], foot[1], cv[0], cv[1], cv[2], cv[3]);
		...
		if (c.isParabola()) {
			...
			botanaPolynomials[0][0] = PPolynomial.collinear(foot[0], foot[1], lv[0], lv[1], lv[2], lv[3]);
			botanaPolynomials[0][1] =
					PPolynomial.perpendicular(foot[0], foot[1], lv[0], lv[1], foot[0], foot[1], cv[8], cv[9]);
			botanaPolynomials[0][2] = new PPolynomial(foot[0])
					.multiply(new PPolynomial(2))
					.subtract(new PPolynomial(mirror[0]))
					.subtract(new PPolynomial(cv[8]));
			botanaPolynomials[0][3] = new PPolynomial(foot[1])
					.multiply(new PPolynomial(2))
					.subtract(new PPolynomial(mirror[1]))
					.subtract(new PPolynomial(cv[9]));
			botanaPolynomials[0][4] =
					PPolynomial.collinear(mirror[0], mirror[1], cv[4], cv[5], cv[6], cv[7]);
