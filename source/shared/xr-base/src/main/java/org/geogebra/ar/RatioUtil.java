...
class RatioUtil {
	...
	double setARScaleAtStart(
			EuclidianView3D view3D,
			CoordMatrix4x4 viewModelMatrix,
			CoordMatrix4x4 projectMatrix,
			double ggbToRw) {
		...
		double precision =
				Math.round(projectFactor / precisionPoT) * precisionPoT * PROJECT_FACTOR_RELATIVE_PRECISION;
	...
	double calculateRatio(EuclidianView3D view3D, ARGestureManager arGestureManager) {
		...
		if (arGestureManager != null) {
			ratio = ratioAtStart
					* arGestureManager.getScaleFactor()
					* ratioChange
