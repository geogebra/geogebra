...
public abstract class ARManager<TouchEventType> implements XRManagerInterface<TouchEventType> {
	...
	public abstract void arButtonClicked() throws ARException;

	public abstract void setSession() throws ARException;
	...
	public abstract void setBackgroundStyle(Renderer.BackgroundStyle backgroundStyle);
	...
	public abstract void setHittingOriginAndDirection(float x, float y);

	public abstract void setHittingOriginAndDirectionFromScreenCenter();

	private void wrapMouseMoved(int x, int y) {
		view3D
				.getEuclidianController()
				.wrapMouseMoved(view3D.getEuclidianController().createTouchEvent(x, y));
	}
			...
			if (((EuclidianController3D) renderer.getView().getEuclidianController())
				...
				if (arMotionEvent == null) {
					if (mouseTouchGestureQueueHelper.isCurrentlyUp()) {
						...
						arMotionEvent =
								getARMotionEventMove((float) view3D.getWidth() / 2, (float) view3D.getHeight() / 2);
						setHittingOriginAndDirectionFromScreenCenter();
					}
				} else {
					// force event to be screen-centered
					arMotionEvent.setLocation(
							renderer.getWidthInPixels() / 2, renderer.getHeightInPixels() / 2);
					setHittingOriginAndDirectionFromScreenCenter();
				}
			} else {
				...
				if (arMotionEvent != null) {
					...
					if (mouseTouchGestureQueueHelper.isCurrentlyUp()) {
						...
						if (lastARMotionEvent != null) {
							if (lastARMotionEvent.getAction() == ARMotionEvent.FIRST_FINGER_DOWN) {
								arMotionEvent =
										getARMotionEventMove(lastARMotionEvent.getX(), lastARMotionEvent.getY());
								setHittingOriginAndDirection(arMotionEvent);
							} else if (lastARMotionEvent.getAction() == ARMotionEvent.ON_MOVE) {
	...
	private void updateModelMatrix() {
		...
		modelOrigin.setX(
				anchorOrigin.getX() + translationOffset.getX() + previousTranslationOffset.getX());
		modelOrigin.setY(anchorOrigin.getY());
		modelOrigin.setZ(
				anchorOrigin.getZ() + translationOffset.getZ() + previousTranslationOffset.getZ());
	}

	private Coords setRay() {
		viewMatrix.solve(Coords.O, rayOrigin);
		rayDirection.setSub3(cHitMatrix.getOrigin(), rayOrigin);
		rayOrigin.projectPlane(
				modelMatrix.getVx(),
				modelMatrix.getVz(),
				rayDirection,
				modelMatrix.getOrigin(),
				projection);
		return projection;
	}
	...
	protected void setClipCenterAndComputeRayDirection() {
		...
		tmpCoords2.setY(0);
		tmpCoords2.setZ(
				projectMatrix.getOrigin().getZ() / (1 - projectMatrix.getVz().getZ()));
		tmpCoords2.setW(1);
	...
	public void setXRScaleAtStart() {
		...
		double ratio = this.ratioUtil.setARScaleAtStart(view3D, modelMatrix, projectMatrix, ggbToRw);
	...
	public void fitThickness() {
		if (isDrawing()) {
			...
			double thicknessMin = getThicknessMin(mDistance);
			arScale =
					(float) (thicknessMin / (arGestureManager.getScaleFactor() * ratioUtil.getRatioChange()));
			arScaleFactor = arScaleAtStart / arScale;
	...
	private void updateSettingsScale(float factor) {
		...
		settings.setXYZscale(
				settings.getXscale() * factor,
