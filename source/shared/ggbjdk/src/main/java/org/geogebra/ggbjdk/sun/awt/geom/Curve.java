...
public abstract class Curve {
	 ...
	 */
	public static int pointCrossingsForPath(GPathIterator pi, double px, double py) {
		if (pi.isDone()) {
			return 0;
		}
		...
		while (!pi.isDone()) {
			switch (pi.currentSegment(coords)) {
				...
				case GPathIterator.SEG_QUADTO:
					...
					endy = coords[3];
					crossings +=
							pointCrossingsForQuad(px, py, curx, cury, coords[0], coords[1], endx, endy, 0);
					curx = endx;
					cury = endy;
					break;
				case GPathIterator.SEG_CUBICTO:
					...
					crossings += pointCrossingsForCubic(
							px, py, curx, cury, coords[0], coords[1], coords[2], coords[3], endx, endy, 0);
					curx = endx;
					cury = endy;
					break;
	...
	public static int rectCrossingsForPath(
			GPathIterator pi, double rxmin, double rymin, double rxmax, double rymax) {
			...
			switch (pi.currentSegment(coords)) {
				...
				case GPathIterator.SEG_QUADTO:
					...
					crossings = rectCrossingsForQuad(
							crossings, rxmin, rymin, rxmax, rymax, curx, cury, coords[0], coords[1], endx, endy,
							0);
					curx = endx;
					cury = endy;
					break;
				case GPathIterator.SEG_CUBICTO:
					...
					crossings = rectCrossingsForCubic(
							crossings, rxmin, rymin, rxmax, rymax, curx, cury, coords[0], coords[1], coords[2],
							coords[3], endx, endy, 0);
					curx = endx;
					cury = endy;
					break;
				case GPathIterator.SEG_CLOSE:
	...
	public static int rectCrossingsForQuad(
		...
		crossings = rectCrossingsForQuad(
				crossings, rxmin, rymin, rxmax, rymax, x0, y0, x0c, y0c, xc, yc, level + 1);
		if (crossings != RECT_INTERSECTS) {
			crossings = rectCrossingsForQuad(
					crossings, rxmin, rymin, rxmax, rymax, xc, yc, xc1, yc1, x1, y1, level + 1);
		}
		return crossings;
	}
	...
	public static int rectCrossingsForCubic(
		...
		crossings = rectCrossingsForCubic(
				crossings, rxmin, rymin, rxmax, rymax, x0, y0, xc0, yc0, xc0m, yc0m, xmid, ymid, level + 1);
		if (crossings != RECT_INTERSECTS) {
			crossings = rectCrossingsForCubic(
					crossings, rxmin, rymin, rxmax, rymax, xmid, ymid, xmc1, ymc1, xc1, yc1, x1, y1,
					level + 1);
		}
		return crossings;
	}
		...
		*/
		while (s0 < s1 && t0 < t1) {
			...
			*/
			try {
				if (findIntersect(
						that, yrange, ymin, 0, 0, s0, xs0, ys0, sh, xsh, ysh, t0, xt0, yt0, th, xth, yth)) {
					break;
				}
			} catch (Throwable t) {
