...
public abstract class Path2D implements Shape {
	...
	public static class Double extends Path2D {
		...
		GPoint2D getPoint(int coordindex) {
			return new GPoint2D(doubleCoords[coordindex], doubleCoords[coordindex + 1]);
		}
			...
			for (int i = 1; i < numTypes; i++) {
				switch (pointTypes[i]) {
					...
					case GPathIterator.SEG_LINETO:
						crossings += Curve.pointCrossingsForLine(
								px, py, curx, cury, endx = coords[ci++], endy = coords[ci++]);
						curx = endx;
						cury = endy;
						break;
	 ...
	 * @since 1.6
	 */
	public static boolean contains(GPathIterator pi, double x, double y, double w, double h) {
	 ...
	 * @since 1.6
	 */
	public static boolean intersects(GPathIterator pi, double x, double y, double w, double h) {
		if (java.lang.Double.isNaN(x + w) || java.lang.Double.isNaN(y + h)) {
			/* [xy]+[wh] is NaN if any of those values are NaN,
			 * or if adding the two together would produce NaN
			 * by virtue of adding opposing Infinite values.
	 ...
	 */
	@Override
	public final GPathIterator getPathIterator(GAffineTransform at, double flatness) {
	...
	abstract static class Iterator implements GPathIterator {
		int typeIdx;
		int pointIdx;
		Path2D path;
