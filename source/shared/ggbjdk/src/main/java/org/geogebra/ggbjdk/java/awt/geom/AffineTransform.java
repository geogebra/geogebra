...
public class AffineTransform implements GAffineTransform {
		...
		switch (state1) {
			...
			case (APPLY_SHEAR):
				...
				if (m01 == 0 && m10 == 0) {
					...
					if (m00 == 1.0 && m11 == 1.0) {
						this.type = (state1 == APPLY_IDENTITY ? TYPE_IDENTITY : TYPE_TRANSLATION);
					} else {
						state1 |= APPLY_SCALE;
						this.type = TYPE_UNKNOWN;
					}
	...
	public AffineTransform createInverse() throws NoninvertibleTransformException {
		...
		switch (state) {
			...
			case (APPLY_SHEAR | APPLY_SCALE):
				...
				return new AffineTransform(
						m11 / det, -m10 / det, -m01 / det, m00 / det, 0.0, 0.0, (APPLY_SHEAR | APPLY_SCALE));
			case (APPLY_SHEAR | APPLY_TRANSLATE):
				if (m01 == 0.0 || m10 == 0.0) {
	 ...
	 * @since 1.2
	 */
	public void transform(GPoint2D[] ptSrc, int srcOff, GPoint2D[] ptDst, int dstOff, int numPts) {
		int state = this.state;
		while (--numPts >= 0) {
			// Copy source coords into local variables in case src == dst
	...
	public void transform(
		...
		if (destCoordinates == srcCoordinates
				...
				&& destOffset < srcOffset + nPoints * 2) {
