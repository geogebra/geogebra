...
import org.geogebra.editor.share.editor.MathFieldAsync;
...
public class MathFieldW implements MathField, IsWidget, MathFieldAsync, BlurHandler {
	...
	protected static TemplateCatalog sTemplateCatalog = new TemplateCatalog();
	...
	public MathFieldW(SyntaxAdapter converter, Panel parent, Canvas canvas,
		...
		mathFieldInternal.setFormula(new Formula(sTemplateCatalog));
		initTimer();
		instances.add(this);
		if (canvas != null) {
	...
	public void setCatalog(TemplateCatalog catalog) {
		this.catalog = catalog;
	}
	...
	private void startEditing() {
		if (mathFieldInternal.getEditorState().getCurrentNode() == null) {
	...
	public void setInputBoxFunctionVariables(List<String> funcVars) {
		catalog.setInputBoxFunctionVars(funcVars);
