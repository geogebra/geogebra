 ...
 *
 */
public final class RowHeaderPopupMenuW extends org.geogebra.common.cas.view.RowHeaderPopupMenu
	...
	private void initMenu() {
		...
		AriaMenuItem miCopyInput = new AriaMenuItem(
				loc.getMenu("CopyInput"), null, () -> actionPerformed(CellAction.COPY_INPUT));
		rowHeaderPopupMenu.addItem(miCopyInput);

		AriaMenuItem miPaste =
				new AriaMenuItem(loc.getMenu("Paste"), null, () -> actionPerformed(CellAction.PASTE));
		rowHeaderPopupMenu.addItem(miPaste);
		rowHeaderPopupMenu.addSeparator();

		AriaMenuItem miInsertAbove = new AriaMenuItem(
				loc.getMenu("InsertAbove"), null, () -> actionPerformed(CellAction.INSERT_ABOVE));
		rowHeaderPopupMenu.addItem(miInsertAbove);

		// "Insert Below" menu item
		AriaMenuItem miInsertBelow = new AriaMenuItem(
				loc.getMenu("InsertBelow"), null, () -> actionPerformed(CellAction.INSERT_BELOW));
		rowHeaderPopupMenu.addItem(miInsertBelow);

		int[] selRows = table.getSelectedRows();
		String strRows = getDeleteString(selRows);
		AriaMenuItem miDelete =
				new AriaMenuItem(strRows, null, () -> actionPerformed(CellAction.DELETE));
		rowHeaderPopupMenu.addItem(miDelete);

		rowHeaderPopupMenu.addSeparator();

		AriaMenuItem miUseAsText = new AriaMenuItem(
				loc.getMenu("CasCellUseAsText"), null, () -> actionPerformed(CellAction.TEXT));
		rowHeaderPopupMenu.addItem(miUseAsText);

		if (Js.isTruthy(navigator.clipboard)) {

			AriaMenuItem copyItem =
					new AriaMenuItem(loc.getMenu("Copy"), null, () -> actionPerformed(CellAction.COPY));
			rowHeaderPopupMenu.addItem(copyItem);

			AriaMenuItem latexItem = new AriaMenuItem(
					loc.getMenu("CopyAsLaTeX"), null, () -> actionPerformed(CellAction.COPY_LATEX));
			...
			case COPY_LATEX:
				...
				if (toBeCopied != null) {
					...
					if (toBeCopied.endsWith("\\\\ undefined \\\\ ")) {
						toBeCopied = toBeCopied.substring(0, toBeCopied.length() - 14);
					} else if (toBeCopied.endsWith("\\\\ undefined \\\\")) {
						toBeCopied = toBeCopied.substring(0, toBeCopied.length() - 13);
					}
