...
public class ThumbnailTest {
	...
	public void thumbnailShouldUseNonemptyView() {
		...
		app.getKernel().getAlgebraProcessor().processAlgebraCommand("SetActiveView(1)", true);
		app.getKernel().getAlgebraProcessor().processAlgebraCommand("\"Text\"", true);
		thumbnailShouldUse(App.VIEW_EUCLIDIAN, app);
	}

	private static void thumbnailShouldUse(int viewId, AppW app) {
		assertEquals(viewId, app.getGgbApi().getViewForThumbnail().getViewID());
	}
}
