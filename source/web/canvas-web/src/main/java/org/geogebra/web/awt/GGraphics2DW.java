...
public class GGraphics2DW implements GGraphics2DWI, GGraphics2D {
	...
	public void setPaint(final GPaint paint) {
		if (paint instanceof VectorPatternPaint) {
			if (((VectorPatternPaint) paint).type == VectorPatternPaint.VectorType.SVG) {
				setPaintSVG((VectorPatternPaint) paint);
			}
			if (((VectorPatternPaint) paint).type == VectorPatternPaint.VectorType.PDF) {
	...
	public void setFont(GFont font) {
		...
		if (font instanceof GFontW) {
			...
			try {
				...
				setFontFailed = true;
				Log.debug("problem setting font: " + currentFont.getFullFontString());
			}
		}
	}
	...
	public void drawImage(GBufferedImage img, int x, int y) {
		...
		try {
			...
			Log.debug("error in drawImage(): " + e.getMessage());
		}
	}
