...
final class SandboxConverter {
	...
	private QuickJS.QuickJSHandle toSandboxObject(
			Object rawResult, QuickJS.QuickJSContext vm, int maxDepth, Object topLevel) {
		...
		switch (Js.typeof(rawResult)) {
			...
			case "object":
				...
				QuickJS.QuickJSHandle sandboxed =
						JsArray.isArray(rawResult) ? vm.newArray() : vm.newObject();
				JsPropertyMap<Object> props = JsObject.of(rawResult);
				if (maxDepth > 0) {
					props.forEach(propName -> vm.setProp(
							sandboxed,
							propName,
